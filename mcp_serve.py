"""
Seraphiel MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Seraphiel-specific extra)

Usage:
    seraphiel mcp serve
    seraphiel mcp serve --verbose
...
logger = logging.getLogger("seraphiel.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SERAPHIEL_HOME."""
    try:
        from seraphiel_constants import get_seraphiel_home
        return get_seraphiel_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SERAPHIEL_HOME", Path.home() / ".seraphiel")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from seraphiel_constants import get_seraphiel_home
        directory_file = get_seraphiel_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SERAPHIEL_HOME", Path.home() / ".seraphiel")
...
class EventBridge:
    ...
    This is the Seraphiel equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from seraphiel_constants import get_seraphiel_home
            db_file = get_seraphiel_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SERAPHIEL_HOME", Path.home() / ".seraphiel")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Seraphiel MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "seraphiel",
        instructions=(
            "Seraphiel Brain messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Seraphiel MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
