...
# 5. Symlinks the 'seraphiel' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Seraphiel Brain Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/seraphiel-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink seraphiel into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up seraphiel command..."

SERAPHIEL_BIN="$SCRIPT_DIR/venv/bin/seraphiel"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SERAPHIEL_BIN" "$COMMAND_LINK_DIR/seraphiel"
echo -e "${GREEN}✓${NC} Symlinked seraphiel → $COMMAND_LINK_DISPLAY_DIR/seraphiel"
...
# ============================================================================
# Seed bundled skills into ~/.seraphiel/skills/
# ============================================================================

SERAPHIEL_SKILLS_DIR="${SERAPHIEL_HOME:-$HOME/.seraphiel}/skills"
mkdir -p "$SERAPHIEL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.seraphiel/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SERAPHIEL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  seraphiel status        # Check configuration"
if is_termux; then
    echo "  seraphiel gateway       # Run gateway in foreground"
else
    echo "  seraphiel gateway install # Install gateway service (messaging + cron)"
fi
echo "  seraphiel cron list     # View scheduled jobs"
echo "  seraphiel doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m seraphiel_cli.main setup
fi
