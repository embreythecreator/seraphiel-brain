...
interface SeraphielConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SeraphielConsoleModal({ open, onClose }: SeraphielConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("seraphiel> ");
  const inputPromptRef = useRef("seraphiel> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "seraphiel> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Seraphiel Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="seraphiel-console-title"
    >
      <div
        className={cn(
          themedBody,
