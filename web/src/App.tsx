...
const SIDEBAR_COLLAPSED_KEY = "seraphiel-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="seraphiel-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="seraphiel-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSeraphiel = status?.can_update_seraphiel === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'seraphiel update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "seraphiel update";
    return (
      t.status.updateSeraphielConfirmMessage ??
      `This will run 'seraphiel update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSeraphielConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSeraphiel) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSeraphiel,
      runningLabel: t.status.updatingSeraphiel,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Seraphiel gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSeraphielConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSeraphielConfirmTitle ?? `${t.status.updateSeraphiel}?`}
    />
    </>
  );
}
