# OpenAI-Compatible API Server for Seraphiel Brain
...
SSE streaming. By exposing this endpoint, seraphiel-brain becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  seraphiel-brain   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.seraphiel/.env`
2. Run `seraphiel gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with seraphiel-brain through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns seraphiel-brain as a model) |
...
Authorization: Bearer seraphiel-api-key-here
...
But seraphiel-brain sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "seraphiel-brain"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured seraphiel-brain model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of seraphiel-brain's core prompt
...
Once implemented, seraphiel-brain works as a drop-in backend for:
