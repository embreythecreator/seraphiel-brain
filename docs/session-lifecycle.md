...
The canonical transcript store is SQLite via `SessionDB` (from `seraphiel_state`). The
...
Counts consecutive restarts via a JSON file (`{SERAPHIEL_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `seraphiel update`, `seraphiel gateway restart`,
or `/restart`.
