# Seraphiel Middleware

Seraphiel middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Seraphiel'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Seraphiel calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Seraphiel retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Seraphiel guardrails,
...
- `telemetry_schema_version`: currently `seraphiel.observer.v1`
- `middleware_schema_version`: currently `seraphiel.middleware.v1`
...
Seraphiel stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Seraphiel runs
them as a nested chain in registration order. Middleware failures are fail-open:
Seraphiel logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Seraphiel applies middleware in this order:
...
For each tool call, Seraphiel applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Seraphiel pre-execution path against the effective arguments:
...
```bash
seraphiel plugins enable <plugin-name>
```

For isolated local testing, use one `SERAPHIEL_HOME` for plugin enablement and the
agent run:

```bash
export SERAPHIEL_HOME=/tmp/seraphiel-middleware-test
mkdir -p "$SERAPHIEL_HOME"
seraphiel plugins enable <plugin-name>
seraphiel chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run seraphiel plugins enable <plugin-name>
uv run seraphiel chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["seraphiel_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/seraphiel-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Seraphiel expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Seraphiel treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Seraphiel preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Seraphiel does not convert downstream
