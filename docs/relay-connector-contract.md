...
This document is the formal interface between the **Seraphiel gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`embreythecreator/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `seraphiel gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`seraphiel gateway enroll` → connector `/relay/enroll`, or
...
(`embreythecreator/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `seraphiel` CLI.
