# Seraphiel Observer Hooks

Seraphiel observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Seraphiel' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Seraphiel catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Seraphiel only builds
