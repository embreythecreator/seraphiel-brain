...
`seraphiel config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.seraphiel/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `SERAPHIEL_HOME`, secret scope, and
