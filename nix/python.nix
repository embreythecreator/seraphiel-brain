...
let
  ...
  # $SERAPHIEL_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SERAPHIEL_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        seraphiel-brain = prev.seraphiel-brain.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "seraphiel-brain-env" {
    seraphiel-brain = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "seraphiel-brain-editable-env" {
    seraphiel-brain = dependency-groups;
  };
}
