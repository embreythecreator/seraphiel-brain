...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      seraphiel-brain = self'.packages.default;
      seraphielVenv = seraphiel-brain.seraphielVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "seraphiel-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${seraphielVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "seraphiel-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "seraphiel-build-package" { } ''
          echo "PASS: package built at ${seraphiel-brain}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "seraphiel-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "seraphiel-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${seraphiel-brain}/bin/seraphiel || (echo "FAIL: seraphiel binary missing"; exit 1)
          test -x ${seraphiel-brain}/bin/seraphiel-brain || (echo "FAIL: seraphiel-brain binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${seraphiel-brain}/bin/seraphiel version 2>&1 | grep -qi "seraphiel" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "seraphiel-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in seraphiel seraphiel-brain seraphiel-acp; do
            test -x ${seraphiel-brain}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "seraphiel-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking seraphiel --help ==="
          ${seraphiel-brain}/bin/seraphiel --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${seraphiel-brain}/bin/seraphiel --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "seraphiel-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${seraphiel-brain}/share/seraphiel-brain/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${seraphiel-brain}/share/seraphiel-brain/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SERAPHIEL_BUNDLED_SKILLS" ${seraphiel-brain}/bin/seraphiel || \
            (echo "FAIL: SERAPHIEL_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SERAPHIEL_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${seraphiel-brain}/share/seraphiel-brain/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${seraphiel-brain}/share/seraphiel-brain/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SERAPHIEL_OPTIONAL_SKILLS" ${seraphiel-brain}/bin/seraphiel || \
            (echo "FAIL: SERAPHIEL_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SERAPHIEL_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "seraphiel-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${seraphiel-brain}/share/seraphiel-brain/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${seraphiel-brain}/share/seraphiel-brain/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SERAPHIEL_BUNDLED_PLUGINS" ${seraphiel-brain}/bin/seraphiel || \
            (echo "FAIL: SERAPHIEL_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SERAPHIEL_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "seraphiel-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${seraphiel-brain}/share/seraphiel-brain/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${seraphiel-brain}/share/seraphiel-brain/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${seraphiel-brain}/share/seraphiel-brain/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SERAPHIEL_BUNDLED_LOCALES" ${seraphiel-brain}/bin/seraphiel || \
            (echo "FAIL: SERAPHIEL_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SERAPHIEL_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (SERAPHIEL_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SERAPHIEL_BUNDLED_LOCALES=${seraphiel-brain}/share/seraphiel-brain/locales \
            ${seraphielVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SERAPHIEL_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${seraphielVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${seraphielVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without SERAPHIEL_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "seraphiel-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${seraphiel-brain}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${seraphiel-brain}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SERAPHIEL_TUI_DIR" ${seraphiel-brain}/bin/seraphiel || \
            (echo "FAIL: SERAPHIEL_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SERAPHIEL_TUI_DIR set in wrapper"
        ...
        # Verify SERAPHIEL_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        seraphiel-node = pkgs.runCommand "seraphiel-node-version" { } ''
          set -e
          echo "=== Checking SERAPHIEL_NODE in wrapper ==="
          grep -q "SERAPHIEL_NODE" ${seraphiel-brain}/bin/seraphiel || \
            (echo "FAIL: SERAPHIEL_NODE not set in wrapper"; exit 1)
          echo "PASS: SERAPHIEL_NODE present in wrapper"

          SERAPHIEL_NODE=$(sed -n "s/^export SERAPHIEL_NODE='\(.*\)'/\1/p" ${seraphiel-brain}/bin/seraphiel)
          test -x "$SERAPHIEL_NODE" || (echo "FAIL: SERAPHIEL_NODE=$SERAPHIEL_NODE not executable"; exit 1)
          echo "PASS: SERAPHIEL_NODE executable at $SERAPHIEL_NODE"

          NODE_MAJOR=$("$SERAPHIEL_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All SERAPHIEL_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SERAPHIEL_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "seraphiel-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SERAPHIEL_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SERAPHIEL_MANAGED guards ==="
          check_blocked "config set" ${seraphiel-brain}/bin/seraphiel config set model foo
          check_blocked "config edit" ${seraphiel-brain}/bin/seraphiel config edit
        ...
        extra-python-packages = let
          ...
          seraphielWithExtra = seraphiel-brain.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "seraphiel-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${seraphielWithExtra}/bin/seraphiel || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${seraphielWithExtra}/bin/seraphiel || \
          ...
          if grep -q "PYTHONPATH" ${seraphiel-brain}/bin/seraphiel; then
        ...
        extra-dependency-groups = let
          seraphielWithGroups = seraphiel-brain.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "seraphiel-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${seraphielWithGroups}"
          echo "venv: ${seraphielWithGroups.seraphielVenv}"
        ...
        messaging-variant = pkgs.runCommand "seraphiel-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.seraphielVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "seraphiel-config-roundtrip" {
          ...
          merge_and_load() {
            local seraphiel_home="$1"
            export SERAPHIEL_HOME="$seraphiel_home"
            ${configMergeScript} ${nixSettings} "$seraphiel_home/config.yaml"
            ${seraphielVenv}/bin/python3 -c '
import json, sys
