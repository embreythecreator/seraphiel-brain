# nix/nixosModules.nix — NixOS module for seraphiel-brain
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: seraphiel runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SERAPHIEL_HOME/.env
# and read by seraphiel at startup — no container recreation needed for env changes.
#
# Tool resolution: the seraphiel wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.seraphiel-brain;
    effectivePackage =
    ...
    seraphiel-brain = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-seraphiel-brain)
    deepConfigType = lib.types.mkOptionType {
      name = "seraphiel-config-attrs";
      description = "Seraphiel YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — seraphiel
    ...
    generatedConfigFile = pkgs.writeText "seraphiel-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # SERAPHIEL_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "seraphiel-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SERAPHIEL_DOC_EOF'\n${value}\nSERAPHIEL_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "seraphiel-brain";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/seraphiel";
    ...
    # seraphiel user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "seraphiel-container-entrypoint" ''
      set -eu

      SERAPHIEL_UID="''${SERAPHIEL_UID:?SERAPHIEL_UID must be set}"
      SERAPHIEL_GID="''${SERAPHIEL_GID:?SERAPHIEL_GID must be set}"

      # ── Group: ensure a group with GID=$SERAPHIEL_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SERAPHIEL_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="seraphiel"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$SERAPHIEL_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$SERAPHIEL_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SERAPHIEL_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SERAPHIEL_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="seraphiel"
        TARGET_HOME="/home/seraphiel"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$SERAPHIEL_UID" -g "$SERAPHIEL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$SERAPHIEL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SERAPHIEL_UID:$SERAPHIEL_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure SERAPHIEL_HOME is owned by the target user.
      ...
      if [ -n "''${SERAPHIEL_HOME:-}" ] && [ -d "$SERAPHIEL_HOME" ]; then
        find "$SERAPHIEL_HOME" \! -user "$SERAPHIEL_UID" -exec chown "$SERAPHIEL_UID:$SERAPHIEL_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/seraphiel-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/seraphiel-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/seraphiel ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/seraphiel
        chmod 0440 /etc/sudoers.d/seraphiel
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SERAPHIEL_UID" --regid="$SERAPHIEL_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $SERAPHIEL_HOME/.env.
    ...
    # Default: /var/lib/seraphiel/workspace → /data/workspace.
  ...
  in {
    options.services.seraphiel-brain = with lib; {
      enable = mkEnableOption "Seraphiel Brain gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = seraphiel-brain;
        description = "The seraphiel-brain package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/seraphiel";
        description = "State directory. Contains .seraphiel/ subdir (SERAPHIEL_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Seraphiel config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $SERAPHIEL_HOME/.env at activation time.
          Seraphiel reads this file on every startup via load_seraphiel_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SERAPHIEL_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $SERAPHIEL_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `seraphiel gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the seraphiel user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the seraphiel plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Seraphiel discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          seraphiel_brain.plugins entry-point group. Each package must be built
          with the same Python interpreter as seraphiel (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in seraphiel-brain's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the seraphiel CLI to environment.systemPackages and export
          SERAPHIEL_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.seraphiel symlink to the service
            stateDir. These users are automatically added to the seraphiel group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.seraphiel-brain.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the seraphiel CLI to system PATH and export SERAPHIEL_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.seraphiel/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.SERAPHIEL_HOME = "${cfg.stateDir}/.seraphiel";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.seraphiel-brain.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the seraphiel user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.seraphiel-brain: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed seraphiel
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure seraphiel is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.seraphiel        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seraphiel/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seraphiel/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seraphiel/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seraphiel/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seraphiel/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."seraphiel-brain-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.seraphiel
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.seraphiel ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.seraphiel ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.seraphiel -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.seraphiel/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.seraphiel/$_subdir"
            chmod 2770 "${cfg.stateDir}/.seraphiel/$_subdir"
            find "${cfg.stateDir}/.seraphiel/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # seraphiel-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.seraphiel/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.seraphiel/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seraphiel/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.seraphiel/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.seraphiel/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seraphiel/.managed
          chmod 0644 ${cfg.stateDir}/.seraphiel/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.seraphiel/.container-mode <<'SERAPHIEL_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    seraphiel_bin=${containerDataDir}/current-package/bin/seraphiel
    SERAPHIEL_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seraphiel/.container-mode
            chmod 0644 ${cfg.stateDir}/.seraphiel/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.seraphiel/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.seraphiel";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.seraphiel" ]; then
                  rm -f "${symlinkPath}"
                  echo "seraphiel-brain: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.seraphiel -> stateDir/.seraphiel for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.seraphiel";
                target = "${cfg.stateDir}/.seraphiel";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "seraphiel-brain: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.seraphiel/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.seraphiel/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.seraphiel/auth.json
              fi
            ''}
          ''}
          ...
          # Seraphiel reads $SERAPHIEL_HOME/.env at startup via load_seraphiel_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.seraphiel/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'SERAPHIEL_NIX_ENV_EOF'
    ${envFileContent}
    SERAPHIEL_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.seraphiel/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.seraphiel/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seraphiel/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.seraphiel-brain = {
          description = "Seraphiel Brain Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            SERAPHIEL_HOME = "${cfg.stateDir}/.seraphiel";
            SERAPHIEL_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $SERAPHIEL_HOME/.env by the activation script. load_seraphiel_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/seraphiel"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the seraphiel group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.seraphiel-brain = {
          description = "Seraphiel Brain Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SERAPHIEL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SERAPHIEL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env SERAPHIEL_UID="$SERAPHIEL_UID" \
                --env SERAPHIEL_GID="$SERAPHIEL_GID" \
                --env SERAPHIEL_HOME=${containerDataDir}/.seraphiel \
                --env SERAPHIEL_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/seraphiel gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
