# nix/seraphiel-brain.nix — Overridable Seraphiel Brain package
#
...
let
  nodejs = nodejs_22;
  seraphielVenv = callPackage ./python.nix {
  ...
  seraphielTui = callPackage ./tui.nix {
    inherit seraphielNpmLib;
  };

  seraphielWeb = callPackage ./web.nix {
    inherit seraphielNpmLib;
  };
  ...
  # to a dev checkout — the loader reads them from SERAPHIEL_BUNDLED_PLUGINS.
  ...
  # at by SERAPHIEL_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${seraphielVenv}/${sitePackagesPath}')
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in seraphiel sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${seraphielVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/seraphiel-brain"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "seraphiel-brain: installing Python dependencies..."
