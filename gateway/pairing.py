...
from seraphiel_constants import get_seraphiel_dir, get_seraphiel_home
...
PAIRING_DIR = get_seraphiel_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from seraphiel_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{SERAPHIEL_HOME}/pairing`` while newer code/docs may
    write ``{SERAPHIEL_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<SERAPHIEL_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<SERAPHIEL_HOME>/pairing/``
    directory (backward-compat for the ``seraphiel pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp SERAPHIEL_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from seraphiel_constants import get_seraphiel_home
            self._dir = get_seraphiel_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `seraphiel` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> seraphiel pairing approve ...` as root, "
                    "re-run with `docker exec -u seraphiel <container> ...` and "
                    "chown the existing file to the seraphiel user, or restart the "
