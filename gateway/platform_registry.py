...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``seraphiel setup`` display).
    ...
    # built-ins).  Used by ``seraphiel gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `seraphiel`
        # invocation -- including plain `seraphiel chat`, which never touches any
        ...
        # `seraphiel setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``seraphiel setup``/``gateway status``, channel
