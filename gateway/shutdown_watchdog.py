...
2. An event-loop heartbeat file at ``<SERAPHIEL_HOME>/state/gateway.heartbeat`` so
...
def _process_seraphiel_home() -> Path:
    """SERAPHIEL_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<SERAPHIEL_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_seraphiel_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from seraphiel_logging import drain_log_queue
