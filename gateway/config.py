...
# dashboard's pre-write mutation validation (seraphiel_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Seraphiel is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_seraphiel_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SERAPHIEL_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.seraphiel/config.yaml (primary user-facing config)
    3. ~/.seraphiel/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_seraphiel_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through seraphiel_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `seraphiel config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `seraphiel config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``seraphiel config set gateway.streaming.*``
