    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if seraphiel_home_str:
            takeover_path = Path(seraphiel_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(seraphiel_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded seraphiel-brain but never re-ran ``seraphiel setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../seraphiel-gateway.service"
    ...
    # that's the common case for seraphiel.
    ...
    # constant in seraphiel_cli/gateway.py.
    headroom = 30.0
