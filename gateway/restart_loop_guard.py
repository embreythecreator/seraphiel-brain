...
Defenses 1 and 2 (the ``_SERAPHIEL_GATEWAY`` guard on ``seraphiel gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.seraphiel.gateway")``,
...
State lives in ``<SERAPHIEL_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_seraphiel_home() / "gateway" / "restart_loop.json"
