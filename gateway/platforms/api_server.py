...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Seraphiel-Session-Id header; opt-in long-term memory scoping via X-Seraphiel-Session-Key header)
...
- GET  /v1/models                  — lists the versioned Seraphiel model name and configured model_routes aliases
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to seraphiel-brain
...
def _seraphiel_version() -> str:
    """Return the seraphiel-brain version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``seraphiel_cli.__version__`` (covers editable /
    ...
    try:
        ...
        return version("seraphiel-brain")
    except Exception:
        pass
    try:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_seraphiel_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted SERAPHIEL_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # seraphiel_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Seraphiel session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through seraphiel-brain's AIAgent.
    """
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            return None
        try:
            from seraphiel_cli.profiles import profiles_to_serve
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_seraphiel_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Seraphiel-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Seraphiel-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Seraphiel-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Seraphiel-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Seraphiel-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``seraphiel sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_seraphiel_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_seraphiel_home())
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the seraphiel-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Seraphiel-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "seraphiel-brain", "version": _seraphiel_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list seraphiel-brain and any configured model_routes aliases.
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Seraphiel version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Seraphiel AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Seraphiel-Session-Id",
                "session_key_header": "X-Seraphiel-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Seraphiel sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Seraphiel session row.
        ...
        return web.json_response({"object": "seraphiel.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "seraphiel.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Seraphiel-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Seraphiel-Session-Key"] = gateway_session_key
        return web.json_response(
            {
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Seraphiel-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Seraphiel-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Seraphiel-Session-Key.  This
        # is independent of X-Seraphiel-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Seraphiel-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Seraphiel-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Seraphiel-Session-Id rejected: "
            ...
            # conversation map to the same Seraphiel session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``seraphiel.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Seraphiel-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Seraphiel-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["seraphiel"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Seraphiel-Completed"] = "false"
            response_headers["X-Seraphiel-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Seraphiel-specific extras.
        response_data = {
            "id": completion_id,
            "object": "chat.completion",
            "created": created,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Seraphiel-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Seraphiel-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if cors:
            sse_headers.update(cors)
        if session_id:
            sse_headers["X-Seraphiel-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Seraphiel-Session-Key"] = gateway_session_key
        response = web.StreamResponse(status=200, headers=sse_headers)
        await response.prepare(request)
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: seraphiel.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: seraphiel.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Seraphiel-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Seraphiel-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Seraphiel-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Seraphiel-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "seraphiel.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Seraphiel-Relay
            ...
            # that is the exact surface the seraphiel-0day campaign abused to write
            # ~/.seraphiel/config.yaml and plant persistence. Sandboxing (Docker /
