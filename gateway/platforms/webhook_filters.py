...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.seraphiel to the active profile home."""
    ...
    if raw.startswith("~/.seraphiel/"):
        return seraphiel_home / raw.removeprefix("~/.seraphiel/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return seraphiel_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SERAPHIEL_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from seraphiel_constants import get_seraphiel_home

    scripts_root = (get_seraphiel_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.seraphiel" or raw_text.startswith("~/.seraphiel/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__seraphiel_ignore__") is True
        ):
            return False, None
