...
# Default location: {SERAPHIEL_HOME}/cache/images/ (legacy: image_cache/)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    for seraphiel_root in (_SERAPHIEL_HOME, _SERAPHIEL_ROOT):
        denied.append(seraphiel_root / ".env")
        denied.append(seraphiel_root / "auth.json")
        denied.append(seraphiel_root / "credentials")
        denied.append(seraphiel_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.seraphiel/.env,
    # ~/.seraphiel/auth.json, etc. — so the obvious prompt-injection sites
