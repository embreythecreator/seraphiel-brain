...
There are two ways to add a platform to the Seraphiel gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.seraphiel/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Seraphiel code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `seraphiel gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `seraphiel_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Seraphiel core.
...
And add it to the `seraphiel-gateway` composite:

```python
"seraphiel-gateway": {
    "includes": [..., "seraphiel-your-platform"]
}
```
...
## 12. Status Display (`seraphiel_cli/status.py`)
...
## 13. Gateway Setup Wizard (`seraphiel_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ seraphiel_cli/ toolsets.py \
  --include="*.py" -l | sort -u
