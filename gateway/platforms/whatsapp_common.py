...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "✶ *Seraphiel Brain*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SERAPHIEL_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/seraphiel), this function
    mirrors the bridge source to a writable SERAPHIEL_HOME location and returns that
    ...
    # Try SERAPHIEL_HOME location first
    seraphiel_home = get_seraphiel_home()
    seraphiel_home_bridge = seraphiel_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SERAPHIEL_HOME if needed
    if seraphiel_home_bridge.exists():
        return seraphiel_home_bridge

    # Mirror the bridge source to SERAPHIEL_HOME
    try:
        seraphiel_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            seraphiel_home_bridge,
            dirs_exist_ok=False,
        )
        return seraphiel_home_bridge
    except Exception:
