...
``{title, url, description, position}`` rows every other Seraphiel web
provider produces.
...
prefers Seraphiel-managed xAI Grok OAuth (via ``seraphiel auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.seraphiel/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``seraphiel tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `seraphiel auth` to sign in with "
