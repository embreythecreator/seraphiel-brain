...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Seraphiel Brain')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Seraphiel Brain" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.seraphiel/.env,
    ...
    print("  Start the gateway:  seraphiel gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`seraphiel photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.seraphiel/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `seraphiel photon telemetry on` / `seraphiel photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.seraphiel/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  seraphiel gateway restart")
    return 0
