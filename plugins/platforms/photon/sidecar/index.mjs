// Seraphiel Brain — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Seraphiel): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Seraphiel -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Seraphiel-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `seraphiel photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Seraphiel can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Seraphiel'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Seraphiel allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Seraphiel, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-seraphiel-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Seraphiel supplied them so a known-good
