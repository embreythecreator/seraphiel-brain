...
Small Node helper that bridges Seraphiel Brain to Photon's Spectrum SDK
(`spectrum-ts`).  Seraphiel is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Seraphiel-Sidecar-Token`)
...
The Seraphiel plugin's `seraphiel photon setup` command runs `npm install`
here automatically.
