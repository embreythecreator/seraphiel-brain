...
TypeScript-only, Seraphiel runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Seraphiel-Sidecar-Token`.
...
`seraphiel photon setup` does, in order:
...
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.seraphiel/.env` so the
...
There is no separate `login` command; like every other Seraphiel channel,
...
Run `seraphiel photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.seraphiel/.env` (the same place every other
...
Management metadata lives in `~/.seraphiel/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `seraphiel photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `seraphiel photon status`, a DM and a group roundtrip,
