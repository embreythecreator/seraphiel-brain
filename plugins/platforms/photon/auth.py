...
Credential storage mirrors every other Seraphiel channel:

    * runtime SDK creds  -> ``~/.seraphiel/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.seraphiel/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a seraphiel dependency
...
# registers Seraphiel as its own client_id.
...
# Default name of the project Seraphiel provisions for the operator.
DEFAULT_PROJECT_NAME = "Seraphiel Brain"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of seraphiel-brain.

def _auth_json_path() -> Path:
    """Resolve ``~/.seraphiel/auth.json`` honouring the active Seraphiel profile."""
    try:
        from seraphiel_constants import get_seraphiel_home
        return Path(get_seraphiel_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.seraphiel")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.seraphiel/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.seraphiel/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.seraphiel/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: seraphiel_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `seraphiel photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `seraphiel photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `seraphiel photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `seraphiel photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `seraphiel photon setup`)"
