...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("SERAPHIEL_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("SERAPHIEL_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
...
_QR_CODE_PAGE = "https://work.weixin.qq.com/ai/qc/gen?source=seraphiel&scode="
...
def qr_scan_for_bot_info(
    *,
    ...
    generate_url = f"{_QR_GENERATE_URL}?source=seraphiel"

    # ── Step 1: Fetch QR code ──
    print("  Connecting to WeCom...", end="", flush=True)
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "SeraphielAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "SeraphielAgent/1.0"})
...
# wizard + _PLATFORMS["wecom"] static dict in seraphiel_cli/gateway.py, and the
...
def interactive_setup() -> None:
    ...
    Replaces seraphiel_cli/gateway.py::_setup_wecom and the static
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'seraphiel-brain[wecom]'",
