"""
IRC Platform Adapter for Seraphiel Brain.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Seraphiel agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "seraphiel-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Seraphiel Brain")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Seraphiel Brain shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: seraphiel_, seraphiel_1, seraphiel_2...
...
def check_requirements() -> bool:
    ...
    # seraphiel setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `seraphiel gateway setup` flow for the IRC platform.

    Lazy-imports ``seraphiel_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Seraphiel to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. seraphiel-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #seraphiel — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.seraphiel/.env")
    print_info("Restart the gateway for changes to take effect: seraphiel gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "seraphiel-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "seraphiel-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Seraphiel Brain (cron)")
