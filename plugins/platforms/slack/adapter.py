...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `seraphiel slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_seraphiel_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Seraphiel does not currently need to route them into the agent loop.
            ...
            # /model work directly without /seraphiel prefix). A single regex
            ...
            # manifest (see `seraphiel slack manifest`). In Socket Mode, Slack
            ...
            from seraphiel_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/seraphiel$")

            @self._app.command(_slash_pattern)
            async def handle_seraphiel_command(ack, command):
            ...
            for _action_id in (
                "seraphiel_approve_once",
                "seraphiel_approve_session",
                "seraphiel_approve_always",
                "seraphiel_deny",
            ):
            ...
            for _action_id in (
                "seraphiel_confirm_once",
                "seraphiel_confirm_always",
                "seraphiel_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("seraphiel_feedback")(self._handle_feedback_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Seraphiel handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Seraphiel session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Seraphiel owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if original_text.startswith("!"):
            try:
                from seraphiel_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@seraphiel`` still resolve.
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "seraphiel_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "seraphiel_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "seraphiel_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "seraphiel_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "seraphiel_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "seraphiel_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "seraphiel_confirm_once": "once",
            "seraphiel_confirm_always": "always",
            "seraphiel_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "seraphiel_approve_once": "once",
            "seraphiel_approve_session": "session",
            "seraphiel_approve_always": "always",
            "seraphiel_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/seraphiel <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/seraphiel
        ...
        if slash_name in {"seraphiel", ""}:
            # Legacy /seraphiel <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from seraphiel_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/seraphiel   ``).
        ...
        # "Running /cmd…" ack shown by handle_seraphiel_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/seraphiel <question>" must produce public replies so
...
# ``seraphiel_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``seraphiel_cli/setup.py::_setup_slack``.
    """
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``seraphiel_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'seraphiel-brain[slack]'",
        # Interactive setup wizard — replaces seraphiel_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in seraphiel_cli/gateway.py.
