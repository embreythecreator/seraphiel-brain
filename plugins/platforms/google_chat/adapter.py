...
# ``model_tools`` import time, so ``seraphiel status``, ``seraphiel chat``, etc.
...
# ``seraphiel_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Seraphiel is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a seraphiel MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "seraphiel-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SERAPHIEL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _seraphiel_home = _get_seraphiel_home()
        except (ModuleNotFoundError, ImportError):
            _seraphiel_home = _Path.home() / ".seraphiel"
        self._thread_count_store = _ThreadCountStore(
            _seraphiel_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Seraphiel is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SERAPHIEL_HOME", str(_Path.home() / ".seraphiel"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SERAPHIEL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a seraphiel MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Seraphiel is thinking…' marker message.
        ...
        "Seraphiel is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Seraphiel is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Seraphiel is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Seraphiel is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``seraphiel setup``.

    The setup wizard at ``seraphiel_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.seraphiel/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. seraphiel-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.seraphiel/.env")
    print_info("Restart the gateway: seraphiel gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``seraphiel cron`` running as a
    separate process from ``seraphiel gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Seraphiel plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'seraphiel-brain[google_chat]'",
        ...
        # the "Seraphiel is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Seraphiel is thinking…' marker message "
