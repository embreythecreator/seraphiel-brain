"""
LINE Messaging API platform adapter for Seraphiel Brain.
...
to Seraphiel Brain. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SERAPHIEL_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            seraphiel_home = Path.home().joinpath(".seraphiel").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``seraphiel status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``seraphiel status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``seraphiel setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.seraphiel/.env`` via ``seraphiel_cli.config``.
    """
    print()
    ...
    try:
        from seraphiel_cli.config import get_env_var, set_env_var
    except ImportError:
        print("seraphiel_cli.config not available; set LINE_* vars manually in ~/.seraphiel/.env")
        return
