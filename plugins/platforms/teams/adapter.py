"""
Microsoft Teams platform adapter for Seraphiel Brain.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Seraphiel"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; SeraphielAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        seraphiel_action = data.get("seraphiel_action", "")
        session_key = data.get("session_key", "")

        if not seraphiel_action or not session_key:
        ...
        choice = choice_map.get(seraphiel_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="seraphiel_approve",
            data={**btn_data_base, "seraphiel_action": "approve_once"}, style="positive",
        )]
        if not smart_denied:
            actions.append(ExecuteAction(
                title="Allow Session", verb="seraphiel_approve",
                data={**btn_data_base, "seraphiel_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="seraphiel_approve",
                    data={**btn_data_base, "seraphiel_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="seraphiel_approve",
            data={**btn_data_base, "seraphiel_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Seraphiel\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.seraphiel/.env")
