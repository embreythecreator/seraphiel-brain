"""SimpleX Chat platform adapter (Seraphiel plugin).
...
This adapter ships as a Seraphiel platform plugin under
``plugins/platforms/simplex/``. The Seraphiel plugin loader scans the
...
Optional environment variables:
    ...
    SERAPHIEL_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``seraphiel setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Seraphiel — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SERAPHIEL_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``seraphiel setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.seraphiel/.env`` via
    ...
    try:
        ...
        print(
            "seraphiel_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.seraphiel/.env"
        )
        return
