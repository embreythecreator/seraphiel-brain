"""
Email platform adapter for the Seraphiel gateway.

Allows users to interact with Seraphiel by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _seraphiel_version = "0"
        imap.xatom(
            "ID",
            f'("name" "seraphiel-brain" "version" "{_seraphiel_version}" '
            '"vendor" "embreythecreator" '
            '"support-email" "noreply@embreythecreator.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `seraphiel config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `seraphiel gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Seraphiel Brain")
        ...
        msg_id = f"<seraphiel-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# seraphiel_cli/gateway.py, and the _send_email dispatch in
