"""ntfy platform adapter (Seraphiel plugin).
...
a Seraphiel dependency.

This adapter ships as a Seraphiel platform plugin under
``plugins/platforms/ntfy/``. The Seraphiel plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "seraphiel-in"              # subscribe topic (incoming)
          publish_topic: "seraphiel-out"     # optional — defaults to topic
...
_ECHO_TAG = "seraphiel-brain"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``seraphiel cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Seraphiel dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `seraphiel gateway status` without
