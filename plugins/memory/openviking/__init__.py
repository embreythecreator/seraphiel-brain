...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Seraphiel peer ID in OpenViking (default: seraphiel)
...
_DEFAULT_AGENT = "seraphiel"
_AGENT_PROMPT_LABEL = "Seraphiel peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "SERAPHIEL_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Seraphiel slash-skill scaffolding before sending content to OpenViking.
...
def _load_seraphiel_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("SERAPHIEL_HOME", "")).expanduser() if os.environ.get("SERAPHIEL_HOME") else Path.home() / ".seraphiel"
...
def _save_seraphiel_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Seraphiel session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Seraphiel only", "write values only to Seraphiel .env"),
    ...
    _save_seraphiel_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Seraphiel .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See seraphiel-brain#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_seraphiel_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Seraphiel peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, seraphiel_home: str, config: dict) -> None:
        ...
        from seraphiel_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        seraphiel_home_path = Path(seraphiel_home)
        env_path = seraphiel_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_seraphiel_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Seraphiel run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Seraphiel' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Seraphiel canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See seraphiel-brain#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/seraphiel/.overview.md).
