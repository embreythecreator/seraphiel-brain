...
_DEFAULT_CONTAINER_TAG = "seraphiel"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=seraphiel"
...
def _load_supermemory_config(seraphiel_home: str) -> dict:
    config = _default_config()
    config_path = Path(seraphiel_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, seraphiel_home: str) -> None:
    config_path = Path(seraphiel_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "seraphiel"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Seraphiel writes into the "Seraphiel" Space in the Supermemory
        ...
        merged = {"sm_source": "seraphiel", **(metadata or {})}
...
def _resolve_container_tag_for_setup(seraphiel_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(seraphiel_home)
...
def _probe_supermemory_connection(api_key: str, seraphiel_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(seraphiel_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(seraphiel_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `seraphiel memory setup`.
        # All other options are documented for $SERAPHIEL_HOME/supermemory.json
    ...
    def save_config(self, values, seraphiel_home):
        ...
        _save_supermemory_config(sanitized, seraphiel_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, seraphiel_home)
    ...
    def post_setup(self, seraphiel_home: str, config: dict) -> None:
        ...
        from seraphiel_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(seraphiel_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from seraphiel_constants import get_seraphiel_home
        self._seraphiel_home = kwargs.get("seraphiel_home") or str(get_seraphiel_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._seraphiel_home)
