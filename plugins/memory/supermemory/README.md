...
- Hosted: API key from [app.supermemory.ai/integrations?connect=seraphiel](http://app.supermemory.ai/integrations?connect=seraphiel)
...
Before running `seraphiel memory setup`, add the local endpoint to
...
Then run `seraphiel memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SERAPHIEL_HOME/supermemory.json`
...
| `container_tag` | `seraphiel` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `seraphiel-{identity}` → `seraphiel-coder`). |
...
`https://api.supermemory.ai`. Seraphiel resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: seraphiel`, and document writes stamp
`metadata.sm_source: seraphiel`. This is a **functional routing key, not telemetry**:
it groups Seraphiel-written memories into a dedicated "Seraphiel" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Seraphiel profile:

```json
{
  "container_tag": "seraphiel-{identity}"
}
```

For a profile named `coder`, this resolves to `seraphiel-coder`. The default profile resolves to `seraphiel-default`. Without `{identity}`, all profiles share the same container.
