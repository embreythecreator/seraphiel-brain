...
**Deployment shapes** (`seraphiel memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Seraphiel to your own Telegram/Discord/etc.
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-seraphiel-brain`.
