...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/seraphiel>
...
```bash
seraphiel memory setup honcho   # configure Honcho directly (works on a fresh install)
seraphiel memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
seraphiel config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.seraphiel/.env
```

> `seraphiel honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `seraphiel memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `seraphiel model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SERAPHIEL_HOME/honcho.json` | Profile-local (isolated Seraphiel instances) |
| 2 | `~/.seraphiel/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Seraphiel profile: `seraphiel` (default) or `seraphiel_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `seraphiel honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Seraphiel session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`seraphiel-brain`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`seraphiel`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-seraphiel-brain`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening seraphiel in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Seraphiel session ID (timestamp + hex). Every `seraphiel` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Seraphiel profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`seraphiel`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Seraphiel profile: `seraphiel` (default) or `seraphiel_<profile>` (e.g. `seraphiel -p coder` -> host key `seraphiel_coder`). Older `seraphiel.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SERAPHIEL_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `seraphiel-brain`) |
...
|---------|-------------|
| `seraphiel memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `seraphiel honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `seraphiel memory setup`) |
| `seraphiel honcho status` | Show resolved config for active profile |
| `seraphiel honcho enable` / `disable` | Toggle Honcho for active profile |
| `seraphiel honcho mode <mode>` | Change recall or observation mode |
| `seraphiel honcho peer --user <name>` | Update user peer name |
| `seraphiel honcho peer --ai <name>` | Update AI peer name |
| `seraphiel honcho tokens --context <N>` | Set context token budget |
| `seraphiel honcho tokens --dialectic <N>` | Set dialectic max chars |
| `seraphiel honcho map <name>` | Map current directory to a session name |
| `seraphiel honcho sync` | Create host blocks for all Seraphiel profiles |
