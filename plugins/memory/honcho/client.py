...
def resolve_active_host() -> str:
    ...
    Resolution order:
      ...
      2. Active profile name via profiles system -> ``seraphiel.<profile>``
      3. Fallback: ``"seraphiel"`` (default profile)
    """
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Seraphiel system prompt
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Seraphiel session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Seraphiel session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Seraphiel session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        # the "go run seraphiel honcho setup" hint they used to.
        try:
