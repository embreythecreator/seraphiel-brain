...
``seraphiel://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Seraphiel OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "seraphiel-brain"
...
def begin_authorization(
    ...
    *,
    ...
    (``seraphiel-desktop`` / ``seraphiel-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Seraphiel.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SERAPHIEL_HOME override can't reach.
