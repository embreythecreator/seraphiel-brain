...
def _save_mem0_json(seraphiel_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(seraphiel_home) / "mem0.json"
    existing = {}
...
def _setup_platform(seraphiel_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "seraphiel-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "seraphiel"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.seraphiel/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.seraphiel/.env (or unset it) or Seraphiel will keep "
    ...
    provider.save_config(provider_config, seraphiel_home)

    if env_writes:
        _write_env(Path(seraphiel_home) / ".env", env_writes)
...
def _setup_selfhosted(seraphiel_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "seraphiel-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "seraphiel")
...
def _setup_oss(seraphiel_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(seraphiel_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "seraphiel-user")
    ...
    if env_writes:
        _write_env(Path(seraphiel_home) / ".env", env_writes)
    _save_mem0_json(seraphiel_home, {"mode": "oss", "user_id": user_id, "agent_id": "seraphiel", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, seraphiel_home: str) -> str:
...
_PGVECTOR_CONTAINER = "seraphiel-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "seraphiel"
...
def _setup_oss_interactive(seraphiel_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], seraphiel_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], seraphiel_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'seraphiel-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "seraphiel-user")

    agent_id = input("  Agent ID [seraphiel]: ").strip()
    agent_id = agent_id or "seraphiel"
    ...
    if env_writes:
        _write_env(Path(seraphiel_home) / ".env", env_writes)
    _save_mem0_json(seraphiel_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(seraphiel_home: str, config: dict) -> None:
    """Entry point called by seraphiel memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(seraphiel_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(seraphiel_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(seraphiel_home, config, flags)
        return
