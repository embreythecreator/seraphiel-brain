...
Configuration
-------------
Secret (lives in $SERAPHIEL_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $SERAPHIEL_HOME/mem0.json, set via `seraphiel memory
setup`):
  ...
  agent_id           — Agent identifier (default: seraphiel)
...
_DEFAULT_USER_ID = "seraphiel-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $SERAPHIEL_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "seraphiel"),
        "oss": {},
    }
    ...
    config_path = get_seraphiel_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, seraphiel_home):
        """Write config to $SERAPHIEL_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, seraphiel_home: str, config: dict) -> None:
        ...
        post_setup(seraphiel_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $SERAPHIEL_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "seraphiel")
