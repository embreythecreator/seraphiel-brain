...
Behavioral settings live in `$SERAPHIEL_HOME/mem0.json` (set them via `seraphiel memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.seraphiel/.env`.
...
| `user_id` | `seraphiel-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
   or in `$SERAPHIEL_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Seraphiel session and call `mem0_search` — it connects to your server.
...
```bash
seraphiel memory setup mem0 --mode oss \
...
```bash
seraphiel memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SERAPHIEL_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.seraphiel/mem0_qdrant"}}
  }
}
```
...
```bash
seraphiel memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SERAPHIEL_HOME/mem0.json`).
