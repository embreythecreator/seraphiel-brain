...
Optional Seraphiel observability plugin that maps Seraphiel observer hooks to
...
not replace Seraphiel Brain's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Seraphiel emit NeMo Relay lifecycle events for
the work Seraphiel already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Seraphiel Brain can:

- Preserve Seraphiel execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
seraphiel plugins enable observability/nemo_relay
```

The `SERAPHIEL_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SERAPHIEL_HOME` that the
agent run will use:

```bash
env SERAPHIEL_HOME=/tmp/seraphiel-nemo-relay-test \
  seraphiel plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SERAPHIEL_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SERAPHIEL_HOME` is the Seraphiel profile/config home used by both
`seraphiel plugins enable ...` and the later `seraphiel chat ...` run. If unset,
Seraphiel uses the user's default home, usually `~/.seraphiel`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SERAPHIEL_HOME=/tmp/seraphiel-nemo-relay-test
seraphiel plugins enable observability/nemo_relay
seraphiel chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `seraphiel` command you run is built from the
...
uv run seraphiel plugins enable observability/nemo_relay
uv run seraphiel chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/seraphiel_brain-*.whl
...
The plugin can configure exporters directly from `SERAPHIEL_NEMO_RELAY_*`
...
```bash
export SERAPHIEL_NEMO_RELAY_ATOF_ENABLED=1
export SERAPHIEL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SERAPHIEL_NEMO_RELAY_ATIF_ENABLED=1
export SERAPHIEL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SERAPHIEL_NEMO_RELAY_ATOF_FILENAME`
- `SERAPHIEL_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SERAPHIEL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SERAPHIEL_NEMO_RELAY_ATIF_AGENT_NAME`
- `SERAPHIEL_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SERAPHIEL_NEMO_RELAY_ATIF_MODEL_NAME`
- `SERAPHIEL_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Seraphiel at it:

```bash
export SERAPHIEL_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `SERAPHIEL_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`SERAPHIEL_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`SERAPHIEL_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Seraphiel does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Seraphiel keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Seraphiel routes LLM and tool
...
Seraphiel feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Seraphiel-owned
...
with this Seraphiel-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Seraphiel rejects
...
Seraphiel activates these plugins before registering its managed LLM and tool
...
activation API, Seraphiel logs an actionable warning and continues with the
...
For the full generic Seraphiel middleware contract, see
...
export SERAPHIEL_HOME=/tmp/seraphiel-nemo-relay-docs/seraphiel-home
mkdir -p "$SERAPHIEL_HOME"

cat > "$SERAPHIEL_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Seraphiel session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SERAPHIEL_NEMO_RELAY_ATOF_ENABLED=1
export SERAPHIEL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/seraphiel-nemo-relay-docs/subagent/atof
export SERAPHIEL_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SERAPHIEL_NEMO_RELAY_ATOF_MODE=overwrite
export SERAPHIEL_NEMO_RELAY_ATIF_ENABLED=1
export SERAPHIEL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/seraphiel-nemo-relay-docs/subagent/atif
export SERAPHIEL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SERAPHIEL_NEMO_RELAY_ATIF_AGENT_NAME='Seraphiel Brain E2E'
export SERAPHIEL_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SERAPHIEL_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"seraphiel.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Seraphiel Brain E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Seraphiel dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/seraphiel-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/seraphiel-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/seraphiel-nemo-relay-docs/workdir/beta.txt
cd /tmp/seraphiel-nemo-relay-docs/workdir

export SERAPHIEL_NEMO_RELAY_ATOF_ENABLED=1
export SERAPHIEL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/seraphiel-nemo-relay-docs/parallel/atof
export SERAPHIEL_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SERAPHIEL_NEMO_RELAY_ATOF_MODE=overwrite
export SERAPHIEL_NEMO_RELAY_ATIF_ENABLED=1
export SERAPHIEL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/seraphiel-nemo-relay-docs/parallel/atif
export SERAPHIEL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SERAPHIEL_NEMO_RELAY_ATIF_AGENT_NAME='Seraphiel Brain E2E'
export SERAPHIEL_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Seraphiel sessions map to `agent` scopes.
- Seraphiel API request hooks map to `llm` scope start/end events.
- Seraphiel tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Seraphiel execution middleware to hand
...
Enable it for Seraphiel:

```bash
export SERAPHIEL_NEMO_RELAY_PLUGINS_TOML=/tmp/seraphiel-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Seraphiel routes execution
through these boundaries:

```text
Seraphiel provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Seraphiel provider adapter next_call(...)

Seraphiel tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Seraphiel tool dispatcher next_call(...)
```
...
middleware for a local Seraphiel run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export SERAPHIEL_HOME=/tmp/seraphiel-middleware-test/seraphiel-home
mkdir -p "$SERAPHIEL_HOME" /tmp/seraphiel-middleware-test/nemo-relay
...
cat > /tmp/seraphiel-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/seraphiel-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/seraphiel-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Seraphiel Middleware E2E"
agent_version = "local"
...
export SERAPHIEL_NEMO_RELAY_PLUGINS_TOML=/tmp/seraphiel-middleware-test/nemo-relay/plugins.toml
