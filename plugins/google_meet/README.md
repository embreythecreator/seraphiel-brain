...
Let the seraphiel agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where seraphiel runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `seraphiel meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `seraphiel meet` CLI |
...
| `cli.py` | `seraphiel meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SERAPHIEL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `seraphiel meet node {run,list,approve,remove,status,ping}` |
...
seraphiel meet install                                      # pip + Chromium
seraphiel meet setup                                        # preflight
seraphiel meet auth                                         # optional
seraphiel meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
seraphiel meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.seraphiel/.env
seraphiel meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
seraphiel meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
seraphiel meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.seraphiel/.env
seraphiel meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, seraphiel will **not** switch your system audio input automatically — the
...
seraphiel plugins enable google_meet
seraphiel meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
seraphiel meet node ping my-mac
