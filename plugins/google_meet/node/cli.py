"""`seraphiel meet node ...` subcommand tree.

Wired into the existing ``seraphiel meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``seraphiel meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="seraphiel-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``seraphiel meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             seraphiel meet node approve <name> ws://<host>:{args.port} {token}")
        try:
