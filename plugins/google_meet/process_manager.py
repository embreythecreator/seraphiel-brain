...
session-scoped state file under ``$SERAPHIEL_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $SERAPHIEL_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_seraphiel_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "Seraphiel Brain",
    ...
    If a bot is already running for this seraphiel install, leave it first —
    ...
    env = os.environ.copy()
    env["SERAPHIEL_MEET_URL"] = url
    env["SERAPHIEL_MEET_OUT_DIR"] = str(out)
    env["SERAPHIEL_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["SERAPHIEL_MEET_HEADED"] = "1"
    if auth_state:
        env["SERAPHIEL_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["SERAPHIEL_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if SERAPHIEL_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["SERAPHIEL_MEET_MODE"] = mode
    if realtime_model:
        env["SERAPHIEL_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["SERAPHIEL_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["SERAPHIEL_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["SERAPHIEL_MEET_REALTIME_KEY"] = realtime_api_key
