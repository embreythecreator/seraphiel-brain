...
request omits ``thinking``.  Seraphiel' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Seraphiel reasoning
...
(per Z.AI / BigModel docs).  Seraphiel' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Seraphiel reasoning effort onto GLM-5.2's native ``high``/``max``.
...
zai = ZaiProfile(
    ...
    env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY", "ZAI_GLM_API_KEY"),
