/**
 * Seraphiel Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.seraphiel/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SERAPHIEL_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SERAPHIEL_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "seraphiel-kanban-dot-triage",
    todo: "seraphiel-kanban-dot-todo",
    ready: "seraphiel-kanban-dot-ready",
    running: "seraphiel-kanban-dot-running",
    blocked: "seraphiel-kanban-dot-blocked",
    done: "seraphiel-kanban-dot-done",
    archived: "seraphiel-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-seraphiel-task";
  ...
  const DOCS_URL = "https://seraphiel-brain.embreythecreator.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://seraphiel-brain.embreythecreator.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "seraphiel.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="seraphiel-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])seraphiel-kanban-md-code\1/i.test(attrs)) {
      return ' class="seraphiel-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "seraphiel-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "seraphiel-kanban-md",
  ...
  // standard `drop` event and our `seraphiel-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("seraphiel-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("seraphiel-kanban-column--drop");
          if (target) target.classList.add("seraphiel-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("seraphiel-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("seraphiel-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("seraphiel-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SERAPHIEL_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "seraphiel-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "seraphiel-kanban-attention",
        "seraphiel-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "seraphiel-kanban-attention-bar" },
        h("span", { className: "seraphiel-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "seraphiel-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "seraphiel-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "seraphiel-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "seraphiel-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "seraphiel-kanban-attention-row",
                  "seraphiel-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "seraphiel-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "seraphiel-kanban-attention-row-id" }, task.id),
                h("span", { className: "seraphiel-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "seraphiel-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "seraphiel-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "seraphiel-kanban-diag-action-btn",
      action.suggested ? "seraphiel-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " seraphiel-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".seraphiel-kanban-drawer-comment-row input, .seraphiel-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "seraphiel-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("seraphiel-kanban-diag", sevClass) },
      h("div", { className: "seraphiel-kanban-diag-header" },
        h("span", { className: "seraphiel-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "seraphiel-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "seraphiel-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "seraphiel-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "seraphiel-kanban-diag-data-row" },
                  h("span", { className: "seraphiel-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "seraphiel-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "seraphiel-kanban-diag-data-row" },
                h("span", { className: "seraphiel-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "seraphiel-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "seraphiel-kanban-diag-reassign-row" },
            h("span", { className: "seraphiel-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "seraphiel-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "seraphiel-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "seraphiel-kanban-diag-msg",
              msg.ok ? "seraphiel-kanban-diag-msg--ok" : "seraphiel-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "seraphiel-kanban-section" },
      h("div", { className: "seraphiel-kanban-section-head-row" },
        h("span", { className: "seraphiel-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "seraphiel-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "seraphiel-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "seraphiel-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "seraphiel-kanban-docs-link",
      title: "Open Seraphiel Kanban docs in a new tab",
      "aria-label": "Seraphiel Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `seraphiel kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "seraphiel-kanban-msg-ok" : "seraphiel-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "seraphiel-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "seraphiel-kanban-boardswitcher" },
      h("div", { className: "seraphiel-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "seraphiel-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "seraphiel-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "seraphiel-kanban-dialog-title" },
        ...
        h("div", { className: "seraphiel-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Seraphiel profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "seraphiel-kanban-bulk" },
      h("span", { className: "seraphiel-kanban-bulk-count" },
      ...
      h("div", { className: "seraphiel-kanban-bulk-priority",
      ...
      h("div", { className: "seraphiel-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Seraphiel profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "seraphiel-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("seraphiel-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("seraphiel-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "seraphiel-kanban-trash",
        dragOver ? "seraphiel-kanban-trash--drop" : "",
        props.draggingTaskId ? "seraphiel-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "seraphiel-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "seraphiel-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".seraphiel-kanban-card")) return true;
      if (target.closest && target.closest(".seraphiel-kanban-column-add")) return true;
      if (target.closest && target.closest(".seraphiel-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("seraphiel-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("seraphiel-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".seraphiel-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "seraphiel-kanban-columns",
        isScrollable ? "seraphiel-kanban-columns--scrollable" : "",
        isPanning ? "seraphiel-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("seraphiel-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("seraphiel-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "seraphiel-kanban-column",
        dragOver ? "seraphiel-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "seraphiel-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "seraphiel-kanban-col-check",
        ...
        h("span", { className: cn("seraphiel-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "seraphiel-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "seraphiel-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "seraphiel-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "seraphiel-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "seraphiel-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "seraphiel-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "seraphiel-kanban-lane" },
                  h("div", { className: "seraphiel-kanban-lane-head" },
                    h("span", { className: "seraphiel-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "seraphiel-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "seraphiel-kanban-card--stale-red";
    if (age >= tier.amber) return "seraphiel-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".seraphiel-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "seraphiel-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "seraphiel-kanban-card",
        props.selected ? "seraphiel-kanban-card--selected" : "",
        props.failed ? "seraphiel-kanban-card--failed" : "",
        props.draggingSource ? "seraphiel-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "seraphiel-kanban-card-content" },
          h("div", { className: "seraphiel-kanban-card-row" },
            h("label", {
              className: "seraphiel-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "seraphiel-kanban-card-check",
            ...
            h("span", { className: "seraphiel-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or seraphiel kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "seraphiel-kanban-warning-badge",
                    "seraphiel-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "seraphiel-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "seraphiel-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "seraphiel-kanban-progress",
                    progress.done === progress.total ? "seraphiel-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "seraphiel-kanban-needs-assignee",
          ...
          h("div", { className: "seraphiel-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "seraphiel-kanban-card-row seraphiel-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "seraphiel-kanban-assignee",
                            title: `Assigned to Seraphiel profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "seraphiel-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "seraphiel-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "seraphiel-kanban-ago",
  ...
  // centered modal reusing the seraphiel-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "seraphiel-kanban-dialog seraphiel-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Seraphiel profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Seraphiel profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "seraphiel-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "seraphiel-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "seraphiel-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "seraphiel-kanban-drawer-close",
        ...
        data ? h("div", { className: "seraphiel-kanban-drawer-comment-foot" },
          h("div", {
            className: "seraphiel-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "seraphiel-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "seraphiel-kanban-section" },
      h("div", { className: "seraphiel-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "seraphiel-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "seraphiel-kanban-drawer-body" },
      h("div", { className: "seraphiel-kanban-drawer-title" },
        h("span", { className: cn("seraphiel-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "seraphiel-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "seraphiel-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "seraphiel-kanban-section" },
            h("div", { className: "seraphiel-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "seraphiel-kanban-section" },
            h("div", { className: "seraphiel-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "seraphiel-kanban-done-no-result seraphiel-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "seraphiel-kanban-section" },
            h("div", { className: "seraphiel-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "seraphiel-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "seraphiel-kanban-section" },
        h("div", { className: "seraphiel-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "seraphiel-kanban-comment" },
            h("div", { className: "seraphiel-kanban-comment-head" },
              h("span", { className: "seraphiel-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "seraphiel-kanban-diag-action-btn",
      ...
      h("div", { className: "seraphiel-kanban-section" },
        h("div", { className: "seraphiel-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "seraphiel-kanban-comment" },
                h("div", { className: "seraphiel-kanban-comment-head" },
                  h("span", { className: "seraphiel-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "seraphiel-kanban-comment-ago" },
      ...
      h("div", { className: "seraphiel-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "seraphiel-kanban-event",
              isDiag ? "seraphiel-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "seraphiel-kanban-event-header" },
                  h("span", { className: "seraphiel-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "seraphiel-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "seraphiel-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "seraphiel-kanban-event-header-plain" },
                  h("span", { className: "seraphiel-kanban-event-kind" }, e.kind),
                  h("span", { className: "seraphiel-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "seraphiel-kanban-event-phantom-row" },
                  h("span", { className: "seraphiel-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "seraphiel-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "seraphiel-kanban-event-payload" },
    ...
    return h("div", { className: "seraphiel-kanban-section" },
      h("div", { className: "seraphiel-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "seraphiel-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `seraphiel-kanban-run--${r.outcome || r.status || "ended"}`
          : "seraphiel-kanban-run--active";
        return h("div", { key: r.id, className: cn("seraphiel-kanban-run", outcomeClass) },
          h("div", { className: "seraphiel-kanban-run-head" },
            h("span", { className: "seraphiel-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "seraphiel-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "seraphiel-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "seraphiel-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "seraphiel-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "seraphiel-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "seraphiel-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "seraphiel-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "seraphiel-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "seraphiel-kanban-pre seraphiel-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "seraphiel-kanban-meta-row" },
      h("span", { className: "seraphiel-kanban-meta-label" }, props.label),
      h("span", { className: "seraphiel-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "seraphiel-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "seraphiel-kanban-meta-row" },
        h("span", { className: "seraphiel-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "seraphiel-kanban-meta-value seraphiel-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "seraphiel-kanban-meta-row" },
        h("span", { className: "seraphiel-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "seraphiel-kanban-meta-value seraphiel-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "seraphiel-kanban-section" },
      h("div", { className: "seraphiel-kanban-section-head-row" },
        h("span", { className: "seraphiel-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "seraphiel-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "seraphiel-kanban-section" },
      h("div", { className: "seraphiel-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "seraphiel-kanban-deps-row" },
        h("span", { className: "seraphiel-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "seraphiel-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "seraphiel-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "seraphiel-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "seraphiel-kanban-dep-chip-x",
      ...
      h("div", { className: "seraphiel-kanban-deps-row" },
        h("span", { className: "seraphiel-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "seraphiel-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "seraphiel-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "seraphiel-kanban-msg-ok"
          : "seraphiel-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "seraphiel-kanban-section" },
      ...
      h("div", { className: "seraphiel-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "seraphiel-kanban-home-sub seraphiel-kanban-home-sub--on"
              : "seraphiel-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SERAPHIEL_PLUGINS__ && typeof window.__SERAPHIEL_PLUGINS__.register === "function") {
    window.__SERAPHIEL_PLUGINS__.register("kanban", KanbanPage);
  }
})();
