...
Auto-tracks and cleans up ephemeral files created during Seraphiel Brain
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SERAPHIEL_HOME` and `/tmp/seraphiel-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SERAPHIEL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SERAPHIEL_HOME` or `/tmp/seraphiel-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SERAPHIEL_HOME/disk-cleanup/` is itself excluded
- `$SERAPHIEL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
