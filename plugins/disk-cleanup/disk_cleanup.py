"""disk_cleanup — ephemeral file cleanup for Seraphiel Brain.
...
Rules:
  ...
  - empty dirs    → always delete (under SERAPHIEL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SERAPHIEL_HOME and /tmp/seraphiel-*
Never touches: ~/.seraphiel/logs/ or any system directory.
"""
...
try:
    ...
    def get_seraphiel_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SERAPHIEL_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".seraphiel").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SERAPHIEL_HOME/logs/``."""
    return get_seraphiel_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SERAPHIEL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SERAPHIEL_HOME or ``/tmp/seraphiel-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    seraphiel_home = get_seraphiel_home()
    try:
        path.resolve().relative_to(seraphiel_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/seraphiel-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("seraphiel-"):
        return True
    return False
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SERAPHIEL_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        seraphiel_home = get_seraphiel_home()
        for parent in ("cron", "cronjobs"):
            base = seraphiel_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SERAPHIEL_HOME)")
        return False
...
def guess_category(path: Path) -> Optional[str]:
    ...
    seraphiel_home = get_seraphiel_home()
    try:
        rel = path.resolve().relative_to(seraphiel_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "seraphiel-brain",
        }:
            return None
        ...
        # Path isn't under SERAPHIEL_HOME (e.g. /tmp/seraphiel-*) — fall through.
        pass
