...
## Client ID
- Need to register a "Desktop app" OAuth client on a Seraphiel GCP project
...
## Token Lifecycle
- Store at `~/.seraphiel/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `seraphiel_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `seraphiel_cli/models.py` — Add Gemini model catalog
4. `seraphiel_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `seraphiel_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `seraphiel_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Seraphiel GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via SERAPHIEL_GEMINI_CLIENT_ID env var
...
- seraphiel-brain Copilot OAuth: `seraphiel_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
