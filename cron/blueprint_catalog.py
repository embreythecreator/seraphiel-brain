...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``seraphiel://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``seraphiel://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"seraphiel://blueprint/{quote(blueprint.key)}{qs}"
