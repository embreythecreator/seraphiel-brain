...
``seraphiel gateway restart`` (or ``launchctl kickstart ai.seraphiel.gateway``
or ``systemctl restart seraphiel-gateway``).  When the cron fires, the
...
``seraphiel cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``seraphiel gateway``, ``launchctl ... seraphiel-gateway``,
``systemctl ... seraphiel-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_SERAPHIEL_GATEWAY=1``, and
``seraphiel gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `seraphiel gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:seraphiel\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a seraphiel-gateway label. macOS launchd
    # labels look like `ai.seraphiel.gateway` / `seraphiel-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated seraphiel services (e.g.
    # `launchctl unload ai.seraphiel.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bseraphiel[.\-]?gateway)"
    # Branch C: systemctl ops on a seraphiel-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bseraphiel[.\-]?gateway)"
    # Branch D: pkill / kill targeting the seraphiel gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<SERAPHIEL_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.seraphiel/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_seraphiel_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `seraphiel gateway restart` from a shell outside "
