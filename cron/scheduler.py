...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Seraphiel profile.
    ...
    the scheduler's test/override hook and a context-local Seraphiel home override
    both point at the resolved profile directory so _get_seraphiel_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_seraphiel_home() callers agree on the same home.
    ...
    global _seraphiel_home
    prior_override = _seraphiel_home
    env_snapshot = os.environ.copy()

    from seraphiel_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    override_token = None
    try:
        override_token = set_seraphiel_home_override(profile_home)
        _seraphiel_home = profile_home
        logger.info(
            "Job '%s': using Seraphiel profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _seraphiel_home = prior_override
        if override_token is not None:
            reset_seraphiel_home_override(override_token)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    run_env["SERAPHIEL_HOME"] = str(_get_seraphiel_home())
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_seraphiel_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_seraphiel_home() / ".env"), override=True, encoding="latin-1")
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Seraphiel home override, scheduler _seraphiel_home hook, and temporary
