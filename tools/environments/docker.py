...
def _load_seraphiel_env_vars() -> dict[str, str]:
    """Load ~/.seraphiel/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Seraphiel profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale seraphiel-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=seraphiel-brain=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Seraphiel process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=seraphiel-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a seraphiel process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Seraphiel
    process leaves its container behind permanently because there's no
    subsequent Seraphiel process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=seraphiel-brain=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=seraphiel-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``SERAPHIEL_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("SERAPHIEL_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using SERAPHIEL_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'seraphiel'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``seraphiel-brain:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.seraphiel/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. seraphiel-brain:latest) already use /init as PID 1
        ...
        container_name = f"seraphiel-{uuid.uuid4().hex[:8]}"
        # Labels make seraphiel-created containers identifiable to:
        #   * the orphan reaper (`seraphiel-brain=1` for the global sweep filter)
        #   * future cross-process reuse (`seraphiel-task-id`, `seraphiel-profile`)
        #   * operators running `docker ps --filter label=seraphiel-brain=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Seraphiel profile is captured at
        ...
        label_args = [
            "--label", "seraphiel-brain=1",
            "--label", f"seraphiel-task-id={task_label}",
            "--label", f"seraphiel-profile={profile_name}",
        ]
        ...
        self._labels = {
            "seraphiel-brain": "1",
            "seraphiel-task-id": task_label,
            "seraphiel-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Seraphiel process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Seraphiel secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Seraphiel-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_seraphiel_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_seraphiel_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _SERAPHIEL_PROVIDER_ENV_BLOCKLIST)
        seraphiel_env = _load_seraphiel_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = seraphiel_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("seraphiel-task-id", "")
        profile_label = self._labels.get("seraphiel-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"seraphiel-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``seraphiel-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=seraphiel-brain=1",
                    "--filter", f"label=seraphiel-task-id={task_label}",
                    "--filter", f"label=seraphiel-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Seraphiel process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Seraphiel exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Seraphiel process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Seraphiel startup. That covers the
        ...
        # outlives Seraphiel processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"seraphiel-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``seraphiel /quit``
