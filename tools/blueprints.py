...
`seraphiel skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.seraphiel.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.seraphiel.blueprint`` is a mapping containing
    ...
    seraphiel = meta.get("seraphiel") if isinstance(meta, dict) else None
    blueprint = seraphiel.get("blueprint") if isinstance(seraphiel, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.seraphiel.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.seraphiel.blueprint`` block) they can hand
    to ``seraphiel skills publish`` to share. ``body`` is the plain-language
