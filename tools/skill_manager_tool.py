...
~/.seraphiel/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `seraphiel config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.seraphiel/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch SERAPHIEL_HOME and
    ...
    live profile-scoped SERAPHIEL_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`seraphiel curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`seraphiel curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.seraphiel/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Seraphiel profiles.
    ...
    try:
        root = get_default_seraphiel_root()
    except Exception:
        return matches
    ...
    # Default profile (~/.seraphiel/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.seraphiel/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`seraphiel -p "
            ...
            base += (
                ...
                f"Switch profiles (`seraphiel -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.seraphiel/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `seraphiel curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `seraphiel curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_seraphiel_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `seraphiel curator unpin <name>`. "
