#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Seraphiel desktop GUI.
...
``read_terminal`` it is gated on ``SERAPHIEL_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — SERAPHIEL_DESKTOP is set on the gateway the app spawns."""
    return (os.getenv("SERAPHIEL_DESKTOP") or "").strip().lower() in ("1", "true", "yes")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Seraphiel desktop GUI (the tabs mirroring terminal(background=true) runs). "
