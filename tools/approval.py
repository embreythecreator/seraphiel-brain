...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("SERAPHIEL_YOLO_MODE", ""))
...
# os.environ["SERAPHIEL_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export SERAPHIEL_INTERACTIVE.
_seraphiel_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "seraphiel_interactive",
    default=None,
)


def set_seraphiel_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["SERAPHIEL_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``SERAPHIEL_INTERACTIVE`` env var for legacy callers.
    """
    return _seraphiel_interactive_ctx.set("1" if interactive else "")


def reset_seraphiel_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_seraphiel_interactive_context`."""
    _seraphiel_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``SERAPHIEL_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _seraphiel_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("SERAPHIEL_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SERAPHIEL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SERAPHIEL_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SERAPHIEL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SERAPHIEL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SERAPHIEL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SERAPHIEL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("SERAPHIEL_CRON_SESSION"):
        return False
    if env_var_enabled("SERAPHIEL_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $SERAPHIEL_HOME, or by the resolved absolute
# active profile home path such as /home/seraphiel/.seraphiel/config.yaml. The
# resolved-absolute form is folded into the ~/.seraphiel/ patterns at detection
...
# go stale when SERAPHIEL_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SERAPHIEL_ENV_PATH = (
    r'(?:~\/\.seraphiel/|'
    r'(?:\$home|\$\{home\})/\.seraphiel/|'
    r'(?:\$seraphiel_home|\$\{seraphiel_home\})/)'
    r'\.env\b'
)
# ~/.seraphiel/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _SERAPHIEL_ENV_PATH; matches the SERAPHIEL_HOME override form as
# well as ~/.seraphiel/.
_SERAPHIEL_CONFIG_PATH = (
    r'(?:~\/\.seraphiel/|'
    r'(?:\$home|\$\{home\})/\.seraphiel/|'
    r'(?:\$seraphiel_home|\$\{seraphiel_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_SERAPHIEL_ENV_PATH}|'
    rf'{_SERAPHIEL_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # `seraphiel` and `gateway` (e.g. `seraphiel -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bseraphiel\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart seraphiel gateway (kills running agents)"),
    (r'\bseraphiel\s+update\b', "seraphiel update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `seraphiel gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart seraphiel-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart seraphiel-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(seraphiel|gateway|cli\.py)\b', "kill seraphiel/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill seraphiel` but not
    # `kill -9 $(pgrep -f seraphiel)` because the substitution is opaque
    ...
    # the `seraphiel gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.seraphiel.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(seraphiel|ai\.seraphiel)\b', "stop/restart seraphiel launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Seraphiel file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.seraphiel/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Seraphiel-managed security file (~/.seraphiel/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SERAPHIEL_CONFIG_PATH}|{_SERAPHIEL_ENV_PATH})', "in-place edit of Seraphiel config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SERAPHIEL_CONFIG_PATH}|{_SERAPHIEL_ENV_PATH})', "in-place edit of Seraphiel config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SERAPHIEL_CONFIG_PATH}|{_SERAPHIEL_ENV_PATH})', "in-place edit of Seraphiel config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.seraphiel/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / SERAPHIEL_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Seraphiel home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\seraphiel), so folding the user home
    # first would eat the prefix the Seraphiel-home fold needs.
    command = _rewrite_resolved_seraphiel_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / SERAPHIEL_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.seraphiel``); the matched
...
def _rewrite_resolved_seraphiel_home(command: str) -> str:
    """Rewrite the resolved absolute Seraphiel home prefix to ``~/.seraphiel/``.

    Resolves the active ``SERAPHIEL_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.seraphiel/`` so the static ``_SERAPHIEL_CONFIG_PATH`` / ``_SERAPHIEL_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/seraphiel/.seraphiel/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$SERAPHIEL_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from seraphiel_constants import get_seraphiel_home
        home = get_seraphiel_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.seraphiel")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Seraphiel ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"seraphiel-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["SERAPHIEL_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "SERAPHIEL_SPINNER_PAUSE" in os.environ:
            del os.environ["SERAPHIEL_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Seraphiel approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``SERAPHIEL_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            SERAPHIEL_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("SERAPHIEL_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set SERAPHIEL_INTERACTIVE or "
                "SERAPHIEL_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set SERAPHIEL_INTERACTIVE or "
            "SERAPHIEL_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("SERAPHIEL_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``SERAPHIEL_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("SERAPHIEL_EXEC_ASK")
