...
DANGEROUS_PATTERNS = [
    ...
    (r'\bseraphiel\s+gateway\s+(stop|restart)\b', "stop/restart seraphiel gateway (kills running agents)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.seraphiel/ form so the Seraphiel config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/seraphiel/.seraphiel/config.yaml`) rather than
    # ~, $HOME, or $SERAPHIEL_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live SERAPHIEL_HOME even when that is set
...
def _rewrite_resolved_seraphiel_home(command: str) -> str:
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate SERAPHIEL_HOME (e.g. "/" or "") rewriting
        ...
        # /home/seraphiel/.seraphiel or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.seraphiel/")
    return command
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("SERAPHIEL_INTERACTIVE")
