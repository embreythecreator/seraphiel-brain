...
* Never raises. Any I/O error (disk full, permission denied, missing
  SERAPHIEL_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_seraphiel_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: SERAPHIEL_HOME env var, then ~/.seraphiel
            home = os.environ.get("SERAPHIEL_HOME") or os.path.expanduser("~/.seraphiel")
