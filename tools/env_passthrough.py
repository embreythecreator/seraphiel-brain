...
def _is_seraphiel_provider_credential(name: str) -> bool:
    """True if ``name`` is a Seraphiel-managed provider credential (API key,
    token, or similar) per ``_SERAPHIEL_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Seraphiel API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Seraphiel credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Seraphiel-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_seraphiel_internal_secret(name):
        return True
    return name in _SERAPHIEL_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Seraphiel-managed provider credentials (from
    ``_SERAPHIEL_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Seraphiel-managed
    ...
    Non-Seraphiel third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_seraphiel_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Seraphiel provider "
                "credential %r (blocked by _SERAPHIEL_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Seraphiel-managed provider credentials must not be passed
                ...
                if _is_seraphiel_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Seraphiel "
                        "provider credential %r from config.yaml (blocked "
                        "by _SERAPHIEL_PROVIDER_ENV_BLOCKLIST). Operator "
