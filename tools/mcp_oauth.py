...
This module provides the glue:
    - ``SeraphielTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Seraphiel Brain"
"""
...
from seraphiel_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__seraphiel_user_skipped__"
...
def _get_token_dir(seraphiel_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses SERAPHIEL_HOME so each profile gets its own OAuth tokens.
    Layout: ``SERAPHIEL_HOME/mcp-tokens/``
    """
    try:
        from seraphiel_constants import get_seraphiel_home
        base = Path(seraphiel_home) if seraphiel_home is not None else Path(get_seraphiel_home())
    except ImportError:
        base = Path(os.environ.get("SERAPHIEL_HOME", str(Path.home() / ".seraphiel")))
...
def _cached_redirect_port(storage: "SeraphielTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Seraphiel restarts and chooses a
...
def _cached_redirect_uri(storage: "SeraphielTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``seraphiel mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `seraphiel mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# SeraphielTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SeraphielTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SERAPHIEL_HOME/mcp-tokens/<server_name>.json         -- tokens
        SERAPHIEL_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SERAPHIEL_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, seraphiel_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._seraphiel_home = Path(seraphiel_home) if seraphiel_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._seraphiel_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._seraphiel_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._seraphiel_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Seraphiel records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._seraphiel_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Seraphiel.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://seraphiel-brain.embreythecreator.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `seraphiel mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = SeraphielTokenStorage(server_name, seraphiel_home=seraphiel_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "SeraphielTokenStorage | None" = None,
) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Seraphiel Brain")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    storage = SeraphielTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`seraphiel mcp login {server_name}` interactively first to complete "
