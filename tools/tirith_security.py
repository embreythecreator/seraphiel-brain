...
it is automatically downloaded from GitHub releases to $SERAPHIEL_HOME/bin/tirith.
...
def _get_seraphiel_home() -> str:
    """Return the Seraphiel home directory, respecting SERAPHIEL_HOME env var."""
    return str(get_seraphiel_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_seraphiel_home(), ".tirith-install-failed")
...
def _seraphiel_bin_dir() -> str:
    """Return $SERAPHIEL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_seraphiel_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SERAPHIEL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_seraphiel_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SERAPHIEL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SERAPHIEL_HOME/bin/tirith
    ...
    seraphiel_bin = os.path.join(_seraphiel_bin_dir(), "tirith")
    if os.path.isfile(seraphiel_bin) and os.access(seraphiel_bin, os.X_OK):
        _resolved_path = seraphiel_bin
        _install_failure_reason = ""
        _clear_install_failed()
