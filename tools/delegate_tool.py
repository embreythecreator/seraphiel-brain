...
# Must match seraphiel_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``seraphiel-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``seraphiel-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. seraphiel-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (seraphiel-cli,
    # seraphiel-telegram, etc.) that _strip_blocked_tools must keep because they
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.seraphiel/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_seraphiel_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `seraphiel -z` or a cron job, or a "
        ...
        try:
            ...
            _source = get_session_env("SERAPHIEL_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("SERAPHIEL_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and SERAPHIEL_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'seraphiel auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Seraphiel config.

    Prefer the shared persistent loader because it follows the active
    SERAPHIEL_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``SERAPHIEL_IGNORE_USER_CONFIG=1`` (``seraphiel chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("SERAPHIEL_IGNORE_USER_CONFIG") == "1"
