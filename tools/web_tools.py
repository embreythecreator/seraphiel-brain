...
Backend is selected during ``seraphiel tools`` setup (web.backend in config.yaml).
When available, Seraphiel can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Seraphiel config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Seraphiel' config/.env layer (``seraphiel config set``, ``seraphiel tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.seraphiel/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``seraphiel tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `seraphiel tools` repaint.
        try:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `seraphiel plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `seraphiel tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`seraphiel plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from seraphiel_cli.config import get_env_value as _gev
