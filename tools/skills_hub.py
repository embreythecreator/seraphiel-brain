#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Seraphiel Skills Hub.
...
Used by seraphiel_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _seraphiel_home() -> Path:
    return get_seraphiel_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _seraphiel_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            seraphiel_meta = metadata.get("seraphiel", {})
            if isinstance(seraphiel_meta, dict):
                tags = seraphiel_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            seraphiel_meta = metadata.get("seraphiel", {})
            if isinstance(seraphiel_meta, dict):
                raw_tags = seraphiel_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Seraphiel) but not activated
    ...
    OFFICIAL_REPO = "embreythecreator/seraphiel-brain"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                seraphiel_meta = meta_block.get("seraphiel", {})
                if isinstance(seraphiel_meta, dict):
                    tags = seraphiel_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Seraphiel centralized index source
# ---------------------------------------------------------------------------

SERAPHIEL_INDEX_URL = "https://seraphiel-brain.embreythecreator.com/docs/api/skills-index.json"
SERAPHIEL_INDEX_TTL = 6 * 3600  # 6 hours


def _seraphiel_index_cache_file() -> Path:
    return _index_cache_dir() / "seraphiel-index.json"


def _load_seraphiel_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SERAPHIEL_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    seraphiel_index_cache_file = _seraphiel_index_cache_file()
    if seraphiel_index_cache_file.exists():
        try:
            age = time.time() - seraphiel_index_cache_file.stat().st_mtime
            if age < SERAPHIEL_INDEX_TTL:
                return json.loads(seraphiel_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(SERAPHIEL_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Seraphiel index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Seraphiel index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        seraphiel_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        seraphiel_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class SeraphielIndexSource(SkillSource):
    """Skill source backed by the centralized Seraphiel Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_seraphiel_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "seraphiel-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "seraphiel-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SeraphielIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "seraphiel-index"
