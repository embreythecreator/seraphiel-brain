...
One ``CDPSupervisor`` runs per Seraphiel ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Seraphiel logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "seraphiel-dialog-bridge.invalid"
...
(() => {
  if (window.__seraphielDialogBridgeInstalled) return;
  window.__seraphielDialogBridgeInstalled = true;
  const ENDPOINT = "http://seraphiel-dialog-bridge.invalid/";
