...
<url>``) is spawned as a direct child of the Seraphiel process. Seraphiel's own
...
Seraphiel process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Seraphiel startup's opt-in
