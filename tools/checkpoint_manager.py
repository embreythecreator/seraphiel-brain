...
-----------------------------------------------------------------------------
    ...
    ~/.seraphiel/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/seraphiel/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_seraphiel_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/seraphiel"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Seraphiel convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("SERAPHIEL_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Seraphiel infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``seraphiel checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `seraphiel checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "seraphiel@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Seraphiel Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``SERAPHIEL_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at SERAPHIEL_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "SERAPHIEL_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "SERAPHIEL_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `seraphiel checkpoints` CLI
# ---------------------------------------------------------------------------
