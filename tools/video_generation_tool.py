...
provider** — turn on a backend by enabling a plugin (``seraphiel plugins
enable video_gen/<name>``) and selecting it in ``seraphiel tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/seraphiel-brain-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `seraphiel tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `seraphiel tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `seraphiel tools` → "
...
# `seraphiel tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`seraphiel tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `seraphiel tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
