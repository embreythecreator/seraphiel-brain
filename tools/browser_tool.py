...
def _find_agent_browser() -> str:
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                seraphiel_nm = str(get_seraphiel_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=seraphiel_nm)
            if not recheck:
                seraphiel_node_bin = str(get_seraphiel_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=seraphiel_node_bin)
            if not recheck:
                seraphiel_node_root = str(get_seraphiel_home() / "node")
                recheck = shutil.which("agent-browser", path=seraphiel_node_root)
            if recheck:
