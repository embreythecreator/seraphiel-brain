# Seraphiel Brain - Development Guide

Instructions for AI coding assistants and developers working on the seraphiel-brain codebase.

**Never give up on the right solution.**

## What Seraphiel Is

Seraphiel is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Seraphiel ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`seraphiel tools`, `seraphiel setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `SERAPHIEL_HOME`. Mocks hide integration bugs.
...
- **New `SERAPHIEL_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `seraphiel tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.seraphiel/plugins/` (or via a
  pip entry point), and promote them in the Seraphiel Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `seraphiel <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `seraphiel webhook`, `seraphiel cron`, `seraphiel tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.seraphiel/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.seraphiel/seraphiel-brain/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _SERAPHIEL_CORE_TOOLS list
├── cli.py                # SeraphielCLI class — interactive CLI orchestrator (~11k LOC)
├── seraphiel_state.py       # SessionDB — SQLite session store (FTS5 search)
├── seraphiel_constants.py   # get_seraphiel_home(), display_seraphiel_home() — profile-aware paths
├── seraphiel_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── seraphiel_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── seraphiel-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `seraphiel --tui`
...
**User config:** `~/.seraphiel/config.yaml` (settings), `~/.seraphiel/.env` (API keys only).
**Logs:** `~/.seraphiel/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_seraphiel_home()`.
Browse with `seraphiel logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Seraphiel: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`seraphiel_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `SeraphielCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.seraphiel/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`seraphiel_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/seraphiel` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `seraphiel_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `SeraphielCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `seraphiel --tui` or `SERAPHIEL_TUI=1`.
...
npm run dev       # watch mode (rebuilds seraphiel-ink + tsx --watch)
npm start         # production
npm run build     # full build (seraphiel-ink + tsc)
...
### TUI in the Dashboard (`seraphiel dashboard` → `/chat`)

The dashboard embeds the real `seraphiel --tui` — **not** a rewrite.  See `seraphiel_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `seraphiel_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `seraphiel --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `seraphiel --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@seraphiel/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `seraphiel serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `SERAPHIEL_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `seraphiel` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `seraphiel --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
be core tools. For custom or local-only tools, do **not** edit Seraphiel core.
Use the plugin route instead: create `~/.seraphiel/plugins/<name>/plugin.yaml`
and `~/.seraphiel/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Seraphiel tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_SERAPHIEL_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_SERAPHIEL_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_seraphiel_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `SERAPHIEL_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_seraphiel_home()` for the base directory — never `Path.home() / ".seraphiel"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `seraphiel_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `seraphiel_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `seraphiel tools`, `seraphiel setup`, most CLI subcommands | `seraphiel_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`seraphiel_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
seraphiel_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.seraphiel/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Seraphiel gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `seraphiel_cli/skin_engine.py`:
...
Users create `~/.seraphiel/skins/<name>.yaml`:
...
Seraphiel has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.seraphiel/plugins/` and pip-installed entry points.

### General plugins (`seraphiel_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.seraphiel/plugins/`, `./.seraphiel/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `seraphiel` at startup so
  `seraphiel <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(seraphiel_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `seraphiel <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `seraphiel --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `seraphiel_cli/main.py`, etc.).
...
into `~/.seraphiel/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `seraphiel memory setup` / `post_setup()` without
...
`~/.seraphiel/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Seraphiel Discord
...
2. User: `$SERAPHIEL_HOME/plugins/model-providers/<name>/`
...
[`seraphiel-example-plugins`](https://github.com/embreythecreator/seraphiel-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `seraphiel skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.seraphiel.tags`, `metadata.seraphiel.category`,
`metadata.seraphiel.related_skills`, `metadata.seraphiel.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.seraphiel.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Seraphiel tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Seraphiel Brain" is the secondary collaborator. If the
   contributor's commit shows "Seraphiel Brain" as author (because they
   used Seraphiel to draft the skill), replace it with their actual name
...
lives in the `seraphiel-brain-dev` skill at
...
`"messaging"`); `_SERAPHIEL_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `seraphiel tools` (the curses UI) or the
...
Seraphiel returns a delegation id immediately and the result re-enters the
...
go to `~/.seraphiel/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `seraphiel_cli/curator.py` wires `seraphiel curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.seraphiel/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `seraphiel cron <verb>`
...
- File lock at `~/.seraphiel/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `seraphiel kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `seraphiel_cli/kanban.py` wires `seraphiel kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`seraphiel-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `SERAPHIEL_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Seraphiel Brain ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `SERAPHIEL_BACKGROUND_NOTIFICATIONS` env var):
...
Seraphiel supports **profiles** — multiple fully isolated instances, each with its own
`SERAPHIEL_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `seraphiel_cli/main.py` sets
`SERAPHIEL_HOME` before any module imports. All `get_seraphiel_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_seraphiel_home()` for all SERAPHIEL_HOME paths.** Import from `seraphiel_constants`.
   NEVER hardcode `~/.seraphiel` or `Path.home() / ".seraphiel"` in code that reads/writes state.
   ```python
   # GOOD
   from seraphiel_constants import get_seraphiel_home
   config_path = get_seraphiel_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".seraphiel" / "config.yaml"
   ```

2. **Use `display_seraphiel_home()` for user-facing messages.** Import from `seraphiel_constants`.
   This returns `~/.seraphiel` for default or `~/.seraphiel/profiles/<name>` for profiles.
   ```python
   # GOOD
   from seraphiel_constants import display_seraphiel_home
   print(f"Config saved to {display_seraphiel_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.seraphiel/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_seraphiel_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_seraphiel_home()`,
   not `Path.home() / ".seraphiel"`.

4. **Tests that mock `Path.home()` must also set `SERAPHIEL_HOME`** — since code now uses
   `get_seraphiel_home()` (reads env var), not `Path.home() / ".seraphiel"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"SERAPHIEL_HOME": str(tmp_path / ".seraphiel")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not SERAPHIEL_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".seraphiel" / "profiles"`, NOT `get_seraphiel_home() / "profiles"`.
   This is intentional — it lets `seraphiel -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.seraphiel` paths
Use `get_seraphiel_home()` from `seraphiel_constants` for code paths. Use `display_seraphiel_home()`
for user-facing print/log messages. Hardcoding `~/.seraphiel` breaks profiles — each profile
has its own `SERAPHIEL_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `seraphiel_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `seraphiel_cli/curses_ui.py` — see
`seraphiel_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `SERAPHIEL_HOME`.

### Tests must not write to `~/.seraphiel/`
The `_isolate_seraphiel_home` autouse fixture in `tests/conftest.py` redirects `SERAPHIEL_HOME` to a temp dir. Never hardcode `~/.seraphiel/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_seraphiel_home()` resolve within the temp dir.
Use the pattern from `tests/seraphiel_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `SERAPHIEL_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.seraphiel/` | Your real config+auth.json                  | Temp dir per test                         |
