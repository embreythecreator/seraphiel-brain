...
row taxonomy is inferred from the atlas shape so Seraphiel can render both legacy
...
# rows (1536x1872), while older Seraphiel/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Seraphiel' activity state names. They are not always identical to the
...
# Legacy Seraphiel/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Seraphiel activity names -> accepted row-name aliases in descending
