...
Owns one Codex thread per Seraphiel session. Drives `turn/start`, consumes
...
# Seraphiel' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SERAPHIEL_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Seraphiel/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Seraphiel session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SERAPHIEL_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SERAPHIEL_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="seraphiel",
            client_title="Seraphiel Brain",
            client_version=_get_seraphiel_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Seraphiel' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Seraphiel' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own seraphiel-tools callback we
            # auto-accept — the user already approved Seraphiel' tools
            ...
            if server_name == "seraphiel-tools":
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Seraphiel approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Seraphiel returns 'once', 'session', 'always', or 'deny'.
...
def _get_seraphiel_version() -> str:
    """Best-effort Seraphiel version string for codex's userAgent line."""
    try:
