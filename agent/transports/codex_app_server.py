...
projection into Seraphiel' display, approval bridging, transcript projection into
...
"codex_app_server"`. Seraphiel' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Seraphiel secret — gateway
        ...
        spawn_env = seraphiel_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SERAPHIEL_KANBAN_TASK"):
            kanban_db = spawn_env.get("SERAPHIEL_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "SERAPHIEL_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SERAPHIEL_HOME", os.path.expanduser("~/.seraphiel")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "seraphiel",
        client_title: str = "Seraphiel Brain",
