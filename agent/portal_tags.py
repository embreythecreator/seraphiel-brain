...
Every Seraphiel request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Seraphiel Brain and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=seraphiel-brain",
        "client=seraphiel-client-v<__version__>",
    ]

The version is sourced live from ``seraphiel_cli.__version__`` so it auto-aligns
...
``seraphiel_cli.__version__`` is the canonical source of truth.
"""
...
def _seraphiel_version() -> str:
    """Return the current Seraphiel release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``seraphiel_cli`` cannot be imported (should
...
def seraphiel_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=seraphiel-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=seraphiel-client-v{_seraphiel_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Seraphiel session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Seraphiel conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.seraphiel/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Seraphiel
    ...
    """
    tags = ["product=seraphiel-brain", seraphiel_client_tag()]
