...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<SERAPHIEL_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
    ...
    home = os.environ.get("SERAPHIEL_HOME") or os.path.join(
        os.path.expanduser("~"), ".seraphiel"
    )
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<SERAPHIEL_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        seraphiel_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(seraphiel_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Seraphiel -HostProfileId seraphiel -HostVersion 1.0.0 "
...
def seraphiel_lsp_session_dir() -> str:
