...
cache problem must never block Seraphiel startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Seraphiel home used for cache paths.

    ``home_path`` is whatever ``load_seraphiel_dotenv()`` already resolved;
    falling back to ``$SERAPHIEL_HOME`` / ``~/.seraphiel`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<seraphiel_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SERAPHIEL_HOME is ever made traversable.
            try:
