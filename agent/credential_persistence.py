...
``auth.json``.  They intentionally have no dependency on ``seraphiel_cli.auth`` so
...
# Sources Seraphiel owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Seraphiel-owned OAuth/device-code state)
