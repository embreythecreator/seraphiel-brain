...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Seraphiel-Session-Id header), and an
        ...
        if env_var_enabled("SERAPHIEL_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from seraphiel_cli.config import load_config as _load_moa_cfg
    ...
    is_nous_portal = "embreythecreator" in eff_base_url.lower()
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to seraphiel's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from seraphiel_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
    ...
    try:
        ...
        from seraphiel_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from seraphiel_cli.middleware import apply_tool_request_middleware
    ...
    from seraphiel_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: seraphiel' own dispatch loop intentionally keeps an
