...
``kind: backend``) or ``~/.seraphiel/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`seraphiel_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.seraphiel/.env``) so credentials set
    through Seraphiel' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``seraphiel tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``seraphiel tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``seraphiel tools`` picker.

        Used by ``seraphiel_cli/tools_config.py`` to inject this provider as a
