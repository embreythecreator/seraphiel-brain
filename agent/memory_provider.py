...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `seraphiel backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - seraphiel_home (str): The active SERAPHIEL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.seraphiel``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "seraphiel").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'seraphiel memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], seraphiel_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'seraphiel memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``seraphiel_home`` is the active SERAPHIEL_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SERAPHIEL_HOME.

        ``seraphiel backup`` only walks SERAPHIEL_HOME, so any provider state kept
        ...
        of the archive, and ``seraphiel import`` restores them to their original
