...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SERAPHIEL_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Seraphiel runtime / deployment
    "SERAPHIEL_HOME", "SERAPHIEL_PROFILE", "SERAPHIEL_GATEWAY_LOCK_DIR",
    "SERAPHIEL_MAX_ITERATIONS", "SERAPHIEL_MAX_TOKENS", "SERAPHIEL_API_TIMEOUT",
    "SERAPHIEL_REDACT_SECRETS", "SERAPHIEL_NOUS_TIMEOUT_SECONDS",
    "_SERAPHIEL_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SERAPHIEL_KANBAN_DB", "SERAPHIEL_KANBAN_WORKSPACES_ROOT", "SERAPHIEL_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "SERAPHIEL_KANBAN_",
    "SERAPHIEL_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(seraphiel_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(seraphiel_home) / ".env")
