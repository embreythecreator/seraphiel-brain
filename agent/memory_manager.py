...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SERAPHIEL_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Seraphiel expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``seraphiel_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_seraphiel_home()`` themselves.
        """
        if "seraphiel_home" not in kwargs:
            from seraphiel_constants import get_seraphiel_home
            kwargs["seraphiel_home"] = str(get_seraphiel_home())
        for provider in self._providers:
            try:
