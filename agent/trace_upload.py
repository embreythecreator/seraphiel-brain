"""Upload a Seraphiel session transcript to Hugging Face as an agent trace.

Seraphiel stores sessions in its own SQLite store (``seraphiel_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``seraphiel trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Seraphiel' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "seraphiel-traces"
_SERAPHIEL_VERSION = "seraphiel-brain"
...
# ---------------------------------------------------------------------------
# Conversion: Seraphiel OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Seraphiel' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Seraphiel conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.seraphiel/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `seraphiel trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/seraphiel-traces`` dataset. Returns a
