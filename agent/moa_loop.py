...
MoA-enabled; the normal Seraphiel agent loop still owns tool calling and turn
...
def _slot_label(slot: dict[str, Any]) -> str:
    ...
    display = f"{label} ({runtime})" if label else runtime
    effort = str(slot.get("reasoning_effort") or "").strip()
    return f"{display}[reasoning={effort}]" if effort else display
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Seraphiel boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Seraphiel agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Seraphiel agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Seraphiel agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
