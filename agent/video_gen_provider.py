...
as ``kind: backend``) or ``~/.seraphiel/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``seraphiel tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SERAPHIEL_HOME/cache/videos/``, creating parents as needed."""
    from seraphiel_constants import get_seraphiel_home

    path = get_seraphiel_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SERAPHIEL_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$SERAPHIEL_HOME/cache/videos/``.
