...
def _seraphiel_home_path() -> Path:
    """Resolve the active SERAPHIEL_HOME (profile-aware) without circular imports."""
    try:
        from seraphiel_constants import get_seraphiel_home  # local import to avoid cycles
        return get_seraphiel_home()
    except Exception:
        return Path(os.path.expanduser("~/.seraphiel"))


def _seraphiel_root_path() -> Path:
    """Resolve the Seraphiel root dir (always the parent of any profile, never per-profile)."""
    try:
        from seraphiel_constants import get_default_seraphiel_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    seraphiel_home = _seraphiel_home_path()
    seraphiel_root = _seraphiel_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(seraphiel_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(seraphiel_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(seraphiel_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(seraphiel_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved SERAPHIEL_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("SERAPHIEL_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Seraphiel control-plane files: block both the ACTIVE profile's view
    # (seraphiel_home) AND the global root view. Without the root pass, a
    ...
    seraphiel_dirs = []
    for base in (_seraphiel_home_path(), _seraphiel_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in seraphiel_dirs:
                seraphiel_dirs.append(real)
        except Exception:
            continue

    for base_real in seraphiel_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Seraphiel path.

    Three categories are blocked:

      * Internal Seraphiel cache files under ``SERAPHIEL_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under SERAPHIEL_HOME and the global Seraphiel
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.seraphiel/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active SERAPHIEL_HOME (profile-aware) AND the global
    # Seraphiel root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (SERAPHIEL_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    seraphiel_dirs: list[Path] = []
    for base in (_seraphiel_home_path(), _seraphiel_root_path()):
        try:
            real = base.resolve()
            if real not in seraphiel_dirs:
    ...
    for hd in seraphiel_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Seraphiel cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # SERAPHIEL_HOME or <root>.
    ...
    for hd in seraphiel_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Seraphiel credential store "
    ...
    for hd in seraphiel_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Seraphiel MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Seraphiel MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Seraphiel profiles are separate SERAPHIEL_HOME dirs under
...
#
# Reference: May 2026 incident where a seraphiel-security profile session
# edited skills under both ``~/.seraphiel/profiles/seraphiel-security/skills/``
# AND ``~/.seraphiel/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SERAPHIEL_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SERAPHIEL_HOME.

    ``~/.seraphiel``              -> ``"default"``
    ``~/.seraphiel/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _seraphiel_home_path().resolve()
        root_real = _seraphiel_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Seraphiel scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Seraphiel entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Seraphiel profile {info['target_profile']!r}, but the "
...
#
#   <SERAPHIEL_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.seraphiel/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.seraphiel/…`` segment and warns
# regardless of which Seraphiel profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.seraphiel/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.seraphiel`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.seraphiel/…`` and returns the
    index where the inner Seraphiel-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .seraphiel / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".seraphiel":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Seraphiel state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.seraphiel``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.seraphiel`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Seraphiel resolver to
    succeed, so it works correctly even when called from contexts where
    SERAPHIEL_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Seraphiel process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real SERAPHIEL_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.seraphiel/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.seraphiel/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host SERAPHIEL_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Seraphiel state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Seraphiel "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real SERAPHIEL_HOME. Use the "
