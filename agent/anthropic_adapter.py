"""Anthropic Messages API adapter for Seraphiel Brain.

Translates between Seraphiel's internal OpenAI-style message format and
...
# Seraphiel effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See seraphiel-brain#13848, #17057.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to seraphiel's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to seraphiel's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Seraphiel notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Seraphiel historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``seraphiel models``) never mutates ``~/.seraphiel/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Seraphiel)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.seraphiel/auth.json)
    ...
    # 1. Seraphiel-managed OAuth/setup token env var
    ...
    # 4. Seraphiel credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Seraphiel configs.
...
# ── Seraphiel-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.seraphiel/.anthropic_oauth.json (our own file).
...
_SERAPHIEL_OAUTH_FILE = get_seraphiel_home() / ".anthropic_oauth.json"
...
def run_seraphiel_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Seraphiel-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Seraphiel with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from seraphiel_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_seraphiel_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Seraphiel-managed OAuth credentials from ~/.seraphiel/.anthropic_oauth.json."""
    if _SERAPHIEL_OAUTH_FILE.exists():
        try:
            data = json.loads(_SERAPHIEL_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Seraphiel OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See seraphiel-brain#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See seraphiel-brain: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See seraphiel-brain#13848 (Kimi) and
    seraphiel-brain#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Seraphiel Brain", "Claude Code")
                text = text.replace("Seraphiel agent", "Claude Code")
                text = text.replace("seraphiel-brain", "claude-code")
                text = text.replace("Seraphiel", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Seraphiel-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Seraphiel surfaces in its CLI. We
