...
upstream prefix cache warm.  See ``seraphiel-brain-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``SERAPHIEL_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("SERAPHIEL_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Seraphiel never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SERAPHIEL_HOME persona while keeping
    ...
    # Pointer to the seraphiel-brain skill + docs for user questions about Seraphiel itself.
    stable_parts.append(SERAPHIEL_AGENT_HELP_GUIDANCE)
    ...
    # SERAPHIEL_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Seraphiel, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Seraphiel profile the agent is running
    # under so it doesn't conflate ~/.seraphiel/skills/ (default profile) with
    # ~/.seraphiel/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Seraphiel profile: default. Other profiles (if any) live "
            "under " + str(get_seraphiel_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Seraphiel profile: {active_profile}. This session reads "
            f"and writes {get_seraphiel_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_seraphiel_home()}/skills/, {get_seraphiel_home()}/plugins/, "
            f"{get_seraphiel_home()}/cron/, {get_seraphiel_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Seraphiel). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Seraphiel never rebuilds or reinjects parts of it
