...
as ``kind: backend``) or ``~/.seraphiel/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``seraphiel tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``seraphiel tools`` model picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``seraphiel tools``
...
def _images_cache_dir() -> Path:
    """Return ``$SERAPHIEL_HOME/cache/images/``, creating parents as needed."""
    from seraphiel_constants import get_seraphiel_home

    path = get_seraphiel_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$SERAPHIEL_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$SERAPHIEL_HOME/cache/images/``.
