...
Plugins built on Seraphiel Brain often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Seraphiel
...
:class:`~seraphiel_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Seraphiel' main config
...
handles every provider, fallback chain, and per-task override Seraphiel
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`seraphiel_cli.plugins.PluginContext`
