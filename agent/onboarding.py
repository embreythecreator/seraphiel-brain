...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Seraphiel starts and finds ``~/.openclaw/``.

    Points users at ``seraphiel claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``seraphiel claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Seraphiel, run "
        "`seraphiel claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `seraphiel claw cleanup` (renames it to ~/.openclaw.pre-migration — "
