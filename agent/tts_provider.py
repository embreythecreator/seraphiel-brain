...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Seraphiel with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.seraphiel/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Seraphiel
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``seraphiel tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``seraphiel setup`` for
