...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``SERAPHIEL_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``SERAPHIEL_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``SERAPHIEL_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``SERAPHIEL_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``SERAPHIEL_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
    name = (os.getenv("SERAPHIEL_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    portal = "https://portal.embreythecreator.com/billing?topup=open"
    ...
    return BillingState(logged_in=False, error=f"unknown SERAPHIEL_DEV_BILLING_FIXTURE: {name}")
