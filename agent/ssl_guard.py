"""Preventive SSL CA certificate checks for Seraphiel Brain.
...
def _skip_ssl_guard_enabled() -> bool:
    return os.getenv("SERAPHIEL_SKIP_SSL_GUARD", "").strip().lower() in _SKIP_VALUES
...
def verify_ca_bundle() -> None:
    ...
    if _skip_ssl_guard_enabled():
        logger.debug("SSL CA bundle guard skipped via SERAPHIEL_SKIP_SSL_GUARD")
        return
