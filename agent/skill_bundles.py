...
Storage
-------
Bundles live in ``~/.seraphiel/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SERAPHIEL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``seraphiel bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SERAPHIEL_HOME.

    Honors ``SERAPHIEL_BUNDLES_DIR`` for tests; falls back to
    ``<SERAPHIEL_HOME>/skill-bundles``.
    """
    override = os.environ.get("SERAPHIEL_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_seraphiel_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `seraphiel bundles` CLI subcommand.
# ---------------------------------------------------------------------------
