...
React + Ink terminal UI for Seraphiel. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SERAPHIEL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `seraphiel setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.seraphiel/.seraphiel_history` or under `SERAPHIEL_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `seraphiel setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/seraphiel-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external seraphiel setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.seraphiel/tui-parent.log
    ...
    types/
      seraphiel-ink.d.ts            type declarations for @seraphiel/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SeraphielCLI subprocess for slash commands
```