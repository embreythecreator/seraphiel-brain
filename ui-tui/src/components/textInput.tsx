import type { InputEvent, Key } from '@seraphiel/ink'
import * as Ink from '@seraphiel/ink'
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SERAPHIEL_TUI_TERMUX_FAST_ECHO ?? '').trim().toLowerCase()
