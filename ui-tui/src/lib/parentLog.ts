...
const logDir = join(process.env.SERAPHIEL_HOME?.trim() || join(homedir(), '.seraphiel'), 'logs')
...
// a real ~/.seraphiel (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('seraphiel-tui: parent lifecycle log unavailable\n')
    }
  }
}
