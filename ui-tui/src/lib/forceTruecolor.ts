...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SERAPHIEL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Seraphiel on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SERAPHIEL_TUI_TRUECOLOR=1.
