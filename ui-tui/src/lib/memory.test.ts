...
const ENV_KEYS = ['SERAPHIEL_AUTO_HEAPDUMP', 'SERAPHIEL_HEAPDUMP_DIR', 'SERAPHIEL_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'seraphiel-heapdump-test-'))
    process.env.SERAPHIEL_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without SERAPHIEL_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without SERAPHIEL_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when SERAPHIEL_AUTO_HEAPDUMP=1', async () => {
    process.env.SERAPHIEL_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.SERAPHIEL_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of SERAPHIEL_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.SERAPHIEL_HEAPDUMP_DIR
    savedMax = process.env.SERAPHIEL_HEAPDUMP_MAX_BYTES
    delete process.env.SERAPHIEL_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'seraphiel-heapdump-prune-'))
    process.env.SERAPHIEL_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {delete process.env.SERAPHIEL_HEAPDUMP_DIR}
    else {process.env.SERAPHIEL_HEAPDUMP_DIR = savedDir}

    if (savedMax === undefined) {delete process.env.SERAPHIEL_HEAPDUMP_MAX_BYTES}
    else {process.env.SERAPHIEL_HEAPDUMP_MAX_BYTES = savedMax}
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.SERAPHIEL_HEAPDUMP_MAX_BYTES = String(2 * 1024)
