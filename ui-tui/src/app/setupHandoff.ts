import type { RunExternalProcess } from '@seraphiel/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`seraphiel ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching seraphiel: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`seraphiel ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
