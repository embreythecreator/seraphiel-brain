...
await build({
  ...
  // Skip the prebuilt @seraphiel/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@seraphiel/ink': resolve(root, 'packages/seraphiel-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The seraphiel_cli launcher
