...
Background
----------
A Nous bootstrap session (client_id ``seraphiel-cli-vps``) can take a terminal
...
``stage2-hook.sh`` seeds ``auth.json`` from ``SERAPHIEL_AUTH_JSON_BOOTSTRAP`` only
...
``SERAPHIEL_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
Design constraints
------------------
- Pure stdlib, no seraphiel_cli imports: runs early in the boot hook, before the
...
# from SERAPHIEL_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
...
REBOOTSTRAP_ENV = "SERAPHIEL_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "seraphiel-cli-vps"
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``seraphiel_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a SERAPHIEL_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    SERAPHIEL_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             SERAPHIEL_AUTH_JSON_BOOTSTRAP path handle it)
