# ============================================================================
# Seraphiel Brain Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$SeraphielHome = $(if ($env:SERAPHIEL_HOME) { $env:SERAPHIEL_HOME } else { "$env:LOCALAPPDATA\seraphiel" }),
    [string]$InstallDir = $(if ($env:SERAPHIEL_HOME) { "$env:SERAPHIEL_HOME\seraphiel-brain" } else { "$env:LOCALAPPDATA\seraphiel\seraphiel-brain" }),
    ...
    # builds apps/desktop into a launchable Seraphiel.exe.
    #
    # Why opt-in:
    #   * Seraphiel-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Seraphiel.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Seraphiel.exe
    ...
    #     `seraphiel desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:embreythecreator/seraphiel-brain.git"
$RepoUrlHttps = "https://github.com/embreythecreator/seraphiel-brain.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Seraphiel Brain Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Seraphiel.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $SeraphielHome)) {
        New-Item -ItemType Directory -Force -Path $SeraphielHome | Out-Null
    }
    $envFile = Join-Path $SeraphielHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $SeraphielHome "node"
...
function Install-Uv {
    # Seraphiel owns its own uv at $SeraphielHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (seraphiel_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `seraphiel update` stay in sync.
    $managedUv = Join-Path $SeraphielHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SeraphielHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SeraphielHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SeraphielHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SeraphielHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and SERAPHIEL_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Seraphiel-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Seraphiel setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Seraphiel uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\seraphiel\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Seraphiel needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Seraphiel copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\seraphiel\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SERAPHIEL_GIT_BASH_PATH`` (User scope) so Seraphiel can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Seraphiel-managed PortableGit install instead..."
    }

    # Download PortableGit into $SeraphielHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SeraphielHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Seraphiel features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Seraphiel needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SERAPHIEL_GIT_BASH_PATH`` (User env scope) so Seraphiel can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SeraphielHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SeraphielHome\git\usr\bin\bash.exe
    $candidates += "$SeraphielHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SeraphielHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SERAPHIEL_GIT_BASH_PATH", $candidate, "User")
            $env:SERAPHIEL_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SERAPHIEL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Seraphiel may not find Git Bash."
    Write-Info "If needed, set SERAPHIEL_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$SeraphielHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SeraphielHome\node;$env:Path"
        Write-Success "Node.js $version found (Seraphiel-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Seraphiel-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SeraphielHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SeraphielHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\seraphiel-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$SeraphielHome\node") { Remove-Item -Recurse -Force "$SeraphielHome\node" }
                Move-Item $extractedDir.FullName "$SeraphielHome\node"
                ...
                $version = & "$SeraphielHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SeraphielHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\seraphiel-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `seraphiel
                    # update` path (#4735).
                    ...
                    $stashName = "seraphiel-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``seraphiel update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Seraphiel behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running seraphiel processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/embreythecreator/seraphiel-brain/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/embreythecreator/seraphiel-brain/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/embreythecreator/seraphiel-brain/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\seraphiel-brain-$zipLabel.zip"
                $extractPath = "$env:TEMP\seraphiel-brain-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `seraphiel update` (see the notes at the shared clone-path
    ...
    # next `seraphiel update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Seraphiel-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running seraphiel process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running seraphiel processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Seraphiel_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (seraphiel.exe) plus its child tree.
            & taskkill /F /T /IM seraphiel.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM seraphiel.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m seraphiel_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not seraphiel.exe.
            ...
            # user (see seraphiel_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `seraphiel update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `seraphiel.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install seraphiel-brain package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and seraphiel.exe broken with
    ...
    # here, not 30 seconds later when the user runs `seraphiel`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register seraphiel.exe in dist-info/RECORD but fail to
        ...
        # `seraphiel` command while seraphiel-brain.exe / seraphiel-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m seraphiel_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `seraphiel dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\seraphiel_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `seraphiel dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `seraphiel dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: seraphiel_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up seraphiel command..."
    
    if ($NoVenv) {
        $seraphielBin = "$InstallDir"
    } else {
        $seraphielBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so seraphiel is globally available
    # On Windows, the seraphiel.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$seraphielBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$seraphielBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $seraphielBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SERAPHIEL_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\seraphiel instead
    # of the Unix default ~/.seraphiel
    $currentSeraphielHome = [Environment]::GetEnvironmentVariable("SERAPHIEL_HOME", "User")
    if (-not $currentSeraphielHome -or $currentSeraphielHome -ne $SeraphielHome) {
        [Environment]::SetEnvironmentVariable("SERAPHIEL_HOME", $SeraphielHome, "User")
        Write-Success "Set SERAPHIEL_HOME=$SeraphielHome"
    }
    $env:SERAPHIEL_HOME = $SeraphielHome
    
    # Update current session
    $env:Path = "$seraphielBin;$env:Path"
    
    Write-Success "seraphiel command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.seraphiel-bootstrap-complete which tells the Seraphiel
    ...
    # Seraphiel-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `seraphiel update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".seraphiel-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SERAPHIEL_HOME directory structure ($SeraphielHome, default %LOCALAPPDATA%\seraphiel)
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SeraphielHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SeraphielHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SeraphielHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Seraphiel's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SeraphielHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in seraphiel_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $SeraphielHome"
    
    # Seed bundled skills into $SeraphielHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SeraphielHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $SeraphielHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SeraphielHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SeraphielHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Seraphiel-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'seraphiel setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\seraphiel-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\seraphiel-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\seraphiel-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Seraphiel` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Seraphiel.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_seraphiel_desktop command
    # resolves apps/desktop/release/win-unpacked/Seraphiel.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Seraphiel-managed
    ...
    # The --dir mode produces an unpacked Seraphiel.exe in
    ...
    # are NOT trying to work around). The Seraphiel icon + product name are
    # stamped onto Seraphiel.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\seraphiel-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Seraphiel.exe",
        "$desktopDir\release\win-arm64-unpacked\Seraphiel.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Seraphiel.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Seraphiel icon + identity are stamped onto Seraphiel.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, seraphiel-brain#38216).
    ...
    #    Seraphiel.exe. We deliberately do NOT point them at `seraphiel desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Seraphiel.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Seraphiel.lnk')
        )
        ...
        foreach ($lnkPath in $targets) {
            try {
                ...
                $sc.Description = 'Seraphiel Brain'
                $sc.Save()
        ...
        # with the Seraphiel icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.seraphiel/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'seraphiel setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m seraphiel_cli.main setup
    } else {
        python -m seraphiel_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $seraphielCmd = "$InstallDir\venv\Scripts\seraphiel.exe"
    if (-not (Test-Path $seraphielCmd)) {
        $seraphielCmd = "seraphiel"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SeraphielHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'seraphiel whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: seraphiel gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SeraphielHome\logs\gateway.log"
            Start-Process -FilePath $seraphielCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SeraphielHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: seraphiel gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: seraphiel gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SeraphielHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SeraphielHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SeraphielHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SeraphielHome\seraphiel-brain\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   seraphiel              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   seraphiel setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   seraphiel config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   seraphiel config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   seraphiel gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   seraphiel update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Seraphiel repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Seraphiel-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Seraphiel to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://seraphiel-brain.embreythecreator.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
