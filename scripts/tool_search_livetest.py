#!/usr/bin/env python3
"""Live test harness for Seraphiel Brain's Tool Search feature.
...
# Force-isolate the test environment BEFORE any seraphiel imports.
ORIGINAL_HOME = os.environ.get("SERAPHIEL_HOME")
ORIGINAL_AUTH = Path.home() / ".seraphiel" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.seraphiel/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.seraphiel/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="seraphiel_ts_live_"))
    seraphiel_home = home_dir / ".seraphiel"
    seraphiel_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, seraphiel_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".seraphiel" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, seraphiel_home / ".env")
        ...
        load_seraphiel_dotenv(seraphiel_home=str(Path.home() / ".seraphiel"))
    ...
    (seraphiel_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new SERAPHIEL_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "seraphiel_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SERAPHIEL_HOME"] = str(home)
...
def main():
    ...
    # Restore original SERAPHIEL_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SERAPHIEL_HOME"] = ORIGINAL_HOME
    else:
