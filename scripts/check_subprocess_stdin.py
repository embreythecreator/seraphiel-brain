...
When Seraphiel runs in TUI mode, the gateway child process communicates with
...
# ``get_seraphiel_home() / "plugins"`` (user) and ``./.seraphiel/plugins/`` (project,
# gated behind ``SERAPHIEL_ENABLE_PROJECT_PLUGINS``) — see
# ``seraphiel_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``seraphiel_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import seraphiel_constants (this script
    ...
    # plugins in get_seraphiel_home()/plugins/ and project plugins in
    # ./.seraphiel/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_seraphiel_home() / "plugins"]
    if os.environ.get("SERAPHIEL_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".seraphiel" / "plugins")
