#!/usr/bin/env python3
"""Seraphiel Brain Release Script
...
VERSION_FILE = REPO_ROOT / "seraphiel_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@seraphiel.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.seraphiel": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level SERAPHIEL_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (seraphiel journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "seraphiel.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.seraphiel/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "brett@personalfinancelab.com": "brett539",  # PR #49369 salvage (cap Telegram initialize() with asyncio.wait_for(SERAPHIEL_TELEGRAM_INIT_TIMEOUT, default 30s) per attempt so an unreachable fallback-IP connect chain can't block gateway startup indefinitely; add WARNING progress logs before DoH discovery and each connect attempt)
    ...
    "moonsong@embreythecreator.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows seraphiel-shim resolution + prefer --update on recovery; #52378)
    ...
    "jake@embreythecreator.com": "simpolism",
    ...
    "seraphiel@marian.local": "Schrotti77",
    ...
    "teknium@embreythecreator.com": "teknium1",
    ...
    "274096618+seraphiel-brain-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@embreythecreator.com": "shannonsands",
    ...
    "seraphiel@embreythecreator.com": "embreythecreator",
    ...
    "seraphiel@noushq.ai": "benbarclay",
    ...
    "ben@embreythecreator.com": "benbarclay",
    ...
    "emozilla@embreythecreator.com": "emozilla",
    ...
    "jonny@embreythecreator.com": "yoniebans",
    "jake@embreythecreator.com": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on seraphiel mcp add)
    ...
    "seraphiel@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@seraphiel.local": "jacdevos",
    ...
    "teknium@seraphiel-brain": "teknium1",
    ...
    "github@nadyaseraphiel.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (SERAPHIEL_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SERAPHIEL_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: seraphiel-eval community link)
    ...
    "seraphielagent26@gmail.com": "seraphielagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+seraphielagent26@users.noreply.github.com": "seraphielagent26",
    ...
    "clement@embreythecreator.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@seraphiel": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.seraphiel@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Seraphiel runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level SERAPHIEL_HOME state files)
    ...
    "annguyen@embreythecreator.com": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@seraphiel-brain.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Seraphiel
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"seraphiel-brain[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "seraphiel@embreythecreator.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/embreythecreator/seraphiel-brain",
    ...
    lines.append(f"# Seraphiel Brain v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Seraphiel Brain. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Seraphiel Brain Release Tool")
    ...
    print(f"{'='*60}")
    print("  Seraphiel Brain Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Seraphiel Brain v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Seraphiel Brain v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Seraphiel Brain v{new_version} ({calver_date})' "
