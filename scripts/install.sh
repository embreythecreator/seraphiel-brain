...
Edit this to customize how Seraphiel communicates with you.
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Seraphiel Brain browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Seraphiel will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (seraphiel --tui may not work)"
        }
...
ensure_browser() {
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$SERAPHIEL_HOME/node" --silent --ignore-scripts \
