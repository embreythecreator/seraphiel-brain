#!/usr/bin/env bash
# Canonical test runner for seraphiel-brain. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.seraphiel/...)
#
# Usage:
...
# (SERAPHIEL_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.seraphiel/seraphiel-brain/venv"; do
...
elif [ -n "${SERAPHIEL_PYTHON:-}" ] && [ -x "$SERAPHIEL_PYTHON" ] \
    && "$SERAPHIEL_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: SERAPHIEL_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `seraphiel` binary rather
  # than the devShell hook.
  PYTHON="$SERAPHIEL_PYTHON"
  echo "▶ no local venv — using Nix dev venv via SERAPHIEL_PYTHON: $PYTHON"
else
  echo "error: no virtualenv found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and SERAPHIEL_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  exit 1
fi
...
if [ -f "$HOME/.seraphiel/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.seraphiel"
...
exec env -i \
  ...
  LC_ALL=C.UTF-8 \
  PYTHONHASHSEED=0 \
  ${SERAPHIEL_RUN_SLOW_PET_TESTS:+SERAPHIEL_RUN_SLOW_PET_TESTS="$SERAPHIEL_RUN_SLOW_PET_TESTS"} \
