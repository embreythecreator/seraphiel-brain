#!/usr/bin/env python3
"""Drive the Seraphiel TUI under SERAPHIEL_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.seraphiel/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with SERAPHIEL_TUI_RESUME set so it
bypasses the seraphiel_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  SERAPHIEL_PERF_LOG     (default ~/.seraphiel/perf.log)
  SERAPHIEL_PERF_NODE    (default node from $PATH)
  SERAPHIEL_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests SERAPHIEL_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_seraphiel_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("SERAPHIEL_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".seraphiel"

DEFAULT_TUI_DIR = Path(
    os.environ.get("SERAPHIEL_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("SERAPHIEL_PERF_LOG", str(get_seraphiel_home() / "perf.log")))
DEFAULT_STATE_DB = get_seraphiel_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — SERAPHIEL_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["SERAPHIEL_DEV_PERF"] = "1"
    env["SERAPHIEL_DEV_PERF_MS"] = str(args.threshold_ms)
    env["SERAPHIEL_DEV_PERF_LOG"] = str(log)
    env["SERAPHIEL_TUI_RESUME"] = sid
    ...
    node = os.environ.get("SERAPHIEL_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="SERAPHIEL_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "seraphiel-ink" / "src"
