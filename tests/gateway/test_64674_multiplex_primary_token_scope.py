...
class TestLoadGatewayConfigForRunner:
    ...
    def test_scoped_reload_picks_up_default_profile_token(self, tmp_path, monkeypatch):
        ...
        from gateway import run as run_mod
        import seraphiel_constants as hc
        ...
        monkeypatch.delenv("TELEGRAM_BOT_TOKEN", raising=False)
        # Point both seraphiel_constants and gateway.run at our temp home.
        monkeypatch.setattr(hc, "get_seraphiel_home", lambda: home)
        monkeypatch.setattr(run_mod, "get_seraphiel_home", lambda: home)
        monkeypatch.setattr(run_mod, "_seraphiel_home", home)
