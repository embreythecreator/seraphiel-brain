...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("SERAPHIEL_SESSION_PLATFORM") == "api_server"
        finally:
            clear_session_vars(tokens)
...
class TestDeclareStatelessChannel:
    """``seraphiel -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``SERAPHIEL_SESSION_*`` routing keys, so an async delegation's
