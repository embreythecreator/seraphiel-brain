...
def test_mention_patterns_list_matches():
    adapter = _make_adapter(mention_patterns=["hey seraphiel", "seraphiel,"])
    assert adapter._slack_message_matches_mention_patterns("hey seraphiel, you there?") is True
    assert adapter._slack_message_matches_mention_patterns("just chatting") is False


def test_mention_patterns_case_insensitive():
    adapter = _make_adapter(mention_patterns=["hey seraphiel"])
    assert adapter._slack_message_matches_mention_patterns("HEY SERAPHIEL!") is True


def test_mention_patterns_single_string():
    adapter = _make_adapter(mention_patterns="^seraphiel")
    assert adapter._slack_message_matches_mention_patterns("seraphiel do this") is True
    assert adapter._slack_message_matches_mention_patterns("ok seraphiel") is False


def test_mention_patterns_invalid_regex_skipped_without_crash():
    # An invalid pattern is dropped; valid siblings still work.
    adapter = _make_adapter(mention_patterns=["(unclosed", "hey seraphiel"])
    assert adapter._slack_message_matches_mention_patterns("hey seraphiel") is True


def test_mention_patterns_env_var_fallback(monkeypatch):
    monkeypatch.setenv("SLACK_MENTION_PATTERNS", '["hey seraphiel", "seraphiel,"]')
    adapter = _make_adapter()  # no config value -> falls back to env
    assert adapter._slack_message_matches_mention_patterns("hey seraphiel") is True
...
def test_mention_patterns_env_var_csv_fallback_splits_patterns(monkeypatch):
    ...
    assert [pattern.pattern for pattern in patterns] == ["hey seraphiel", "seraphiel"]
    assert adapter._slack_message_matches_mention_patterns("hey seraphiel") is True
...
def test_mention_patterns_trigger_in_channel_without_literal_mention():
    ...
    adapter = _make_adapter(require_mention=True, mention_patterns=["hey seraphiel"])
    assert _would_process(adapter, text="hey seraphiel what's the status") is True
