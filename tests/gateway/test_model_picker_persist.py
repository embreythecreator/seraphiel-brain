...
closure the PR changed, against a real temp ``SERAPHIEL_HOME``.
"""
...
def _fake_switch_result():
    ...
    from seraphiel_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "seraphiel_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "seraphiel_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_seraphiel_home", seraphiel_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_seraphiel_home() / config.yaml`` — point it here.
    monkeypatch.setattr("seraphiel_constants.get_seraphiel_home", lambda: seraphiel_home)
    monkeypatch.setattr("seraphiel_cli.config.get_seraphiel_home", lambda: seraphiel_home)
    return cfg_path
...
async def test_multiplex_picker_keeps_profile_adapter_and_callback_scope(
    ...
    monkeypatch.setattr("seraphiel_cli.model_switch.switch_model", _profile_switch)
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    named_adapter = _FakePickerAdapter()
    runner = _make_named_runner(monkeypatch, default_adapter, named_adapter, named_home)
    monkeypatch.setattr(gateway_run, "_seraphiel_home", default_home)
    _stub_picker_dependencies(monkeypatch)
