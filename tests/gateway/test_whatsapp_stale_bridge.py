...
``seraphiel update``, serving pre-update bridge.js behavior forever (e.g.
...
def _fresh_node_modules(bridge_dir: Path) -> None:
    ...
    nm.mkdir()
    (nm / ".seraphiel-pkg-hash").write_text(
...
class TestDepRefreshStamp:
    ...
    async def test_reinstalls_when_package_json_changed(self, tmp_path):
        bridge_dir = _setup_bridge_dir(tmp_path)
        _fresh_node_modules(bridge_dir)
        # Simulate `seraphiel update` bumping the Baileys pin
        ...
        from gateway.platforms.whatsapp import _file_content_hash
        stamp = (bridge_dir / "node_modules" / ".seraphiel-pkg-hash").read_text().strip()
...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["SERAPHIEL_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["SERAPHIEL_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["SERAPHIEL_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
