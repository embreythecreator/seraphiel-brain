...
async def test_monitor_to_drain_transcribes_and_echoes_pending_voice_once(
    ...
    monkeypatch.setenv("SERAPHIEL_GATEWAY_NOTIFY_INTERVAL", "0")
    ...
    with (
        patch("gateway.run._seraphiel_home", tmp_path),
...
async def test_busy_voice_interrupt_transcribes_before_pending_drain(monkeypatch):
    ...
    monkeypatch.setenv("SERAPHIEL_GATEWAY_BUSY_ACK_ENABLED", "false")
