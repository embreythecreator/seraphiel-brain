...
``seraphiel_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``seraphiel_cli.config`` (get_env_value / save_env_value) and
``seraphiel_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch those
source modules.
"""
import seraphiel_cli.config as config_mod
import seraphiel_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to seraphiel_cli.slack_cli + filesystem; stub it.
    import seraphiel_cli.slack_cli as slack_cli_mod
