...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_seraphiel_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Seraphiel, summarize this")
        assert adapter._message_matches_mention_patterns("@Seraphiel agent help")
        assert not adapter._message_matches_mention_patterns("casual family chatter")
        assert not adapter._message_matches_mention_patterns("antiseraphiel should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Seraphiel what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Seraphiel, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Seraphiel agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Seraphiel about this") == "please ask Seraphiel about this"
