...
from seraphiel_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(seraphiel_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <seraphiel_home>/plugins/<name> and opt it in."""
    plugin_dir = seraphiel_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path / "seraphiel_empty"))
