...
def _make_agent():
    seraphiel_home = Path(tempfile.mkdtemp(prefix="seraphiel-test-home-"))
    (seraphiel_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._seraphiel_home", seraphiel_home),
