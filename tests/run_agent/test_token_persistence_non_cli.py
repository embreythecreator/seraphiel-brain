...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    seraphiel_state = ModuleType("seraphiel_state")
    seraphiel_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "seraphiel_state", seraphiel_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Seraphiel"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Seraphiel"
