...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.seraphiel/seraphiel-brain/utils.py)
...
2. ``seraphiel update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``seraphiel_cli/main.py`` (the ``seraphiel update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``seraphiel_constants`` after the
