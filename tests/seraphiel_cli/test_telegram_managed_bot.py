"""Tests for seraphiel_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("seraphiel_")
        assert name.endswith("_bot")
        assert len(name) == len("seraphiel_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("seraphiel_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "seraphiel_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="Seraphiel & Friends",
        )
        assert "Seraphiel+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "seraphiel_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/SeraphielSetupBot/seraphiel_abcdefghijklmnop_bot?name=Seraphiel+Agent",
            "qr_payload": "https://t.me/newbot/SeraphielSetupBot/seraphiel_abcdefghijklmnop_bot?name=Seraphiel+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "seraphiel_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Seraphiel Brain")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="seraphiel_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/SeraphielSetupBot/seraphiel_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/SeraphielSetupBot/seraphiel_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "seraphiel_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "seraphiel_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("seraphiel_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("seraphiel_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("seraphiel_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("seraphiel_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("seraphiel_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "seraphiel_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("seraphiel_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("seraphiel_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
