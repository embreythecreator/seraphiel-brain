"""Tests for the unified provider catalog (seraphiel_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``seraphiel model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from seraphiel_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_seraphiel_model_provider():
    """PARITY CONTRACT: the catalog == the `seraphiel model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `seraphiel model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_seraphiel_model():
    """The catalog must not invent providers `seraphiel model` doesn't show."""
