"""Tests for seraphiel_cli.doctor."""
...
import seraphiel_cli.doctor as doctor
import seraphiel_cli.gateway as gateway_cli
from seraphiel_cli import doctor as doctor_mod
from seraphiel_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `seraphiel doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "SERAPHIEL_HOME", seraphiel_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("SERAPHIEL_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("SERAPHIEL_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_seraphiel_home(self, tmp_path, provider=""):
        """Create a minimal SERAPHIEL_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_seraphiel_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "SERAPHIEL_HOME", home)
        ...
        try:
            from seraphiel_cli import auth as _auth_mod
...
def test_run_doctor_accepts_seraphiel_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp SERAPHIEL_HOME.

        ``run_doctor`` reads the module-level ``SERAPHIEL_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("SERAPHIEL_HOME")`` alone
        leaves doctor probing the REAL ~/.seraphiel. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from seraphiel_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("seraphiel_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "seraphiel_cli.doctor", raising=False)
        from seraphiel_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale SERAPHIEL_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir(parents=True)
        (seraphiel_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"SERAPHIEL_MAX_ITERATIONS={ghost}\n")
        (seraphiel_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "SERAPHIEL_HOME", seraphiel_home)
        monkeypatch.setattr(doctor_mod, "get_seraphiel_home", lambda: seraphiel_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("SERAPHIEL_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("SERAPHIEL_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale SERAPHIEL_MAX_ITERATIONS" in out
        env_after = (seraphiel_home / ".env").read_text(encoding="utf-8")
        assert "SERAPHIEL_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`seraphiel doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Seraphiel install.
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_collect_deprecated_env_vars(self):
        ...
        assert "SERAPHIEL_TOOL_PROGRESS" in names
        assert "TERMINAL_CWD" in names
        assert "QQ_HOME_CHANNEL" in names
        assert "OPENAI_API_KEY" not in names
        by_name = dict(findings)
        assert "display.tool_progress" in by_name["SERAPHIEL_TOOL_PROGRESS"]
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir(parents=True)
        (seraphiel_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (seraphiel_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, seraphiel_home = self._run_doctor_with_config(monkeypatch, tmp_path, config_yaml=cfg)
        ...
        on_disk = (seraphiel_home / "config.yaml").read_text(encoding="utf-8")
...
"""
        ...
        assert "Deprecated: SERAPHIEL_TOOL_PROGRESS" in out
...
"""
        ...
        assert "Deprecated: SERAPHIEL_TOOL_PROGRESS" not in out
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"SERAPHIEL_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: SERAPHIEL_TOOL_PROGRESS_MODE" in out
