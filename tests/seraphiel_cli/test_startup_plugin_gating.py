...
``seraphiel_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``seraphiel --help``, ``seraphiel version``,
``seraphiel logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``seraphiel --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from seraphiel_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["seraphiel", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["seraphiel", "-w", "--tui"], None),
        (["seraphiel", "version"], "version"),
        (["seraphiel", "--tui", "chat"], "chat"),
        (["seraphiel", "-w", "logs"], "logs"),
        (["seraphiel", "chat", "hello world"], "chat"),
        (["seraphiel", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["seraphiel", "-m", "gpt5", "chat"], "chat"),
        (["seraphiel", "--model", "gpt5", "chat", "hi"], "chat"),
        (["seraphiel", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["seraphiel", "-z", "hello world"], None),
        (["seraphiel", "-z", "hello", "chat"], "chat"),
        (["seraphiel", "--model=gpt5", "chat"], "chat"),     # inline form
        (["seraphiel", "--", "chat"], "chat"),               # -- terminator
        (["seraphiel", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["seraphiel", "some-plugin-cmd"], "some-plugin-cmd"),
        (["seraphiel", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["seraphiel"],                          # bare → chat
        ["seraphiel", "--help"],                # top-level help
        ["seraphiel", "-h"],
        ["seraphiel", "version"],               # known built-in
        ["seraphiel", "logs"],
        ["seraphiel", "gateway", "run"],
        ["seraphiel", "--tui"],
        ["seraphiel", "-w", "--tui"],
        ["seraphiel", "chat", "hi"],
        ["seraphiel", "help"],                  # accepted built-in-ish
        ["seraphiel", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["seraphiel", "meet", "join"],          # potential google_meet plugin
        ["seraphiel", "honcho", "status"],      # potential memory plugin
        ["seraphiel", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``seraphiel help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"seraphiel_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
