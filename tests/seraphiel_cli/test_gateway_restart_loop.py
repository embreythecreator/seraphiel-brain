...
Covers:
- Defense 1: gateway stop/restart refuse when _SERAPHIEL_GATEWAY=1
...
from seraphiel_cli.cron import (
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.seraphiel.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.seraphiel.gateway.plist",
        "launchctl stop ai.seraphiel.gateway",
        "systemctl restart seraphiel-gateway",
        "systemctl stop seraphiel-gateway.service",
        "systemctl start seraphiel-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill seraphiel gateway process",
        "pkill -f seraphiel.*gateway",
        "pkill -f gateway.*seraphiel",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "seraphiel cron list",
        "seraphiel update",
        "seraphiel config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `seraphiel gateway start` is benign — starting a gateway from inside a
        ...
        "seraphiel gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway seraphiel
        # services must not be falsely blocked (the old `.*seraphiel` matched any
        # seraphiel token).
        "launchctl unload ai.seraphiel.update-checker.plist",
        "launchctl restart ai.seraphiel.daemon",
        "systemctl restart seraphiel-meta.service",
        "systemctl restart seraphiel-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_seraphiel_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade seraphiel then run seraphiel gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.seraphiel.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_seraphiel_gateway_once.sh). The script must live under
        # SERAPHIEL_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path / ".seraphiel"))
        scripts_dir = tmp_path / ".seraphiel" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify seraphiel gateway stop/restart refuse when _SERAPHIEL_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SERAPHIEL_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SERAPHIEL_GATEWAY=1.

    Issue #37453: systemctl --user restart seraphiel-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart seraphiel-gateway",
        "systemctl --user restart seraphiel-gateway",
        "systemctl stop seraphiel-gateway.service",
        "seraphiel gateway restart",
        "launchctl kickstart gui/501/ai.seraphiel.gateway",
        "pkill -f seraphiel.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart seraphiel-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-seraphiel systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _SERAPHIEL_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart seraphiel-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart seraphiel-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run seraphiel gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("seraphiel gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeseraphiel gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SERAPHIEL_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.seraphiel.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    seraphiel_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run seraphiel gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path / ".seraphiel"))
        (tmp_path / ".seraphiel").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run seraphiel gateway restart nightly",
        ))
