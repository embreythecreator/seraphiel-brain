...
The Python launcher (`seraphiel --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Seraphiel session
...
from seraphiel_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["seraphiel", "--tui", "-c", "abc"])
        monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
        monkeypatch.delenv("SERAPHIEL_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_seraphiel_tui_env_set(self, monkeypatch):
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
        monkeypatch.setenv("SERAPHIEL_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `seraphiel --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
