...
When ``SERAPHIEL_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.seraphiel/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.seraphiel            (has its own auth.json fixture)
    * Profile     -> tmp_path/.seraphiel/profiles/coder   (active, SERAPHIEL_HOME points here)
    ...
    global_root = tmp_path / ".seraphiel"
    global_root.mkdir()
...
# ``_load_provider_state`` itself, those helpers raise ``"Seraphiel is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from seraphiel_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    seraphiel_home = tmp_path / "classic"
    seraphiel_home.mkdir()
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))

    _write(seraphiel_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SERAPHIEL_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.seraphiel as the "real home" — which is NOT equal
    # to the SERAPHIEL_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    seraphiel_home = tmp_path / "classic"
    ...
    _write(seraphiel_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from seraphiel_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SERAPHIEL_HOME is set to a custom path that is NOT under
    # ~/.seraphiel/profiles/ — get_default_seraphiel_root() returns SERAPHIEL_HOME
...
def test_provider_state_transaction_locks_global_fallback_before_use(
    ...
    import seraphiel_cli.auth as auth
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_seraphiel_home_override(profile_b)
        try:
