"""Regression tests for the OAuth dispatcher in seraphiel_cli.web_server.
...
from seraphiel_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Seraphiel-Session-Token": _SESSION_TOKEN}
...
def _fake_nous_device_data():
    return {
        ...
        "user_code": "NOUS-1234",
        "verification_uri": "https://portal.embreythecreator.com/device",
        "verification_uri_complete": (
            "https://portal.embreythecreator.com/device?user_code=NOUS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.embreythecreator.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "seraphiel_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "seraphiel_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "seraphiel_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    ...
    from seraphiel_cli import web_server as ws
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_seraphiel_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "seraphiel auth add fake-oauth",
...
def test_nous_dashboard_device_flow_does_not_retry_legacy_scope_on_invoke_refusal(monkeypatch):
    ...
    monkeypatch.delenv("SERAPHIEL_AGENT_USE_LEGACY_SESSION_KEYS", raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_nous_poller", lambda sid: None)
...
def test_codex_dashboard_worker_persists_inside_session_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_mod,
        "_save_codex_tokens",
        lambda tokens: saved_homes.append(get_seraphiel_home()),
    )
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "seraphiel auth" not in detail
    finally:
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "seraphiel_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "seraphiel_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "seraphiel_cli.web_server._start_anthropic_pkce",
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `seraphiel model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `seraphiel model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Seraphiel."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``seraphiel auth remove
    xai-oauth``.
    """
    from seraphiel_cli import auth as auth_mod
    from seraphiel_cli import web_server as ws
    ...
    # Prior `seraphiel auth remove xai-oauth` left the source suppressed.
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "seraphiel auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to seraphiel_cli.auth.get_auth_status (the
    ...
    with patch("seraphiel_cli.auth.get_auth_status", return_value=fake_status):
...
def test_status_hardcoded_branch_wins_over_generic_fallback():
    ...
    with patch(
        "seraphiel_cli.auth.get_nous_auth_status",
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("seraphiel_cli.auth.get_auth_status", return_value={"logged_in": False}):
