"""ensure_seraphiel_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".seraphiel"
    second = tmp_path / "b" / ".seraphiel"

    monkeypatch.setenv("SERAPHIEL_HOME", str(first))
    cfg.ensure_seraphiel_home()

    # Profile switch: SERAPHIEL_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SERAPHIEL_HOME", str(second))
    cfg.ensure_seraphiel_home()
