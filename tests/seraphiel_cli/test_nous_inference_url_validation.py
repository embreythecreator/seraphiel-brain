...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.embreythecreator.com/v1"
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="seraphiel_cli.auth"):
            assert (
    ...
    def test_subdomain_of_allowlist_host_rejected(self):
        """*.embreythecreator.com is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.embreythecreator.com would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_nous_inference_url_from_network(
                "https://evil.inference-api.embreythecreator.com/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="seraphiel_cli.auth"):
            assert (
                _validate_nous_inference_url_from_network(
                    "http://inference-api.embreythecreator.com/v1"
                )
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``inference-api.embreythecreator.com``, they MUST update the allowlist
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        import seraphiel_cli.auth as _auth_mod
    ...
    def test_proxy_adapter_also_validates(self):
        ...
        boundary."""
        from pathlib import Path
        import seraphiel_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.embreythecreator.com`` persisted before the
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-inference-api.embreythecreator.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
    ...
    def test_refresh_keeps_valid_url(self, monkeypatch):
        ...
        good = "https://inference-api.embreythecreator.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "seraphiel-cli",
...
class TestEnvOverrideWins:
    ...
    STAGING = "https://stg-inference-api.embreythecreator.com/v1"
