...
``seraphiel update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
import seraphiel_cli.main as m
...
def test_recovery_runs_install_and_clears_marker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("seraphiel_cli.managed_uv.ensure_uv", lambda: None)
    monkeypatch.setattr(
        m,
        "_install_python_dependencies_with_optional_fallback",
...
def test_recovery_self_lock_guard_clears_marker_without_install(tmp_path, monkeypatch):
    # Windows self-lock: seraphiel.exe is an ancestor of this Python process, so a
    ...
    monkeypatch.setattr(m, "_seraphiel_exe_shims", lambda d: [shim])
...
def test_recovery_self_lock_guard_inactive_when_not_ancestor(tmp_path, monkeypatch):
    # Windows, but seraphiel.exe is NOT in the ancestry (launched via `seraphiel
    ...
    class FakeProc:
        ...
        def parents(self):
            # Ancestry is plain pythons / cmd — no seraphiel.exe shim.
