"""Tests for the per-profile Projects store (seraphiel_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Seraphiel Brain", folders=["/tmp/seraphiel"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "seraphiel-brain"
    assert proj.name == "Seraphiel Brain"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/seraphiel"
    assert [f.path for f in proj.folders] == ["/tmp/seraphiel"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "seraphiel-brain").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Seraphiel Brain")
    pdb.create_project(conn, name="Seraphiel Brain")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["seraphiel-brain", "seraphiel-brain-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SERAPHIEL_HOME.
...
def test_db_path_under_seraphiel_home():
    # Resolves under SERAPHIEL_HOME (set by the autouse isolation fixture).
