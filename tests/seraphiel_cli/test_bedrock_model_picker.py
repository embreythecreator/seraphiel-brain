...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SERAPHIEL_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SERAPHIEL_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SERAPHIEL_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from seraphiel_cli.providers import SERAPHIEL_OVERLAYS
        assert "bedrock" in SERAPHIEL_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from seraphiel_cli.providers import SERAPHIEL_OVERLAYS
        assert SERAPHIEL_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from seraphiel_cli.providers import SERAPHIEL_OVERLAYS
        assert SERAPHIEL_OVERLAYS["bedrock"].auth_type == "aws_sdk"
