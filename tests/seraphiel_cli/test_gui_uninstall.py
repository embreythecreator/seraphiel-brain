"""Tests for seraphiel_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import seraphiel_cli.gui_uninstall as gu


def _make_agent(seraphiel_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = seraphiel_home / "seraphiel-brain"
    (agent_root / "seraphiel_cli").mkdir(parents=True)
    (agent_root / "seraphiel_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(seraphiel_home: Path) -> None:
    """Create the source-built GUI artifacts a `seraphiel desktop` run produces."""
    desktop = seraphiel_home / "seraphiel-brain" / "apps" / "desktop"
    ...
    (seraphiel_home / "seraphiel-brain" / "node_modules").mkdir(parents=True)
    (seraphiel_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(seraphiel_home: Path) -> None:
    (seraphiel_home / "config.yaml").write_text("x: 1\n")
    (seraphiel_home / ".env").write_text("KEY=secret\n")
    (seraphiel_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    seraphiel_home = tmp_path / ".seraphiel"
    seraphiel_home.mkdir()
    assert gu.agent_is_installed(seraphiel_home) is False
    _make_agent(seraphiel_home)
    assert gu.agent_is_installed(seraphiel_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    seraphiel_home = tmp_path / ".seraphiel"
    (seraphiel_home / "seraphiel-brain" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(seraphiel_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    seraphiel_home = tmp_path / ".seraphiel"
    _make_gui_build(seraphiel_home)
    artifacts = gu.source_built_gui_artifacts(seraphiel_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(seraphiel_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(seraphiel_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    seraphiel_home = tmp_path / ".seraphiel"
    agent_root = _make_agent(seraphiel_home)
    _make_gui_build(seraphiel_home)
    _make_user_data(seraphiel_home)
    ...
    removed = gu.uninstall_gui(seraphiel_home)
    ...
    assert not (seraphiel_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "seraphiel_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (seraphiel_home / "config.yaml").exists()
    assert (seraphiel_home / ".env").exists()
    assert (seraphiel_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    seraphiel_home = tmp_path / ".seraphiel"
    _make_agent(seraphiel_home)
    userdata = tmp_path / "Seraphiel-userdata"
    userdata.mkdir()
    ...
    gu.uninstall_gui(seraphiel_home)
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(seraphiel_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(seraphiel_home)
    ...
    assert summary["seraphiel_home"] == str(seraphiel_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Seraphiel"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Seraphiel"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Seraphiel"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Seraphiel"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    SERAPHIEL_HOME with all the destructive externals stubbed out.
    """
    import seraphiel_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_seraphiel_home", lambda: seraphiel_home)
    ...
    from seraphiel_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (seraphiel_home / "config.yaml").exists()
    assert seraphiel_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole SERAPHIEL_HOME non-interactively."""
    import seraphiel_cli.uninstall as uninstall

    seraphiel_home = tmp_path / ".seraphiel"
    (seraphiel_home / "seraphiel-brain" / "seraphiel_cli").mkdir(parents=True)
    (seraphiel_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not seraphiel_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m seraphiel_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_seraphiel_home", lambda: seraphiel_home)
    ...
    assert not (desktop / "release").exists()
    assert not (seraphiel_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "seraphiel_cli").exists()
    assert (seraphiel_home / "config.yaml").exists()
