"""Dashboard Seraphiel Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_seraphiel_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from seraphiel_cli.console_engine import ConsoleResult, SeraphielConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.5)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(SeraphielConsoleEngine, "execute", slow_execute)
