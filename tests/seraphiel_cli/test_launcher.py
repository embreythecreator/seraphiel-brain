"""Tests for the top-level `./seraphiel` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./seraphiel` should use `seraphiel_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "seraphiel"
    called = []

    fake_main_module = types.ModuleType("seraphiel_cli.main")

    def fake_main():
        called.append("seraphiel_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "seraphiel_cli.main", fake_main_module)
    ...
    assert called == ["seraphiel_cli.main"]
