...
def main_mod(monkeypatch):
    import seraphiel_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["seraphiel", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["seraphiel", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["seraphiel", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["seraphiel", "--tui"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
    monkeypatch.setattr(
        sys, "argv", ["seraphiel", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
    monkeypatch.delenv("SERAPHIEL_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("SERAPHIEL_FAST_STARTUP_BANNER", raising=False)
    monkeypatch.setattr(sys, "argv", ["seraphiel"])
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
    monkeypatch.setattr(
    ...
    assert os.environ["SERAPHIEL_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["SERAPHIEL_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_version_skips_update_check(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["seraphiel", "version"])
    monkeypatch.setattr(
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("SERAPHIEL_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["seraphiel", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Seraphiel Brain v" in out
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["seraphiel", "chat", "--help"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("SERAPHIEL_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("SERAPHIEL_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["seraphiel", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_seraphiel_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("SERAPHIEL_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_run_and_exit_oneshot_hard_exits_when_oneshot_import_fails(
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "seraphiel_cli.oneshot" and "run_oneshot" in (fromlist or ()):
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import seraphiel_cli.oneshot as oneshot
        from seraphiel_cli.main import _exit_after_oneshot
...
def test_main_oneshot_path_bypasses_late_atexit_abort():
    ...
    program = textwrap.dedent(
        ...
        sys.modules["seraphiel_cli.oneshot"] = types.SimpleNamespace(
...
def test_oneshot_run_agent_closes_agent_after_chat(monkeypatch):
    import seraphiel_cli.oneshot as oneshot_mod
...
def test_oneshot_exit_code_when_failed_without_response(monkeypatch):
    ...
    monkeypatch.setattr(
        "seraphiel_cli.oneshot._run_agent",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from seraphiel_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "seraphiel -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """seraphiel -z bypasses SeraphielCLI, but recall still needs SessionDB."""
    from seraphiel_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "seraphiel_state", mod("seraphiel_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "seraphiel_cli.config",
        mod("seraphiel_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "seraphiel_cli.models",
        mod("seraphiel_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "seraphiel_cli.runtime_provider",
        mod(
            "seraphiel_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "seraphiel_cli.tools_config",
        mod("seraphiel_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SERAPHIEL_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/seraphiel-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["SERAPHIEL_MODEL"] == "nous/seraphiel-test"
    assert env["SERAPHIEL_INFERENCE_MODEL"] == "nous/seraphiel-test"
    assert env["SERAPHIEL_TUI_PROVIDER"] == "nous"
    assert env["SERAPHIEL_INFERENCE_PROVIDER"] == "nous"
    assert env["SERAPHIEL_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["SERAPHIEL_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("seraphiel-tui-active-session-")
...
def test_launch_tui_worktree_validates_relative_python_against_final_cwd(
    ...
    monkeypatch.setenv("SERAPHIEL_CWD", str(parent_cwd))
    ...
    assert captured["env"]["SERAPHIEL_CWD"] == str(worktree)
    assert captured["env"]["SERAPHIEL_PYTHON"] == str(relative_python)


def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_seraphiel_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/seraphiel-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("seraphiel_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("SERAPHIEL_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "SERAPHIEL_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["SERAPHIEL_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_seraphiel_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("SERAPHIEL_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "seraphiel_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "seraphiel --tui --resume 20260409_000001_abc123" in out
    assert 'seraphiel --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "seraphiel --tui --resume actual_session" in out
