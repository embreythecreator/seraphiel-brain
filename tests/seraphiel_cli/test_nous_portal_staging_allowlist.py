...
``SERAPHIEL_PORTAL_BASE_URL=https://portal.staging-embreythecreator.com`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("SERAPHIEL_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert _nous_portal_env_override() is None

    def test_seraphiel_portal_base_url_wins(self, monkeypatch):
        monkeypatch.setenv(
            "SERAPHIEL_PORTAL_BASE_URL", "https://portal.staging-embreythecreator.com/"
        )
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert (
            _nous_portal_env_override() == "https://portal.staging-embreythecreator.com"
        )

    def test_nous_portal_base_url_used_as_fallback(self, monkeypatch):
        monkeypatch.delenv("SERAPHIEL_PORTAL_BASE_URL", raising=False)
        monkeypatch.setenv(
            "NOUS_PORTAL_BASE_URL", "https://portal.staging-embreythecreator.com"
        )
        assert (
            _nous_portal_env_override() == "https://portal.staging-embreythecreator.com"
        )
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        assert "portal.staging-embreythecreator.com" not in _NOUS_PORTAL_ALLOWED_HOSTS
        assert (
            _nous_portal_env_override() == "https://portal.staging-embreythecreator.com"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("seraphiel_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior SERAPHIEL_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-embreythecreator.com"
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        monkeypatch.setenv("SERAPHIEL_PORTAL_BASE_URL", staging_portal)
