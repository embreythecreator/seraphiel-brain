"""Tests for seraphiel_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `seraphiel gateway restart` spawned a
    foreground gateway that fought the supervised one. Detection must key off
    s6 being PID 1 (`_s6_running`) alone."""
    monkeypatch.setattr(
        "seraphiel_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from seraphiel_cli.service_manager import _s6_running
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the seraphiel user. The new
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "seraphiel_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "seraphiel_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "seraphiel_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "seraphiel_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "seraphiel_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "seraphiel_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "seraphiel_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "seraphiel_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `seraphiel_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import seraphiel_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("seraphiel_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "seraphiel_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "seraphiel_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "seraphiel_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "seraphiel_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
# unprivileged seraphiel user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from seraphiel_cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "seraphiel -p coder gateway run" in run_text
    assert "s6-setuidgid seraphiel" in run_text
    ...
    # in seraphiel_cli/gateway.py::_gateway_command_inner.
    assert "export SERAPHIEL_S6_SUPERVISED_CHILD=1" in run_text

    log_run = svc_dir / "log" / "run"
    assert log_run.is_file()
    log_text = log_run.read_text()
    # CRITICAL: SERAPHIEL_HOME must be a runtime env-var expansion, NOT
    # a Python-substituted absolute path. Negative-assert the wrong
    # form so future regressions are caught.
    assert "$SERAPHIEL_HOME" in log_text
    assert "logs/gateways/coder" in log_text
    assert "/opt/data/logs/gateways/coder" not in log_text, (
        "log_dir was hard-coded; must use ${SERAPHIEL_HOME} at run time"
    )
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import seraphiel_cli.service_manager as sm
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid seraphiel seraphiel -p coder gateway run --replace" in run_text
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``seraphiel gateway run``, an agent action, the Open WebUI helper) holds
    the per-SERAPHIEL_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root SERAPHIEL_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `seraphiel gateway run --replace` (no -p flag).
    assert "seraphiel gateway run --replace" in default_text
    assert "seraphiel -p default" not in default_text
...
def test_s6_lifecycle_persists_named_profile_desired_state(
    ...
    profile_dir = seraphiel_home / "profiles" / "coder"
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home / "profiles" / "coder"))

    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.start("gateway-default")
    state = json.loads((seraphiel_home / "gateway_state.json").read_text())
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "seraphiel profile create typo" in msg
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from seraphiel_cli.service_manager import S6CommandError
...
#
# `seraphiel gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    dropped seraphiel user and s6-log crash-loops on `mkdir: Permission denied`.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    parent_chown = 'chown seraphiel:seraphiel "$SERAPHIEL_HOME/logs/gateways"'
    ...
    assert 'chown -R seraphiel:seraphiel "$SERAPHIEL_HOME/logs/gateways"' not in log_text
    ...
    leaf_idx = log_text.index('chown -R seraphiel:seraphiel "$log_dir"')
