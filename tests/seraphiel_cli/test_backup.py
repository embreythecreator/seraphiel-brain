"""Tests for seraphiel backup and import commands."""
...
def _make_seraphiel_tree(root: Path) -> None:
    """Create a realistic ~/.seraphiel directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "seraphiel_state.db"):
    ...
    # seraphiel-brain repo (should be EXCLUDED)
    (root / "seraphiel-brain").mkdir(exist_ok=True)
    (root / "seraphiel-brain" / "run_agent.py").write_text("# big file\n")
    (root / "seraphiel-brain" / ".git").mkdir()
    (root / "seraphiel-brain" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_seraphiel_brain(self):
        from seraphiel_cli.backup import _should_exclude
        assert _should_exclude(Path("seraphiel-brain/run_agent.py"))
        assert _should_exclude(Path("seraphiel-brain/.git/HEAD"))
    ...
    def test_includes_nested_seraphiel_brain_in_skills(self):
        """skills/autonomous-ai-agents/seraphiel-brain/ must NOT be excluded —
        only the root-level seraphiel-brain/ repo is skipped."""
        from seraphiel_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/seraphiel-brain/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/seraphiel-brain/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under SERAPHIEL_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        _make_seraphiel_tree(seraphiel_home)

        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        # get_default_seraphiel_root needs this
    ...
    def test_failed_sqlite_backup_never_raw_copies_live_wal_db(self, tmp_path, monkeypatch, capsys):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "config.yaml").write_text("model: test\n")
        db_path = seraphiel_home / "state.db"
        ...
        import seraphiel_cli.backup as backup_mod
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = seraphiel_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, seraphiel_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_seraphiel_brain(self, tmp_path, monkeypatch):
        """Backup does NOT include seraphiel-brain/ directory."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        _make_seraphiel_tree(seraphiel_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "seraphiel-brain" in n]
            assert agent_files == [], f"seraphiel-brain files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under SERAPHIEL_HOME must be
        ...
        venv_pkg = seraphiel_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = seraphiel_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_seraphiel_brain_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../seraphiel-brain/ but NOT root seraphiel-brain/."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        _make_seraphiel_tree(seraphiel_home)

        # Add a nested seraphiel-brain directory inside skills (like the real layout)
        nested = seraphiel_home / "skills" / "autonomous-ai-agents" / "seraphiel-brain"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Seraphiel Brain Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root seraphiel-brain must be excluded
            root_agent = [n for n in names if n.startswith("seraphiel-brain/")]
            assert root_agent == [], f"root seraphiel-brain leaked: {root_agent}"
            # Nested skill seraphiel-brain must be included
            assert "skills/autonomous-ai-agents/seraphiel-brain/SKILL.md" in names
            assert "skills/autonomous-ai-agents/seraphiel-brain/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/seraphiel-backup-*.zip."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("seraphiel-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside SERAPHIEL_HOME."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        _make_seraphiel_tree(seraphiel_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(seraphiel_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Seraphiel backup."""
        from seraphiel_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only seraphiel_state.db (old wrong name) is rejected."""
        from seraphiel_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["seraphiel_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into seraphiel home."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (seraphiel_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (seraphiel_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (seraphiel_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (seraphiel_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_seraphiel_prefix(self, tmp_path, monkeypatch):
        """Import strips .seraphiel/ prefix if all entries share it."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".seraphiel/config.yaml": "model: test\n",
            ".seraphiel/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (seraphiel_home / "config.yaml").read_text() == "model: test\n"
        assert (seraphiel_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_seraphiel_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a seraphiel backup."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (seraphiel_home / "config.yaml").exists()
        # traversal file should NOT exist outside seraphiel home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (seraphiel_home / "gateway_state.json").write_text(live_state)
        ...
        assert (seraphiel_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (seraphiel_home / "config.yaml").exists()
        assert not (seraphiel_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        (seraphiel_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (seraphiel_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (seraphiel_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            seraphiel_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (seraphiel_home / "gateway.pid").write_text("4242")
        (seraphiel_home / "processes.json").write_text('{"live": true}')
        ...
        assert (seraphiel_home / "gateway.pid").read_text() == "4242"
        assert (seraphiel_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (seraphiel_home / "cron.pid").exists()
        assert not (seraphiel_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (seraphiel_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (seraphiel_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (seraphiel_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (seraphiel_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".seraphiel"
        src_home.mkdir(parents=True)
        _make_seraphiel_tree(src_home)

        monkeypatch.setenv("SERAPHIEL_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from seraphiel_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".seraphiel"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SERAPHIEL_HOME", str(dst_home))
        ...
        # seraphiel-brain should NOT be present
        assert not (dst_home / "seraphiel-brain").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from seraphiel_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without seraphiel markers fails validation."""
        import io
        from seraphiel_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_seraphiel(self):
        """Detects .seraphiel/ prefix wrapping all entries."""
        import io
        from seraphiel_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".seraphiel/config.yaml", "test")
            zf.writestr(".seraphiel/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".seraphiel/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".seraphiel/", "")
            zf.writestr(".seraphiel/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_seraphiel_home(self, tmp_path, monkeypatch):
        """Backup exits when seraphiel home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".seraphiel"
        monkeypatch.setenv("SERAPHIEL_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("seraphiel-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_seraphiel_home(self, tmp_path, monkeypatch):
        """Backup handles empty seraphiel home (no files to back up)."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        # Only excluded dirs, no actual files
        (seraphiel_home / "__pycache__").mkdir()
        (seraphiel_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = seraphiel_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = seraphiel_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_seraphiel(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside seraphiel root."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside seraphiel home
        out_zip = seraphiel_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = seraphiel_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (seraphiel_home / "config.yaml").exists()
        assert (seraphiel_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (seraphiel_home / "profiles" / "coder" / "config.yaml").exists()
        assert (seraphiel_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "seraphiel -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        def fake_import(name, *a, **kw):
            if name == "seraphiel_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from seraphiel_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def seraphiel_home(self, tmp_path):
        """Create a fake SERAPHIEL_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        assert snap_id is not None
        snap_dir = seraphiel_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", seraphiel_home=seraphiel_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        db_copy = seraphiel_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        assert (seraphiel_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_discord_recovery_ledger(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot

        gateway_dir = seraphiel_home / "gateway"
        gateway_dir.mkdir()
        ...
        copied = seraphiel_home / "state-snapshots" / snap_id / "gateway" / ledger.name
        assert copied.exists()
        conn = sqlite3.connect(copied)
        assert conn.execute("SELECT message_id FROM handled").fetchall() == [("123",)]
        conn.close()

    def test_copies_channel_aliases(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        copied = seraphiel_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        with open(seraphiel_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(seraphiel_home=empty) is None

    def test_max_file_size_skips_oversized_file(self, seraphiel_home, capsys):
        ...
        snap_id = create_quick_snapshot(
            seraphiel_home=seraphiel_home, max_file_size=cap
        )
        assert snap_id is not None
        snap_dir = seraphiel_home / "state-snapshots" / snap_id
    ...
    def test_max_file_size_none_copies_everything(self, seraphiel_home):
        """Default (no cap) preserves manual /snapshot behavior."""
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home, max_file_size=None)
        assert (seraphiel_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_max_file_size_under_cap_copies(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(
            seraphiel_home=seraphiel_home, max_file_size=1 << 30
        )
        assert (seraphiel_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_list_snapshots(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", seraphiel_home=seraphiel_home)
        id2 = create_quick_snapshot(label="second", seraphiel_home=seraphiel_home)

        snaps = list_quick_snapshots(seraphiel_home=seraphiel_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", seraphiel_home=seraphiel_home)
        snaps = list_quick_snapshots(limit=3, seraphiel_home=seraphiel_home)
        assert len(snaps) == 3

    def test_restore_config(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)

        (seraphiel_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (seraphiel_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, seraphiel_home=seraphiel_home)
        assert result is True
        assert "openrouter" in (seraphiel_home / "config.yaml").read_text()

    def test_restore_state_db(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)

        conn = sqlite3.connect(str(seraphiel_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, seraphiel_home=seraphiel_home)
    ...
    def test_restore_nonexistent(self, seraphiel_home):
        from seraphiel_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", seraphiel_home=seraphiel_home) is False

    def test_auto_prune(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", seraphiel_home=seraphiel_home)
        snaps = list_quick_snapshots(limit=100, seraphiel_home=seraphiel_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", seraphiel_home=seraphiel_home)
        deleted = prune_quick_snapshots(keep=3, seraphiel_home=seraphiel_home)
        assert deleted == 7
        assert len(list_quick_snapshots(seraphiel_home=seraphiel_home)) == 3

    def test_snapshot_includes_pairing_directories(self, seraphiel_home):
        ...
        (seraphiel_home / "platforms" / "pairing").mkdir(parents=True)
        (seraphiel_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (seraphiel_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (seraphiel_home / "pairing").mkdir()
        (seraphiel_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (seraphiel_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, seraphiel_home):
        ...
        assert restore_quick_snapshot(snap_id, seraphiel_home=seraphiel_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, seraphiel_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (seraphiel update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, seraphiel_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, seraphiel_home):
        ...
        escape_dst = seraphiel_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped SERAPHIEL_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from seraphiel_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        copy = seraphiel_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        copy = seraphiel_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, seraphiel_home):
        from seraphiel_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(seraphiel_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, seraphiel_home):
        ...
        board_dir = seraphiel_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        copy = (
            seraphiel_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, seraphiel_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(seraphiel_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, seraphiel_home):
        ...
        board = seraphiel_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(seraphiel_home=seraphiel_home)
        snap = seraphiel_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, seraphiel_home, monkeypatch):
        ...
        import seraphiel_cli.backup as bk
        from seraphiel_cli.backup import create_quick_snapshot
        ...
        copy = seraphiel_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``seraphiel update``
    ...
    @pytest.fixture
    def seraphiel_home(self, tmp_path):
        root = tmp_path / ".seraphiel"
        root.mkdir()
        _make_seraphiel_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, seraphiel_home):
        from seraphiel_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(seraphiel_home=seraphiel_home)
        assert out is not None
        assert out.exists()
        assert out.parent == seraphiel_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, seraphiel_home):
        """Pre-update backup should include the same user data that
        ``seraphiel backup`` would, and should exclude the same directories."""
        from seraphiel_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(seraphiel_home=seraphiel_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # seraphiel-brain repo excluded
        assert not any(n.startswith("seraphiel-brain/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, seraphiel_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from seraphiel_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(seraphiel_home=seraphiel_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(seraphiel_home=seraphiel_home)
    ...
    def test_rotation_keeps_only_n(self, seraphiel_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(seraphiel_home=seraphiel_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (seraphiel_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, seraphiel_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from seraphiel_cli.backup import create_pre_update_backup

        (seraphiel_home / "backups").mkdir(exist_ok=True)
        manual = seraphiel_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(seraphiel_home=seraphiel_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from seraphiel_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(seraphiel_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, seraphiel_home):
        ...
        """
        from seraphiel_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(seraphiel_home=seraphiel_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, seraphiel_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, seraphiel_home):
        ...
        first = create_pre_update_backup(seraphiel_home=seraphiel_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(seraphiel_home=seraphiel_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(seraphiel_home=seraphiel_home, keep=0)
    ...
    def test_skips_symlinked_files(self, seraphiel_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside SERAPHIEL_HOME."""
        from seraphiel_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def seraphiel_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".seraphiel"
        root.mkdir()
        _make_seraphiel_tree(root)
        # Point SERAPHIEL_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SERAPHIEL_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for seraphiel_cli.config + seraphiel_constants so they pick up SERAPHIEL_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("seraphiel_cli.config") or mod == "seraphiel_constants":
                del __import__("sys").modules[mod]
        return root

    @staticmethod
    def _set_mode(seraphiel_home, value):
        import yaml
        (seraphiel_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("seraphiel_cli.config"):
                del _sys.modules[mod]

    @staticmethod
    def _zips(seraphiel_home):
        d = seraphiel_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(seraphiel_home):
        d = seraphiel_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []

    def test_default_creates_quick_snapshot_only(self, seraphiel_home, capsys):
        """With no config, the default mode is ``quick``: a state snapshot is
        created but NOT the full zip."""
        from seraphiel_cli.main import _run_pre_update_backup
        ...
        assert self._snaps(seraphiel_home)
        assert not self._zips(seraphiel_home)

    def test_backup_flag_forces_full(self, seraphiel_home, capsys):
        ...
        assert "Saved:" in out
        assert "seraphiel import" in out
        assert len(self._zips(seraphiel_home)) == 1

    def test_no_backup_flag_skips_everything(self, seraphiel_home, capsys):
        ...
        assert not self._snaps(seraphiel_home)
        assert not self._zips(seraphiel_home)

    def test_config_off_disables_everything_silently(self, seraphiel_home, capsys):
        ...
        self._set_mode(seraphiel_home, "off")
        from seraphiel_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
    ...
    def test_legacy_false_maps_to_off(self, seraphiel_home, capsys):
        """Legacy boolean ``false`` (the old zip opt-out) now means off."""
        self._set_mode(seraphiel_home, False)
        from seraphiel_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        assert snap_id is None
    ...
    def test_legacy_true_maps_to_full(self, seraphiel_home, capsys):
        """Legacy boolean ``true`` (the old always-zip opt-in) means full."""
        self._set_mode(seraphiel_home, True)
        from seraphiel_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_full_mode(self, seraphiel_home, capsys):
        self._set_mode(seraphiel_home, "full")
        from seraphiel_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_quick_mode(self, seraphiel_home, capsys):
        self._set_mode(seraphiel_home, "quick")
        from seraphiel_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_unknown_mode_falls_back_to_quick(self, seraphiel_home, capsys):
        self._set_mode(seraphiel_home, "bogus-mode")
        from seraphiel_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_no_backup_flag_overrides_full_config(self, seraphiel_home, capsys):
    ...
    def test_backup_flag_overrides_off_config(self, seraphiel_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (seraphiel claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``seraphiel claw migrate`` runs before mutating ~/.seraphiel/."""
    ...
    def test_creates_backup_under_backups_dir(self, seraphiel_home):
        from seraphiel_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(seraphiel_home=seraphiel_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `seraphiel
        # import` and the update-backup listing both pick them up.
        assert out.parent == seraphiel_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, seraphiel_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``seraphiel backup`` / ``create_pre_update_backup`` — no drift."""
        from seraphiel_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(seraphiel_home=seraphiel_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_seraphiel_import(self, seraphiel_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Seraphiel
        backup — `seraphiel import` should accept it."""
        from seraphiel_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(seraphiel_home=seraphiel_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, seraphiel_home):
        from seraphiel_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(seraphiel_home=seraphiel_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(seraphiel_home=seraphiel_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, seraphiel_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(seraphiel_home=seraphiel_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((seraphiel_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_seraphiel_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.seraphiel yet — nothing to back up."""
        from seraphiel_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(seraphiel_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, seraphiel_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from seraphiel_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(seraphiel_home=seraphiel_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(seraphiel_home=seraphiel_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`seraphiel update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, seraphiel_home: Path, label="pre-update"):
        from seraphiel_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, seraphiel_home=seraphiel_home, keep=5)
    ...
    def test_restores_when_emptied_after_migration(self, tmp_path):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        jobs_path = seraphiel_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(seraphiel_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, seraphiel_home=seraphiel_home)
        assert result is not None
    ...
    def test_bom_live_file_still_counted(self, tmp_path):
        ...
        from seraphiel_cli.backup import _count_cron_jobs, restore_cron_jobs_if_emptied
        seraphiel_home = tmp_path / ".seraphiel"
        jobs_path = seraphiel_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(seraphiel_home)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = seraphiel_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, seraphiel_home=seraphiel_home) is None
        assert restore_cron_jobs_if_emptied("", seraphiel_home=seraphiel_home) is None
...
# location, NOT under SERAPHIEL_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, seraphiel_home: Path) -> None:
        seraphiel_home.mkdir(parents=True, exist_ok=True)
        (seraphiel_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (seraphiel_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (seraphiel_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        self._make_min_tree(seraphiel_home)
        # External provider state living OUTSIDE SERAPHIEL_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under SERAPHIEL_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        seraphiel_home = dst_home / ".seraphiel"
        seraphiel_home.mkdir()
        ...
        # External state did NOT leak into SERAPHIEL_HOME.
        assert not (seraphiel_home / "_external").exists()
