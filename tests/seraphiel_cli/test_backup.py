...
class TestRunPreUpdateBackup:
    ...
    def test_default_enabled_creates_backup(self, seraphiel_home, capsys):
        """With the new safe default (``pre_update_backup: true``), every
        ``seraphiel update`` creates a backup before any destructive step
        runs — the cost is a few minutes of zip time vs. the alternative
        of silent total data loss of ``~/.seraphiel/`` observed in #48200
