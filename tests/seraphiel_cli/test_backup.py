...
def _make_seraphiel_tree(root: Path) -> None:
    ...
    (root / "seraphiel_state.db").write_bytes(b"fake-state")
...
class TestRunPreUpdateBackup:
    ...
    def test_backup_flag_creates_backup(self, seraphiel_home, capsys):
        ...
        backups = list((seraphiel_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, seraphiel_home, capsys):
        """With the default (``pre_update_backup: false``), ``seraphiel update``
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((seraphiel_home / "backups").glob("pre-update-*.zip")) \
            if (seraphiel_home / "backups").exists() else True

    def test_no_backup_flag_skips(self, seraphiel_home, capsys):
        from seraphiel_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (seraphiel_home / "backups").exists() or not list(
            (seraphiel_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, seraphiel_home, capsys):
    ...
    def test_config_disabled_is_silent(self, seraphiel_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, seraphiel_home, capsys):
