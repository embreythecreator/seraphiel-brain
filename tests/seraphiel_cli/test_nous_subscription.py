...
from seraphiel_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from seraphiel_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "seraphiel_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `seraphiel tools`
# ---------------------------------------------------------------------------
