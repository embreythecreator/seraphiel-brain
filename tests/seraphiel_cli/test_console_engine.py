...
from seraphiel_cli.console_engine import SeraphielConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/seraphiel-console-test.zip",
    "import /tmp/seraphiel-console-test.zip",
...
def test_console_parses_bare_and_seraphiel_prefixed_commands(_isolate_seraphiel_home):
    engine = SeraphielConsoleEngine()

    bare = engine.execute("config path")
    prefixed = engine.execute("seraphiel config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    _isolate_seraphiel_home,
):
    import seraphiel_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'seraphiel doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'seraphiel setup' to configure\x1b[0m")
        print()

    monkeypatch.setattr(status_mod, "show_status", fake_show_status)

    result = SeraphielConsoleEngine().execute("status")

    assert result.status == "ok"
    assert "Sessions" in result.output
    assert "Active: 3 session(s)" in result.output
    assert "seraphiel doctor" not in result.output
    assert "seraphiel setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'seraphiel doctor' for detailed diagnostics"))
        print(osc_link("  Run 'seraphiel setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    help_text = SeraphielConsoleEngine().help_text()
    ...
    assert "Run `seraphiel skills list`" not in help_text
    assert "Run `seraphiel tools list`" not in help_text
...
def test_console_help_for_command_uses_cli_summary():
    help_text = SeraphielConsoleEngine().help_text("skills list")

    assert help_text == "skills list\nList installed skills"


def test_console_registry_covers_non_admin_cli_surface():
    registered = set(SeraphielConsoleEngine().commands)
...
def test_console_rejects_destructive_and_shell_like_commands(line):
    result = SeraphielConsoleEngine().execute(line)
...
def test_help_lists_supported_commands_and_not_full_cli():
    result = SeraphielConsoleEngine().execute("help")
...
def test_config_set_requires_confirmation_then_writes(_isolate_seraphiel_home):
    engine = SeraphielConsoleEngine()
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_seraphiel_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_seraphiel_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_seraphiel_home):
    ...
    assert code == 0
    assert "Seraphiel Console" in stdout.getvalue()
    assert "seraphiel>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_repl_refuses_non_interactive_confirmation(_isolate_seraphiel_home):
...
def test_main_console_subcommand_smoke(_isolate_seraphiel_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "seraphiel_cli.main", "console"],
    ...
    assert "Seraphiel Console" in result.stdout
