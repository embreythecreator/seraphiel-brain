...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    SERAPHIEL_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_seraphiel_argv", lambda: ["seraphiel"])
    ...
    assert pid == 4242
    assert captured["env"]["SERAPHIEL_HOME"] == str(profile)
    assert captured["env"]["SERAPHIEL_KANBAN_TASK"] == "t_spawn_tools"
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited SERAPHIEL_TUI=1 (or a TUI-default
    ...
    flag) and strips SERAPHIEL_TUI from the child env."""
    root = tmp_path / ".seraphiel"
    ...
    assert "SERAPHIEL_TUI" not in captured["env"]
