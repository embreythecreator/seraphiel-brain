...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """seraphiel auth add xai-oauth must set active_provider and write a pool entry.
    ...
    monkeypatch.setattr(
        "seraphiel_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``seraphiel auth add xai-oauth`` runs must produce independent pool entries.
