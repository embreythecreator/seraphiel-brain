...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SERAPHIEL_HOME with .container-mode file."""
    seraphiel_home = tmp_path / ".seraphiel"
    seraphiel_home.mkdir()
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
    monkeypatch.delenv("SERAPHIEL_DEV", raising=False)

    container_mode = seraphiel_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=seraphiel-brain\n"
        "exec_user=seraphiel\n"
        "seraphiel_bin=/data/current-package/bin/seraphiel\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("seraphiel_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "seraphiel-brain"
    assert info["exec_user"] == "seraphiel"
    assert info["seraphiel_bin"] == "/data/current-package/bin/seraphiel"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("seraphiel_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_seraphiel_dev(container_env, monkeypatch):
    """Returns None when SERAPHIEL_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SERAPHIEL_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_seraphiel_dev_zero(container_env, monkeypatch):
    """SERAPHIEL_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SERAPHIEL_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        seraphiel_home = Path(tmpdir) / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("seraphiel_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_seraphiel_home": lambda: seraphiel_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SERAPHIEL_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=seraphiel-custom\n"
        "exec_user=myuser\n"
        "seraphiel_bin=/opt/seraphiel/bin/seraphiel\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "seraphiel-custom"
    assert info["exec_user"] == "myuser"
    assert info["seraphiel_bin"] == "/opt/seraphiel/bin/seraphiel"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from seraphiel_cli.main import _exec_in_container
