...
def test_billing_logged_out(cli, monkeypatch, capsys):
    ...
    assert "seraphiel portal" in out
...
def test_buy_flow_no_card_guides_then_continues_after_recheck(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(SeraphielCLI, "_prompt_text_input_modal", _scripted("recheck", "cancel"), raising=False)
...
def test_buy_flow_no_card_back_abandons(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(SeraphielCLI, "_prompt_text_input_modal", _scripted("cancel"), raising=False)
