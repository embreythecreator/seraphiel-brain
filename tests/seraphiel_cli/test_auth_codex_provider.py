"""Tests for Codex auth — tokens stored in Seraphiel auth store (~/.seraphiel/auth.json)."""
...
def _setup_seraphiel_auth(seraphiel_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Seraphiel auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_seraphiel_auth(seraphiel_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_seraphiel_auth(seraphiel_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
    ...
    monkeypatch.setattr("seraphiel_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    seraphiel_home = tmp_path / "seraphiel"
    _setup_seraphiel_auth(seraphiel_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((seraphiel_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``seraphiel auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``seraphiel auth add openai-codex``.
    ...
    (seraphiel_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `seraphiel auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``seraphiel auth add openai-codex`` for accounts B and C
    ...
    (seraphiel_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``seraphiel auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``seraphiel auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Seraphiel auth store, not ~/.codex/."""
    seraphiel_home = tmp_path / "seraphiel"
    codex_home = tmp_path / "codex-cli"
    seraphiel_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "seraphiel-at", "refresh_token": "seraphiel-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Seraphiel store
    assert not (codex_home / "auth.json").exists()

    # Seraphiel auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "seraphiel-at"


def test_resolve_returns_seraphiel_auth_store_source(tmp_path, monkeypatch):
    seraphiel_home = tmp_path / "seraphiel"
    _setup_seraphiel_auth(seraphiel_home)
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run seraphiel auth".
    """
    from seraphiel_cli.auth import (
    ...
    assert "seraphiel auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from seraphiel_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from seraphiel_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "seraphiel_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("seraphiel_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("seraphiel_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch seraphiel_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("seraphiel_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
