...
seraphiel_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `seraphiel config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_seraphiel_home = get_seraphiel_home()`` at import time (module
    singleton), so monkeypatching SERAPHIEL_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SERAPHIEL_HOME.)
    """
    import cli

    cli._seraphiel_home = home
