...
import seraphiel_cli.memory_setup as memory_setup
from seraphiel_cli.memory_setup import _CANCELLED, _curses_select
...
def test_curses_select_cancel_defaults_to_selected(monkeypatch):
    ...
    monkeypatch.setattr("seraphiel_cli.curses_ui.curses_radiolist", fake_radiolist)
...
def test_cmd_setup_top_level_cancel_writes_nothing(monkeypatch):
    ...
    monkeypatch.setattr("seraphiel_cli.config.load_config", load_config)
...
def test_cmd_setup_builtin_selection_still_saves_builtin(monkeypatch):
    ...
    monkeypatch.setattr("seraphiel_cli.config.load_config", lambda: config)
    monkeypatch.setattr("seraphiel_cli.config.save_config", save_config)
...
def test_cmd_setup_clears_interactive_picker_before_provider_post_setup(monkeypatch):
    ...
    class PostSetupProvider:
        def post_setup(self, seraphiel_home, config):
    ...
    monkeypatch.setattr(memory_setup, "get_seraphiel_home", lambda: "/tmp/seraphiel-test")
    monkeypatch.setattr("seraphiel_cli.config.load_config", lambda: {"memory": {}})
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_seraphiel_home", lambda: tmp_path)
    monkeypatch.setattr("seraphiel_cli.config.load_config", lambda: {"memory": {}})
    monkeypatch.setattr("seraphiel_cli.config.save_config", save_config)
