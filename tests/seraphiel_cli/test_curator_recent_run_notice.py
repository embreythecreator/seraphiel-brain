...
The notice prints the most recent curator run summary on `seraphiel update`,
...
session start) so users normally never see the rename map. `seraphiel update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(seraphiel_main)
