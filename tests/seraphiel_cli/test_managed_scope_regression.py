...
def seraphiel_home(tmp_path, monkeypatch):
    ...
    # /etc/seraphiel on the dev/CI box can't influence the test.
    monkeypatch.setenv("SERAPHIEL_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(seraphiel_home, monkeypatch):
    from seraphiel_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(seraphiel_home, monkeypatch):
    from seraphiel_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(seraphiel_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_seraphiel_dotenv(seraphiel_home=str(home))
