...
def test_user_env_overrides_stale_shell_values(tmp_path, monkeypatch):
    ...
    loaded = load_seraphiel_dotenv(seraphiel_home=home)
...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_seraphiel_dotenv(seraphiel_home=home, project_env=project_env)
...
def test_main_import_applies_user_env_over_shell_values(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "OPENAI_BASE_URL=https://new.example/v1\nSERAPHIEL_INFERENCE_PROVIDER=custom\n",
        encoding="utf-8",
    )
    ...
    sys.modules.pop("seraphiel_cli.main", None)
    importlib.import_module("seraphiel_cli.main")

    assert os.getenv("OPENAI_BASE_URL") == "https://new.example/v1"
    assert os.getenv("SERAPHIEL_INFERENCE_PROVIDER") == "custom"
...
def test_utf16_le_bom_env_loads_and_rewrites_clean_utf8(tmp_path, monkeypatch):
    ...
    home.mkdir()
    env_file = home / ".env"
    content = "SERAPHIEL_TEST_KEY=hello_utf16\nSECOND_KEY=world\n"
    env_file.write_bytes(codecs.BOM_UTF16_LE + content.encode("utf-16-le"))

    monkeypatch.delenv("SERAPHIEL_TEST_KEY", raising=False)
    ...
    assert loaded == [env_file]
    assert os.getenv("SERAPHIEL_TEST_KEY") == "hello_utf16"
    assert os.getenv("SECOND_KEY") == "world"
    assert os.environ.get("\ufffd\ufffdSERAPHIEL_TEST_KEY") is None
    _assert_clean_utf8_env_on_disk(env_file, first_key="SERAPHIEL_TEST_KEY")
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from seraphiel_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "SERAPHIEL_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="seraphiel_cli.env_loader"):
...
def test_utf32_warning_fires_once_per_path(tmp_path, caplog, monkeypatch):
    ...
    import seraphiel_cli.env_loader as env_loader
    from seraphiel_cli.env_loader import _sanitize_env_file_if_needed
...
def test_leading_replacement_char_does_not_rewrite(tmp_path):
    ...
    env_file = tmp_path / ".env"
    raw = b"\xffSERAPHIEL_TEST_KEY=should-not-rewrite\nSECOND_KEY=ok\n"
    env_file.write_bytes(raw)
