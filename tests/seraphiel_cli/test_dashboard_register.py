"""Tests for ``seraphiel dashboard register``.
...
import seraphiel_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "seraphiel_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("seraphiel_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Nous Portal" in out
        assert "seraphiel setup" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.embreythecreator.com",
        ...
        # (idempotency key) and once for SERAPHIEL_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID":
                return existing_client_id
            return None

        with patch(
            "seraphiel_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("seraphiel_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "seraphiel_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "seraphiel_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "SERAPHIEL_DASHBOARD_PORTAL_URL" not in saved
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://seraphiel.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://seraphiel.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["SERAPHIEL_DASHBOARD_PORTAL_URL"]
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / SERAPHIEL_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        SERAPHIEL_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "SERAPHIEL_DASHBOARD_PORTAL_URL":
                return existing_portal
            return None

        with patch(
            "seraphiel_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("seraphiel_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "seraphiel_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "seraphiel_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
        ):
            # The ambient process env may carry SERAPHIEL_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("SERAPHIEL_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
    ...
    def test_explicit_custom_url_persisted_when_var_absent(self, capsys):
        ...
        assert saved["SERAPHIEL_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["SERAPHIEL_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        saved = self._run(
            args=_ns(portal_url="https://portal.embreythecreator.com"),
            portal="https://portal.embreythecreator.com",
            existing_portal=None,
        )
        assert (
            saved["SERAPHIEL_DASHBOARD_PORTAL_URL"] == "https://portal.embreythecreator.com"
        )
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists SERAPHIEL_DASHBOARD_PUBLIC_URL in .env.

    --redirect-uri is the full public callback (e.g.
    https://seraphiel.example.com/auth/callback). At serve time the dashboard auth
    layer reconstructs that callback by appending "/auth/callback" to
    SERAPHIEL_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as SERAPHIEL_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        SERAPHIEL_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "SERAPHIEL_DASHBOARD_PUBLIC_URL":
        ...
        with patch(
            ...
            dr, "_resolve_portal_base_url", return_value="https://portal.embreythecreator.com"
        ), patch(
            "seraphiel_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "seraphiel_cli.config.save_env_value", side_effect=fake_save
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["SERAPHIEL_DASHBOARD_PUBLIC_URL"] == "https://seraphiel.example.com"
        # The full callback path must NOT be persisted verbatim (would double
        # the path at serve time).
        assert "/auth/callback" not in saved["SERAPHIEL_DASHBOARD_PUBLIC_URL"]

    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://seraphiel.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["SERAPHIEL_DASHBOARD_PUBLIC_URL"] == "https://seraphiel.example.com:8443"
    ...
    def test_public_url_updates_existing_in_place(self, capsys):
        ...
        assert saved["SERAPHIEL_DASHBOARD_PUBLIC_URL"] == "https://new.example.com"

    def test_public_url_equal_to_existing_is_noop(self, capsys):
        # Derived origin already matches what's stored → no redundant write.
        saved = self._run(
            args=_ns(redirect_uri="https://seraphiel.example.com/auth/callback"),
            existing_public="https://seraphiel.example.com",
        )
        assert "SERAPHIEL_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://seraphiel.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "seraphiel_cli.config.get_env_value", return_value=None
        ), patch(
            "seraphiel_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://seraphiel.example.com/auth/callback",
                )
            )
        assert saved["SERAPHIEL_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["SERAPHIEL_DASHBOARD_PUBLIC_URL"] == "https://seraphiel.example.com"
...
class TestPortalResolution:
    ...
    def test_falls_back_to_stored_login_portal(self):
        with patch(
            ...
            return_value={"portal_base_url": "https://portal.staging-embreythecreator.com"},
        ):
            assert (
                dr._resolve_portal_base_url(None)
                == "https://portal.staging-embreythecreator.com"
            )
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        err = urllib.error.HTTPError(
            url="https://portal.embreythecreator.com/api/oauth/self-hosted-client",
            code=code,
            msg="err",
            hdrs=None,
