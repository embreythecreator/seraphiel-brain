...
the SAME provider universe as ``seraphiel model`` (the CLI/TUI picker).
...
def test_every_seraphiel_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `seraphiel model` universe."""
    ...
    assert not missing, (
        "providers shown in `seraphiel model` but not configurable in the desktop "
