"""Tests for the `seraphiel memory reset` CLI command.
...
- Profile-scoped reset (uses SERAPHIEL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SERAPHIEL_HOME with memory files."""
    seraphiel_home = tmp_path / ".seraphiel"
    memories = seraphiel_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSeraphiel repo is at ~/.seraphiel/seraphiel-brain\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return seraphiel_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `seraphiel memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        (seraphiel_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SERAPHIEL_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_seraphiel_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
