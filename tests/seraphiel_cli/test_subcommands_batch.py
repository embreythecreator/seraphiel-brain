...
def test_config_get_unset_subcommands_parse():
    """`seraphiel config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="seraphiel")
    sub = parser.add_subparsers(dest="command")
    handler = _h("config")
...
# ── deprecated `seraphiel login` fails gracefully, not with argparse error ────
#
# `seraphiel login` is a removed command; its handler (`login_command` in
# `seraphiel_cli/auth.py`) prints a deprecation notice pointing at `seraphiel auth` /
# `seraphiel model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `seraphiel --help`.
