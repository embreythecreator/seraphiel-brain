...
subparser, placing the flag BEFORE the subcommand (e.g. 'seraphiel --yolo chat')
...
def _build_parser():
    """Build the seraphiel argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("seraphiel_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "seraphiel_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets SERAPHIEL_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `seraphiel gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: seraphiel_cli.main is a very heavy
    # import (previously 11 separate `python -m seraphiel_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `seraphiel <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `seraphiel -t web chat` silently dropped the
    ...
    Uses the real `seraphiel_cli._parser.build_top_level_parser()` rather than
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`seraphiel {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `seraphiel --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`seraphiel {flag} chat` lost the flag — got "
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `seraphiel <flag> <value> chat`:\n  "
