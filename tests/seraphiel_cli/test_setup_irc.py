"""Tests for IRC gateway configuration via `seraphiel setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Seraphiel install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Seraphiel install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#seraphiel")
            monkeypatch.setenv("IRC_NICKNAME", "seraphiel-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `seraphiel setup gateway` experience with IRC present."""
