"""Runtime smoke test for Docker $SERAPHIEL_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the seraphiel user after container boot.
...
registered later runs its log service as the dropped seraphiel user and
...
def test_logs_gateways_seeded_and_seraphiel_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by seraphiel after boot."""
    ...
    # Both must be owned by seraphiel
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=seraphiel" in r.stdout, (
        f"logs/ not owned by seraphiel: {r.stdout}"
    )
    assert "gateways=seraphiel" in r.stdout, (
        f"logs/gateways/ not owned by seraphiel: {r.stdout}"
    )
