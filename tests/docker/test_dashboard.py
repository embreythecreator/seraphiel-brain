"""Harness: dashboard opt-in via SERAPHIEL_DASHBOARD.

Today (tini): dashboard starts once when SERAPHIEL_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``seraphiel`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without SERAPHIEL_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "seraphiel dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without SERAPHIEL_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without SERAPHIEL_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `seraphiel doctor` and any other health check).
    ...
    SERAPHIEL_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without SERAPHIEL_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with SERAPHIEL_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
         ...
         "-e", "SERAPHIEL_DASHBOARD_INSECURE=1",
...
def test_dashboard_opt_in_starts(
    ...
    """With SERAPHIEL_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SERAPHIEL_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'seraphiel dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with SERAPHIEL_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """SERAPHIEL_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SERAPHIEL_DASHBOARD=1", "-e", "SERAPHIEL_DASHBOARD_PORT=9120",
         # Default bind is 0.0.0.0; pin insecure opt-in so the auth gate
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "seraphiel dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as seraphiel, so the
    # seraphiel user can kill it (same UID).
...
# whenever `SERAPHIEL_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/seraphiel/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
    end = time.monotonic() + deadline_s
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SERAPHIEL_DASHBOARD=1",
         "-e", "SERAPHIEL_DASHBOARD_HOST=0.0.0.0",
         "-e", "SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and SERAPHIEL_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``SERAPHIEL_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    assert status.get("auth_required") is False, (
        "SERAPHIEL_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
