...
Override the image with ``SERAPHIEL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SERAPHIEL_TEST_IMAGE", "seraphiel-brain-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SERAPHIEL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SERAPHIEL_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged seraphiel user
# ---------------------------------------------------------------------------
#
# Background: every Seraphiel runtime path inside the container drops to UID
# 10000 (the ``seraphiel`` user) via ``s6-setuidgid seraphiel``. ``docker exec``
...
# fails (PermissionError swallowed) for seraphiel, so a test that ran as root
...
# helpers below run every probe under ``-u seraphiel`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: seraphiel).
