"""Regression test: ``seraphiel dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``seraphiel dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$SERAPHIEL_GIT_SHA`` build-arg to ``/opt/seraphiel/.seraphiel_build_sha`` and
``seraphiel_cli/build_info.py`` reads it as a fallback.
...
is absent and ``seraphiel dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/seraphiel/.seraphiel_build_sha`` exists in the image, assert that
  ``seraphiel dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's seraphiel user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"seraphiel dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/seraphiel/.seraphiel_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/seraphiel/.seraphiel_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``SERAPHIEL_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``seraphiel dump``
