...
Before the s6 migration, ``docker run embreythecreator/seraphiel-brain gateway
...
crash and a supervised dashboard alongside (when ``SERAPHIEL_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``SERAPHIEL_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``SERAPHIEL_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``seraphiel`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'seraphiel.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'seraphiel.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → seraphiel gateway run) can take
    ...
    # (main-wrapper.sh → python → seraphiel gateway run) can take well over
    ...
    # top), NOT the static main-seraphiel service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-seraphiel
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``seraphiel gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    start_container(
        ...
        "SERAPHIEL_GATEWAY_NO_SUPERVISE=1",
        cmd="gateway run",
    )
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "SERAPHIEL_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The SERAPHIEL_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``seraphiel gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``seraphiel gateway
    ...
    # (main-wrapper.sh → python → seraphiel gateway run) can take well
    ...
    # started either — so we'd see the CMD's `seraphiel gateway run`
    ...
    # `seraphiel gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*seraphiel.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `seraphiel gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-seraphiel`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``SERAPHIEL_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${SERAPHIEL_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Seraphiel Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Seraphiel Gateway
    ...
    assert "⚕" in combined or "Seraphiel Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Seraphiel Gateway Starting" in file_contents, (
