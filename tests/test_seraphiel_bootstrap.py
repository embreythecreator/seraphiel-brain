"""Tests for seraphiel_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Seraphiel entry point
(seraphiel, seraphiel-brain, seraphiel-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Seraphiel entry point imports seraphiel_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported seraphiel_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("seraphiel_bootstrap", None)
    import seraphiel_bootstrap  # noqa: WPS433
    return seraphiel_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Seraphiel CLI
...
class TestEntryPointsImportBootstrap:
    """Every Seraphiel entry point must import seraphiel_bootstrap as its
    ...
    # Entry points that invoke Seraphiel as a process.  Each one must
    # import seraphiel_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "seraphiel_cli/main.py",   # seraphiel CLI (console_script)
        "run_agent.py",          # seraphiel-brain (console_script)
        "acp_adapter/entry.py",  # seraphiel-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import seraphiel_bootstrap' and that
        ...
        half-finished ``seraphiel update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``seraphiel_bootstrap`` as a top-level module) leaves seraphiel
        ...
        """
        # Resolve relative to the seraphiel-brain repo root.  Tests live
        # at tests/test_seraphiel_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # seraphiel start even when seraphiel_bootstrap hasn't been
        ...
        assert first_import_name == "seraphiel_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'seraphiel_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import seraphiel_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Seraphiel's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SERAPHIEL_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SERAPHIEL_PYTHON_SRC_ROOT"] = env
            elif "SERAPHIEL_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SERAPHIEL_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/seraphiel")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SERAPHIEL_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SERAPHIEL_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/seraphiel", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Seraphiel root.  The guard must relocate Seraphiel to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/seraphiel"])
        assert result[0] == "/opt/seraphiel"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Seraphiel root.
        assert result.index("/opt/seraphiel") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/seraphiel", "/opt/seraphiel", ""])
        assert result.count("/opt/seraphiel") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SERAPHIEL_PYTHON_SRC_ROOT"] = "/env/seraphiel"
            hb.harden_import_path()
            assert sys.path[0] == "/env/seraphiel"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SERAPHIEL_PYTHON_SRC_ROOT", None)
