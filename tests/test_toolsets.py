...
class TestToolsetConsistency:
    ...
    def test_seraphiel_platforms_share_core_tools(self):
        """All seraphiel-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on seraphiel-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["seraphiel-cli", "seraphiel-telegram", "seraphiel-discord", "seraphiel-whatsapp", "seraphiel-slack", "seraphiel-signal", "seraphiel-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_seraphiel_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("seraphiel-whatsapp")

    def test_seraphiel_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("seraphiel-api-server")
