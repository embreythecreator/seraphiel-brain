...
The ``seraphiel update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m seraphiel_cli.main gateway run`` — image name
    ``pythonw``, not ``seraphiel.exe`` — so the ``taskkill /IM seraphiel.exe`` guard
    ...
    # The seraphiel.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM seraphiel.exe' in text
