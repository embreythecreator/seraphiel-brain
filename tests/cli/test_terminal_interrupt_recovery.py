...
The recovery path lives in ``SeraphielCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A SeraphielCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(SeraphielCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise SeraphielCLI._recover_terminal_after_interrupt()."""
    ...
    def test_drains_stdin_then_redraws(self, bare_cli):
        ...
        with patch("seraphiel_cli.curses_ui.flush_stdin") as mock_flush:
    ...
    def test_redraw_still_runs_when_flush_fails(self, bare_cli):
        ...
        bare_cli._force_full_redraw = MagicMock()
        with patch(
            "seraphiel_cli.curses_ui.flush_stdin", side_effect=OSError("no tty")
        ):
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "seraphiel_cli.curses_ui.flush_stdin",
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(SeraphielCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(SeraphielCLI, "_recover_terminal_after_interrupt")
        assert callable(SeraphielCLI._recover_terminal_after_interrupt)
