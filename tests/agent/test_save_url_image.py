...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SERAPHIEL_HOME under tmp_path."""
    monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path / ".seraphiel"))
    (tmp_path / ".seraphiel").mkdir()

    # Force the constants/image cache helpers to re-read SERAPHIEL_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("seraphiel_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_seraphiel_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SERAPHIEL_HOME — gateway cleanup
