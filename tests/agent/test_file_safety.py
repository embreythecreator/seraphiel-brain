...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_seraphiel_env(self):
        """Seraphiel' own .env inside SERAPHIEL_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: seraphiel internal .env is in ~/.seraphiel/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.seraphiel/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.seraphiel/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Seraphiel cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        seraphiel_home = tmp_path / ".seraphiel"
        cache = seraphiel_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._seraphiel_home_path", return_value=seraphiel_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Seraphiel cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        seraphiel_home = tmp_path / ".seraphiel"
        hub = seraphiel_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_seraphiel_home(self, tmp_path):
        """The env basename guard does not depend on SERAPHIEL_HOME resolution."""
        seraphiel_home = tmp_path / ".seraphiel"
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        cache = seraphiel_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
