...
def isolated_home(monkeypatch):
    """Temp SERAPHIEL_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="seraphiel_test_31179_")
    seraphiel_home = os.path.join(test_home, ".seraphiel")
    os.makedirs(seraphiel_home)
...
def _fresh_modules():
    """Drop cached seraphiel modules so each test reloads against current env."""
