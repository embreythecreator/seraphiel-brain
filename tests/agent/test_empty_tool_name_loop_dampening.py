...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SERAPHIEL_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="seraphiel_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".seraphiel"))
    prev_home = os.environ.get("SERAPHIEL_HOME")
    os.environ["SERAPHIEL_HOME"] = os.path.join(test_home, ".seraphiel")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("seraphiel_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SERAPHIEL_HOME"] = prev_home
