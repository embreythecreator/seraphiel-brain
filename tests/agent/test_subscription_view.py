...
def test_manage_url_attaches_org_and_path_to_portal_origin():
    s = SubscriptionState(
        logged_in=True,
        org_id="org_x",
        portal_url="https://portal.embreythecreator.com/billing/whatever",
    )
    # Path is replaced with /manage-subscription; org_id is pinned; origin kept.
    assert (
        subscription_manage_url(s)
        == "https://portal.embreythecreator.com/manage-subscription?org_id=org_x"
    )
...
def test_no_fixture_when_env_unset(monkeypatch):
    monkeypatch.delenv("SERAPHIEL_DEV_SUBSCRIPTION_FIXTURE", raising=False)
...
def test_dev_fixture_states(monkeypatch, name, checker):
    monkeypatch.setenv("SERAPHIEL_DEV_SUBSCRIPTION_FIXTURE", name)
...
def test_dev_fixture_exposes_tier_catalog(monkeypatch):
    ...
    monkeypatch.setenv("SERAPHIEL_DEV_SUBSCRIPTION_FIXTURE", "mid")
    s = dev_fixture_subscription_state()
...
def test_dev_fixture_unknown_name_fails_safe(monkeypatch):
    monkeypatch.setenv("SERAPHIEL_DEV_SUBSCRIPTION_FIXTURE", "bogus")
    s = dev_fixture_subscription_state()
    assert s is not None
