...
""",
        ...
        from seraphiel_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        from seraphiel_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from seraphiel_cli.plugins import PluginContext, PluginManifest, PluginManager
