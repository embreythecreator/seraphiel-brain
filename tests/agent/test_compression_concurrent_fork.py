...
Damien's incident (Discord, 2026-05-28): a long Seraphiel session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``seraphiel_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "seraphiel_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "seraphiel_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``seraphiel_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = seraphiel_state.SessionDB
    monkeypatch.setattr(seraphiel_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(seraphiel_state, "SessionDB", real_session_db_type)
