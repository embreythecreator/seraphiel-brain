...
to a generic "Your API key was rejected... run seraphiel setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "seraphiel setup".
    assert "seraphiel portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.embreythecreator.com" in source
