...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Seraphiel Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_seraphiel_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path / ".seraphiel"))

    seraphiel_env = tmp_path / ".seraphiel" / ".env"
    seraphiel_env.parent.mkdir(parents=True)
    seraphiel_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.seraphiel/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.seraphiel/auth.json` and read the
    ...
    oauth = seraphiel_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = seraphiel_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.seraphiel/auth.json and @file:.seraphiel/.anthropic_oauth.json "
        "and @file:.seraphiel/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
async def test_canonical_guard_fails_closed_when_lookup_raises(tmp_path: Path, monkeypatch):
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.seraphiel/auth.json",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
