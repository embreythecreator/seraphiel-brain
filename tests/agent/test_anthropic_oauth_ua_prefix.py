...
class TestOAuthUserAgentPrefix:
    ...
    def test_token_exchange_ua_prefix(self):
        """run_seraphiel_oauth_login_pure must not send claude-cli/ UA."""
        import inspect
        import agent.anthropic_adapter as mod

        # Get the source of the exchange function
        try:
            source = inspect.getsource(mod.run_seraphiel_oauth_login_pure)
        except AttributeError:
            pytest.skip("run_seraphiel_oauth_login_pure not found")
        ...
        for i, line in enumerate(source.split("\n"), 1):
            ...
            if "claude-cli/" in stripped and ("User-Agent" in stripped or "user-agent" in stripped):
                pytest.fail(
                    f"Line {i}: run_seraphiel_oauth_login_pure still uses claude-cli/ UA header: {stripped}"
                )
        assert "claude-code/" in source, (
            "run_seraphiel_oauth_login_pure should use claude-code/ UA"
        )
