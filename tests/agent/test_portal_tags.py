...
def test_seraphiel_client_tag_includes_current_version():
    """The client tag must reflect seraphiel_cli.__version__ verbatim."""
    from seraphiel_cli import __version__
    from agent.portal_tags import seraphiel_client_tag

    assert seraphiel_client_tag() == f"client=seraphiel-client-v{__version__}"


def test_seraphiel_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import seraphiel_client_tag

    tag = seraphiel_client_tag()
    assert tag.startswith("client=seraphiel-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import seraphiel_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=seraphiel-brain" in tags
    assert seraphiel_client_tag() in tags
    assert len(tags) == 2
