...
path via ``seraphiel chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SERAPHIEL_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SERAPHIEL_HOME at a temp dir and turn moa.save_traces on."""
    seraphiel_home = tmp_path / ".seraphiel"
    seraphiel_home.mkdir()
    monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))

    # save_moa_turn reads config via seraphiel_cli.config.load_config; stub it to
    ...
    return seraphiel_home / "moa-traces"
