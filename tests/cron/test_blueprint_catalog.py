...
the shared /blueprint command handler (seraphiel_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated SERAPHIEL_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("seraphiel://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("seraphiel://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from seraphiel_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
