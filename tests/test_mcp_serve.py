"""
Tests for mcp_serve — Seraphiel MCP server.
...
def _isolate_seraphiel_home(tmp_path, monkeypatch):
    """Redirect SERAPHIEL_HOME to a temp directory."""
    monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
    try:
        import seraphiel_constants
        monkeypatch.setattr(seraphiel_constants, "get_seraphiel_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking seraphiel_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_when_only_sessions_json_changed(
        ...
        # _poll_once reads <SERAPHIEL_HOME>/state.db for its mtime gate; the autouse
        # fixture points SERAPHIEL_HOME at tmp_path.
        db_path = tmp_path / "state.db"
