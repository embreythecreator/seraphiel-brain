...
def _restore_tool_modules():
    original_seraphiel_home = os.environ.get("SERAPHIEL_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "seraphiel_cli"
        or name.startswith("seraphiel_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_seraphiel_home is None:
            os.environ.pop("SERAPHIEL_HOME", None)
        else:
            os.environ["SERAPHIEL_HOME"] = original_seraphiel_home
        _reset_modules(("tools", "seraphiel_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    seraphiel_cli = types.ModuleType("seraphiel_cli")
    seraphiel_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["seraphiel_cli"] = seraphiel_cli
    seraphiel_home = tmp_path / "seraphiel-home"
    os.environ["SERAPHIEL_HOME"] = str(seraphiel_home)
    sys.modules["seraphiel_cli.config"] = types.SimpleNamespace(
        get_seraphiel_home=lambda: seraphiel_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="seraphiel-brain", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": seraphiel_home / "modal_snapshots.json",
