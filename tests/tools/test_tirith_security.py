...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._seraphiel_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(seraphiel_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(seraphiel_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_seraphiel_bin(self):
        """After _INSTALL_FAILED, manual install in SERAPHIEL_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        seraphiel_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(seraphiel_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(seraphiel_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._seraphiel_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == seraphiel_bin
            assert _tirith_mod._resolved_path == seraphiel_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# SERAPHIEL_HOME isolation
# ---------------------------------------------------------------------------

class TestSeraphielHomeIsolation:
    def test_seraphiel_bin_dir_respects_seraphiel_home(self):
        """_seraphiel_bin_dir must use SERAPHIEL_HOME, not hardcoded ~/.seraphiel."""
        from tools.tirith_security import _seraphiel_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SERAPHIEL_HOME": tmpdir}):
            result = _seraphiel_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_seraphiel_home(self):
        """_failure_marker_path must use SERAPHIEL_HOME, not hardcoded ~/.seraphiel."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SERAPHIEL_HOME": "/custom/seraphiel"}):
            result = _failure_marker_path()
        assert result == "/custom/seraphiel/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SERAPHIEL_HOME; verify it's active."""
        seraphiel_home = os.getenv("SERAPHIEL_HOME")
        assert seraphiel_home is not None, "SERAPHIEL_HOME should be set by conftest"
        assert "seraphiel_test" in seraphiel_home, "Should point to test temp dir"

    def test_get_seraphiel_home_fallback(self):
        """Without SERAPHIEL_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_seraphiel_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SERAPHIEL_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".seraphiel")
            result = _get_seraphiel_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._seraphiel_bin_dir",
