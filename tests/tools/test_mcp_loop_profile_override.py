"""Regression tests for SERAPHIEL_HOME override propagation onto the MCP loop.
...
get_seraphiel_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: SeraphielTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(SeraphielTokenStorage("probe-srv")._tokens_path())
