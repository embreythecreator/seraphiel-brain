...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``seraphiel tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.seraphiel/auth.json
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `seraphiel tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``seraphiel tools``,
...
class TestXAIProviderOAuthPath:
    ...
    credential pool (``seraphiel auth`` sign-in) rather than an env-var key.
    ...
    def test_search_uses_oauth_bearer_token_and_base_url(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("XAI_API_KEY", raising=False)
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        monkeypatch.setenv("SERAPHIEL_XAI_BASE_URL", "https://proxy.x.ai/v1/")
    ...
    def test_pool_only_direct_refresh_updates_main_runtime(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "seraphiel_cli.auth.refresh_xai_oauth_pure",
            fake_refresh,
        )
