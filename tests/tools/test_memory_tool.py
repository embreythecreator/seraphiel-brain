...
class TestScanMemoryContent:
    ...
    def test_seraphiel_config_mod_blocked(self):
        result = _scan_memory_content("edit .seraphiel/config.yaml to change settings")
        assert "Blocked" in result
        assert "seraphiel_config_mod" in result
        result = _scan_memory_content("update .seraphiel/SOUL.md with new personality")
        assert "Blocked" in result
        assert "seraphiel_config_mod" in result
    ...
    def test_seraphiel_config_mod_no_false_positives(self):
        """Merely mentioning seraphiel config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .seraphiel/config.yaml for settings") is None
        assert _scan_memory_content("Read .seraphiel/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .seraphiel/config.yaml file contains runtime options") is None
