...
When SERAPHIEL_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the seraphiel user's home directory ($SERAPHIEL_HOME == /opt/data).
Runtime-writable trees under ``/opt/seraphiel`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/seraphiel/gateway`` is one such
