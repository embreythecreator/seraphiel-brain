...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.seraphiel/)``
    ...
    def test_refuses_seraphiel_home_itself(self, tmp_path):
        """``~/.seraphiel/`` itself is what the #48200 wipe destroyed."""
        from tools.skills_sync import _rmtree_writable

        seraphiel = tmp_path / "home"
        seraphiel.mkdir()
        (seraphiel / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", seraphiel / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(seraphiel)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        seraphiel = tmp_path / "home"
        seraphiel.mkdir()
        skills = seraphiel / "skills"
        skills.mkdir()
        not_skills = seraphiel / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "seraphiel skills reset new-skill" in captured
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """SERAPHIEL_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("SERAPHIEL_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("SERAPHIEL_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty SERAPHIEL_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("SERAPHIEL_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `seraphiel profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.seraphiel) rely on so bundled
    skills are never seeded at install time NOR re-injected by `seraphiel update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        seraphiel_home = tmp_path / "home"
        seraphiel_home.mkdir()
        (seraphiel_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.SERAPHIEL_HOME", seraphiel_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`seraphiel skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.SERAPHIEL_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.SERAPHIEL_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
