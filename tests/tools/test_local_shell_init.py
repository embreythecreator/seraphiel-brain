...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export SERAPHIEL_SESSION_ENV_PROBE="sticky"; '
                'export PATH="/tmp/seraphiel-session-bin:$PATH"; '
                'echo "first=$SERAPHIEL_SESSION_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$SERAPHIEL_SESSION_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
        ...
        assert "/tmp/seraphiel-session-bin" in output
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        init_file.write_text(
            'export SERAPHIEL_SHELL_INIT_PROBE="probe-ok"\n'
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$SERAPHIEL_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
