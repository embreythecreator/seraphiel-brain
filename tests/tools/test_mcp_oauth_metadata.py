...
Covers:
- :class:`SeraphielTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.SeraphielMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SERAPHIEL_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# SeraphielTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = SeraphielTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        storage = SeraphielTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (SeraphielMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: SeraphielTokenStorage, **context_attrs):
    ...
    provider = _SERAPHIEL_PROVIDER_CLS.__new__(_SERAPHIEL_PROVIDER_CLS)
    provider._seraphiel_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        storage = SeraphielTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SERAPHIEL_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        storage = SeraphielTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        storage = SeraphielTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        storage = SeraphielTokenStorage("noop-srv")
        ...
        with patch.object(
            SeraphielTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
        storage = SeraphielTokenStorage("flow-srv")
        ...
        with patch.object(
            _SERAPHIEL_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
