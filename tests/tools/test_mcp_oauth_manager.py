...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    for home, access_token in ((profile_a, "TOKEN_A"), (profile_b, "TOKEN_B")):
        ...
        try:
            storage = SeraphielTokenStorage("shared")
...
def test_manager_explicit_home_removes_only_that_profiles_tokens(tmp_path):
    ...
    token = set_seraphiel_home_override(profile_a)
    try:
        MCPOAuthManager().remove("shared", seraphiel_home=profile_b)
    finally:
        reset_seraphiel_home_override(token)
...
def test_seraphiel_provider_subclass_exists():
    """SeraphielMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SERAPHIEL_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SERAPHIEL_PROVIDER_CLS is not None
    assert issubclass(_SERAPHIEL_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_seraphiel_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns SeraphielMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SERAPHIEL_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _SERAPHIEL_PROVIDER_CLS is not None
    assert isinstance(provider, _SERAPHIEL_PROVIDER_CLS)
    assert provider._seraphiel_server_name == "srv"
