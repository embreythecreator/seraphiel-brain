...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "embreythecreator.com"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.embreythecreator.com",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "embreythecreator.com",
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.embreythecreator.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_seraphiel_home_override(self, tmp_path):
        """Auth lookup should read from SERAPHIEL_HOME/auth.json, not ~/.seraphiel/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".seraphiel").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "SERAPHIEL_HOME": str(seraphiel_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``seraphiel tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("seraphiel_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "seraphiel_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from seraphiel_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.seraphiel/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.seraphiel/.env), not bare os.getenv. Parametrized over the four
