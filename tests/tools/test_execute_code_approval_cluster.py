...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SERAPHIEL_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SERAPHIEL_GATEWAY_SESSION set, a bound session
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("SERAPHIEL_CRON_SESSION", "1")
    monkeypatch.delenv("SERAPHIEL_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_seraphiel_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SERAPHIEL_HOME": "/h", "SERAPHIEL_PROFILE": "p",
        "SERAPHIEL_CONFIG": "/c.yaml", "SERAPHIEL_ENV": "/e",
        # other SERAPHIEL_* → dropped (broad prefix removed)
        "SERAPHIEL_BASE_URL": "https://x", "SERAPHIEL_INTERACTIVE": "1",
        "SERAPHIEL_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("SERAPHIEL_HOME", "SERAPHIEL_PROFILE", "SERAPHIEL_CONFIG", "SERAPHIEL_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SERAPHIEL_BASE_URL", "SERAPHIEL_INTERACTIVE", "SERAPHIEL_KANBAN_DB",
...
def test_env_scrub_logs_dropped_seraphiel_vars(caplog):
    """Dropping a non-allowlisted, non-secret SERAPHIEL_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset SERAPHIEL_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "SERAPHIEL_HOME": "/h",          # allowlisted → kept, not logged
        "SERAPHIEL_BASE_URL": "https://x",   # dropped → logged
        "SERAPHIEL_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SERAPHIEL_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SERAPHIEL_HOME" in out and "PATH" in out
    assert "SERAPHIEL_BASE_URL" not in out and "SERAPHIEL_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SERAPHIEL_BASE_URL" in msgs and "SERAPHIEL_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SERAPHIEL_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SERAPHIEL_* vars."""
    import logging
