"""Contract test: the s6-overlay stage2 hook seeds $SERAPHIEL_HOME/logs/gateways
as the seraphiel user.
...
seraphiel user and s6-log crash-loops on `mkdir: Permission denied`.

Seeding `logs/gateways` in stage2 (cont-init runs before any service starts)
guarantees the parent already exists seraphiel-owned by the time the first
...
def _seed_mkdir_block(text: str) -> str:
    """Extract the `as_seraphiel mkdir -p \\ ...` seed block."""
    m = re.search(r"as_seraphiel mkdir -p \\\n(?:[^\n]*\\\n)*[^\n]*\n", text)
    assert m, "stage2-hook.sh must contain the as_seraphiel mkdir -p seed block"
    return m.group(0)


def test_logs_gateways_is_seeded(stage2_text: str) -> None:
    block = _seed_mkdir_block(stage2_text)
    assert '"$SERAPHIEL_HOME/logs/gateways"' in block, (
        "logs/gateways must be seeded seraphiel-owned in stage2 so profiles "
    ...
    assert '"$SERAPHIEL_HOME/logs"' in block
