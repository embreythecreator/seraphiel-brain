...
SERAPHIEL_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
SERAPHIEL_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-seraphiel-setup, which runs lexicographically BEFORE
...
This mirrors the existing SERAPHIEL_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def _seed_block(text: str) -> str:
    """Extract the ``if [ ! -f "$SERAPHIEL_HOME/gateway_state.json" ] && … fi``
    block that seeds the gateway state file from the bootstrap env var."""
    m = re.search(
        r'(if \[ ! -f "\$SERAPHIEL_HOME/gateway_state\.json" \] && \\\n'
        r"(?:.*\n)*?fi)",
        text,
    )
    assert m, (
        "stage2-hook.sh must contain the gateway_state.json bootstrap-seed block "
        "guarded on SERAPHIEL_GATEWAY_BOOTSTRAP_STATE"
    )
    return m.group(1)
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$SERAPHIEL_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "SERAPHIEL_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $SERAPHIEL_HOME.

    ``env_value`` is the SERAPHIEL_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export SERAPHIEL_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset SERAPHIEL_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when SERAPHIEL_GATEWAY_BOOTSTRAP_STATE is unset"
