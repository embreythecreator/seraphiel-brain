...
def test_dockerfile_makes_opt_seraphiel_readonly_for_seraphiel_user() -> None:
    ...
    assert "chown -R root:root /opt/seraphiel" not in text
    assert "chmod -R a+rX /opt/seraphiel" not in text
    assert "chmod -R a-w /opt/seraphiel" not in text
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    The stamp is created by root in the shim-wiring RUN block; the seraphiel
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/seraphiel tree.
    assert f"ENV SERAPHIEL_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV SERAPHIEL_LAZY_INSTALL_TARGET=/opt/seraphiel" not in text
    ...
    # succeed as the unprivileged seraphiel runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$SERAPHIEL_HOME/lazy-packages"' in stage2, (
