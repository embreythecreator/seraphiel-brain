...
def _chown_seraphiel_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    seraphiel_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    seraphiel_home = target if seraphiel_home is None else seraphiel_home
    script = (
        "set -eu\n"
        f'SERAPHIEL_HOME="{seraphiel_home}"\n'
        f"{_chown_seraphiel_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_seraphiel_tree "{target}"\n'
    )
...
def test_chown_helper_repairs_real_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    assert log_path.read_text().splitlines() == [
        f"-R seraphiel:seraphiel {target}",
    ]
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "seraphiel-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        seraphiel_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked SERAPHIEL_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_seraphiel_home_trees(stage2_text: str) -> None:
    assert 'chown_seraphiel_tree "$SERAPHIEL_HOME/$sub"' in stage2_text
    assert 'chown_seraphiel_tree "$SERAPHIEL_HOME/profiles"' in stage2_text
    assert 'chown_seraphiel_tree "$SERAPHIEL_HOME/cron"' in stage2_text
    assert 'chown -R seraphiel:seraphiel "$SERAPHIEL_HOME/$sub"' not in stage2_text
    assert 'chown -R seraphiel:seraphiel "$SERAPHIEL_HOME/profiles"' not in stage2_text
    assert 'chown -R seraphiel:seraphiel "$SERAPHIEL_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_seraphiel_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$SERAPHIEL_HOME"' in stage2_text
