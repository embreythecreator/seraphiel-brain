...
def _clean_state():
    ...
    saved = {}
    for k in ("SERAPHIEL_INTERACTIVE", "SERAPHIEL_GATEWAY_SESSION", "SERAPHIEL_EXEC_ASK", "SERAPHIEL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SERAPHIEL_INTERACTIVE"] = "1"
        session_key = os.getenv("SERAPHIEL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SERAPHIEL_INTERACTIVE or SERAPHIEL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
