...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (seraphiel_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(seraphiel_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.seraphiel/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        (seraphiel_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        seraphiel_home = tmp_path / ".seraphiel"
        skills_dir = seraphiel_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.seraphiel/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        seraphiel_home = tmp_path / ".seraphiel"
        (seraphiel_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SERAPHIEL_HOME": str(seraphiel_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.seraphiel")

        assert mounts[0]["container_path"] == "/home/user/.seraphiel/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.seraphiel/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.seraphiel/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SERAPHIEL_HOME."""
        monkeypatch.setenv("SERAPHIEL_HOME", str(tmp_path / ".seraphiel"))
        (tmp_path / ".seraphiel").mkdir()

        # Create a sensitive file one level above seraphiel_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside seraphiel_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SERAPHIEL_HOME must still be registered."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        (seraphiel_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_seraphiel_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SERAPHIEL_HOME must be allowed."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        subdir = seraphiel_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SERAPHIEL_HOME pointing outside must be rejected."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))

        # Create a sensitive file outside seraphiel_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside seraphiel_home pointing outside
        symlink = seraphiel_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SERAPHIEL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, seraphiel_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SERAPHIEL_HOME."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
        ...
        sensitive.write_text("{}")
        self._write_config(seraphiel_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(seraphiel_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SERAPHIEL_HOME via config must still mount."""
        seraphiel_home = tmp_path / ".seraphiel"
        seraphiel_home.mkdir()
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))

        (seraphiel_home / "oauth.json").write_text("{}")
        self._write_config(seraphiel_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(seraphiel_home / "document_cache") in host_paths
        assert str(seraphiel_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.seraphiel/cache/documents" in container_paths
        assert "/root/.seraphiel/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        seraphiel_home = tmp_path / ".seraphiel"
        img_dir = seraphiel_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.seraphiel/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.seraphiel")
            == "/home/agent/.seraphiel/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        seraphiel_home = tmp_path / ".seraphiel"
        (seraphiel_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("SERAPHIEL_HOME", str(seraphiel_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(seraphiel_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        doc_dir = seraphiel_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        seraphiel_home = tmp_path / ".seraphiel"
        ss_dir = seraphiel_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.seraphiel/cache/screenshots/session_abc/screen1.png"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — SERAPHIEL_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping SERAPHIEL_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"SERAPHIEL_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.seraphiel/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.seraphiel/google_token.json" in paths
        assert "/root/.seraphiel/.env" not in paths
