...
class TestWriteDenyExactPaths:
    ...
    def test_seraphiel_env(self):
        # ``.env`` under the active SERAPHIEL_HOME (profile-aware, not just
        # ``~/.seraphiel``) must be write-denied. The hermetic test conftest
        # points SERAPHIEL_HOME at a tempdir — resolve via get_seraphiel_home()
        # to match the denylist.
        from seraphiel_constants import get_seraphiel_home
        path = str(get_seraphiel_home() / ".env")
        assert _is_write_denied(path) is True

    def test_seraphiel_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.seraphiel/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "seraphiel_root"
        ...
        # Sanity check: SERAPHIEL_HOME does point to the profile dir, not the root.
        from seraphiel_constants import get_seraphiel_home, get_default_seraphiel_root
        assert get_seraphiel_home() == profile_home
        assert get_default_seraphiel_root() == root
...
class TestWriteAllowed:
    ...
    def test_seraphiel_config_not_env(self):
        path = os.path.join(str(Path.home()), ".seraphiel", "config.yaml")
