...
plugin SDK (``window.__SERAPHIEL_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-Seraphiel-Session-Token`` header) and gated OAuth
(``seraphiel_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__SERAPHIEL_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__SERAPHIEL_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + seraphiel-achievements are bundled today; assert at least one is
