"""Tests for ACP Registry metadata shipped with Seraphiel."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "seraphiel-brain"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Seraphiel Brain"
    assert data["description"]
    assert data["repository"] == "https://github.com/embreythecreator/seraphiel-brain"
    assert data["website"].startswith("https://seraphiel-brain.embreythecreator.com/")
    assert data["authors"] == ["Seraphiel"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"seraphiel-brain[acp]=={data['version']}"
    assert uvx["args"] == ["seraphiel-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"seraphiel-brain[acp]=={_pyproject_version()}"
    )
