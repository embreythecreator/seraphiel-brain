...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSeraphielSessionId"] == "root1"
    assert prov["rootSeraphielSessionId"] == "root1"
    assert prov["parentSeraphielSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_seraphiel_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSeraphielSessionId"] == "old"
    assert prov["rootSeraphielSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSeraphielSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootSeraphielSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSeraphielSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_seraphiel_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousSeraphielSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"seraphiel"}
    assert "sessionProvenance" in meta["seraphiel"]
    assert meta["seraphiel"]["sessionProvenance"]["currentSeraphielSessionId"] == "root1"
