"""Shared fixtures for the seraphiel-brain test suite.
...
2. **Isolated SERAPHIEL_HOME.** SERAPHIEL_HOME points to a per-test tempdir so
   code reading ``~/.seraphiel/*`` via ``get_seraphiel_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".seraphiel"`` instead of the canonical
   ``get_seraphiel_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SERAPHIEL_SESSION_* inheritance** — the agent's current gateway
...
# SERAPHIEL_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SERAPHIEL_BEHAVIORAL_VARS = frozenset({
    "SERAPHIEL_YOLO_MODE",
    "SERAPHIEL_INTERACTIVE",
    "SERAPHIEL_QUIET",
    "SERAPHIEL_TOOL_PROGRESS",
    "SERAPHIEL_TOOL_PROGRESS_MODE",
    "SERAPHIEL_MAX_ITERATIONS",
    "SERAPHIEL_SESSION_PLATFORM",
    "SERAPHIEL_SESSION_CHAT_ID",
    "SERAPHIEL_SESSION_CHAT_NAME",
    "SERAPHIEL_SESSION_THREAD_ID",
    "SERAPHIEL_SESSION_SOURCE",
    "SERAPHIEL_SESSION_KEY",
    "SERAPHIEL_GATEWAY_SESSION",
    "SERAPHIEL_CRON_SESSION",
    "_SERAPHIEL_GATEWAY",
    "SERAPHIEL_PLATFORM",
    "SERAPHIEL_MODEL",
    "SERAPHIEL_INFERENCE_MODEL",
    "SERAPHIEL_INFERENCE_PROVIDER",
    "SERAPHIEL_TUI_PROVIDER",
    "SERAPHIEL_MANAGED",
    "SERAPHIEL_MANAGED_DIR",
    "SERAPHIEL_DEV",
    "SERAPHIEL_CONTAINER",
    "SERAPHIEL_EPHEMERAL_SYSTEM_PROMPT",
    "SERAPHIEL_TIMEZONE",
    "SERAPHIEL_REDACT_SECRETS",
    "SERAPHIEL_BACKGROUND_NOTIFICATIONS",
    "SERAPHIEL_EXEC_ASK",
    "SERAPHIEL_HOME_MODE",
    "SERAPHIEL_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.seraphiel/kanban.db instead of the per-test SERAPHIEL_HOME.
    "SERAPHIEL_KANBAN_DB",
    "SERAPHIEL_KANBAN_BOARD",
    "SERAPHIEL_KANBAN_HOME",
    "SERAPHIEL_KANBAN_WORKSPACES_ROOT",
    "SERAPHIEL_KANBAN_LOGS_ROOT",
    "SERAPHIEL_KANBAN_TASK",
    "SERAPHIEL_KANBAN_WORKSPACE",
    "SERAPHIEL_KANBAN_RUN_ID",
    ...
    "SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID",
    "SERAPHIEL_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SERAPHIEL_HOME to per-test tempdirs so code that
    reads ``~/.seraphiel/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    fixture. Any code in the codebase reading ``~/.seraphiel/*`` via
    #    ``Path.home() / ".seraphiel"`` instead of ``get_seraphiel_home()``
    #    is a bug to fix at the callsite.
    fake_seraphiel_home = tmp_path / "seraphiel_test"
    fake_seraphiel_home.mkdir()
    (fake_seraphiel_home / "sessions").mkdir()
    (fake_seraphiel_home / "cron").mkdir()
    (fake_seraphiel_home / "memories").mkdir()
    (fake_seraphiel_home / "skills").mkdir()
    monkeypatch.setenv("SERAPHIEL_HOME", str(fake_seraphiel_home))
    ...
    #    ~/.seraphiel/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import seraphiel_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_seraphiel_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal seraphiel config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``seraphiel-gateway`` process
...
# correlated with a ``tests/seraphiel_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> seraphiel-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart seraphiel-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting seraphiel/python patterns are also blocked.
    """
    ...
    def _matches_seraphiel_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SERAPHIEL_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_seraphiel_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch seraphiel-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m seraphiel_cli.main".
                if (
                    "seraphiel" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live seraphiel-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting seraphiel/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `seraphiel update` (or the
        # equivalent `python -m seraphiel_cli.main update`).  These commands
        ...
        if "update" in low and (
            # seraphiel update / seraphiel update --gateway / setsid bash -c ... seraphiel update
            ("seraphiel" in low and "update" in low.split())
            or
            # python -m seraphiel_cli.main update --gateway
            ("seraphiel_cli" in low and "update" in low.split())
            or
            # venv/bin/seraphiel update  (absolute path variant used in tests)
            (".venv/bin/seraphiel" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`seraphiel update` against the real checkout, fetching "
