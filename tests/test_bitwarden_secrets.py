...
binary download are exercised manually by `seraphiel secrets bitwarden
...
def seraphiel_home(tmp_path, monkeypatch):
    """Point Seraphiel at an isolated home directory."""
    home = tmp_path / ".seraphiel"
    home.mkdir()
    monkeypatch.setenv("SERAPHIEL_HOME", str(home))
    # Some modules cache get_seraphiel_home; clear if needed.
    import seraphiel_constants
    if hasattr(seraphiel_constants, "_SERAPHIEL_HOME_CACHE"):
        seraphiel_constants._SERAPHIEL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(seraphiel_home, monkeypatch):
...
def test_install_bws_happy_path(seraphiel_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(seraphiel_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(seraphiel_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from seraphiel_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under seraphiel_home/cache/."""
    home = tmp_path / ".seraphiel"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
