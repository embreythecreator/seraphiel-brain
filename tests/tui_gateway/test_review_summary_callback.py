...
def server():
    with patch.dict(
        "sys.modules",
        {
            "seraphiel_constants": MagicMock(
                get_seraphiel_home=MagicMock(return_value="/tmp/seraphiel_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'seraphiel-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'seraphiel-release' patched"
    }
