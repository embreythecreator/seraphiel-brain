...
Usage (via cron with --no-agent):
    ...
    seraphiel cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SERAPHIEL_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Seraphiel-Watcher/1.0"})
