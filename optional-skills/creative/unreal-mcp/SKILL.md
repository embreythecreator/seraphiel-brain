...
Companion skill for the `unreal-engine` entry in the Seraphiel MCP catalog. The
...
Two halves, in this order: the editor side must be up before Seraphiel connects.
...
### One-time, Seraphiel side

    seraphiel mcp install unreal-engine
...
`~/.seraphiel/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Seraphiel — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Seraphiel connects
...
2. Start the Seraphiel session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Seraphiel
they appear as:

| Seraphiel tool | Purpose |
|---|---|
...
then happens at `seraphiel mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Seraphiel runs batched calls
...
- **Start order matters.** Editor + server up first, then the Seraphiel
...
- **Timeouts on long operations.** Seraphiel' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.seraphiel/config.yaml` for
