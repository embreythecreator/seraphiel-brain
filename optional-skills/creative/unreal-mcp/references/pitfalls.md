...
### 1. Start order: editor first, Seraphiel second

Seraphiel probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Seraphiel session.
...
restart the editor, restart the Seraphiel session.
...
server fails to bind (Output Log) or Seraphiel' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.seraphiel/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Seraphiel

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Seraphiel' connection
lives in `~/.seraphiel/config.yaml` via `seraphiel mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Seraphiel — just don't
mistake it for the Seraphiel setup step.
...
explicitly warns against overlapping calls. Seraphiel executes same-turn tool
...
### 9. Timeouts: Seraphiel gives up before Unreal does

Seraphiel' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Seraphiel runs elsewhere (SSH backend,
