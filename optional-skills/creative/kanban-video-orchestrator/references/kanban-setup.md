...
the actual `setup.sh` that creates the project workspace, configures Seraphiel
...
> [embreythecreator/kanban-video-pipeline](https://github.com/embreythecreator/kanban-video-pipeline).
...
2. **Create profiles** — `seraphiel profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.seraphiel/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `seraphiel kanban create` assigned to the director
...
```bash
seraphiel profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.seraphiel/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.seraphiel/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Seraphiel
...
Each profile gets a `SOUL.md` at `~/.seraphiel/profiles/<name>/SOUL.md` that
...
```bash
seraphiel kanban create "Direct production of <video title>" \
    --assignee director \
...
check_key ELEVENLABS_API_KEY seraphiel ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY seraphiel OPENROUTER_API_KEY || exit 1
# ...
```
