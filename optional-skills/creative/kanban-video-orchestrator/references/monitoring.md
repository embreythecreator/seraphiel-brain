...
seraphiel kanban watch --tenant <project-slug>

# Snapshot of the board
seraphiel kanban list --tenant <project-slug>
seraphiel kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
seraphiel kanban stats --tenant <project-slug>

# Visual dashboard (browser)
seraphiel dashboard

# Inspect a specific task (includes comments + events)
seraphiel kanban show <task-id>

# Follow a single task's event stream
seraphiel kanban tail <task-id>
```

Verify available subcommands with `seraphiel kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `seraphiel kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `seraphiel kanban block <id>` then `seraphiel kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `seraphiel kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `seraphiel kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `seraphiel profile list` (profile exists?), `seraphiel status` (gateway/dispatcher up?), and verify tenant. |
...
seraphiel kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
seraphiel kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(seraphiel kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
seraphiel kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
seraphiel kanban block <task-id>
seraphiel kanban archive <task-id>

# Diagnose what happened
seraphiel kanban show <task-id>      # task body, comments, recent events
seraphiel kanban tail <task-id>      # follow the live event stream
seraphiel kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `seraphiel kanban create` for the director
...
while true; do
    clear
    seraphiel kanban list --tenant <slug>
    echo "---"
    seraphiel kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `seraphiel kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `seraphiel profile list` and start
