...
Usage:
  ...
  <server_name> matches the files in $SERAPHIEL_HOME/mcp-tokens/<server>.json etc.
...
def _seraphiel_home():
    # Prefer Seraphiel' own resolver (profile-safe); fall back to env then ~/.seraphiel.
    try:
        from seraphiel_constants import get_seraphiel_home
        return str(get_seraphiel_home())
    except Exception:
        return os.environ.get("SERAPHIEL_HOME") or os.path.expanduser("~/.seraphiel")


def _tokens_dir():
    return os.path.join(_seraphiel_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "seraphiel-diag", "version": "1.0"}},
        },
        headers={
