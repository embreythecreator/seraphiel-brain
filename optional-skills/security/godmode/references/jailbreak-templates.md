...
## 5. GODMODE FAST — Seraphiel 4 405B (Zero Refusal)

**Model:** `embreythecreator/seraphiel-4-405b`
**Strategy:** Seraphiel models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Seraphiel Brain

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.seraphiel/config.yaml`:
