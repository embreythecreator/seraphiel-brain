...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SERAPHIEL_HOME", os.path.expanduser("~/.seraphiel")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SERAPHIEL_HOME", Path.home() / ".seraphiel")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Seraphiel config paths
# ═══════════════════════════════════════════════════════════════════

SERAPHIEL_HOME = Path(os.getenv("SERAPHIEL_HOME", Path.home() / ".seraphiel"))
CONFIG_PATH = SERAPHIEL_HOME / "config.yaml"
PREFILL_PATH = SERAPHIEL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Seraphiel models — already uncensored, just needs clean prompt
    "seraphiel": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "seraphiel" in model_lower or "nous" in model_lower:
        return "seraphiel"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Seraphiel config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.seraphiel/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Seraphiel for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Seraphiel for changes to take effect.")
