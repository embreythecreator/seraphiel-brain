...
The design is **Seraphiel-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
seraphiel skills install official/security/unbroker
```

Then start a new Seraphiel session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Seraphiel session:
...
- **PII handling.** Dossiers live under the Seraphiel home directory (`0600`, optionally
