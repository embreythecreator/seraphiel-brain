...
All per-subject data lives under PDD_DATA_DIR (default: $SERAPHIEL_HOME/unbroker),
which is the same trust boundary Seraphiel uses for .env and OAuth tokens.
"""
...
def seraphiel_home() -> Path:
    return Path(os.environ.get("SERAPHIEL_HOME") or (Path.home() / ".seraphiel"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else seraphiel_home() / "unbroker"
