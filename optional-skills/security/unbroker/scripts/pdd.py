...
The Seraphiel agent orchestrates scanning and opt-out submission with native tools
...
Run it through the `terminal` tool (it can read PII files under SERAPHIEL_HOME);
...
def cmd_setup(args) -> None:
    if getattr(args, "auto", False):
        ...
        # asking anyone. Read creds from $SERAPHIEL_HOME/.env too (the terminal shell doesn't export
...
def cmd_doctor(args) -> None:
    ...
    cfg = config_mod.load_config()
    caps = config_mod.detect_capabilities(config_mod.dotenv_env())  # see creds in $SERAPHIEL_HOME/.env too
    ...
    if engaged:
        L.append(f"        key: {crypto.identity_path()} (0600) - guards casual/backup/commit "
                 "exposure, NOT a full-SERAPHIEL_HOME read")
    ...
    if cfg["email_mode"] == "draft_only":
        ...
        L.append("  Email mode: browser (no password) - the agent sends opt-outs and opens verify "
                 "links via the operator's logged-in webmail. This needs Seraphiel pointed at the "
                 "operator's OWN Chrome over CDP (launch with --remote-debugging-port=9222 "
                 "--user-data-dir=~/.seraphiel/chrome-debug, signed into the webmail once); else it falls "
    ...
    if not crypto.is_engaged():
        L.append("  Storage: dossiers are PLAINTEXT JSON (0600 under SERAPHIEL_HOME). "
...
def cmd_cdp(args) -> None:
    ...
    Seraphiel at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({"running": True, "endpoint": f"127.0.0.1:{port}",
              ...
              "note": "a debuggable browser is already listening; point Seraphiel's browser tools at "
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Seraphiel's browser tools at 127.0.0.1:{port} (CDP)",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--profile",
                   help="user-data-dir (default: $SERAPHIEL_HOME/chrome-debug, a dedicated debug profile)")
