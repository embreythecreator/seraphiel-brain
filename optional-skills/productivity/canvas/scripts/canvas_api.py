#!/usr/bin/env python3
"""Canvas LMS API CLI for Seraphiel Brain.
...
def _check_config():
    ...
    if missing:
        seraphiel_env = os.path.join(
            os.environ.get("SERAPHIEL_HOME", os.path.expanduser("~/.seraphiel")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {seraphiel_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Seraphiel Brain"
    )
