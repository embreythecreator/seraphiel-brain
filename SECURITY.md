# Seraphiel Brain Security Policy

This document describes Seraphiel Brain's trust model, names the one
...
Seraphiel Brain is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Seraphiel Brain,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Seraphiel Brain access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Seraphiel Brain's documentation
...
Seraphiel Brain supports two OS-level isolation postures. They address
...
Seraphiel Brain supports this in two ways:

- **Seraphiel Brain's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Seraphiel Brain's in-process heuristics
...
Seraphiel Brain filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Seraphiel Brain
itself. Bugs in Seraphiel Brain's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Seraphiel Brain does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Seraphiel Brain's own documentation, or reasonable
  operator expectations would predict — including cases where
  Seraphiel Brain has documented a stance about how its output should
...
- Seraphiel Brain includes supply-chain guards for MCP server
