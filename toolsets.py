...
_SERAPHIEL_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane, and close an agent's
    # read-only terminal tab (both gated on SERAPHIEL_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (SERAPHIEL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `seraphiel tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``seraphiel tools`` → Video Generation."
        ),
    ...
    "plan": {
        "description": "Plan-mode artifact: save a markdown plan for operator approval",
        "tools": ["save_plan"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SERAPHIEL_KANBAN_TASK env "
    ...
    # Coding posture (base Seraphiel — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in seraphiel_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Seraphiel toolsets (CLI + messaging platforms)
    #
    ...
    # the `seraphiel send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "seraphiel-cli": {
        ...
        "tools": _SERAPHIEL_CORE_TOOLS,
        "includes": []
    },

    "seraphiel-cron": {
        # Mirrors seraphiel-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `seraphiel tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as seraphiel-cli; gated by `seraphiel tools`",
        "tools": _SERAPHIEL_CORE_TOOLS,
        "includes": []
    },
    ...
    "seraphiel-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _SERAPHIEL_CORE_TOOLS,
        "includes": []
    },

    "seraphiel-homeassistant": {
    ...
    "seraphiel-email": {
        "description": "Email bot toolset - interact with Seraphiel via email (IMAP/SMTP)",
        "tools": _SERAPHIEL_CORE_TOOLS,
        "includes": []
    },

    "seraphiel-mattermost": {
    ...
    "seraphiel-sms": {
        "description": "SMS bot toolset - interact with Seraphiel via SMS (Twilio)",
        "tools": _SERAPHIEL_CORE_TOOLS,
        "includes": []
    },
    ...
    "seraphiel-webhook": {
        ...
        "tools": _SERAPHIEL_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "seraphiel-gateway": {
        ...
        "tools": [],
        "includes": ["seraphiel-telegram", "seraphiel-discord", "seraphiel-whatsapp", "seraphiel-slack", "seraphiel-signal", "seraphiel-bluebubbles", "seraphiel-homeassistant", "seraphiel-email", "seraphiel-sms", "seraphiel-mattermost", "seraphiel-matrix", "seraphiel-dingtalk", "seraphiel-feishu", "seraphiel-wecom", "seraphiel-wecom-callback", "seraphiel-weixin", "seraphiel-qqbot", "seraphiel-webhook", "seraphiel-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``seraphiel-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SERAPHIEL_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``seraphiel-gateway``
    ...
    """
    core = set(_SERAPHIEL_CORE_TOOLS)
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (seraphiel-<name>).
        # Gives them _SERAPHIEL_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("seraphiel-"):
            platform_name = name[len("seraphiel-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SERAPHIEL_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
