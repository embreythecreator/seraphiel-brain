...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/embreythecreator/seraphiel-brain/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    seraphiel_meta = (fm.get("metadata") or {}).get("seraphiel") or {}
    tags = seraphiel_meta.get("tags") or []
    related = seraphiel_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `seraphiel skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Seraphiel Brain"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Seraphiel ships with a large built-in skill library copied into `~/.seraphiel/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Seraphiel also syncs bundled skills on `seraphiel update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.seraphiel/skills/` tree, it is still shipped with Seraphiel; restore it with `seraphiel skills reset <name> --restore`.",
        "",
