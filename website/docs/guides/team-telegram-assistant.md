...
This tutorial walks you through setting up a Telegram bot powered by Seraphiel Brain that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Seraphiel Brain installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.seraphiel/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Seraphiel itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Seraphiel Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_seraphiel_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Seraphiel Brain. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.seraphiel/.env`:
...
```
[Gateway] Starting Seraphiel Gateway...
...
# View live logs
journalctl --user -u seraphiel-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `seraphiel gateway install` to update the plist.
:::
...
```bash
seraphiel gateway stop && seraphiel gateway start
```
...
seraphiel pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.seraphiel/.env`:
...
Control how much detail the bot shows when using tools. In `~/.seraphiel/config.yaml`:
...
Customize how the bot communicates by editing `~/.seraphiel/SOUL.md`:

For a full guide, see [Use SOUL.md with Seraphiel](/guides/use-soul-with-seraphiel).
...
```bash
# From the CLI
seraphiel cron list          # View all scheduled jobs
seraphiel cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u seraphiel-gateway` (Linux) or `~/.seraphiel/logs/gateway.log` (macOS) |
| Cron job output | `~/.seraphiel/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.seraphiel/cron/jobs.json` |
| Pairing data | `~/.seraphiel/pairing/` |
| Session history | `~/.seraphiel/sessions/` |
