...
description: "Diagnose and fix common Seraphiel cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`seraphiel gateway` for foreground, or `seraphiel gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `seraphiel cron tick`.
...
```bash
date
seraphiel cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.seraphiel/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.seraphiel/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.seraphiel/.env` |
...
| `local` | Write access to `~/.seraphiel/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `seraphiel cron list` for updated `last_error` field (if available).
...
```bash
seraphiel skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `seraphiel skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `seraphiel skills list`.
...
2. `~/.seraphiel/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `seraphiel cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Seraphiel config directory). Verify:
```bash
ls ~/.seraphiel/scripts/your-script.py   # Must exist
seraphiel cron edit <job_id> --script ~/.seraphiel/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `seraphiel skills install <skill-name>`.
...
```bash
ps aux | grep seraphiel
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.seraphiel/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.seraphiel/cron/jobs.json
chmod 600 ~/.seraphiel/cron/jobs.json   # Your user should own it
```
...
```bash
seraphiel cron list                    # Show all jobs, states, next_run times
seraphiel cron run <job_id>            # Schedule for next tick (for testing)
seraphiel cron edit <job_id>           # Fix configuration issues
seraphiel logs                         # View recent Seraphiel logs
seraphiel skills list                  # Verify installed skills
```
...
1. Run the job with `seraphiel cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.seraphiel/logs/agent.log` for scheduler messages and `~/.seraphiel/logs/errors.log` for warnings
3. Open an issue at [github.com/embreythecreator/seraphiel-brain](https://github.com/embreythecreator/seraphiel-brain) with:
