...
Copy-paste blueprints for common automation patterns. Each blueprint uses Seraphiel's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`seraphiel webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `seraphiel webhook subscribe`) |
...
```bash
seraphiel cron create "0 2 * * *" \
  "You are a project manager triaging the embreythecreator/seraphiel-brain GitHub repo.

1. Run: gh issue list --repo embreythecreator/seraphiel-brain --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
seraphiel webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
seraphiel cron create "0 9 * * 1" \
  "Scan the embreythecreator/seraphiel-brain repo for documentation drift.

1. Run: gh pr list --repo embreythecreator/seraphiel-brain --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (seraphiel_cli/commands.py, seraphiel_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (seraphiel_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
seraphiel cron create "0 6 * * *" \
  "Run a dependency security audit on the seraphiel-brain project.

1. cd ~/.seraphiel/seraphiel-brain && source .venv/bin/activate
...
```bash
seraphiel webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
seraphiel webhook subscribe alert-triage \
...
```python title="~/.seraphiel/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Seraphiel-Monitor/1.0"})
...
```bash
seraphiel cron create "every 30m" \
  ...
  --script ~/.seraphiel/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
seraphiel cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Seraphiel Brain development." \
...
```bash
seraphiel webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
seraphiel webhook subscribe stripe-payments \
...
```bash
seraphiel cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the seraphiel-brain codebase.
