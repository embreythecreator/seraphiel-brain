...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `seraphiel send`."
---

# Pipe Script Output to Messaging Platforms

`seraphiel send` is a small, scriptable CLI that pushes a message to any
messaging platform Seraphiel is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | seraphiel send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `seraphiel
...
seraphiel send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
seraphiel send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
seraphiel send --list

# Filter by platform
seraphiel send --list telegram
```
...
Any platform Seraphiel ships adapters for works as a target:
...
`seraphiel send` resolves the message body in this order:

1. **Positional argument** — `seraphiel send --to telegram "hi"`
2. **`--file PATH`** — `seraphiel send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | seraphiel send --to telegram`

When stdin is a TTY (no pipe), Seraphiel does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  seraphiel send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `seraphiel send` reuses your Seraphiel config, the same script works on
any host where Seraphiel is installed — no need to export bot tokens into
...
`seraphiel send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  seraphiel send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | seraphiel send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.seraphiel/bin/seraphiel send \
...
```bash
./train.py --epochs 200 && \
  seraphiel send --to telegram "training done" || \
  seraphiel send --to telegram "training failed (exit $?)"
```
...
seraphiel send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(seraphiel send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `seraphiel send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `seraphiel send` calls
the platform's REST endpoint directly using credentials from
`~/.seraphiel/.env` and `~/.seraphiel/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `seraphiel gateway start` and retry.
...
# Machine-readable
seraphiel send --list --json
```

The listing is built from `~/.seraphiel/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`seraphiel gateway
...
| Approach | Multi-platform | Reuses Seraphiel creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `seraphiel send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`seraphiel send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `seraphiel send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `seraphiel send` shares with cron delivery.
