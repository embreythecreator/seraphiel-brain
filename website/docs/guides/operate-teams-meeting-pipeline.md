...
```bash
seraphiel teams-pipeline validate
```
...
```bash
seraphiel teams-pipeline token-health
seraphiel teams-pipeline token-health --force-refresh
```
...
```bash
seraphiel teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
seraphiel teams-pipeline maintain-subscriptions
seraphiel teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Seraphiel cron (recommended if you already run the Seraphiel gateway)

Seraphiel ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.seraphiel/scripts/`. First create the script:

```bash
mkdir -p ~/.seraphiel/scripts
cat > ~/.seraphiel/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec seraphiel teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.seraphiel/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
seraphiel cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/seraphiel-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Seraphiel Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=seraphiel
EnvironmentFile=/etc/seraphiel/env
ExecStart=/usr/local/bin/seraphiel teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/seraphiel-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Seraphiel Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now seraphiel-teams-pipeline-maintain.timer
systemctl list-timers seraphiel-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/seraphiel teams-pipeline maintain-subscriptions >> /var/log/seraphiel/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.seraphiel/.env` at the top of a wrapper script that crontab calls.
...
```bash
seraphiel teams-pipeline subscriptions   # should show expirationDateTime advanced
seraphiel teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
seraphiel teams-pipeline list
seraphiel teams-pipeline list --status failed
seraphiel teams-pipeline show <job-id>
```

### Replay a stored job

```bash
seraphiel teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
seraphiel teams-pipeline fetch --meeting-id <meeting-id>
seraphiel teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `seraphiel teams-pipeline maintain-subscriptions --dry-run`
- inspect `seraphiel teams-pipeline list --status failed`
...
- run `seraphiel teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `seraphiel teams-pipeline run`
...
- [ ] `seraphiel teams-pipeline validate` returns an OK snapshot
- [ ] `seraphiel teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Seraphiel cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
