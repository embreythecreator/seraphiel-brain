...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Seraphiel Brain  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Seraphiel instantly when PRs are opened or updated.
:::
...
- **Seraphiel Brain installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  seraphiel gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.seraphiel/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Seraphiel can access GitHub. Start a chat:
...
```
Run: gh pr list --repo embreythecreator/seraphiel-brain --state open --limit 3
```
...
Still in the chat, ask Seraphiel to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo embreythecreator/seraphiel-brain
```
...
A skill gives Seraphiel consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.seraphiel/skills/code-review
```

Create `~/.seraphiel/skills/code-review/SKILL.md`:
...
Verify it loaded — start `seraphiel` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Seraphiel your team's standards:
...
2. Teach Seraphiel your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
seraphiel gateway status    # Is the gateway running?
seraphiel cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-seraphiel-plugin)** — wrap the review logic into a shareable plugin
