...
`~/.seraphiel/config.yaml`, `~/.seraphiel/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/seraphiel`:

```text
/etc/seraphiel/
├── config.yaml     # managed config layer (wins over ~/.seraphiel/config.yaml)
└── .env            # managed env layer (wins over ~/.seraphiel/.env + shell)
```
...
The location can be relocated with the `SERAPHIEL_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SERAPHIEL_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Seraphiel.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SERAPHIEL_MANAGED_DIR=/opt/org/seraphiel-policy
```

:::warning
A user who can set `SERAPHIEL_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `seraphiel doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/seraphiel/config.yaml` (managed) | `/etc/seraphiel/.env` (managed) |
| 2 | `~/.seraphiel/config.yaml` (user) | `~/.seraphiel/.env` (user) |
...
```bash
seraphiel config        # shows a header naming the managed source + the pinned keys
seraphiel doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Seraphiel refuses and names the source:

```bash
$ seraphiel config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/seraphiel/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `seraphiel config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/seraphiel

# Pin some config values for every user on this machine
sudo tee /etc/seraphiel/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/seraphiel/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/seraphiel
sudo chmod 0644 /etc/seraphiel/config.yaml /etc/seraphiel/.env
```

Changes take effect on the next Seraphiel start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`seraphiel doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Seraphiel as `root`), managed scope is advisory.
