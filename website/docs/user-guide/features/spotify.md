...
Seraphiel can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.seraphiel/auth.json` and refreshed automatically on 401; you only log in once per machine.
