...
Seraphiel Brain can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `seraphiel computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `seraphiel tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Seraphiel refreshes the binary in two
places so you don't get stuck on a stale release:

- **`seraphiel update`** — when you update Seraphiel itself, if `cua-driver` is
...
- **`seraphiel computer-use install --upgrade`** — manual force-refresh.
...
`seraphiel computer-use status` shows the installed version next to the
binary path.
...
Pair with `approvals.mode: manual` in `~/.seraphiel/config.yaml` if you want every action confirmed.
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `SERAPHIEL_CUA_DRIVER_VERSION`
...
`seraphiel computer-use install` to fetch the cua-driver binary, or run
`seraphiel tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/embreythecreator/seraphiel-brain/blob/main/skills/apple/macos-computer-use/SKILL.md)
