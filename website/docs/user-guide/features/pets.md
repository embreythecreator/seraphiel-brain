...
Seraphiel can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SERAPHIEL_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Seraphiel renders the sprite at full fidelity when your
...
seraphiel pets list
seraphiel pets list cat

# Install a pet and make it active in one step
seraphiel pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
seraphiel pets show

# Check your setup
seraphiel pets doctor
```

## `seraphiel pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `seraphiel pets list [query] [--limit N]` |
| List installed pets | `seraphiel pets list --installed` |
| Install a pet | `seraphiel pets install <slug> [--select] [--force]` |
| Set the active pet | `seraphiel pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `seraphiel pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `seraphiel pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `seraphiel pets off` |
| Remove an installed pet | `seraphiel pets remove <slug>` |
| Diagnose setup | `seraphiel pets doctor` |

`seraphiel pets show` flags:
...
Beyond installing pre-made pets from the gallery, Seraphiel can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `seraphiel pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `seraphiel tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SERAPHIEL_PET_IMAGE_PROVIDER` env var (e.g. `SERAPHIEL_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Seraphiel is
...
Run `seraphiel pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Seraphiel uses its own
  profile-scoped `<SERAPHIEL_HOME>/pets/` instead — install through `seraphiel pets`.
