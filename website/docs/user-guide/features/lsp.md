...
Seraphiel runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Seraphiel
...
1. Seraphiel captures a baseline of current diagnostics for the file.
...
…). Seraphiel auto-detects the binary on PATH or in
`<SERAPHIEL_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Seraphiel at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SERAPHIEL_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`seraphiel lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Seraphiel installs both packages together when you
run `seraphiel lsp install typescript` or auto-install fires on first
use.

## CLI

```
seraphiel lsp status          # service state + per-server install status
seraphiel lsp list            # registry, optionally --installed-only
seraphiel lsp install <id>    # eagerly install one server
seraphiel lsp install-all     # try every server with a known recipe
seraphiel lsp restart         # tear down running clients
seraphiel lsp which <id>      # print resolved binary path
```

`seraphiel lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SERAPHIEL_HOME>/lsp/bin
...
When `install_strategy: auto`, Seraphiel installs binaries into
`<SERAPHIEL_HOME>/lsp/bin/`. NPM packages land in
`<SERAPHIEL_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Seraphiel-owned and is
...
Servers are kept alive for the life of the Seraphiel process. There's
...
**`seraphiel lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SERAPHIEL_HOME>/lsp/bin/`. Run
`seraphiel lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `seraphiel lsp status`**
...
When `seraphiel lsp status` shows a `Backend warnings` section, install
...
`~/.seraphiel/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.seraphiel/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `seraphiel lsp restart` to clear the set;
