...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.seraphiel/skills/.archive/`.
...
seraphiel curator run            # trigger a review now (blocks until the LLM pass finishes)
seraphiel curator run --background  # fire-and-forget: start the LLM pass in a background thread
