...
description: "Plugins shipped with Seraphiel Brain that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
2. **User** — `~/.seraphiel/plugins/<name>/`
3. **Project** — `./.seraphiel/plugins/<name>/` (requires `SERAPHIEL_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `seraphiel_brain.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `seraphiel memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `seraphiel plugins list` and the interactive `seraphiel plugins` UI), but none load until you explicitly enable them:

```bash
seraphiel plugins enable disk-cleanup
```

Or via `~/.seraphiel/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Seraphiel. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
seraphiel plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `seraphiel plugins enable <name>`.
...
| `seraphiel-achievements` | dashboard tab | Steam-style collectible badges generated from your real Seraphiel session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `seraphiel memory` and `seraphiel plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SERAPHIEL_HOME` or `/tmp/seraphiel-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under SERAPHIEL_HOME | always | Never |
...
**State** — everything lives at `$SERAPHIEL_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SERAPHIEL_HOME` or `/tmp/seraphiel-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `seraphiel plugins enable disk-cleanup` (or check the box in `seraphiel plugins`).

**Disabling again:** `seraphiel plugins disable disk-cleanup`.
...
**Enabling:** `seraphiel plugins enable security-guidance` (or check the box in `seraphiel plugins`).

**Disabling again:** `seraphiel plugins disable security-guidance`.
...
Traces Seraphiel turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Seraphiel' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `seraphiel logs`.
...
```bash
seraphiel tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Seraphiel and the next turn ships a trace.
...
Then put the credentials in `~/.seraphiel/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Seraphiel turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `SERAPHIEL_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Seraphiel session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `seraphiel chat` session lives under one Langfuse session.

**Verify:**

```bash
seraphiel plugins list                 # observability/langfuse should show "enabled"
seraphiel chat -q "hello"              # check the Langfuse UI for a "Seraphiel turn" trace
```
...
| `SERAPHIEL_LANGFUSE_RELEASE` | — | Release/version tag |
| `SERAPHIEL_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `SERAPHIEL_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `SERAPHIEL_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Seraphiel-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Seraphiel-prefixed wins when both are set.
...
**Disabling:** `seraphiel plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.seraphiel/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
seraphiel plugins enable google_meet
...
**Disabling:** `seraphiel plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.seraphiel/cache/google_meet/` until you remove them.

### seraphiel-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Seraphiel session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Seraphiel feature changes.

**How it works:**

- Scans your entire `~/.seraphiel/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SERAPHIEL_HOME/plugins/seraphiel-achievements/state.json`
...
| Secret | Hidden until Seraphiel detects the first related signal in your history |

**API** — routes mount under `/api/plugins/seraphiel-achievements/`:
...
**State files** — live under `$SERAPHIEL_HOME/plugins/seraphiel-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Seraphiel updates. |
...
**Enabling:** Nothing to enable — `seraphiel-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `seraphiel dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/seraphiel-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.seraphiel/plugins/seraphiel-achievements/` that ships no dashboard. The plugin's state files under `$SERAPHIEL_HOME/plugins/seraphiel-achievements/` survive — reinstalling preserves your unlock history.
