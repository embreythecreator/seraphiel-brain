...
chat completions — use your Seraphiel-managed provider subscription as their
...
| Use case | "Use Seraphiel as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Seraphiel stores
the refresh token in `~/.seraphiel/auth.json` — the same place all Seraphiel
...
Model:      Seraphiel-4-70B    # or Seraphiel-4.3-36B, Seraphiel-4-405B
```
...
interface in `seraphiel_cli/proxy/adapters/`.
...
If you see `not logged in`, run `seraphiel portal`. If you see
...
supports; check `portal.embreythecreator.com/models`.
...
[portal.embreythecreator.com](https://portal.embreythecreator.com).
...
`seraphiel_cli/proxy/adapters/<provider>.py` and registering it in
