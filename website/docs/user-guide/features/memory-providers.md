...
Seraphiel Brain ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
seraphiel memory setup      # interactive picker + configuration
seraphiel memory status     # check what's active
seraphiel memory off        # disable external provider
```

You can also select the active memory provider via `seraphiel plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.seraphiel/config.yaml`:
...
When a memory provider is active, Seraphiel automatically:
...
**Setup Wizard:**
```bash
seraphiel memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `seraphiel honcho setup` command still works (it now redirects to `seraphiel memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$SERAPHIEL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$SERAPHIEL_HOME/honcho.json` > `~/.seraphiel/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/embreythecreator/seraphiel-brain/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/seraphiel).
...
:::tip Migrating from `seraphiel honcho`
If you previously used `seraphiel honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Seraphiel profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Seraphiel profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Seraphiel profile. Host key `seraphiel` → default; `seraphiel.<profile>` for others. |
...
```bash
seraphiel profile create coder --clone
```

`--clone` creates a `seraphiel.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
seraphiel honcho sync
```

Scans every Seraphiel profile, creates host blocks for any profile without one, inherits settings from the default `seraphiel` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `seraphiel memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/embreythecreator/seraphiel-brain/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/seraphiel).
...
seraphiel memory setup    # select "openviking"
# Or manually:
seraphiel config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.seraphiel/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.seraphiel/.env
```
...
`OPENVIKING_AGENT` is Seraphiel' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
seraphiel memory setup    # select "mem0" → "Platform"
# Or manually:
seraphiel config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.seraphiel/.env
```

**Setup (OSS):**
```bash
seraphiel memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
seraphiel memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
seraphiel memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
seraphiel memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
seraphiel memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.seraphiel/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.seraphiel/.env
```
...
**Config:** `$SERAPHIEL_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.seraphiel/.env`.
...
| `user_id` | `seraphiel-user` | User identifier |
| `agent_id` | `seraphiel` | Agent identifier |
...
**Switching modes:** Re-run `seraphiel memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
seraphiel memory setup    # select "hindsight"
# Or manually:
seraphiel config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.seraphiel/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p seraphiel ui start`

**Config:** `$SERAPHIEL_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `seraphiel` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Seraphiel Brain and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/embreythecreator/seraphiel-brain/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
seraphiel memory setup    # select "holographic"
# Or manually:
seraphiel config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.seraphiel-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$SERAPHIEL_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
seraphiel memory setup    # select "retaindb"
# Or manually:
seraphiel config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.seraphiel/.env
```
...
# Then configure Seraphiel
seraphiel memory setup    # select "byterover"
# Or manually:
seraphiel config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$SERAPHIEL_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=seraphiel), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
seraphiel memory setup    # select "supermemory"
# Or manually:
seraphiel config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.seraphiel/.env
```
...
Before running `seraphiel memory setup`, set `base_url` in
`$SERAPHIEL_HOME/supermemory.json`:
...
Then run `seraphiel memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$SERAPHIEL_HOME/supermemory.json`
...
| `container_tag` | `seraphiel` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `seraphiel-{identity}` → `seraphiel-coder`) to isolate memories per Seraphiel profile
...
| **Requires** | `pip install seraphiel-memori` + `seraphiel-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install seraphiel-memori
seraphiel-memori install
seraphiel config set memory.provider memori
seraphiel memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `seraphiel-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$SERAPHIEL_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$SERAPHIEL_HOME/` so each profile has its own credentials
