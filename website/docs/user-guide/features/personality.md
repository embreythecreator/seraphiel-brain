...
description: "Customize Seraphiel Brain's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Seraphiel Brain's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SERAPHIEL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Seraphiel is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Seraphiel now seeds a default `SOUL.md` automatically in:

```text
~/.seraphiel/SOUL.md
```

More precisely, it uses the current instance's `SERAPHIEL_HOME`, so if you run Seraphiel with a custom home directory, it will use:

```text
$SERAPHIEL_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Seraphiel creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Seraphiel loads `SOUL.md` only from `SERAPHIEL_HOME`
- Seraphiel does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Seraphiel falls back to a built-in default identity
...
If Seraphiel loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SERAPHIEL_HOME`, the personality belongs to the Seraphiel instance itself.

That also makes it easier to teach users:
- "Edit `~/.seraphiel/SOUL.md` to change Seraphiel' default personality."
...
- how Seraphiel should handle uncertainty, disagreement, or ambiguity
...
## What Seraphiel injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Seraphiel falls back to a built-in default identity ("You are Seraphiel Brain, an intelligent AI assistant created by Embrey The Creator / The Voice..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Seraphiel ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Seraphiel, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Seraphiel its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.seraphiel/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.seraphiel/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-seraphiel)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Seraphiel speaks
- `display.skin` and `/skin` affect how Seraphiel looks in the terminal
