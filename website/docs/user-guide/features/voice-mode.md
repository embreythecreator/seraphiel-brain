...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Seraphiel Brain — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Seraphiel Brain supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Seraphiel](/guides/use-voice-mode-with-seraphiel).
...
:::tip
The `~/.seraphiel/` directory and default `config.yaml` are created automatically the first time you run `seraphiel`. You only need to create `~/.seraphiel/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `seraphiel setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`seraphiel chat`) and the **TUI** (`seraphiel --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.seraphiel/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
seraphiel                # Start the interactive CLI
```
...
1. Start the CLI with `seraphiel` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.seraphiel/config.yaml` (default: `ctrl+b`).
:::
...
```bash
seraphiel gateway        # Start the gateway (connects to configured platforms)
seraphiel gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@seraphielbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.seraphiel/.env`:
```bash
...
```bash
seraphiel gateway        # Start with existing configuration
```
...
If you are running Seraphiel inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.seraphiel/.env`
...
- Review gateway logs: `tail -f ~/.seraphiel/logs/gateway.log`
