...
When Seraphiel Brain runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.seraphiel/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.seraphiel/config.yaml` (switchable per session
via `/personality`).
...
If you use Seraphiel' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.seraphiel/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Seraphiel Brain's deliverable
