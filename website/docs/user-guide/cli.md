...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Seraphiel Brain terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Seraphiel Brain's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `seraphiel setup --portal` — and you're ready to `seraphiel chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Seraphiel also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `seraphiel --tui` — see the [TUI](tui.md) guide.
:::
...
seraphiel chat --model "anthropic/claude-sonnet-4"

# With a specific provider
seraphiel chat --provider nous        # Use Nous Portal
seraphiel chat --provider openrouter  # Force OpenRouter

# With specific toolsets
seraphiel chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
seraphiel -s seraphiel-brain-dev,github-auth
seraphiel chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
seraphiel --continue             # Resume the most recent CLI session (-c)
seraphiel --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
seraphiel chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
seraphiel -w                         # Interactive mode in worktree
seraphiel -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Seraphiel CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Seraphiel CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SERAPHIEL_YOLO_MODE` is on (either `seraphiel --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Seraphiel to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Seraphiel supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Seraphiel replies |
...
```bash
seraphiel -s seraphiel-brain-dev,github-auth
seraphiel chat -s github-pr-workflow -s github-auth
```

Seraphiel loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.seraphiel/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.seraphiel/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Seraphiel..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Seraphiel recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Seraphiel — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Seraphiel is working, Seraphiel prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Seraphiel to the background — just like any terminal process. The shell prints a confirmation:

```
Seraphiel Brain has been suspended. Run `fg` to bring Seraphiel Brain back.
```
...
Resume this session with:
  seraphiel --resume 20260225_143052_a1b2c3
...
```bash
seraphiel --continue                          # Resume the most recent CLI session
seraphiel -c                                  # Short form
seraphiel -c "my project"                     # Resume a named session (latest in lineage)
seraphiel --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
seraphiel --resume "refactoring auth"         # Resume by title
seraphiel -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `seraphiel sessions rename <id> <title>` from the command line. Use `seraphiel sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Seraphiel's SQLite state database under `~/.seraphiel/state.db`. The database keeps:
...
Seraphiel immediately confirms the task and gives you back the prompt:
