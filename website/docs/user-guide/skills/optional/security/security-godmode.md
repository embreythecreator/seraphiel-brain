...
12. **execute_code sandbox has no env vars** — When Seraphiel runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.seraphiel/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.seraphiel/.env"))`
