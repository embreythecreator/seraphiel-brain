...
| | |
|---|---|
| Source | Optional — install with `seraphiel skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Seraphiel Brain |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Seraphiel terminal calls (desktop app flow only)
...
## Seraphiel Execution Pattern (desktop app flow)

Seraphiel terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/seraphiel-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/seraphiel-op.sock"
