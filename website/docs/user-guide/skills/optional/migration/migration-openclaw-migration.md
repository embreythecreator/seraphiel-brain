---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Seraphiel Brain"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Seraphiel Brain"
---
...
Migrate a user's OpenClaw customization footprint into Seraphiel Brain. Imports Seraphiel-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `seraphiel skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Seraphiel Brain (Seraphiel) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Seraphiel`, `Memory`, `Persona`, `Import` |
| Related skills | [`seraphiel-brain`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-seraphiel-brain) |
...
# OpenClaw -> Seraphiel Migration

Use this skill when a user wants to move their OpenClaw setup into Seraphiel Brain with minimal manual cleanup.
...
```bash
seraphiel claw migrate              # Full interactive migration
...
seraphiel claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `seraphiel setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_seraphiel.py` to:

- import `SOUL.md` into the Seraphiel home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Seraphiel memory entries
- merge OpenClaw command approval patterns into Seraphiel `command_allowlist`
- migrate Seraphiel-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Seraphiel working-directory configuration
- copy OpenClaw skills into `~/.seraphiel/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Seraphiel workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.seraphiel/tts/`
- archive non-secret docs that do not have a direct Seraphiel destination
...
- `scripts/openclaw_to_seraphiel.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py`

Do not guess a shorter path like `~/.seraphiel/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.seraphiel/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Seraphiel-compatible secrets, currently:
...
Seraphiel CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Seraphiel. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Seraphiel workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Seraphiel skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.seraphiel/skills/migration/openclaw-migration/scripts/openclaw_to_seraphiel.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Seraphiel unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Seraphiel targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Seraphiel destination. Unsupported auth blobs must still be reported as skipped.
...
- Seraphiel persona state imported
- Seraphiel memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.seraphiel/skills/openclaw-imports/`
