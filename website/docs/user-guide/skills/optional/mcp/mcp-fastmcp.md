...
| | |
|---|---|
| Source | Optional — install with `seraphiel skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Seraphiel or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Seraphiel. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.seraphiel/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.seraphiel/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Seraphiel integration, either:

- configure the server in `~/.seraphiel/config.yaml` using the `native-mcp` skill, or
...
### Seraphiel cannot see the deployed server

The server-building part may be correct while the Seraphiel config is not. Load the `native-mcp` skill and configure the server in `~/.seraphiel/config.yaml`, then restart Seraphiel.
