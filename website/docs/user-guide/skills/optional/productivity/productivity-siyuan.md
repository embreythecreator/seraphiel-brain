...
| | |
|---|---|
| Source | Optional — install with `seraphiel skills install official/productivity/siyuan` |
...
3. Store it in `${SERAPHIEL_HOME:-~/.seraphiel}/.env`:
   ```
...
```yaml
# In ~/.seraphiel/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
