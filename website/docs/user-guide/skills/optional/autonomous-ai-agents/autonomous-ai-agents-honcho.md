...
description: "Configure and use Honcho memory with Seraphiel -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Seraphiel -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `seraphiel skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Seraphiel

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Seraphiel profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/seraphiel#running-honcho-locally-with-seraphiel

### Verify

```bash
seraphiel honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Seraphiel profile's AI peer
...
Honcho models conversations as interactions between **peers**. Seraphiel creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Seraphiel instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Seraphiel run |
| `global` | Single session across all directories |

Manual override: `seraphiel honcho map my-project-name`
...
Each Seraphiel profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block seraphiel.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `seraphiel.coder` host block in `honcho.json`
...
```bash
seraphiel honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Seraphiel when Honcho memory is active.
...
Config file: `$SERAPHIEL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `seraphiel honcho setup`. Ensure `memory.provider: honcho` is in `~/.seraphiel/config.yaml`.

### Memory not persisting across sessions
Check `seraphiel honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `seraphiel profile create <name> --clone`. For existing profiles: `seraphiel honcho sync`.
...
| Command | Description |
|---------|-------------|
| `seraphiel honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `seraphiel honcho status` | Show resolved config, connection test, peer info for active profile |
| `seraphiel honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `seraphiel honcho disable` | Disable Honcho for the active profile |
| `seraphiel honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `seraphiel honcho peers` | Show peer identities across all profiles |
| `seraphiel honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `seraphiel honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `seraphiel honcho sessions` | List known directory-to-session-name mappings |
| `seraphiel honcho map <name>` | Map current working directory to a Honcho session name |
| `seraphiel honcho identity` | Seed AI peer identity or show both peer representations |
| `seraphiel honcho sync` | Create host blocks for all Seraphiel profiles that don't have one yet |
| `seraphiel honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Seraphiel + Honcho |
| `seraphiel memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `seraphiel memory status` | Show active memory provider and config |
| `seraphiel memory off` | Disable external memory provider |
