...
| | |
|---|---|
| Source | Optional — install with `seraphiel skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Seraphiel core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Seraphiel native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Seraphiel native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Seraphiel native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Seraphiel workflow:
...
## Recommended Seraphiel usage patterns
...
- Do not silently choose Parallel when Seraphiel native tools are already sufficient.
