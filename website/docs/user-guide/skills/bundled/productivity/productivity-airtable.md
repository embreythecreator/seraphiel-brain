...
4. Store the token in `~/.seraphiel/.env` (or via `seraphiel setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.seraphiel/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
