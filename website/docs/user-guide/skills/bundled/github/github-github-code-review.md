...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _seraphiel_env="${SERAPHIEL_HOME:-$HOME/.seraphiel}/.env"; [ -f "$_seraphiel_env" ] && grep -q "^GITHUB_TOKEN=" "$_seraphiel_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_seraphiel_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Seraphiel Brain\",
    \"comments\": [
...
```bash
source "${SERAPHIEL_HOME:-$HOME/.seraphiel}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Seraphiel Brain. Code looks clean — good test coverage, no security concerns."
