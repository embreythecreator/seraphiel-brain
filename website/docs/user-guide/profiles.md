...
Run multiple independent Seraphiel agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Seraphiel home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Seraphiel state.
...
```bash
seraphiel profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Seraphiel profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `seraphiel setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
seraphiel profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `seraphiel profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
The alias works with every seraphiel subcommand — it's just `seraphiel -p <name>` under the hood.
...
```bash
seraphiel -p coder chat
seraphiel --profile=coder doctor
seraphiel chat -p coder -q "hello"    # works in any position
```

### Sticky default (`seraphiel profile use`)

```bash
seraphiel profile use coder
seraphiel chat                   # now targets coder
seraphiel tools                  # configures coder's tools
seraphiel profile use default    # switch back
```

Sets a default so plain `seraphiel` commands target that profile. Like `kubectl config use-context`.
...
- **`seraphiel profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Seraphiel its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Seraphiel was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.seraphiel/profiles/coder/.env

# Edit assistant's tokens
nano ~/.seraphiel/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates seraphiel-gateway-coder systemd/launchd service
assistant gateway install     # creates seraphiel-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `seraphiel profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `seraphiel -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.seraphiel/profiles/coder/SOUL.md
```
...
```bash
seraphiel profile list           # show all profiles with status
seraphiel profile show coder     # detailed info for one profile
seraphiel profile rename coder dev-bot   # rename (updates alias + service)
seraphiel profile export coder   # export to coder.tar.gz
seraphiel profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
seraphiel profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `seraphiel profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.seraphiel`). To remove everything, use `seraphiel uninstall`.
:::
...
```bash
# Bash
eval "$(seraphiel completion bash)"

# Zsh
eval "$(seraphiel completion zsh)"
```
