...
description: Control your smart home with Seraphiel Brain via Home Assistant integration.
...
Seraphiel Brain integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Seraphiel Brain"
5. Copy the token
...
Seraphiel Brain registers four tools for smart home control:
...
Configure which events the agent sees in `~/.seraphiel/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Seraphiel Brain".
...
The adapter reads credentials from `~/.seraphiel/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Seraphiel profile
...
running Seraphiel — `curl -H "Authorization: Bearer <token>" <url>/api/` should
