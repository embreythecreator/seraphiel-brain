...
Connect Seraphiel to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Seraphiel supports two WeCom integration modes:
...
> Run `seraphiel gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Seraphiel decrypts the message, queues it for the agent
...
(Use `seraphiel gateway start` only after `seraphiel gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Seraphiel and the token the
...
in `~/.seraphiel/.env` values around `=` will also break signature checks. After
fixing, restart `seraphiel gateway run`.
...
Check `seraphiel gateway run` logs for the bound host/port. If the adapter bound to
