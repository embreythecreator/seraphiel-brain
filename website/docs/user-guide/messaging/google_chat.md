...
sidebar_position: 12
title: "Google Chat"
description: "Set up Seraphiel Brain as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Seraphiel Brain to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Seraphiel
...
> Run `seraphiel gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Seraphiel can read (e.g.,
`~/.seraphiel/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a seraphiel restart)
...
- Principal: `seraphiel-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Seraphiel calls
...
- **App name**: whatever you want users to see ("Seraphiel" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/seraphiel-chat-events`.
...
`ADDED_TO_SPACE` event that Seraphiel uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Seraphiel

Add the Google Chat section to `~/.seraphiel/.env`:
...
Install the dependencies the Google Chat adapter needs (no Seraphiel extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Seraphiel is thinking…" marker, then
...
`~/.seraphiel/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Seraphiel is thinking…").
...
Thread support: when a user replies inside a thread, Seraphiel detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Seraphiel session.
...
3. Download the JSON. Move it onto the host that runs Seraphiel.
4. Register the client with Seraphiel (run under the profile you want it scoped to):
...
seraphiel -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Seraphiel home (e.g.
`~/.seraphiel/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.seraphiel/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.seraphiel/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Seraphiel isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `seraphiel gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Seraphiel will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.seraphiel/google_chat_user_tokens/<sanitized_email>.json` (filesystem
