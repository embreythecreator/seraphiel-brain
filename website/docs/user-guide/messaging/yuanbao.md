...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Seraphiel Brain to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Seraphiel to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.seraphiel/.env`:
...
All standard Seraphiel commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.seraphiel/logs/gateway.log`
...
4. Consider enabling verbose logging: `SERAPHIEL_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Seraphiel automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
SERAPHIEL_LOG_LEVEL=debug seraphiel gateway
```
...
```bash
seraphiel chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
