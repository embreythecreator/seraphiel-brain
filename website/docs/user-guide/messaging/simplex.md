...
`simplex:group:12` as a cron `deliver=` target or in a `seraphiel send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`seraphiel send` CLI](/guides/pipe-script-output):

```bash
seraphiel send simplex:<contact-id> "Done!"
```
