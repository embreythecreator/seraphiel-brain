...
description: "Chat with Seraphiel from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Seraphiel from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Seraphiel](/guides/use-voice-mode-with-seraphiel).
...
```mermaid
flowchart TB
    subgraph Gateway["Seraphiel Gateway"]
...
For group chats, hooks, and automation flows, Seraphiel supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Seraphiel keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Seraphiel does not hide failures just because the text resembles a silence token.
...
```bash
seraphiel gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
seraphiel gateway              # Run in foreground
seraphiel gateway setup        # Configure messaging platforms interactively
seraphiel gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo seraphiel gateway install --system   # Linux only: install a boot-time system service
seraphiel gateway start        # Start the default service
seraphiel gateway stop         # Stop the default service
seraphiel gateway status       # Check default service status
seraphiel gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.seraphiel/config.yaml"
gateway:
...
```bash
seraphiel gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Seraphiel sends heartbeats
...
| `/update` | Update Seraphiel Brain to the latest version |
...
with the `session_reset` section in `~/.seraphiel/config.yaml`:
...
Configure per-platform overrides in `~/.seraphiel/gateway.json`:
...
seraphiel pairing approve telegram XKGH5N7P

# Other pairing commands:
seraphiel pairing list          # View pending + approved users
seraphiel pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Seraphiel appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.seraphiel/config.yaml`:
...
Off by default. When enabled, Seraphiel prepends a human-readable timestamp
...
Seraphiel confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.seraphiel/config.yaml`:
...
```bash
SERAPHIEL_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
seraphiel gateway install               # Install as user service
seraphiel gateway start                 # Start the service
seraphiel gateway stop                  # Stop the service
seraphiel gateway status                # Check status
journalctl --user -u seraphiel-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo seraphiel gateway install --system
sudo seraphiel gateway start --system
sudo seraphiel gateway status --system
journalctl -u seraphiel-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Seraphiel installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit seraphiel-gateway` (or `sudo systemctl edit seraphiel-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `seraphiel update`. When `seraphiel update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart seraphiel-gateway` command (it never blocks on an interactive password prompt).
...
```bash
seraphiel gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `seraphiel update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo seraphiel update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/seraphiel-gateway`:

```
seraphiel ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed seraphiel-gateway*, /usr/bin/systemctl --no-ask-password start seraphiel-gateway*, /usr/bin/systemctl --no-ask-password restart seraphiel-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Seraphiel will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Seraphiel installations on the same machine (with different `SERAPHIEL_HOME` directories), each gets its own systemd service name. The default `~/.seraphiel` uses `seraphiel-gateway`; other installations use `seraphiel-gateway-<hash>`. The `seraphiel gateway` commands automatically target the correct service for your current `SERAPHIEL_HOME`.
:::

### macOS (launchd)

```bash
seraphiel gateway install               # Install as launchd agent
seraphiel gateway start                 # Start the service
seraphiel gateway stop                  # Stop the service
seraphiel gateway status                # Check status
tail -f ~/.seraphiel/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.seraphiel.gateway.plist`. It includes three environment variables:
...
- **SERAPHIEL_HOME** — scopes the gateway to your Seraphiel installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `seraphiel gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SERAPHIEL_HOME` directory gets its own launchd label. The default `~/.seraphiel` uses `ai.seraphiel.gateway`; other installations use `ai.seraphiel.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `seraphiel-cli` | Full access |
| Telegram | `seraphiel-telegram` | Full tools including terminal |
| Discord | `seraphiel-discord` | Full tools including terminal |
| WhatsApp | `seraphiel-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `seraphiel-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `seraphiel-slack` | Full tools including terminal |
| Google Chat | `seraphiel-google_chat` | Full tools including terminal |
| Signal | `seraphiel-signal` | Full tools including terminal |
| SMS | `seraphiel-sms` | Full tools including terminal |
| Email | `seraphiel-email` | Full tools including terminal |
| Home Assistant | `seraphiel-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `seraphiel-mattermost` | Full tools including terminal |
| Matrix | `seraphiel-matrix` | Full tools including terminal |
| DingTalk | `seraphiel-dingtalk` | Full tools including terminal |
| Feishu/Lark | `seraphiel-feishu` | Full tools including terminal |
| WeCom | `seraphiel-wecom` | Full tools including terminal |
| WeCom Callback | `seraphiel-wecom-callback` | Full tools including terminal |
| Weixin | `seraphiel-weixin` | Full tools including terminal |
| BlueBubbles | `seraphiel-bluebubbles` | Full tools including terminal |
| QQBot | `seraphiel-qqbot` | Full tools including terminal |
| Yuanbao | `seraphiel-yuanbao` | Full tools including terminal |
| Microsoft Teams | `seraphiel-teams` | Full tools including terminal |
| API Server | `seraphiel-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `seraphiel-webhook` | Full tools including terminal |
| Raft | `seraphiel-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.seraphiel/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
