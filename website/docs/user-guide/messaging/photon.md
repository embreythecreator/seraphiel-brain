...
Connect Seraphiel to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Seraphiel — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Seraphiel runs it in a
...
seraphiel photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Seraphiel Brain` project on your account.
...
Runtime credentials are written to `~/.seraphiel/.env`
...
(device token, dashboard project id) lives in `~/.seraphiel/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Seraphiel
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Seraphiel replies with a pairing code. Approve it with:

```bash
seraphiel pairing approve photon <CODE>
```

Use `seraphiel pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.seraphiel/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.seraphiel/.env`):
...
By default Seraphiel responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Seraphiel` and
`@Seraphiel agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Seraphiel will reply.

## Status & troubleshooting

```bash
seraphiel photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Seraphiel uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run seraphiel photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `seraphiel photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `seraphiel photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `seraphiel photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Seraphiel sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Seraphiel wake words  | JSON list / comma / newline regex patterns for group mentions |
