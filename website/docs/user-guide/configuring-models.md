...
Every auxiliary task defaults to `auto` — meaning Seraphiel uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
`provider: auto` with `model: ''` tells Seraphiel to use the main model for that task.
