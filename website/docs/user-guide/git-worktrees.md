...
title: "Git Worktrees"
description: "Run multiple Seraphiel agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Seraphiel Brain is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Seraphiel so each session has a clean, isolated working directory.

## Why Use Worktrees with Seraphiel?

Seraphiel treats the **current working directory** as the project root:

- CLI: the directory where you run `seraphiel` or `seraphiel chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.seraphiel/config.yaml`
...
git worktree add ../repo-feature feature/seraphiel-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/seraphiel-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Seraphiel there:

```bash
cd ../repo-feature

# Start Seraphiel in the worktree
seraphiel
```
...
git worktree add ../repo-experiment-a feature/seraphiel-a
git worktree add ../repo-experiment-b feature/seraphiel-b
```
...
Each Seraphiel process:

- Works on its own branch (`feature/seraphiel-a` vs `feature/seraphiel-b`).
...
- Seraphiel checkpoint data under `~/.seraphiel/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Seraphiel experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/seraphiel-checkpoints-docs`, `feature/seraphiel-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Seraphiel from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `seraphiel -w` (Automatic Worktree Mode)

Seraphiel has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `seraphiel/seraphiel-<hash>`).
...
```bash
seraphiel -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `seraphiel -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Seraphiel session its own clean checkout.
