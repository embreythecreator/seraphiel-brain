...
description: "Run Seraphiel Brain directly on an Android phone with Termux"
---

# Seraphiel on Android with Termux

This is the tested path for running Seraphiel Brain directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Seraphiel from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Seraphiel now ships a Termux-aware installer path:
...
- links `seraphiel` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `seraphiel` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/seraphiel" "$PREFIX/bin/seraphiel"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `seraphiel` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.seraphiel/.env`.
...
### `seraphiel doctor` says ripgrep or Node is missing
