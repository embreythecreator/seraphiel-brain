...
| **Google Gemini (OAuth)** | `seraphiel model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Seraphiel marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `seraphiel auth add codex-oauth` (or `seraphiel model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
# → done — Seraphiel auto-provisions your free tier on first request
```

Seraphiel ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.seraphiel/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `SERAPHIEL_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Seraphiel works unchanged
...
| Workspace / Standard / Enterprise account | Set `SERAPHIEL_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Seraphiel detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Seraphiel shows
...
```bash
SERAPHIEL_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
SERAPHIEL_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
Seraphiel will automatically load a LM Studio model with 64K context length
