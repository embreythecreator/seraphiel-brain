...
- Building a custom/local tool without modifying Seraphiel core? Start with [Build a Seraphiel Plugin](../developer-guide/plugins/index.md)
...
You can also run a fully isolated Seraphiel instance (throwaway SERAPHIEL_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m seraphiel_cli.main
scripts/dev-sandbox.sh --persistent python -m seraphiel_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
