...
description: "How to build an image-generation backend plugin for Seraphiel Brain"
---
...
Seraphiel scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.seraphiel/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `seraphiel_brain.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `seraphiel tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `seraphiel tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.seraphiel/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `seraphiel plugins enable <name>`).
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `seraphiel plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `seraphiel tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `seraphiel tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$SERAPHIEL_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.seraphiel/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `seraphiel plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.
...
Or interactively: `seraphiel tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
