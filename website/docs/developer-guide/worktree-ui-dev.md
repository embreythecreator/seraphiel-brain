...
| Variable | Meaning |
|----------|---------|
| `SERAPHIEL_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SERAPHIEL_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SERAPHIEL_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Seraphiel itself — they're private to these helpers. The variables Seraphiel *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `seraphiel --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_seraphiel_root)" || { echo "htui: not in a Seraphiel checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SERAPHIEL_MAIN_CHECKOUT/.venv/bin/python" -m seraphiel_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SERAPHIEL_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_seraphiel_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SERAPHIEL_TUI_DIR` are mutually exclusive
`SERAPHIEL_TUI_DIR` points Seraphiel at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `seraphiel --tui --dev` exits with an error. Run `unset SERAPHIEL_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_seraphiel_root)" || { echo "hgui: not in a Seraphiel checkout" >&2; return 1; }
  deps="${SERAPHIEL_GUI_DEPS_CHECKOUT:-$SERAPHIEL_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _seraphiel_link_deps "$desktop" "$deps/apps/desktop"
    _seraphiel_link_deps "$root" "$deps"
  else
  ...
  trap '_seraphiel_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SERAPHIEL_DESKTOP_SERAPHIEL_ROOT="$root" \
    SERAPHIEL_DESKTOP_PYTHON="$SERAPHIEL_MAIN_CHECKOUT/.venv/bin/python" \
    SERAPHIEL_DESKTOP_IGNORE_EXISTING=1 \
    SERAPHIEL_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SERAPHIEL_DESKTOP_SERAPHIEL_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `seraphiel`. |
| `SERAPHIEL_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SERAPHIEL_DESKTOP_IGNORE_EXISTING` | Ignores any `seraphiel` on `PATH` so it can't shadow the worktree. |
| `SERAPHIEL_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Seraphiel checkout.
_seraphiel_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/seraphiel_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}

# Symlink node_modules from the deps checkout — never over an existing tree.
_seraphiel_link_deps() {
...
_seraphiel_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'seraphiel_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `seraphiel --tui --dev` and the `SERAPHIEL_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/embreythecreator/seraphiel-brain/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `SERAPHIEL_*` variable Seraphiel reads
