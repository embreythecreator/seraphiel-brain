...
sidebar_position: 1
title: "Architecture"
description: "Seraphiel Brain internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Seraphiel Brain internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ seraphiel_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SeraphielCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── seraphiel_state.py           # SQLite session/state database with FTS5
├── seraphiel_constants.py       # SERAPHIEL_HOME, profile-aware paths
...
│
├── seraphiel_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `seraphiel` subcommands (large file)
...
│   ├── skills_config.py      # seraphiel skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # seraphiel tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # seraphiel gateway start/stop
│
...
```text
User input → SeraphielCLI.process_input()
...
Three discovery sources: `~/.seraphiel/plugins/` (user), `.seraphiel/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `seraphiel plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-seraphiel-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Seraphiel as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`seraphiel -p <name>`) gets its own SERAPHIEL_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
