...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Seraphiel Brain — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Seraphiel tool** to the repository itself.
...
2. **`toolsets.py`** — add tool name to `_SERAPHIEL_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `seraphiel_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `seraphiel_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `seraphiel chat -q "Use the weather tool for London"`
