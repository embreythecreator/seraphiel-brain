...
title: "Desktop Plugin SDK (@seraphiel/plugin-sdk)"
description: "Extend the native Seraphiel Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Seraphiel Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SeraphielPlugin`.
It imports one module — `@seraphiel/plugin-sdk` — and gets everything: the app's
...
`$SERAPHIEL_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Seraphiel. This page is the **native
desktop app** (`seraphiel desktop`) SDK — the `@seraphiel/plugin-sdk` module and
`$SERAPHIEL_HOME/desktop-plugins/`. The **web dashboard** (`seraphiel dashboard`) has
its own, unrelated plugin system on `window.__SERAPHIEL_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Seraphiel Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SERAPHIEL_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `SeraphielPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$SERAPHIEL_HOME/desktop-plugins/hello/plugin.js` (that's `~/.seraphiel/...`
by default, or `~/.seraphiel/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.seraphiel/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@seraphiel/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Seraphiel' }),
      jsx('div', {
...
The only importable specifiers are `@seraphiel/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SeraphielPlugin`:

```ts
interface SeraphielPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `seraphiel.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@seraphiel/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@seraphiel/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@seraphiel/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@seraphiel/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@seraphiel/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@seraphiel/plugin-sdk'
...
```javascript
import { queryClient } from '@seraphiel/plugin-sdk'
...
`dashboard/` subfolder of a regular Seraphiel plugin and declare it in a
`manifest.json`:

```
~/.seraphiel/plugins/<id>/
└── dashboard/
...
seraphiel-brain codebase directly (`seraphiel_state`, `seraphiel_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.seraphiel/`)
...
(`seraphiel.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SeraphielPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@seraphiel/plugin-sdk` alias.
2. It's still lint-fenced to `@seraphiel/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@seraphiel/plugin-sdk`, `react`,
...
| Plugin contract | `SeraphielPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `seraphiel-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`seraphiel-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SERAPHIEL_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `seraphiel logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@seraphiel/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.seraphiel/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.seraphiel/logs/errors.log` for
