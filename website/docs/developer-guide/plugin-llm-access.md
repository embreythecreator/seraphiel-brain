...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Seraphiel already has applies. The plugin never sees a
...
[`seraphiel-example-plugins`](https://github.com/embreythecreator/seraphiel-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
seraphiel-brain itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/embreythecreator/seraphiel-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Seraphiel provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Seraphiel provider
        # (must be one Seraphiel already knows about — same names as
        # `seraphiel model` and config.yaml model.provider).
        ...
        # plugin sends — Seraphiel does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.seraphiel/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Seraphiel' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Seraphiel subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/embreythecreator/seraphiel-brain/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/embreythecreator/seraphiel-brain/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/embreythecreator/seraphiel-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/embreythecreator/seraphiel-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
