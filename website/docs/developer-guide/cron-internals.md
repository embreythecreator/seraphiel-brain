...
description: "How Seraphiel stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `seraphiel_cli/cron.py` | CLI `seraphiel cron` subcommands |
...
Jobs are stored in `~/.seraphiel/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `seraphiel cron` commands are run or during active CLI sessions.
...
```python
# ~/.seraphiel/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.seraphiel/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `seraphiel cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `seraphiel cron` CLI provides direct job management:

```bash
seraphiel cron list                    # Show all jobs
seraphiel cron create                  # Interactive job creation (alias: add)
seraphiel cron edit <job_id>           # Edit job configuration
seraphiel cron pause <job_id>          # Pause a running job
seraphiel cron resume <job_id>         # Resume a paused job
seraphiel cron run <job_id>            # Trigger immediate execution
seraphiel cron remove <job_id>         # Delete a job
```
