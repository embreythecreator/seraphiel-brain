...
The ACP adapter wraps Seraphiel' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
seraphiel acp / seraphiel-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.seraphiel/.env
  -> configure stderr logging
  -> construct SeraphielACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'seraphiel-brain[acp]==<version>' seraphiel-acp`, pointed at the `seraphiel-brain` PyPI release.
...
- `allow_once` -> Seraphiel `once`
- `allow_always` -> Seraphiel `always`
- reject options -> Seraphiel `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Seraphiel tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["seraphiel-acp"])
...
Instead it reuses Seraphiel' runtime resolver:

- `acp_adapter/auth.py`
- `seraphiel_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Seraphiel provider/credentials. It also always advertises a terminal setup auth method (`seraphiel-setup`, args `--setup`) so first-run registry clients can open Seraphiel' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.seraphiel/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `seraphiel-acp` toolset definition
- `seraphiel_cli/main.py` — `seraphiel acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `seraphiel-acp` script
