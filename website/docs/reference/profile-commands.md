...
This page covers all commands related to [Seraphiel profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `seraphiel profile` without a subcommand shows help.
...
## `seraphiel profile use`

```bash
seraphiel profile use <name>
```

Sets `<name>` as the active profile. All subsequent `seraphiel` commands (without `-p`) will use this profile.
...
seraphiel profile use default
```

## `seraphiel profile create`

```bash
seraphiel profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `seraphiel profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `seraphiel update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.seraphiel`), use `seraphiel skills opt-out` / `seraphiel skills opt-in`. |
...
seraphiel profile create mybot
...
seraphiel profile create work2 --clone-from work

# Clone everything from a specific profile
seraphiel profile create work2-backup --clone-from work --clone-all
```

## `seraphiel profile describe`

```bash
seraphiel profile describe [<name>] [options]
```
...
seraphiel profile describe researcher

# Set it explicitly
seraphiel profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
seraphiel profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
seraphiel profile describe --all --auto
```

## `seraphiel profile delete`

```bash
seraphiel profile delete <name> [options]
```
...
```bash
seraphiel profile delete mybot
seraphiel profile delete mybot --yes
```
...
## `seraphiel profile show`

```bash
seraphiel profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Seraphiel home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ seraphiel profile show work
Profile: work
Path:    ~/.seraphiel/profiles/work
...
## `seraphiel profile alias`

```bash
seraphiel profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Seraphiel installation.
...
```bash
seraphiel profile alias work
# Creates/updates ~/.local/bin/work

seraphiel profile alias work --name mywork
# Creates ~/.local/bin/mywork

seraphiel profile alias work --remove
# Removes the wrapper script
```

## `seraphiel profile rename`

```bash
seraphiel profile rename <old-name> <new-name>
```
...
```bash
seraphiel profile rename mybot assistant
# ~/.seraphiel/profiles/mybot → ~/.seraphiel/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `seraphiel profile export`

```bash
seraphiel profile export <name> [options]
```
...
```bash
seraphiel profile export work
# Creates work.tar.gz in the current directory

seraphiel profile export work -o ./work-2026-03-29.tar.gz
```

## `seraphiel profile import`

```bash
seraphiel profile import <archive> [options]
```
...
```bash
seraphiel profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

seraphiel profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`seraphiel profile export` / `import` are still the right commands for
...
### `seraphiel profile install`

```bash
seraphiel profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `seraphiel -p telemetry`). |
...
seraphiel profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
seraphiel profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
seraphiel profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
seraphiel profile install ./telemetry/
```

### `seraphiel profile update`

```bash
seraphiel profile update <name> [--force-config] [--yes]
```
...
### `seraphiel profile info`

```bash
seraphiel profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Seraphiel version, author, env var requirements, the source URL/path, and
...
`seraphiel profile list` also shows the distribution name and version in a
`Distribution` column, and `seraphiel profile show <name>` / `delete <name>`
...
seraphiel profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
seraphiel profile install https://github.com/your-org/internal-assistant.git
```
...
`seraphiel_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Seraphiel version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Seraphiel can clone from.
3. Tell recipients to run `seraphiel profile install <your-repo-url>`.
...
## `seraphiel -p` / `seraphiel --profile`

```bash
seraphiel -p <name> <command> [options]
seraphiel --profile <name> <command> [options]
```

Global flag to run any Seraphiel command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
seraphiel -p work chat -q "Check the server status"
seraphiel --profile dev gateway start
seraphiel -p personal skills list
seraphiel -p work config edit
```

## `seraphiel completion`

```bash
seraphiel completion <shell>
```
...
- `seraphiel profile <TAB>` — subcommands (list, use, create, etc.)
- `seraphiel profile use <TAB>` — profile names
- `seraphiel -p <TAB>` — profile names
