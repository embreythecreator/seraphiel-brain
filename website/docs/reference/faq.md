...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `seraphiel profile create newname --clone-all` to copy everything from the current profile.
