"""CLI entry point for the seraphiel-brain ACP adapter.

Loads environment variables from ``~/.seraphiel/.env``, configures logging
...
try:
    ...
    # shadowing Seraphiel's own modules — ``seraphiel acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    seraphiel_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from SERAPHIEL_HOME (default ``~/.seraphiel``)."""
    from seraphiel_cli.env_loader import load_seraphiel_dotenv

    seraphiel_home = get_seraphiel_home()
    loaded = load_seraphiel_dotenv(seraphiel_home=seraphiel_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", seraphiel_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="seraphiel-acp",
        description="Run Seraphiel Brain as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Seraphiel version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Seraphiel provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.seraphiel/node/ "
...
def _print_version() -> None:
    ...
    print(seraphiel_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import SeraphielACPAgent  # noqa: F401

    print("Seraphiel ACP check OK")


def _run_setup() -> None:
    from seraphiel_cli.main import main as seraphiel_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "seraphiel", "model"]
        seraphiel_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``seraphiel postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting seraphiel-brain ACP adapter")
    ...
    import acp
    from .server import SeraphielACPAgent
