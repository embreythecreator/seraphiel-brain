"""ACP auth helpers — detect and advertise Seraphiel authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "seraphiel-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Seraphiel runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Seraphiel can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Seraphiel.
    ...
    not have Seraphiel provider credentials configured yet, so Seraphiel always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Seraphiel using the currently configured "
