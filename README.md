...
Use any model you want — [Nous Portal](https://portal.embreythecreator.com), OpenRouter, OpenAI, your own endpoint, and [many others](https://seraphiel-brain.embreythecreator.com/docs/integrations/providers). Switch with `seraphiel model` — no code changes, no lock-in.
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Seraphiel `bin` folder (`%LOCALAPPDATA%\seraphiel\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Seraphiel bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\seraphiel\bin\uv.exe"
...
**To whitelist Seraphiel:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\seraphiel\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Seraphiel updates `uv` and the hash changes every version
...
source ~/.seraphiel/venvs/seraphiel-dev/bin/activate
uv pip install -e ".[all,dev]"
scripts/run_tests.sh
```
