"""Windows UTF-8 bootstrap for Seraphiel entry points.
...
should be imported at the very top of every Seraphiel entry point
(``seraphiel``, ``seraphiel-brain``, ``seraphiel-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Seraphiel modules.

    Seraphiel ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Seraphiel source root
    ...
    """
    root = src_root or os.environ.get("SERAPHIEL_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SERAPHIEL_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SERAPHIEL_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import seraphiel_bootstrap``
# (or ``from seraphiel_bootstrap import apply_windows_utf8_bootstrap``) at
