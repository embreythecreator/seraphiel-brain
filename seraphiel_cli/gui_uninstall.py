"""
Seraphiel Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``seraphiel desktop`` / ``seraphiel gui``)
     Built inside the agent checkout under ``$SERAPHIEL_HOME/seraphiel-brain/``:
       ...
       - ``$SERAPHIEL_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Seraphiel.app`` or ``~/Applications/Seraphiel.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Seraphiel`` (NSIS per-user)
...
the app name ("Seraphiel"), separate from ``$SERAPHIEL_HOME``:
  - macOS:   ``~/Library/Application Support/Seraphiel``
  - Windows: ``%APPDATA%\\Seraphiel``
  - Linux:   ``$XDG_CONFIG_HOME/Seraphiel`` (default ``~/.config/Seraphiel``)
...
``seraphiel uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(seraphiel_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return seraphiel_home / "seraphiel-brain"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Seraphiel"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Seraphiel"
...
def source_built_gui_artifacts(seraphiel_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``seraphiel desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``seraphiel-brain/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(seraphiel_home)
    ...
    return [
        ...
        seraphiel_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Seraphiel" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Seraphiel.app"),
            home / "Applications" / "Seraphiel.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Seraphiel).
            local_base / "Programs" / "Seraphiel",
            # Older / alternate layout some builds used.
            local_base / "seraphiel-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Seraphiel")
    else:
        ...
        paths += [
            data_base / "applications" / "seraphiel.desktop",
            data_base / "applications" / "Seraphiel.desktop",
        ]
    return paths


def agent_is_installed(seraphiel_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SERAPHIEL_HOME.
    ...
    if (agent_root / "seraphiel_cli").is_dir():
        return True
...
def gui_is_installed(seraphiel_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(seraphiel_home):
        if p.exists():
            return True
...
def gui_install_summary(seraphiel_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = seraphiel_home if seraphiel_home is not None else get_seraphiel_home()
...
def uninstall_gui(seraphiel_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``seraphiel-brain/seraphiel_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SERAPHIEL_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove seraphiel' or "
            "'sudo dnf remove seraphiel'). AppImage builds are a single file you "
