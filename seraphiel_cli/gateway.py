...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Seraphiel can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo seraphiel gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo seraphiel gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo seraphiel gateway start --system")
            return scope, False
...
# (the documented `nohup seraphiel gateway run` workaround). See #23387.
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Seraphiel will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @seraphiel:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Seraphiel agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Seraphiel will use (e.g., seraphiel@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
...
def _setup_whatsapp():
    ...
    from seraphiel_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from seraphiel_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: seraphiel pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'seraphiel gateway setup'")
