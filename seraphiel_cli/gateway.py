"""
Gateway subcommand for seraphiel CLI.

Handles: seraphiel gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_seraphiel_home is imported lazily at call sites to avoid ImportError
# when seraphiel_constants is cached from a pre-update version during `seraphiel update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "seraphiel CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``seraphiel gateway status`` from falsely counting the ``seraphiel`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``seraphiel gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_seraphiel_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"seraphiel_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. SERAPHIEL_HOME
        ...
        # SERAPHIEL_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "seraphiel_home=" in command_lc
            and f"seraphiel_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``seraphiel update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Seraphiel profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Seraphiel profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from seraphiel_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "seraphiel_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m seraphiel_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``seraphiel update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Seraphiel's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose SERAPHIEL_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``seraphiel update`` users
    # with no running gateway until they re-invoke ``seraphiel gateway``
    ...
    from seraphiel_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from seraphiel_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / SERAPHIEL_HOME) the windowless
        # base interpreter needs to import seraphiel_cli.  Empty on POSIX, where
...
def _seraphiel_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``SERAPHIEL_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("SERAPHIEL_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_seraphiel_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SERAPHIEL_HOME``.

    Under ``sudo``, ``SERAPHIEL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_seraphiel_home` falls back to ``/root/.seraphiel`` — the wrong
    profile. The unit file pins ``SERAPHIEL_HOME`` for the actual gateway
    ...
    unit_home = (_seraphiel_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("SERAPHIEL_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SERAPHIEL_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SERAPHIEL_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}seraphiel gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}seraphiel gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from seraphiel_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: seraphiel gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `seraphiel gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``seraphiel gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from seraphiel_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (seraphiel_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SERAPHIEL_HOME-scoped).
...
from seraphiel_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Seraphiel Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``seraphiel gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``SERAPHIEL_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("SERAPHIEL_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "seraphiel-gateway"
SERVICE_DESCRIPTION = "Seraphiel Brain Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SERAPHIEL_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SERAPHIEL_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from seraphiel_constants import get_default_seraphiel_root

    home = get_seraphiel_home().resolve()
    default = get_default_seraphiel_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SERAPHIEL_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(seraphiel_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when SERAPHIEL_HOME is a named profile.

    For ``~/.seraphiel/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        seraphiel_home: Optional explicit SERAPHIEL_HOME path. Defaults to the current
            ``get_seraphiel_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Seraphiel root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_seraphiel_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from seraphiel_constants import get_default_seraphiel_root

    home = Path(seraphiel_home or str(get_seraphiel_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_seraphiel_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(seraphiel_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".seraphiel"
    try:
        Path(seraphiel_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(seraphiel_home, default_root=target_root)
    except ValueError:
        return _profile_arg(seraphiel_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this SERAPHIEL_HOME.

    Default ``~/.seraphiel`` returns ``seraphiel-gateway`` (backward compatible).
    Profile ``~/.seraphiel/profiles/coder`` returns ``seraphiel-gateway-coder``.
    Any other SERAPHIEL_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Seraphiel installs that predate the
# seraphiel-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (seraphiel-gateway-*.service) and unrelated third-party
# "seraphiel" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("seraphiel.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "seraphiel_cli.main gateway",
    "seraphiel_cli/main.py gateway",
    "gateway/run.py",
    " seraphiel gateway ",
    "/seraphiel gateway ",
)
...
def _find_legacy_seraphiel_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Seraphiel gateway units.

    Detects unit files installed by older Seraphiel versions that used a
    different service name (e.g. ``seraphiel.service`` before the rename to
    ``seraphiel-gateway.service``). When both a legacy unit and the current
    ``seraphiel-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``seraphiel-gateway-coder.service`` and unrelated third-party
      ``seraphiel-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``seraphiel.service`` running an unrelated
...
def has_legacy_seraphiel_units() -> bool:
    """Return True when any legacy Seraphiel gateway unit files exist."""
    return bool(_find_legacy_seraphiel_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Seraphiel gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_seraphiel_units()
    if not legacy:
        return
    print_warning("Legacy Seraphiel gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current seraphiel-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    seraphiel gateway migrate-legacy")


def remove_legacy_seraphiel_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Seraphiel gateway unit files.

    Iterates over whatever ``_find_legacy_seraphiel_units()`` returns — which is
    ...
    legacy = _find_legacy_seraphiel_units()
    if not legacy:
        print("No legacy Seraphiel gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Seraphiel gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: seraphiel gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo seraphiel gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    seraphiel gateway uninstall")
    print_info("    sudo seraphiel gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: seraphiel gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Seraphiel often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.seraphiel`` → ``ai.seraphiel.gateway.plist`` (backward compatible).
    Profile ``~/.seraphiel/profiles/coder`` → ``ai.seraphiel.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.seraphiel.gateway-{suffix}" if suffix else "ai.seraphiel.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.seraphiel/seraphiel-brain  -> /home/alice/.seraphiel/seraphiel-brain
      /opt/seraphiel                 -> /opt/seraphiel  (kept as-is)
...
def _seraphiel_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SERAPHIEL_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_seraphiel_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.seraphiel                    → /home/alice/.seraphiel
      /root/.seraphiel/profiles/coder     → /home/alice/.seraphiel/profiles/coder
      /opt/custom-seraphiel               → /opt/custom-seraphiel  (kept as-is)
    """
    current_seraphiel_raw = os.environ.get("SERAPHIEL_HOME", "").strip()
    current_seraphiel = (
        Path(current_seraphiel_raw).expanduser()
        if current_seraphiel_raw
        else get_seraphiel_home()
    )
    ...
    # different SERAPHIEL_HOME into the generated service unit.
    current_default = Path.home() / ".seraphiel"
    target_default = Path(target_home_dir) / ".seraphiel"

    # Default ~/.seraphiel → remap to target user's default
    if current_seraphiel == current_default:
        return str(target_default)

    # Profile or subdir of ~/.seraphiel → preserve the relative structure
    try:
        relative = current_seraphiel.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.seraphiel) — keep as-is
        return str(current_seraphiel)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    seraphiel_node = seraphiel_home / "node" / "bin"
    if _is_dir(seraphiel_node):
        candidates.append(str(seraphiel_node))
    seraphiel_nm = seraphiel_home / "node_modules" / ".bin"
    if _is_dir(seraphiel_nm):
        candidates.append(str(seraphiel_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m seraphiel_cli.main``, so module
    ...
    dir, or a clone that ``seraphiel update`` later relocates/removes — the path
    ...
    ``SERAPHIEL_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if SERAPHIEL_HOME
...
def _systemd_watchdog_seconds(seraphiel_home: str | Path | None = None) -> int:
    ...
    if seraphiel_home is not None:
        ...
        override_token = set_seraphiel_home_override(seraphiel_home)
        reset_home_override = reset_seraphiel_home_override
    try:
...
def _systemd_watchdog_service_fields(
    seraphiel_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(seraphiel_home)
    if seconds <= 0:
        return "simple", ""
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        seraphiel_home = _seraphiel_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            seraphiel_home
        )
        profile_arg = _profile_arg_for_target_user(seraphiel_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's SERAPHIEL_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(seraphiel_home) if seraphiel_home else _remap_path_for_user(working_dir, home_dir)
...
Group={group_name}
ExecStart={python_path} -m seraphiel_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="SERAPHIEL_HOME={seraphiel_home}"
Restart=always
RestartSec=5
...
"""
    ...
    seraphiel_home = str(get_seraphiel_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        seraphiel_home
    )
    profile_arg = _profile_arg(seraphiel_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m seraphiel_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="SERAPHIEL_HOME={seraphiel_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    # ── SERAPHIEL_HOME sync chokepoint ──────────────────────────────────────
    ...
    # pinned SERAPHIEL_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo seraphiel gateway … --system``, SERAPHIEL_HOME is often stripped
    # and falls back to ``/root/.seraphiel``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / SERAPHIEL_HOME comparisons
    ...
    _sync_seraphiel_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir SERAPHIEL_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved SERAPHIEL_HOME
    ...
    exported a throwaway ``SERAPHIEL_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``seraphiel gateway restart``
    with ``SERAPHIEL_HOME=/tmp/seraphiel-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="SERAPHIEL_HOME=..."`` lines and launchd
    ``<key>SERAPHIEL_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'SERAPHIEL_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>SERAPHIEL_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp SERAPHIEL_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: SERAPHIEL_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported SERAPHIEL_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single SERAPHIEL_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only SERAPHIEL_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp SERAPHIEL_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../seraphiel_test`` path into
    # ``Environment="SERAPHIEL_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/seraphiel_test"' in new_unit
        or "/seraphiel_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # SERAPHIEL_HOME into the unit (manual E2E homes like /tmp/seraphiel-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Seraphiel install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo seraphiel gateway uninstall --system")
    print_info("         seraphiel gateway install")
    print_info("         seraphiel gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("SERAPHIEL_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (seraphiel.service from pre-rename installs)
    # before installing the new seraphiel-gateway.service. If both remain, they
    ...
    if has_legacy_seraphiel_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_seraphiel_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin SERAPHIEL_HOME; adopt it before any
    ...
    # ``sudo seraphiel gateway install --system --force`` would bake /root/.seraphiel
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}seraphiel gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}seraphiel gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}seraphiel gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # SERAPHIEL_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `seraphiel gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `seraphiel gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}seraphiel gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}seraphiel gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}seraphiel gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}seraphiel gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.seraphiel.gateway-{suffix}" if suffix else "ai.seraphiel.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``seraphiel gateway start/stop/status`` call).
...
#      background process (the `nohup seraphiel gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_seraphiel_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_seraphiel_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m seraphiel_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active SERAPHIEL_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "seraphiel_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup seraphiel gateway run --replace`
    ...
    """
    from seraphiel_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_seraphiel_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup seraphiel gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>seraphiel_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>SERAPHIEL_HOME</key>
        <string>{seraphiel_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.seraphiel/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See seraphiel-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_seraphiel_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Seraphiel install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  seraphiel gateway status             # Check status")
    from seraphiel_constants import display_seraphiel_home as _dhh
...
def launchd_stop():
    ...
    # `seraphiel gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Seraphiel PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Seraphiel PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Seraphiel install")
    else:
        print("⚠ Service definition is stale relative to the current Seraphiel install")
        print("  Run: seraphiel gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: seraphiel gateway stop")
            else:
    ...
    if deep:
        log_file = get_seraphiel_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/seraphiel"
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``SERAPHIEL_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``seraphiel gateway run`` is always a misconfiguration, so we
    ...
    try:
        from seraphiel_constants import get_default_seraphiel_root
        default_root = get_default_seraphiel_root()
    ...
    print()
    print("    seraphiel gateway restart")
    print()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``seraphiel gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``seraphiel gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other SERAPHIEL_HOME roots.
    """
    ...
    print("  Use 'seraphiel gateway restart' to replace it,")
    print("  or 'seraphiel gateway stop' first.")
    print("  Or use 'seraphiel gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SERAPHIEL_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Seraphiel gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'seraphiel' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/seraphiel/docker/entrypoint.sh before the Seraphiel command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SERAPHIEL_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set SERAPHIEL_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `seraphiel gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set SERAPHIEL_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `seraphiel gateway restart` which already
    ...
    # `seraphiel gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ✶ Seraphiel Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # SERAPHIEL_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SERAPHIEL_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from seraphiel_constants import get_seraphiel_home as _ghh
    ...
    # the env vars ``SERAPHIEL_GATEWAY_MAX_STARTS`` /
    # ``SERAPHIEL_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("SERAPHIEL_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("SERAPHIEL_GATEWAY_START_WINDOW_S")
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. seraphiel) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Seraphiel delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Seraphiel connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: seraphiel pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Seraphiel will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``seraphiel setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run seraphiel under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.seraphiel/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_seraphiel_home() / "whatsapp" / "session" / "creds.json"
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'seraphiel pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: seraphiel pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'seraphiel gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Seraphiel will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Seraphiel will store the returned account_id/token in ~/.seraphiel/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `seraphiel gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_seraphiel_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `seraphiel pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `seraphiel pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "SeraphielAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``seraphiel_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from seraphiel_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.seraphiel/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.seraphiel/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: seraphiel gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: seraphiel gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: seraphiel gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo seraphiel gateway install --system"
                        )
                    print_info("  Or run in foreground:  seraphiel gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: seraphiel gateway run")
                print_info(
                    "  For persistence:   tmux new -s seraphiel 'seraphiel gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: seraphiel gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup seraphiel gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'seraphiel gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`seraphiel_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from seraphiel_cli.service_manager import (
    ...
    if profile is None:
        ...
        # SERAPHIEL_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``seraphiel gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``seraphiel gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``seraphiel gateway run`` are unaffected.
      2. ``SERAPHIEL_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``seraphiel gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``SERAPHIEL_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("SERAPHIEL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("SERAPHIEL_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${SERAPHIEL_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if SERAPHIEL_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or SERAPHIEL_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-seraphiel service's
    # pattern in docker/s6-rc.d/main-seraphiel/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: seraphiel gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: seraphiel gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s seraphiel 'seraphiel gateway run'"
                )
                print()
            ...
            print()
            print(
                "  seraphiel gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s seraphiel 'seraphiel gateway run'         # persistent via tmux"
            )
            print(
                "  nohup seraphiel gateway run > ~/.seraphiel/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from seraphiel_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  seraphiel profile create <name>     # creates the s6 service slot")
                print("  seraphiel -p <name> gateway start   # bring it up via s6")
                print("  seraphiel status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: seraphiel gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: seraphiel gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: seraphiel gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  seraphiel profile delete <name>     # tears down the s6 service slot")
                print("  seraphiel -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `seraphiel -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: seraphiel gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_SERAPHIEL_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `seraphiel gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_SERAPHIEL_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `seraphiel gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: seraphiel gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  seraphiel gateway install")
                else:
                    print("To install as a service:")
                    print("  seraphiel gateway install")
                    print("  sudo seraphiel gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  seraphiel gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup seraphiel gateway run > ~/.seraphiel/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s seraphiel 'seraphiel gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  seraphiel gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  seraphiel gateway install  # Install as user service")
                    print(
                        "  sudo seraphiel gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Seraphiel gateway unit files from
        # pre-rename installs (e.g. seraphiel.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_seraphiel_units(interactive=not yes, dry_run=dry_run)
