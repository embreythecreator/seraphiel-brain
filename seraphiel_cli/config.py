...
from seraphiel_constants import get_seraphiel_home  # noqa: F811,E402
...
DEFAULT_CONFIG = {
    ...
    "updates": {
        # Run a full ``seraphiel backup``-style zip of SERAPHIEL_HOME before every
        # ``seraphiel update``.  Backups land in ``<SERAPHIEL_HOME>/backups/`` and
        # can be restored with ``seraphiel import <path>``.  Off by default:
        # zipping a large SERAPHIEL_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``seraphiel update
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Seraphiel versions) write ``models`` as
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: seraphiel config set <key> <value>")
            print()
            print("Examples:")
