"""
Configuration management for Seraphiel Brain.

Config files are stored in ~/.seraphiel/ for easy access:
- ~/.seraphiel/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.seraphiel/.env         - API keys and secrets

This module provides:
- seraphiel config          - Show current configuration
- seraphiel config edit     - Open config in editor
- seraphiel config get      - Print a resolved configuration value
- seraphiel config set      - Set a specific value
- seraphiel config unset    - Remove a user configuration value
- seraphiel config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``seraphiel config set`` (which rewrites
    ...
    ``config.yaml`` in place: seraphiel never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.seraphiel/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``seraphiel logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  seraphiel config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Seraphiel makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Seraphiel itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``seraphiel update``, the TUI build.
...
#   on every plugin install / ``seraphiel update``.
...
# * ``SERAPHIEL_HOME`` / ``SERAPHIEL_PROFILE`` / ``SERAPHIEL_CONFIG`` /
#   ``SERAPHIEL_ENV`` — Seraphiel runtime location flags. Writing these into
...
#
# IMPORTANT: ``SERAPHIEL_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SERAPHIEL_LANGFUSE_PUBLIC_KEY,
# SERAPHIEL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Seraphiel runtime location — never via dashboard env writer.
    # NOT a SERAPHIEL_* blanket: integration credentials (SERAPHIEL_GEMINI_*,
    # SERAPHIEL_LANGFUSE_*, SERAPHIEL_SPOTIFY_*, ...) ARE allowed.
    "SERAPHIEL_HOME", "SERAPHIEL_PROFILE", "SERAPHIEL_CONFIG", "SERAPHIEL_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Seraphiel runtime location "
            "(SERAPHIEL_HOME, SERAPHIEL_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.seraphiel/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SERAPHIEL_TOOL_PROGRESS", "SERAPHIEL_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `seraphiel plugins enable
    # observability/langfuse` or `seraphiel tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SERAPHIEL_LANGFUSE_ENV",
    "SERAPHIEL_LANGFUSE_RELEASE",
    "SERAPHIEL_LANGFUSE_SAMPLE_RATE",
    "SERAPHIEL_LANGFUSE_MAX_CHARS",
    "SERAPHIEL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from seraphiel_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SERAPHIEL_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_seraphiel_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Seraphiel is running in package-manager-managed mode.

    Two signals: the SERAPHIEL_MANAGED env var (set by the systemd service),
    or a .managed marker file in SERAPHIEL_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade seraphiel-brain"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``seraphiel_cli/`` — i.e. the git checkout for source
    installs, ``/opt/seraphiel`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$SERAPHIEL_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Seraphiel was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$SERAPHIEL_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SERAPHIEL_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.seraphiel``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SERAPHIEL_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.seraphiel:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SERAPHIEL_HOME``, a
    ...
    ``docker`` and ``seraphiel update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``embreythecreator/seraphiel-brain`` image bakes a ``docker``
        stamp into ``/opt/seraphiel`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SERAPHIEL_HOME.
    try:
    ...
    #    host install whose shared $SERAPHIEL_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``seraphiel update``.
    try:
        method = (
            (get_seraphiel_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``seraphiel_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SERAPHIEL_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/seraphiel`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Seraphiel lives in a ``uv tool`` layout.

    ``uv tool install seraphiel-brain`` places the install at
    ``.../uv/tools/seraphiel-brain/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``seraphiel-brain`` happens to be uv-tool-installed on the machine while
    the *active* Seraphiel is a regular pip/venv install, causing
    ``seraphiel update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/seraphiel-brain/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade seraphiel-brain"
    if method == "docker":
        return "docker pull embreythecreator/seraphiel-brain:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade seraphiel-brain"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade seraphiel-brain"
        return "pip install --upgrade seraphiel-brain"
    return "seraphiel update"
...
# banner, the TUI/desktop session info panel, and ``seraphiel update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://seraphiel-brain.embreythecreator.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``seraphiel update`` / ``seraphiel update --check`` so the wording — and the
...
# Long-form text for ``seraphiel update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# seraphiel_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Seraphiel, it doesn't update
...
✗ ``seraphiel update`` doesn't apply inside the Docker container.

Seraphiel Brain runs as a published image (embreythecreator/seraphiel-brain), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull embreythecreator/seraphiel-brain:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate seraphiel-brain
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm embreythecreator/seraphiel-brain:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/embreythecreator/seraphiel-brain/tags
  • Your config and session history live under ``$SERAPHIEL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``seraphiel update`` inside Docker.
...
def format_managed_message(action: str = "modify this Seraphiel installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SERAPHIEL_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Seraphiel installation is managed by NixOS "
            f"(SERAPHIEL_MANAGED={env_hint}).\n"
            "Edit services.seraphiel-brain.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Seraphiel installation is managed by Homebrew "
            f"(SERAPHIEL_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade seraphiel-brain"
        )

    return (
        f"Cannot {action}: this Seraphiel installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Seraphiel."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SERAPHIEL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, seraphiel_bin
    or None if container mode is not active, we're already inside the
    container, or SERAPHIEL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SERAPHIEL_DEV") == "1":
        return None
    ...
    container_mode_file = get_seraphiel_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "seraphiel-brain")
    exec_user = info.get("exec_user", "seraphiel")
    seraphiel_bin = info.get("seraphiel_bin", "/data/current-package/bin/seraphiel")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "seraphiel_bin": seraphiel_bin,
    }
...
# Re-export from seraphiel_constants — canonical definition lives there.
from seraphiel_constants import get_seraphiel_home, get_process_seraphiel_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_seraphiel_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_seraphiel_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_seraphiel_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SERAPHIEL_UID / SERAPHIEL_GID env vars set by Docker deployments.

    Docker containers running Seraphiel commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SERAPHIEL_HOME once, but
    subdirectories created at runtime by ``ensure_seraphiel_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SERAPHIEL_UID", "").strip()
    gid_str = os.environ.get("SERAPHIEL_GID", "").strip()
    try:
...
def _chown_to_seraphiel_uid(path) -> None:
    """Chown ``path`` to ``SERAPHIEL_UID:SERAPHIEL_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_seraphiel_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_seraphiel_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the seraphiel group can
    share state with the gateway service.

    The mode can be overridden via the SERAPHIEL_HOME_MODE environment variable
    (e.g. SERAPHIEL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SERAPHIEL_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SERAPHIEL_UID``/``SERAPHIEL_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SERAPHIEL_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_seraphiel_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Seraphiel runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SERAPHIEL_CONTAINER") or os.environ.get("SERAPHIEL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SERAPHIEL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SERAPHIEL_HOME, upgrading legacy empty templates.
...
# ensure_seraphiel_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_SERAPHIEL_HOME_ENSURED: set = set()


def ensure_seraphiel_home():
    """Ensure ~/.seraphiel directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``SERAPHIEL_HOME`` we only re-run
    ...
    ``get_seraphiel_home()`` and therefore re-run for the new path.
    """
    home = get_seraphiel_home()
    key = str(home)

    if key in _SERAPHIEL_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``seraphiel profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old SERAPHIEL_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_seraphiel_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _SERAPHIEL_HOME_ENSURED.add(key)


def _ensure_seraphiel_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SERAPHIEL_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Seraphiel-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Seraphiel
        ...
        # identity slot (SOUL.md). Empty by default. The SERAPHIEL_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Seraphiel adds a coding operating brief
        ...
        # ``SERAPHIEL_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             SERAPHIEL_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SERAPHIEL_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Seraphiel auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Seraphiel sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Seraphiel
        ...
        # bundled Seraphiel image, which drops to the `seraphiel` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Seraphiel sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Seraphiel should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``seraphiel chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.seraphiel/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: seraphiel sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .seraphiel.md, or .cursorrules before Seraphiel applies
    ...
    # single read_file page exceeds these limits, Seraphiel truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Seraphiel' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); seraphiel =
                                      # Seraphiel' compression threshold triggers
    ...
    # See tools/kanban_tools.py and seraphiel_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``seraphiel dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``seraphiel kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``seraphiel kanban decompose`` and the kanban
        ...
        # ``seraphiel profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `seraphiel model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `seraphiel` (and `seraphiel chat`) launches by default:
        ...
        # REPL and `--tui` (or SERAPHIEL_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `seraphiel --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `seraphiel -c` muscle memory.  Default off so existing
        # users aren't surprised.  SERAPHIEL_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `seraphiel --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to SERAPHIEL_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/seraphiel`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `seraphiel pets`. Disabled until
        ...
        "pet": {
            ...
            # get_seraphiel_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SERAPHIEL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.embreythecreator.com).
        "oauth": {
        ...
        # (``SERAPHIEL_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SERAPHIEL_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SERAPHIEL_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/seraphiel``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Seraphiel appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.seraphiel/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.seraphiel/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.seraphiel/cache/delegation/
    ...
    # response. If not, Seraphiel feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Seraphiel auto-pauses the goal and
    ...
    "moa": {
        ...
        # <seraphiel_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.seraphiel/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SERAPHIEL_SKILL_DIR} and ${SERAPHIEL_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `seraphiel curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `seraphiel curator run --consolidate` overrides
        ...
        # normally restored on every `seraphiel update`, so pruning them only
        ...
        # skipped), snapshot ~/.seraphiel/skills/ into
        # ~/.seraphiel/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `seraphiel curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for seraphiel-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "✶ *Seraphiel Brain*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SERAPHIEL_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Seraphiel' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.seraphiel/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SERAPHIEL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `seraphiel doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``seraphiel_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Seraphiel to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SERAPHIEL_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        "chronos": {
            ...
            "portal_url": "https://portal.embreythecreator.com",
        ...
        # Also overridable via SERAPHIEL_CRON_MAX_PARALLEL env var.
        ...
        # SERAPHIEL_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `seraphiel -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `seraphiel` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `seraphiel kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     seraphiel-brain's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Seraphiel tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.seraphiel/logs/.
    ...
    # update model picker lists without shipping a seraphiel-brain release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://seraphiel-brain.embreythecreator.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `seraphiel model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal SERAPHIEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.seraphiel/sessions/sessions.json entirely.
        ...
        # the SERAPHIEL_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.seraphiel.gateway` that defenses 1-2 don't
        ...
        # ``SERAPHIEL_GATEWAY_MAX_STARTS`` / ``SERAPHIEL_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.seraphiel/.env,
        ...
        # behavior: files must live under the Seraphiel cache, under
        ...
        # user. Bridged to SERAPHIEL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Seraphiel
        # cache (~/.seraphiel/cache/{documents,images,audio,video,screenshots})
        ...
        # to SERAPHIEL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SERAPHIEL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SERAPHIEL_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.seraphiel/state.db.
    ...
    "sessions": {
        ...
        # default of ``seraphiel sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.seraphiel/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        ...
        #     DBs) into <SERAPHIEL_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``seraphiel backup``-style zip
        #     of SERAPHIEL_HOME into <SERAPHIEL_HOME>/backups/, restorable with
        #     ``seraphiel import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `seraphiel update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<SERAPHIEL_HOME>/lsp/bin/`` on first use.
    ...
    # `seraphiel tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.seraphiel/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.seraphiel/.env (or your shell) and never in config.yaml.
            ...
            # ~/.seraphiel/bin/ on first use.  When False you must install
            ...
            # `seraphiel secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.seraphiel/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Seraphiel disables it for our users unless they
        # explicitly opt in here. When false (default), Seraphiel sets
    ...
    # Seraphiel Desktop (Electron app) launch options. These only affect
    # `seraphiel desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SERAPHIEL_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Seraphiel mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'seraphiel model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. embreythecreator.com -> firecrawl-gateway.embreythecreator.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Seraphiel auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Seraphiel, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @seraphiel:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SERAPHIEL_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #seraphiel)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: seraphiel-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'seraphiel-brain' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Seraphiel API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Seraphiel API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Seraphiel API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SERAPHIEL_TOOL_PROGRESS and SERAPHIEL_TOOL_PROGRESS_MODE are deprecated —
    ...
    "SERAPHIEL_PREFILL_MESSAGES_FILE": {
    ...
    "SERAPHIEL_EPHEMERAL_SYSTEM_PROMPT": {
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `seraphiel config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.seraphiel.config`` entries, then checks
    ...
    try:
        ...
        # should never break `seraphiel update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Seraphiel' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Seraphiel versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``seraphiel_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (seraphiel_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (seraphiel_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (seraphiel_cli/setup.py)
    "known_plugin_toolsets", # written/read by seraphiel_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'seraphiel setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Seraphiel won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, seraphiel send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'seraphiel doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SERAPHIEL_HOME", "~/.seraphiel")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "seraphiel update / seraphiel -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SERAPHIEL_TOOL_PROGRESS")
            old_mode = get_env_value("SERAPHIEL_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SERAPHIEL_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SERAPHIEL_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SERAPHIEL_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SERAPHIEL_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SERAPHIEL_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_seraphiel_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `seraphiel plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `seraphiel curator status` has no
    ...
    #   3. Creates `~/.seraphiel/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_seraphiel_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_seraphiel_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `seraphiel config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `seraphiel config set`"
                    )
    ...
    # behavior set it to true explicitly via `seraphiel config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from seraphiel_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: seraphiel config set <key> <value>")
    ...
    # `seraphiel update`, but only the version bump is persisted.
    ...
    # metadata.seraphiel.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_seraphiel_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``seraphiel config set``
    ...
    backends) — while display paths (``seraphiel status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _seraphiel_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.seraphiel/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.seraphiel/config.yaml.
    ...
    (which change ``SERAPHIEL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/seraphiel/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_seraphiel_dotenv()
...
#   openai-codex (OAuth — seraphiel auth) — OpenAI Codex
#   nous         (OAuth — seraphiel auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.seraphiel/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Seraphiel writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.seraphiel/.env.
    ...
    interactive menu render (`seraphiel tools`, `seraphiel setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `seraphiel tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Seraphiel env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.seraphiel/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.seraphiel/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.seraphiel/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.seraphiel/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Seraphiel — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Seraphiel vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.seraphiel/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.seraphiel/.env`` over ``os.environ``.

    Used for Seraphiel-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ✶ Seraphiel Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from seraphiel_cli.auth import get_anthropic_key
    ...
    # Warn on stale SERAPHIEL_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SERAPHIEL_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SERAPHIEL_MAX_ITERATIONS={_env_ghost} "
                f"(run 'seraphiel doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  seraphiel config edit     # Edit config file", Colors.DIM))
    print(color("  seraphiel config set <key> <value>", Colors.DIM))
    print(color("  seraphiel setup           # Run setup wizard", Colors.DIM))
    print()
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``seraphiel config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``seraphiel config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``seraphiel config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `seraphiel config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `seraphiel config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `seraphiel config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: seraphiel config get <key> [--json]")
            print()
            print("Examples:")
            print("  seraphiel config get model")
            print("  seraphiel config get terminal.backend")
            print("  seraphiel config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: seraphiel config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  seraphiel config set model anthropic/claude-sonnet-4")
            print("  seraphiel config set terminal.backend docker")
            print("  seraphiel config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: seraphiel config unset <key>")
            print()
            print("Examples:")
            print("  seraphiel config unset model")
            print("  seraphiel config unset terminal.backend")
            print("  seraphiel config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'seraphiel config migrate' to add them")
        ...
        print("  seraphiel config           Show current configuration")
        print("  seraphiel config edit      Open config in editor")
        print("  seraphiel config get <key>          Print a resolved config value")
        print("  seraphiel config set <key> <value>   Set a config value")
        print("  seraphiel config unset <key>        Remove a config value")
        print("  seraphiel config check     Check for missing/outdated config")
        print("  seraphiel config migrate   Update config with new options")
        print("  seraphiel config path      Show config file path")
        print("  seraphiel config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``seraphiel config`` UI so users
