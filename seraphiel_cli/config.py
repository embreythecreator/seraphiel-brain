...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.seraphiel/.install_method`` file (written by installers)
    2. SERAPHIEL_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``embreythecreator/seraphiel-brain`` image stamps ``docker``
    ...
    so ``seraphiel update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_seraphiel_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.seraphiel/.install_method."""
    stamp = get_seraphiel_home() / ".install_method"
    try:
...
DEFAULT_CONFIG = {
    ...
    "updates": {
        ...
        # on large SERAPHIEL_HOME directories the zip can add minutes to every
