...
Pure display functions with no SeraphielCLI state dependency.
"""
import json
import logging
import os
...
# (e.g. nix-built seraphiel — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/embreythecreator/seraphiel-brain.git"
_OFFICIAL_REPO_CANONICAL = "github.com/embreythecreator/seraphiel-brain"
...
def _fetch_pypi_latest(package: str = "seraphiel-brain") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Seraphiel update is available.

    Two paths: if ``SERAPHIEL_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = seraphiel_home / ".update_check"
    embedded_rev = os.environ.get("SERAPHIEL_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SERAPHIEL_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `seraphiel update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/seraphiel/update/check`
    ...
    try:
        from seraphiel_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $SERAPHIEL_HOME/seraphiel-brain/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = seraphiel_home / "seraphiel-brain"
        if not (repo_dir / ".git").exists():
...
def absorb_offer_line(repo: str) -> Optional[str]:
    ...
    try:
        ...
        if not tag:
            return None
        return f"✶ upstream {tag} available to absorb · run `seraphiel absorb {tag}`"
    except Exception:
        return None
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$SERAPHIEL_HOME/seraphiel-brain/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``seraphiel_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from seraphiel_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/embreythecreator/seraphiel-brain/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Seraphiel checkout. Cached per-process. Release URL always points at the
    canonical embreythecreator/seraphiel-brain repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Seraphiel Brain v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SERAPHIEL_SIGIL
    except Exception:
        _bskin = None
        _hero = SERAPHIEL_SIGIL
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Seraphiel[/]")
    else:
    ...
    try:
        from seraphiel_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from seraphiel_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # release is available to fold into the fork. Silent (None) on
