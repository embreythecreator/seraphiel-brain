"""Profile distributions — shareable, packaged Seraphiel profiles via git.

A distribution is a Seraphiel profile published as a git repository (or
...
* ``seraphiel profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``seraphiel skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``seraphiel profile``, not a parallel tree):

    seraphiel profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "seraphiel_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".seraphiel_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "seraphiel-brain", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            seraphiel_requires=str(data.get("seraphiel_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.seraphiel_requires:
            out["seraphiel_requires"] = self.seraphiel_requires
        if self.author:
...
def check_seraphiel_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Seraphiel {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``seraphiel profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Seraphiel profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Seraphiel distribution."
        )

    # Version check up-front so we fail fast
    check_seraphiel_requires(manifest.seraphiel_requires, seraphiel_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.seraphiel).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="seraphiel_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `seraphiel profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`seraphiel profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="seraphiel_dist_update_") as tmp:
        plan = plan_install(
