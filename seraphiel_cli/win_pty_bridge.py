"""Windows ConPTY bridge for the `seraphiel dashboard` chat tab.

Drop-in counterpart to ``seraphiel_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``seraphiel_cli.web_server`` consumes: ``spawn``, ``read``,
