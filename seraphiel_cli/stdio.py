...
on Japanese locales, etc.).  Seraphiel's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``seraphiel_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Seraphiel is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SERAPHIEL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SERAPHIEL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Seraphiel-managed Git install directories so
    ...
    # already-running shells don't see the change — which means seraphiel
    ...
    # install without Seraphiel-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Seraphiel's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Seraphiel's
    ``seraphiel config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Seraphiel (User env var in Windows Settings, or
    export in a PowerShell profile) and Seraphiel picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Seraphiel-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\seraphiel\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``seraphiel`` from right after install) retain their old PATH.

    Any subprocess Seraphiel spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Seraphiel-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "seraphiel", "git", "cmd"),
        os.path.join(local_appdata, "seraphiel", "git", "bin"),
        os.path.join(local_appdata, "seraphiel", "git", "usr", "bin"),
        # Seraphiel venv Scripts directory — host of the seraphiel.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes seraphiel, but harmless to include.
        os.path.join(local_appdata, "seraphiel", "seraphiel-brain", "venv", "Scripts"),
