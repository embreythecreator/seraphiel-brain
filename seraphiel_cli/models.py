...
Add, remove, or reorder entries here — both `seraphiel setup` and
`seraphiel` provider-selection will pick up the change automatically.
"""
...
from seraphiel_cli import __version__ as _SERAPHIEL_VERSION
from seraphiel_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_SERAPHIEL_USER_AGENT = f"seraphiel-cli/{_SERAPHIEL_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `seraphiel model`
    flow without maintaining a separate static list.
    """
    from seraphiel_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $SERAPHIEL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Seraphiel-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SERAPHIEL_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Seraphiel refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Seraphiel that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Seraphiel that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from seraphiel_cli.nous_account import get_nous_portal_account_info
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_seraphiel_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$SERAPHIEL_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.embreythecreator.com").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.embreythecreator.com"
...
# this list:  seraphiel model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `seraphiel model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `seraphiel model` TUI
...
#
# Some vendors expose several Seraphiel provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `seraphiel model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``seraphiel model``, the
...
# In-repo fallback for the model Seraphiel silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from seraphiel_cli.model_catalog import get_default_model_from_cache
...
# ``seraphiel model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in seraphiel_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``seraphiel auth add openai-codex`` without
    ``seraphiel model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``seraphiel model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    seraphiel-brain is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from seraphiel_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — seraphiel-brain
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Seraphiel lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from seraphiel_cli.model_catalog import get_curated_nous_models
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.embreythecreator.com"
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from seraphiel_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.embreythecreator.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``seraphiel model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from seraphiel_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:seraphiel-3                           →  ("nous", "seraphiel-3")
...
def _resolve_static_model_alias(
    ...
    try:
        from seraphiel_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Seraphiel' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``seraphiel_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Seraphiel should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``seraphiel auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.seraphiel/.env``.
    ...
    """
    try:
        from seraphiel_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Seraphiel release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Seraphiel release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from seraphiel_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        ...
        try:
            from seraphiel_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Seraphiel release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``seraphiel model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $SERAPHIEL_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from seraphiel_constants import get_seraphiel_home
    return get_seraphiel_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SERAPHIEL_HOME/auth.json`` and external
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    """
    try:
        from seraphiel_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=is_agent_turn)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "SeraphielAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SERAPHIEL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Seraphiel' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SERAPHIEL_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"seraphiel-brain/{_SERAPHIEL_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=seraphiel"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from seraphiel_constants import get_seraphiel_home
    return get_seraphiel_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `seraphiel moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Seraphiel will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Seraphiel cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Seraphiel actually ships.  (#46850)
