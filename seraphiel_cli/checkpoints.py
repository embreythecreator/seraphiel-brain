"""`seraphiel checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.seraphiel/checkpoints/``.  Actions:

    seraphiel checkpoints               # same as `status`
    seraphiel checkpoints status        # total size, project count, breakdown
    seraphiel checkpoints list          # per-project checkpoint counts + workdir
    seraphiel checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    seraphiel checkpoints clear [-f]    # nuke the entire base (asks first)
    seraphiel checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    seraphiel checkpoints
    seraphiel checkpoints prune --retention-days 3 --max-size-mb 200
    seraphiel checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: seraphiel checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``seraphiel checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `seraphiel checkpoints` → status
