...
the existing module-level functions in seraphiel_cli.gateway and
seraphiel_cli.gateway_windows directly. This protocol is a thin facade
...
``seraphiel gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``seraphiel gateway start/stop/restart``).
    """
    ...
    # `seraphiel gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged seraphiel
    ...
    detection always fails. Since every Seraphiel runtime call inside the
    container drops to seraphiel via ``s6-setuidgid``, that silent failure
...
# in ``seraphiel_cli.gateway`` (systemd/launchd) and ``seraphiel_cli.gateway_windows``
...
# active (set via the ``seraphiel -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in seraphiel_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from seraphiel_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from seraphiel_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from seraphiel_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from seraphiel_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in seraphiel_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from seraphiel_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from seraphiel_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from seraphiel_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from seraphiel_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``seraphiel_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `seraphiel profile create`
# runs inside the container (Phase 4). Static services (main-seraphiel, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's SERAPHIEL_HOME blindly;
    derive the shared profile root from the current SERAPHIEL_HOME and map the
    ...
    seraphiel_home = Path(os.environ.get("SERAPHIEL_HOME", "/opt/data"))
    if seraphiel_home.parent.name == "profiles":
        root = seraphiel_home.parent.parent
    else:
        root = seraphiel_home
...
# UID/GID of the in-image ``seraphiel`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``seraphiel`` user — UID 10000 — running every Seraphiel runtime
    ...
    is dispatched as the seraphiel user.
    ...
    with seraphiel ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           seraphiel:seraphiel, 0755 (parent must already exist)
    ``svc_dir/event/``                     seraphiel:seraphiel, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 seraphiel:seraphiel, 0755
    ``svc_dir/supervise/event/``           seraphiel:seraphiel, 03730
    ``svc_dir/supervise/control``          seraphiel:seraphiel, 0660    (FIFO)
    ...
    seraphiel user reads them fine.
    ...
    slot's supervise/ was seraphiel-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _SERAPHIEL_UID, _SERAPHIEL_GID)
        except PermissionError:
            # Running as the seraphiel user already — directory is seraphiel-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _SERAPHIEL_UID, _SERAPHIEL_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is seraphiel-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _SERAPHIEL_UID, _SERAPHIEL_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``seraphiel -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`seraphiel profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-seraphiel, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SERAPHIEL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SERAPHIEL_HOME=/data/seraphiel`` is honored at run
          ...
          4. Drops to the seraphiel user and exec's
             ``seraphiel -p <profile> gateway run`` (or just ``seraphiel
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``seraphiel gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SERAPHIEL_HOME profile" (the implicit profile that exists at
        the top of $SERAPHIEL_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``seraphiel_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SERAPHIEL_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``SERAPHIEL_HOME``,
        ...
        allocator in ``seraphiel_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/seraphiel/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export SERAPHIEL_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's SERAPHIEL_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``seraphiel gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-SERAPHIEL_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The SERAPHIEL_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own SERAPHIEL_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "seraphiel gateway run --replace"
        else:
            gateway_cmd = f"seraphiel -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid seraphiel {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SERAPHIEL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SERAPHIEL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SERAPHIEL_HOME=/data/seraphiel``
        gets its logs under /data/seraphiel/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``seraphiel logs`` reads and what
        ...
        return (
            ...
            f': "${{SERAPHIEL_HOME:=/opt/data}}"\n'
            f'log_dir="$SERAPHIEL_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as seraphiel and crash-loops on
            ...
            f'chown seraphiel:seraphiel "$SERAPHIEL_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R seraphiel:seraphiel "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid seraphiel s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``seraphiel gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``seraphiel -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with seraphiel ownership
            ...
            # the seraphiel user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is seraphiel-owned (see ``_seed_supervise_skeleton``), and on
