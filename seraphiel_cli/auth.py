...
from seraphiel_cli.config import get_seraphiel_home, get_config_path, read_raw_config
...
XAI_OAUTH_DOCS_URL = "https://seraphiel-brain.embreythecreator.com/docs/guides/xai-grok-oauth"
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Seraphiel times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``SERAPHIEL_LOG_LEVEL=INFO`` (or check
            # ``~/.seraphiel/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>seraphiel auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=seraphiel-brain` lets xAI attribute Seraphiel-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Seraphiel — please report at "
            "https://github.com/embreythecreator/seraphiel-brain/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Seraphiel with xAI:")
        print(authorize_url)
