"""Shell completion script generation for seraphiel CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_seraphiel_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Seraphiel Brain bash completion
# Add to ~/.bashrc:
#   eval "$(seraphiel completion bash)"

_seraphiel_profiles() {{
    local profiles_dir="$HOME/.seraphiel/profiles"
...
_seraphiel_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_seraphiel_profiles)" -- "$cur"))
        return
    fi
...
complete -F _seraphiel_completion seraphiel
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef seraphiel
# Seraphiel Brain zsh completion
# Add to ~/.zshrc:
...
_seraphiel_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.seraphiel/profiles" ]]; then
        profiles+=($HOME/.seraphiel/profiles/*(N/:t))
    fi
...
_seraphiel() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_seraphiel_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'seraphiel command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Seraphiel Brain fish completion",
        "# Add to your config:",
        "#   seraphiel completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __seraphiel_profiles",
        "    echo default",
        "    if test -d $HOME/.seraphiel/profiles",
        "        for d in $HOME/.seraphiel/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c seraphiel -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c seraphiel -f -s p -l profile"
        " -d 'Profile name' -xa '(__seraphiel_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__seraphiel_profiles)' -d 'Profile name'"
                )
