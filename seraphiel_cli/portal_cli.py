"""``seraphiel portal`` — the human-readable entry point for Nous Portal.

Running ``seraphiel portal`` with no subcommand performs the one-shot Portal
...
``seraphiel auth add nous --type oauth`` (which still works), is identical to
``seraphiel setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``seraphiel auth`` or ``seraphiel tools``. It's the onboarding + discovery
...
DEFAULT_PORTAL_URL = "https://portal.embreythecreator.com"
SUBSCRIPTION_URL = "https://portal.embreythecreator.com/manage-subscription"
DOCS_URL = "https://seraphiel-brain.embreythecreator.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   seraphiel portal")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `seraphiel model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `seraphiel portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `seraphiel auth add nous --type
    oauth`. It reuses the exact wiring behind `seraphiel setup --portal` (which in
    ...
    """
    from seraphiel_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `seraphiel portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `seraphiel portal` is the
        # human-readable alias for `seraphiel auth add nous --type oauth` /
    ...
    print("Run `seraphiel portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `seraphiel portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `seraphiel portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `seraphiel auth add "
            "nous --type oauth`, identical to `seraphiel setup --portal`). "
