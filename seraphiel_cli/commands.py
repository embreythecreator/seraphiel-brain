"""Slash command definitions and autocomplete for the Seraphiel CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Seraphiel works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Seraphiel is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.seraphiel/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Seraphiel Brain to the latest version", "Info"),
    CommandDef("version", "Show Seraphiel Brain version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`seraphiel_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/seraphiel`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Seraphiel ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Seraphiel
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``seraphiel skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``seraphiel skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``seraphiel skills list`` and
...
# unrelated command landed. These claim their slots right after /seraphiel,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /seraphiel <command>).
...
# native slot, the alias spelling stays reachable via /seraphiel reset).
...
# ``/seraphiel <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/seraphiel" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /seraphiel topup on Slack.
...
#   - plan: plan-mode toggle; reached via /seraphiel plan on Slack.
_SLACK_VIA_SERAPHIEL_ONLY = frozenset({"topup", "moa", "debug", "plan"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/seraphiel <verb>`` subcommand.
    ...
    can still reach them via ``/seraphiel <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/seraphiel`` is always reserved as the first entry so the
    legacy ``/seraphiel <subcommand>`` form keeps working for anything that
    ...
    # Reserve /seraphiel as the catch-all top-level command.
    entries.append(("seraphiel", "Talk to Seraphiel or run a subcommand", "[subcommand] [args]"))
    seen.add("seraphiel")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SERAPHIEL_ONLY:
            # Intentionally Slack-via-/seraphiel only (see _SLACK_VIA_SERAPHIEL_ONLY).
            return
    ...
    # everything except /seraphiel, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://seraphiel-brain.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /seraphiel handler.

    Maps both canonical names and aliases so /seraphiel bg do stuff works
    the same as /seraphiel background do stuff.

    Plugin-registered slash commands are included so ``/seraphiel <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from seraphiel_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
