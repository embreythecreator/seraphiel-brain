"""
Dump command for seraphiel CLI.

Outputs a compact, plain-text summary of the user's Seraphiel setup
...
from seraphiel_cli.config import get_seraphiel_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.seraphiel/.env.
    ...
    exports. ``seraphiel debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.seraphiel_build_sha`` by the Dockerfile's
    ``SERAPHIEL_GIT_SHA`` build-arg (see ``seraphiel_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``seraphiel dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from seraphiel_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(seraphiel_home: Path) -> int:
...
def _cron_summary(seraphiel_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = seraphiel_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_seraphiel_dotenv(
        seraphiel_home=env_path.parent,
    ...
    lines.append(f"seraphiel_home:      {display_seraphiel_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.seraphiel/.env: a managed
        ...
        # A credential added via `seraphiel auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `seraphiel auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["seraphiel-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(seraphiel_home)}")
    lines.append(f"  skills:             {_count_skills(seraphiel_home)}")
