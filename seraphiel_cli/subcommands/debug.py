"""``seraphiel debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Seraphiel Brain. Use 'seraphiel debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    seraphiel debug share              Upload debug report (asks for confirmation)
    seraphiel debug share --yes        Skip confirmation (for scripts/CI)
    seraphiel debug share --lines 500  Include more log lines
    seraphiel debug share --expire 30  Keep paste for 30 days
    seraphiel debug share --local      Print report locally (no upload)
    seraphiel debug share --no-redact  Disable upload-time secret redaction
    seraphiel debug share --nous       Upload to Nous-internal storage (private)
    seraphiel debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'seraphiel debug share'",
    )
