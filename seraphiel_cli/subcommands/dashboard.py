"""``seraphiel dashboard`` subcommand parser.

Extracted verbatim from ``seraphiel_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    # for `seraphiel dashboard` cmdlines and SIGTERM them directly — the same
    # path `seraphiel update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running seraphiel dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running seraphiel dashboard processes and exit",
    )
