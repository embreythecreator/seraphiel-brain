...
Reads the user's Nous OAuth state from ``~/.seraphiel/auth.json`` through the
...
# Endpoints inference-api.embreythecreator.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `seraphiel auth add nous` first."
                )
    ...
    # to seraphiel_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
