...
# xAI's public API is OpenAI-compatible for the endpoints Seraphiel commonly
# uses. The Responses endpoint is included because Seraphiel' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Seraphiel-managed OAuth credentials."""

    auth_hint = "seraphiel auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`seraphiel auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`seraphiel auth reset xai-oauth` or re-authenticate with "
                    "`seraphiel auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `seraphiel auth add xai-oauth --type oauth`."
            )
