...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``SERAPHIEL_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
def _launch_tui(
    ...
    env["SERAPHIEL_PYTHON_SRC_ROOT"] = os.environ.get(
        "SERAPHIEL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SERAPHIEL_PYTHON", sys.executable)
    env.setdefault("SERAPHIEL_CWD", os.getcwd())
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``SERAPHIEL_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("SERAPHIEL_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    # setup (config, rules files, plugins, MCP servers) or from Seraphiel itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (SERAPHIEL_SAFE_MODE is checked by
    # seraphiel_cli/plugins.py and tools/mcp_tool.py).
...
# Critical files that every ``seraphiel`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``seraphiel update`` again to roll forward. The post-pull syntax
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SERAPHIEL_HOME before running the update.
    ...
    SERAPHIEL_HOME directories.  The ``--backup`` flag on ``seraphiel update``
    ...
    # ``seraphiel_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # SERAPHIEL_HOME can add minutes to every update. Users who want the
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # though `git pull` can't touch $SERAPHIEL_HOME, this is cheap
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.seraphiel)")
            else:
...
def cmd_dashboard(args):
    ...
    if "SERAPHIEL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
...
def main():
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.seraphiel/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://seraphiel-brain.embreythecreator.com/docs/user-guide/secrets/bitwarden"
        ),
    )
