#!/usr/bin/env python3
"""
Seraphiel CLI - Main entry point.

Usage:
    seraphiel                     # Interactive chat (default)
    seraphiel chat                # Interactive chat
    seraphiel gateway             # Run gateway in foreground
    seraphiel gateway start       # Start gateway as service
    seraphiel gateway stop        # Stop gateway service
    seraphiel gateway status      # Show gateway status
    seraphiel gateway install     # Install gateway service
    seraphiel gateway uninstall   # Uninstall gateway service
    seraphiel setup               # Interactive setup wizard
    seraphiel logout              # Clear stored authentication
    seraphiel status              # Show status of all components
    seraphiel cron                # Manage cron jobs
    seraphiel cron list           # List cron jobs
    seraphiel cron status         # Check if cron scheduler is running
    seraphiel doctor              # Check configuration and dependencies
    seraphiel honcho setup                    # Configure Honcho AI memory integration
    seraphiel honcho status                   # Show Honcho config and connection status
    seraphiel honcho sessions                 # List directory → session name mappings
    seraphiel honcho map <name>               # Map current directory to a session name
    seraphiel honcho peer                     # Show peer names and dialectic settings
    seraphiel honcho peer --user NAME         # Set user peer name
    seraphiel honcho peer --ai NAME           # Set AI peer name
    seraphiel honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    seraphiel honcho mode                     # Show current memory mode
    seraphiel honcho mode [hybrid|honcho|local]  # Set memory mode
    seraphiel honcho tokens                   # Show token budget settings
    seraphiel honcho tokens --context N       # Set session.context() token cap
    seraphiel honcho tokens --dialectic N     # Set dialectic result char cap
    seraphiel honcho identity                 # Show AI peer identity representation
    seraphiel honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    seraphiel honcho migrate                  # Step-by-step migration guide: OpenClaw native → Seraphiel + Honcho
    seraphiel version             Show version
    seraphiel update              Update to latest version
    seraphiel uninstall           Uninstall Seraphiel Brain
    seraphiel acp                 Run as an ACP server for editor integration
    seraphiel sessions browse     Interactive session picker with search

    seraphiel claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: seraphiel_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``seraphiel_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``seraphiel update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``seraphiel_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, seraphiel crashes on import and the user can't run
# ``seraphiel update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import seraphiel_bootstrap  # noqa: F401
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from seraphiel_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'seraphiel' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``seraphiel tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``seraphiel.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("seraphiel")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"seraphiel", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"seraphiel")
        # Windows: the .exe name is already ``seraphiel.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* seraphiel_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SERAPHIEL_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".seraphiel", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``SERAPHIEL_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``seraphiel … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("SERAPHIEL_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``SERAPHIEL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SERAPHIEL_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`seraphiel --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from seraphiel_cli import __release_date__, __version__

    print(f"Seraphiel Brain v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``seraphiel --version`` before config/logging imports on Termux."""
    if os.environ.get("SERAPHIEL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from seraphiel_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from seraphiel_cli.subcommands.cron import build_cron_parser
from seraphiel_cli.subcommands.gateway import build_gateway_parser
from seraphiel_cli.subcommands.profile import build_profile_parser
from seraphiel_cli.subcommands.model import build_model_parser
from seraphiel_cli.subcommands.setup import build_setup_parser
from seraphiel_cli.subcommands.postinstall import build_postinstall_parser
from seraphiel_cli.subcommands.whatsapp import build_whatsapp_parser
from seraphiel_cli.subcommands.slack import build_slack_parser
from seraphiel_cli.subcommands.login import build_login_parser
from seraphiel_cli.subcommands.logout import build_logout_parser
from seraphiel_cli.subcommands.auth import build_auth_parser
from seraphiel_cli.subcommands.status import build_status_parser
from seraphiel_cli.subcommands.webhook import build_webhook_parser
from seraphiel_cli.subcommands.hooks import build_hooks_parser
from seraphiel_cli.subcommands.doctor import build_doctor_parser
from seraphiel_cli.subcommands.security import build_security_parser
from seraphiel_cli.subcommands.dump import build_dump_parser
from seraphiel_cli.subcommands.debug import build_debug_parser
from seraphiel_cli.subcommands.backup import build_backup_parser
...
from seraphiel_cli.subcommands.dashboard import build_dashboard_parser
from seraphiel_cli.subcommands.gui import build_gui_parser
from seraphiel_cli.subcommands.logs import build_logs_parser
from seraphiel_cli.subcommands.prompt_size import build_prompt_size_parser
from seraphiel_cli.subcommands.memory import build_memory_parser
from seraphiel_cli.subcommands.acp import build_acp_parser
from seraphiel_cli.subcommands.tools import build_tools_parser
from seraphiel_cli.subcommands.insights import build_insights_parser
from seraphiel_cli.subcommands.skills import build_skills_parser
from seraphiel_cli.subcommands.pairing import build_pairing_parser
from seraphiel_cli.subcommands.plugins import build_plugins_parser
from seraphiel_cli.subcommands.mcp import build_mcp_parser
from seraphiel_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (seraphiel tools, seraphiel setup, seraphiel model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'seraphiel {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any seraphiel module import.
#
# Many modules cache SERAPHIEL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SERAPHIEL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.seraphiel/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SERAPHIEL_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `seraphiel mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Seraphiel' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo seraphiel -p <name>` against the invoking user's home.
        ...
        candidate = home / ".seraphiel" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`seraphiel chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors seraphiel_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SERAPHIEL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.seraphiel/profiles/coder or
    # /opt/data/profiles/coder).  If SERAPHIEL_HOME points to the seraphiel root
    # instead (e.g. systemd hardcodes SERAPHIEL_HOME=/root/.seraphiel), we must
    # still read active_profile — the user may have switched profiles via
    # `seraphiel profile use` and the gateway should honour that choice.
    # See issue #22502.
    seraphiel_home_env = os.environ.get("SERAPHIEL_HOME", "")
    if profile_name is None and seraphiel_home_env:
        if Path(seraphiel_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the seraphiel root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as SERAPHIEL_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``seraphiel gateway run`` to mean
    # "the root SERAPHIEL_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("SERAPHIEL_S6_SUPERVISED_CHILD"):
        try:
            from seraphiel_constants import get_default_seraphiel_root

            active_path = get_default_seraphiel_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SERAPHIEL_HOME
    if profile_name is not None:
        try:
            from seraphiel_cli.profiles import resolve_profile_env

            seraphiel_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            seraphiel_home = _resolve_sudo_user_profile_env(profile_name)
            if not seraphiel_home:
            ...
            # A bug in profiles.py must NEVER prevent seraphiel from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SERAPHIEL_HOME"] = seraphiel_home
...
# Load .env from ~/.seraphiel/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from seraphiel_cli.config import get_seraphiel_home
from seraphiel_cli.env_loader import load_seraphiel_dotenv

load_seraphiel_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SERAPHIEL_REDACT_SECRETS env
# var BEFORE seraphiel_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_seraphiel_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from seraphiel_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "SERAPHIEL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SERAPHIEL_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `seraphiel` subcommands
...
try:
    from seraphiel_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from seraphiel_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if seraphiel_constants not importable yet
...
from seraphiel_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to seraphiel_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (seraphiel_cli.main._model_flow_*) keep resolving unchanged.
from seraphiel_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_seraphiel_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SERAPHIEL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SERAPHIEL_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from seraphiel_cli.config import get_env_path, get_seraphiel_home, load_config
    from seraphiel_cli.auth import get_auth_status

    # Determine whether Seraphiel itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from seraphiel_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_seraphiel_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from seraphiel_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_seraphiel_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Seraphiel has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Seraphiel to use their tokens.
    if _has_seraphiel_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, seraphiel_bin
        cli_args: the original CLI arguments (everything after 'seraphiel')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    seraphiel_bin = container_info["seraphiel_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo seraphiel {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo seraphiel {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  seraphiel --tui --resume {target}")
    if title:
        print(f'  seraphiel --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @seraphiel/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@seraphiel" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/seraphiel-ink/package.json",
    "packages/seraphiel-ink/index.js",
    "packages/seraphiel-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SERAPHIEL_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SERAPHIEL_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SERAPHIEL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SERAPHIEL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    seraphiel_home = os.environ.get("SERAPHIEL_HOME") or str(Path.home() / ".seraphiel")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SERAPHIEL_HOME": seraphiel_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(seraphiel_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(seraphiel_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if seraphiel_cli_dir is None:
        seraphiel_cli_dir = Path(__file__).parent
    bundled = seraphiel_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``seraphiel update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("SERAPHIEL_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Seraphiel checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `seraphiel update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Seraphiel checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `seraphiel --tui`\n"
        "If the checkout is still inconsistent, run `seraphiel update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SERAPHIEL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SERAPHIEL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from seraphiel_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("SERAPHIEL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SERAPHIEL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SERAPHIEL_TUI_DIR (e.g. `unset SERAPHIEL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # ships seraphiel_cli/tui_dist/entry.js in the wheel but never ships ui-tui/
    ...
    if tui_dev:
        # Keep the local @seraphiel/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @seraphiel/ink resolves through
        # packages/seraphiel-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "seraphiel-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from seraphiel_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("SERAPHIEL_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["SERAPHIEL_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("SERAPHIEL_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["SERAPHIEL_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("SERAPHIEL_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["SERAPHIEL_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["SERAPHIEL_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from seraphiel_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="seraphiel-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SERAPHIEL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SERAPHIEL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["SERAPHIEL_MODEL"] = model
        env["SERAPHIEL_INFERENCE_MODEL"] = model
    if provider:
        env["SERAPHIEL_TUI_PROVIDER"] = provider
        env["SERAPHIEL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SERAPHIEL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SERAPHIEL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SERAPHIEL_TUI_SKILLS"] = value
    if query:
        env["SERAPHIEL_TUI_QUERY"] = query
    if image:
        env["SERAPHIEL_TUI_IMAGE"] = image
    if checkpoints:
        env["SERAPHIEL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SERAPHIEL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SERAPHIEL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SERAPHIEL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SERAPHIEL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SERAPHIEL_ACCEPT_HOOKS"] = "1"
    ...
    # SERAPHIEL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `seraphiel --tui` try to
    ...
    # still set SERAPHIEL_TUI_RESUME themselves.
    env.pop("SERAPHIEL_TUI_RESUME", None)
    if resume_session_id:
        env["SERAPHIEL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `seraphiel update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SERAPHIEL_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``seraphiel kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``seraphiel kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SERAPHIEL_KANBAN_BOARD"):
        return
    try:
        from seraphiel_cli.kanban_db import get_current_board

        os.environ["SERAPHIEL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.seraphiel/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Seraphiel — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Seraphiel still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``seraphiel … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("SERAPHIEL_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'seraphiel sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from seraphiel_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from seraphiel_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'seraphiel doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Seraphiel isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  seraphiel setup")
        print()

        from seraphiel_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'seraphiel setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from seraphiel_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["SERAPHIEL_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.seraphiel/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SERAPHIEL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SERAPHIEL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SERAPHIEL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from seraphiel_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from seraphiel_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from seraphiel_constants import find_node_executable, with_seraphiel_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Seraphiel?")
        print()
    ...
    # has no creds.json.  Every subsequent `seraphiel gateway` then paid a 30s
    ...
    session_dir = get_seraphiel_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: seraphiel gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `seraphiel gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'seraphiel whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``seraphiel whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``seraphiel_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from seraphiel_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from seraphiel_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from seraphiel_cli.config import stamp_install_method
    from seraphiel_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from seraphiel_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``seraphiel model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from seraphiel_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from seraphiel_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("SERAPHIEL_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from seraphiel_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'seraphiel model' for "
                "available providers, or run 'seraphiel doctor' to diagnose config "
                "issues."
            )
    ...
    from seraphiel_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        _PROVIDER_ALIASES,
        group_providers,
    ...
    # PROVIDER_GROUPS in seraphiel_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``seraphiel model``
    ...
    # matching against seraphiel_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    # a leftover OPENAI_BASE_URL in ~/.seraphiel/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.seraphiel/.env if the active provider is not 'custom'.
    ...
    """
    from seraphiel_cli.config import get_env_value, save_env_value, load_config
...
#
# Seraphiel uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `seraphiel model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `seraphiel model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`seraphiel_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from seraphiel_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from seraphiel_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Seraphiel only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``seraphiel model`` flow, then route aux tasks to them here.
    """
    from seraphiel_cli.config import load_config
    from seraphiel_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from seraphiel_cli.auth import _prompt_model_selection
    from seraphiel_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from seraphiel_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from seraphiel_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Seraphiel URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from seraphiel_cli.curses_ui import curses_radiolist
...
# downstream call sites read `seraphiel_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `seraphiel` invocation — including
# fast paths like `seraphiel --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from seraphiel_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``seraphiel setup`` / ``seraphiel model``.
    ...
    recover from a malformed paste without editing ``~/.seraphiel/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from seraphiel_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from seraphiel_cli.config import save_env_value
    from seraphiel_cli.secret_prompt import masked_secret_prompt
    ...
    from seraphiel_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `seraphiel setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from seraphiel_constants import display_seraphiel_home as _dhh_fn

            print(
                f"    Seraphiel will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               seraphiel model")
        print()
...
def cmd_login(args):
    """Authenticate Seraphiel CLI with a provider."""
    from seraphiel_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from seraphiel_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from seraphiel_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from seraphiel_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from seraphiel_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from seraphiel_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``seraphiel slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: seraphiel slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `seraphiel slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from seraphiel_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from seraphiel_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from seraphiel_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from seraphiel_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from seraphiel_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `seraphiel security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from seraphiel_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from seraphiel_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from seraphiel_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from seraphiel_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Seraphiel home directory to a zip file."""
    if getattr(args, "quick", False):
        from seraphiel_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from seraphiel_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Seraphiel backup from a zip file."""
    from seraphiel_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from seraphiel_cli.config import detect_install_method
    from seraphiel_cli.banner import format_banner_version_label
    ...
    try:
        from seraphiel_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Seraphiel Brain (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from seraphiel_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from seraphiel_cli.uninstall import run_gui_uninstall
    ...
    from seraphiel_cli.uninstall import run_uninstall
...
# Critical files that Seraphiel must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "seraphiel_cli/main.py",
    "seraphiel_cli/config.py",
    "seraphiel_cli/__init__.py",
    "seraphiel_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "seraphiel_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``seraphiel`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="seraphiel-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``seraphiel update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``seraphiel_cli/web_dist/`` (per vite.config.ts
    outDir: "../seraphiel_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "seraphiel_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``seraphiel`` launcher present but ``seraphiel_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the seraphiel venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — seraphiel venv python3, no nix-shell overhead
    ...
    # a seraphiel venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``seraphiel update`` to stash the
    lockfile — repeatedly.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``seraphiel web``).
    ...
    # build path usable on a stock `py -m seraphiel_cli.main web` invocation.
    ...
    from seraphiel_constants import with_seraphiel_node_path
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_seraphiel_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (seraphiel web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``seraphiel update`` can unconditionally call ``seraphiel desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``seraphiel desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $SERAPHIEL_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $SERAPHIEL_HOME."""
    from seraphiel_constants import get_seraphiel_home
    return get_seraphiel_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``seraphiel update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Seraphiel.app/Contents/MacOS/Seraphiel"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Seraphiel.exe",
            release_dir / "win-ia32-unpacked" / "Seraphiel.exe",
            release_dir / "win-arm64-unpacked" / "Seraphiel.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "seraphiel",
            release_dir / "linux-unpacked" / "Seraphiel",
            release_dir / "linux-arm64-unpacked" / "seraphiel",
            release_dir / "linux-arm64-unpacked" / "Seraphiel",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Seraphiel'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_seraphiel_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Seraphiel.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Seraphiel.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Seraphiel.exe: Access is
    ...
    an unrelated "Seraphiel" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Seraphiel is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Seraphiel.app/Contents/MacOS/Seraphiel  ->  app bundle = .../Seraphiel.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``seraphiel desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Seraphiel Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Seraphiel Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the SERAPHIEL_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from seraphiel_logging import setup_logging as _setup_logging_gui
    ...
    # with_seraphiel_node_path() copies os.environ when called with no arg.
    env = with_seraphiel_node_path()
    if getattr(args, "fake_boot", False):
        env["SERAPHIEL_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["SERAPHIEL_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "seraphiel_root", None):
        env["SERAPHIEL_DESKTOP_SERAPHIEL_ROOT"] = str(Path(args.seraphiel_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["SERAPHIEL_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["SERAPHIEL_DESKTOP_CWD"] = os.getcwd()
    ...
    # `SERAPHIEL_DESKTOP_DISABLE_GPU=... seraphiel desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "SERAPHIEL_DESKTOP_DISABLE_GPU" not in os.environ:
        env["SERAPHIEL_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  seraphiel gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Seraphiel-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → seraphiel-setup →
            # seraphiel update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `seraphiel update` path).
            nixos_env = with_seraphiel_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Seraphiel.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Seraphiel.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Seraphiel.exe, close any")
                    print("  running Seraphiel desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> seraphiel desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Seraphiel is
    ...
    if source_mode:
        print("→ Launching Seraphiel Desktop from source build...")
    ...
    print(f"→ Launching packaged Seraphiel Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``seraphiel dashboard`` processes other than ourselves.

    ``seraphiel dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``seraphiel update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Seraphiel Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``seraphiel serve`` as
    ...
    environment variable ``SERAPHIEL_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "seraphiel dashboard",
        "seraphiel_cli.main dashboard",
        "seraphiel_cli/main.py dashboard",
        # The headless backend (`seraphiel serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "seraphiel serve",
        "seraphiel_cli.main serve",
        "seraphiel_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from seraphiel_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "seraphiel.*dashboard"`) keeps us consistent
            # with `seraphiel_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `seraphiel update`.
    ...
    print("  Preview now:  seraphiel curator run --dry-run")
    print("  Pause it:     seraphiel curator pause")
    print(
        "  Docs:         https://seraphiel-brain.embreythecreator.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``seraphiel update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``seraphiel update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: seraphiel curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``seraphiel dashboard`` processes.

    Called at the end of ``seraphiel update`` (default ``reason``) and also
    from ``seraphiel dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Seraphiel Desktop Electron app spawns this dashboard as a
    # backend child, it sets SERAPHIEL_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SERAPHIEL_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    seraphiel dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.seraphiel-update-staging"
    backup = f"{dst}.seraphiel-update-old"
...
def _update_via_zip(args):
    """Update Seraphiel Brain by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `seraphiel update "
            f"--branch {branch}`, or update against main with `seraphiel update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/embreythecreator/seraphiel-brain/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="seraphiel-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"seraphiel-brain-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for seraphiel-brain itself
        ...
        # GitHub ZIPs extract to seraphiel-brain-<branch>/
        extracted = os.path.join(tmp_dir, f"seraphiel-brain-{branch}")
    ...
    from seraphiel_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: seraphiel skills list-modified  "
    ...
    try:
        from seraphiel_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "seraphiel-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Seraphiel behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes seraphiel completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Seraphiel couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Seraphiel couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Seraphiel behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Seraphiel couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Seraphiel couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `seraphiel update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/embreythecreator/seraphiel-brain.git",
    "git@github.com:embreythecreator/seraphiel-brain.git",
    "https://github.com/embreythecreator/seraphiel-brain",
    "git@github.com:embreythecreator/seraphiel-brain",
}
OFFICIAL_REPO_URL = "https://github.com/embreythecreator/seraphiel-brain.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_seraphiel_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_seraphiel_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Seraphiel repository.")
        print("  This means you may miss updates from embreythecreator/seraphiel-brain.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/embreythecreator/seraphiel-brain.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/embreythecreator/seraphiel-brain.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``seraphiel update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from seraphiel_constants import get_default_seraphiel_root

    default_home = get_default_seraphiel_root()
...
# Install-scoped breadcrumb dropped right before ``seraphiel update`` mutates the
...
# survives and the next ``seraphiel`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $SERAPHIEL_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``seraphiel update`` left half-done.
    ...
    protocol stream (``seraphiel acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if seraphiel.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _seraphiel_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Seraphiel is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Seraphiel from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `seraphiel update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from seraphiel_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``seraphiel update`` is still progressing even if pip/uv itself is silent.
    """
...
def _seraphiel_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_seraphiel_instances(
    ...
    The Seraphiel Desktop Electron app spawns ``seraphiel.EXE`` as a backend child,
    so during ``seraphiel update`` the user-invoked process and the desktop's
    ...
    shims (``seraphiel.exe`` / ``seraphiel-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``seraphiel update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``seraphiel.exe``) is a separate process from the Python
    ...
    for shim in _seraphiel_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated seraphiel.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``seraphiel update``
    ...
    #      seraphiel.exe sitting *under* a non-Seraphiel parent (e.g. a Seraphiel
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "seraphiel.exe"
    lines = ["✗ Another seraphiel.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Seraphiel Desktop, exit any open `seraphiel` REPLs, and")
    lines.append("  stop the gateway (`seraphiel gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `seraphiel update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_seraphiel_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``seraphiel.exe``.
    ...
    when ``seraphiel update`` runs, ``seraphiel.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``seraphiel.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next seraphiel invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Seraphiel Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Seraphiel Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _seraphiel_exe_shims(scripts_dir):
        ...
        print(
            "    Close Seraphiel Desktop, exit other `seraphiel` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `seraphiel update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_seraphiel_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``seraphiel.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``seraphiel.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``seraphiel`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_seraphiel_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``seraphiel.exe.old.*`` left by prior updates.

    Called early on every seraphiel invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``seraphiel update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `seraphiel update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``seraphiel.exe`` / ``seraphiel-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_seraphiel_exe`` for the rationale.
    """
    ...
    # hours later inside ``seraphiel update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``seraphiel.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``seraphiel.exe`` shim is locked during ``seraphiel update``, or when uv/distlib
    skips a launcher write. The symptom is ``seraphiel-brain.exe`` and
    ``seraphiel-acp.exe`` present but ``seraphiel.exe`` missing, so ``seraphiel`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `seraphiel update --force` after "
            "closing other seraphiel processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m seraphiel_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``seraphiel update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``seraphiel.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``seraphiel`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `seraphiel update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `seraphiel update --force` after closing other seraphiel processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from seraphiel_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SERAPHIEL_HOME/bin/uv``, but on Termux the official
    ...
    """
    from seraphiel_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `seraphiel update` is exactly the step
...
def _npm_lockfile_changed(seraphiel_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = seraphiel_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(seraphiel_root: Path) -> None:
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from seraphiel_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from seraphiel_constants import is_wsl
        ...
        if is_wsl() and path_npm and _is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `seraphiel update`.")
            failed = ["repo root"]
            if any(
    ...
    # Seraphiel profile using this checkout. Keep one per-checkout cache under the
    # shared Seraphiel root rather than rerunning npm once per named profile.
    shared_seraphiel_root = get_default_seraphiel_root()
    if not _npm_lockfile_changed(shared_seraphiel_root):
    ...
    # need desktop during `seraphiel update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `seraphiel update`.")
        return list(labels)
    ...
    # hung. The chatty npm-deprecation noise during `seraphiel update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_seraphiel_root)
...
class _UpdateOutputStream:
    """Stream wrapper used during ``seraphiel update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.seraphiel/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``seraphiel update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``seraphiel update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.seraphiel/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``seraphiel update --gateway``) the update is already
    ...
    try:
        ...
        # seraphiel_cli.config.get_seraphiel_home to simulate setup failure.
        from seraphiel_cli.config import get_seraphiel_home as _get_seraphiel_home

        logs_dir = _get_seraphiel_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.seraphiel/logs/update.log`` only, never the terminal.

    During ``seraphiel update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``seraphiel update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``seraphiel update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from seraphiel_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from seraphiel_cli.config import recommended_update_command
        from seraphiel_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``seraphiel update`` without requiring a reinstall.  The
    ...
    ``seraphiel`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/seraphiel.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v seraphiel'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/seraphiel, code at /usr/local/lib/seraphiel-brain).
    fhs_link = Path("/usr/local/bin/seraphiel")
    ...
    path_comment = (
        "# Seraphiel Brain — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SERAPHIEL_HOME under
      ``backups/`` (restorable via ``seraphiel import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from seraphiel_cli.backup import create_quick_snapshot
    ...
    try:
        from seraphiel_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_seraphiel_home so the user sees ~/.seraphiel/...
    try:
        from seraphiel_constants import get_seraphiel_home, display_seraphiel_home

        home = get_seraphiel_home()
        try:
            display_path = f"{display_seraphiel_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  seraphiel import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``seraphiel update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``seraphiel update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run seraphiel from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.seraphiel-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".seraphiel-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The seraphiel.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m seraphiel_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``seraphiel
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m seraphiel_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "seraphiel_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Seraphiel processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Seraphiel Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Seraphiel desktop app / other Seraphiel terminals, then re-run:"
    )
    lines.append("    seraphiel update")
    lines.append("  (or use `seraphiel update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    seraphiel.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from seraphiel_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from seraphiel_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Seraphiel...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m seraphiel_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: seraphiel gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``seraphiel gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from seraphiel_cli import gateway_windows
        from seraphiel_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``seraphiel update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Seraphiel Brain to the latest version.
    ...
    if is_managed():
        managed_error("update Seraphiel Brain")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `seraphiel update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Seraphiel via pip (for PyPI installs)."""
    from seraphiel_cli import __version__
    from seraphiel_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "seraphiel-brain"]
        ...
        cmd = [pipx, "upgrade", "seraphiel-brain"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "seraphiel-brain"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "seraphiel-brain"]
    ...
    print("✓ Update complete! Restart seraphiel to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_seraphiel_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `seraphiel serve` backend) will keep .pyd
    ...
    # the seraphiel.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Seraphiel windows/gateways and re-run: seraphiel update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # orphan merge-conflict markers in seraphiel_cli/config.py bricked
        # every user who ran ``seraphiel update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``seraphiel
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``seraphiel update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_seraphiel_home added to seraphiel_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``seraphiel`` launch finishes the
        ...
        # build.  ``seraphiel desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``seraphiel desktop`` shouldn't be forced into a full
        # Electron build by ``seraphiel update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "seraphiel_cli.main", "desktop", "--build-only"]
            ...
            # debuggable.
            #
            # Start the build subprocess with the Seraphiel-managed Node on PATH:
            # when `seraphiel update` runs inside the desktop updater chain
            # (Desktop → seraphiel-setup → seraphiel update), the shell PATH
            # customizations are lost, so a bare-PATH child would fail with
            # `node: not found` before cmd_gui can self-heal.
            from seraphiel_constants import with_seraphiel_node_path

            _build_env = with_seraphiel_node_path()
            build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT, env=_build_env)
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT, env=_build_env)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `seraphiel desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from seraphiel_constants import display_seraphiel_home as _dhh
        ...
        # current — so copy it straight over ~/.seraphiel/cache/model_catalog.json
        ...
        # modules in this process.  Reload seraphiel_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_seraphiel_home() added since the last release.
        try:
            import importlib
            import seraphiel_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SERAPHIEL_HOME so
        # it is not affected by sync_skills()'s module-level SERAPHIEL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SERAPHIEL_HOME env var points at the default or a named profile.
        try:
            from seraphiel_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from seraphiel_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'seraphiel config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: seraphiel config migrate")
            else:
                print()
                print("Skipped. Run 'seraphiel config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from seraphiel_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `seraphiel curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``seraphiel update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from seraphiel_cli.tools_config import install_cua_driver
        ...
        # When running as ``seraphiel update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_seraphiel_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... seraphiel-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... seraphiel-gateway*)
            ...
            try:
                from seraphiel_constants import (
            ...
            # Discover all seraphiel-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. seraphiel-gateway.service or seraphiel-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `seraphiel update` restart it automatically, allow\n"
                            ...
                            # path in `seraphiel gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: seraphiel gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: seraphiel gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: seraphiel -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Seraphiel gateway unit files are still installed.
        # When both seraphiel.service (from a pre-rename install) and the
        # current seraphiel-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `seraphiel update` surfaces the issue until the user migrates.
        try:
            from seraphiel_cli.gateway import (
                has_legacy_seraphiel_units,
                _find_legacy_seraphiel_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_seraphiel_units():
                print()
                print("⚠ Legacy Seraphiel gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_seraphiel_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (seraphiel.service) fight the current")
                print("  seraphiel-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    seraphiel gateway migrate-legacy")
                print()
        ...
        print("  seraphiel model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``seraphiel -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from seraphiel_constants import display_seraphiel_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `seraphiel profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_seraphiel_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → seraphiel -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.seraphiel)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  seraphiel profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     seraphiel -p {name} chat")
                else:
        ...
        from seraphiel_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from seraphiel_constants import get_seraphiel_home as _hh
        ...
        from seraphiel_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `seraphiel profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → seraphiel -p {name}  ({wrapper})")
        print()
        ...
        from seraphiel_cli.profiles import profile_exists, validate_alias_name
        ...
        from seraphiel_cli.profiles import rename_profile
        ...
        from seraphiel_cli.profiles import export_profile
        ...
        from seraphiel_cli.profiles import import_profile
        ...
        import tempfile
        from seraphiel_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="seraphiel_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  seraphiel -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      seraphiel -p {plan.manifest.name} chat")
        ...
        from seraphiel_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `seraphiel profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"seraphiel -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from seraphiel_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("seraphiel_requires"):
            print(f"Requires:     Seraphiel {data['seraphiel_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from seraphiel_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.seraphiel_requires:
        print(f"  Requires: Seraphiel {mf.seraphiel_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``seraphiel dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``seraphiel dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No seraphiel dashboard processes running.")
        return 0

    print(f"{len(pids)} seraphiel dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``seraphiel dashboard register`` for OAuth.
    ...
    try:
        from seraphiel_cli.web_server import should_require_auth
    ...
    try:
        from seraphiel_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `seraphiel dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    seraphiel dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "SERAPHIEL_DASHBOARD_OAUTH_CLIENT_ID into ~/.seraphiel/.env for you.\n"
            "  Docs: https://seraphiel-brain.embreythecreator.com/docs/"
    ...
    try:
        from seraphiel_cli.config import load_config, save_config
        from seraphiel_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
    ...
    try:
        from seraphiel_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No seraphiel dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `seraphiel update`.
    ...
    # launches the dashboard (`worker dashboard` → SERAPHIEL_HOME points into
    ...
    try:
        from seraphiel_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("SERAPHIEL_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "seraphiel_cli.main",
            "-p", "default",
        ...
        # SERAPHIEL_HOME.  We must resolve the root explicitly instead of just
        # dropping SERAPHIEL_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV SERAPHIEL_HOME=/opt/data`), so an unset
        # SERAPHIEL_HOME falls back to $HOME/.seraphiel = /opt/data/.seraphiel — an
        ...
        # update-button guard also misfires).  get_default_seraphiel_root()
        # returns the root for both layouts: ~/.seraphiel for a standard install
        ...
        try:
            from seraphiel_constants import get_default_seraphiel_root
            env["SERAPHIEL_HOME"] = str(get_default_seraphiel_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop SERAPHIEL_HOME) rather than block the reroute.
            env.pop("SERAPHIEL_HOME", None)
    ...
    # the same logs directory as every other Seraphiel surface.
    try:
        from seraphiel_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["SERAPHIEL_SERVE_HEADLESS"] = "1"
    elif "SERAPHIEL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SERAPHIEL_WEB_DIST"])
            if "SERAPHIEL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "seraphiel_cli" / "web_dist"
        )
        ...
        # SERAPHIEL_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["SERAPHIEL_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ SERAPHIEL_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset SERAPHIEL_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads SERAPHIEL_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["SERAPHIEL_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from SERAPHIEL_WEB_DIST: {_dist_root}")
    ...
    try:
        from seraphiel_cli.mcp_startup import start_background_mcp_discovery
    ...
    from seraphiel_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from seraphiel_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from seraphiel_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from seraphiel_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from seraphiel_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Seraphiel log files."""
    from seraphiel_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Seraphiel command console."""
    from seraphiel_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from seraphiel_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``seraphiel -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``seraphiel_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``seraphiel -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``seraphiel`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SERAPHIEL_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that SERAPHIEL_YOLO_MODE is set before ANY
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["SERAPHIEL_SAFE_MODE"] = "1"
    os.environ["SERAPHIEL_IGNORE_USER_CONFIG"] = "1"
    os.environ["SERAPHIEL_IGNORE_RULES"] = "1"
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from seraphiel_cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SERAPHIEL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SERAPHIEL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SERAPHIEL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `seraphiel --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_seraphiel_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_seraphiel_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_seraphiel_home()}/memories/\n")
    else:
        from seraphiel_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Seraphiel Brain as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from seraphiel_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from seraphiel_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from seraphiel_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from seraphiel_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from seraphiel_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from seraphiel_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from seraphiel_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from seraphiel_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from seraphiel_cli.claw import claw_command

    claw_command(args)


def cmd_absorb(args) -> int:
    """`seraphiel absorb` — fold a new upstream release into the fork.
    ...
    try:
        if args.status:
            # Live-tree marker scan first: the running install must NEVER
            # carry conflict markers, in-flight absorb or not (a past absorb
            # bricked the CLI exactly this way — SyntaxError at import).
            marked = subprocess.run(
                ["git", "-C", repo, "grep", "-lI", "-E",
                 "-e", r"^<<<<<<< ", "-e", r"^>>>>>>> "],
                capture_output=True, text=True).stdout.splitlines()
            if marked:
                print(f"  ✗ LIVE TREE BROKEN — conflict markers in "
                      f"{len(marked)} files (the running install!):")
                for f in marked[:10]:
                    print(f"      {f}")
            ...
            wt = driver.worktree_path(repo)
            if os.path.isdir(wt):
                print(f"  sidecar worktree: {wt}")
        ...
        if args.cont:
            ...
            print(f"  ✓ merge materialized in sidecar worktree {wt} — resolve "
                  f"conflicts THERE (live tree untouched), then "
                  f"`seraphiel absorb --verify`")
            return 0
        if args.verify:
...
def main():
    ...
    try:
        from seraphiel_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``seraphiel.exe.old.*`` quarantine files left by previous
    # ``seraphiel update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_seraphiel_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``seraphiel update``
    ...
    # ``seraphiel skills install update``) merely defers recovery one launch;
    # under-matching (missing ``seraphiel -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in seraphiel_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from seraphiel_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from seraphiel_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://seraphiel-brain.embreythecreator.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `seraphiel model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.seraphiel/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://seraphiel-brain.embreythecreator.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from seraphiel_cli import secrets_cli as _secrets_cli
    from seraphiel_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from seraphiel_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in seraphiel_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in seraphiel_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in seraphiel_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in seraphiel_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `seraphiel whatsapp` which sets up "
    ...
    # =========================================================================
    # absorb command — fold a new upstream release into the fork
    #   (maintainer / git-source installs only; dry by default; never main)
    # =========================================================================
    absorb_parser = subparsers.add_parser(
        "absorb",
        help="Absorb a new upstream release into the fork (maintainer/git installs)",
        description=(
            "Fold a new upstream release into Seraphiel's own core via the "
    ...
    # =========================================================================
    from seraphiel_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in seraphiel_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in seraphiel_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in seraphiel_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in seraphiel_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in seraphiel_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in seraphiel_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from seraphiel_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from seraphiel_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from seraphiel_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in seraphiel_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in seraphiel_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in seraphiel_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in seraphiel_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in seraphiel_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in seraphiel_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.seraphiel/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo seraphiel uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from seraphiel_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in seraphiel_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in seraphiel_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in seraphiel_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in seraphiel_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in seraphiel_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from seraphiel_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in seraphiel_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``seraphiel --help``, ``seraphiel version``, ``seraphiel logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from seraphiel_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from seraphiel_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Seraphiel shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from seraphiel_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from seraphiel_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in seraphiel_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in seraphiel_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `seraphiel computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `seraphiel tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `seraphiel computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Seraphiel app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from seraphiel_cli.tools_config import _cua_driver_cmd
            # Honor SERAPHIEL_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from seraphiel_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: seraphiel computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: seraphiel computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: seraphiel computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: seraphiel computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: seraphiel computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: seraphiel computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in seraphiel_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <seraphiel home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from seraphiel_state import workspace_key as _ws_key
            ...
            from seraphiel_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from seraphiel_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from seraphiel_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_seraphiel_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from seraphiel_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_seraphiel_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_seraphiel_home() / "sessions"
            ...
            # `seraphiel sessions prune`: no time window and no filters at all
            ...
            # Launch seraphiel --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in seraphiel_cli/subcommands/insights.py)
    # =========================================================================
    ...
    # =========================================================================
    # update command  (parser built in seraphiel_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in seraphiel_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in seraphiel_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in seraphiel_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in seraphiel_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Seraphiel-Setup.exe success screen tells users to
    # run `seraphiel desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in seraphiel_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in seraphiel_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in seraphiel_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``seraphiel -c Pokemon Agent Dev`` → ``seraphiel -c 'Pokemon Agent Dev'``
    ...
    from seraphiel_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'seraphiel -c model' where 'model' is consumed as the
    ...
    # --yolo: set SERAPHIEL_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (seraphiel hooks list, cron
