...
def cmd_gui(args: argparse.Namespace):
    ...
    print(f"→ Launching packaged Seraphiel Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``seraphiel dashboard`` as
...
def _seraphiel_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "seraphiel.exe",
        scripts_dir / "seraphiel-gateway.exe",
    ]
...
def _run_pre_update_backup(args) -> None:
    ...
    # ``seraphiel_cli/config.py``). Fall back to true if the key is missing
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from seraphiel_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Tying the refresh to ``seraphiel update`` gives users a predictable
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from seraphiel_cli.profiles import profile_exists
