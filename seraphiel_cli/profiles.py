"""
Profile management for multiple isolated Seraphiel instances.

Each profile is a fully independent SERAPHIEL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.seraphiel/profiles/<name>/`` by default.

The "default" profile is ``~/.seraphiel`` itself — backward compatible,
zero migration needed.

Usage::

    seraphiel profile create coder          # fresh profile + bundled skills
    seraphiel profile create coder --clone  # also copy config, .env, SOUL.md, skills
    seraphiel profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    seraphiel -p coder chat                 # or via flag
    seraphiel profile use coder             # set as sticky default
    seraphiel profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See seraphiel_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.seraphiel``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   seraphiel-brain  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `seraphiel backup` archives
...
# Marker file written by `seraphiel profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `seraphiel
...
# `seraphiel skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Seraphiel infrastructure directories that only the default profile
         (``~/.seraphiel``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_seraphiel_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.seraphiel) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "seraphiel-brain",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SERAPHIEL_HOME equals the
...
# into the export. The set below identifies the *known Seraphiel profile
# artifacts* at the root of SERAPHIEL_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``seraphiel_constants``.
...
_RESERVED_NAMES = frozenset({
    "seraphiel", "default", "test", "tmp", "root", "sudo",
})

# Seraphiel subcommands that cannot be used as profile names/aliases
_SERAPHIEL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the seraphiel root, NOT to the current SERAPHIEL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SERAPHIEL_HOME points outside
    ``~/.seraphiel``, profiles live under ``SERAPHIEL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_seraphiel_home() / "profiles"


def _get_default_seraphiel_home() -> Path:
    """Return the default (pre-profile) SERAPHIEL_HOME path.

    In standard deployments this is ``~/.seraphiel``.
    In Docker/custom deployments where SERAPHIEL_HOME is outside ``~/.seraphiel``
    (e.g. ``/opt/data``), returns SERAPHIEL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_seraphiel_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Seraphiel installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SERAPHIEL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_seraphiel_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, seraphiel subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SERAPHIEL_SUBCOMMANDS:
        return f"'{canon}' conflicts with a seraphiel subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            seraphiel_exe = shutil.which("seraphiel") or "seraphiel"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(seraphiel_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Seraphiel. If we
    ...
    try:
        ...
        from seraphiel_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `seraphiel doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``seraphiel -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``seraphiel profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``seraphiel -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``seraphiel profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SERAPHIEL_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Seraphiel config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_seraphiel_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, seraphiel_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SERAPHIEL_HOME.
    ...
    The returned ``seraphiel_home`` is the path to pass to
    ``set_seraphiel_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_seraphiel_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``seraphiel update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.seraphiel)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.seraphiel/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `seraphiel -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Seraphiel profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `seraphiel update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`seraphiel profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Seraphiel (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `seraphiel profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `seraphiel -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SERAPHIEL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``seraphiel profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SERAPHIEL_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_seraphiel_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Seraphiel *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Seraphiel backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SERAPHIEL_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `seraphiel -p <canon> profile
    ...
    seraphiel_markers = ("seraphiel_cli.main", "seraphiel-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Seraphiel process: either an entrypoint marker in argv, or
            # a resolved executable named `seraphiel`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_seraphiel = (
                any(marker in joined for marker in seraphiel_markers)
                or exe_name == "seraphiel"
                or exe_name.startswith("seraphiel")
            )
            if not is_seraphiel:
                continue
            ...
            # ...or by SERAPHIEL_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SERAPHIEL_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.seraphiel).\n"
            "To remove everything, use: seraphiel uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_seraphiel_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SERAPHIEL_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from seraphiel_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SERAPHIEL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SERAPHIEL_HOME")
    try:
        os.environ["SERAPHIEL_HOME"] = str(profile_dir)
        from seraphiel_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SERAPHIEL_HOME"] = old_home
        elif "SERAPHIEL_HOME" in os.environ:
            del os.environ["SERAPHIEL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.seraphiel/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: seraphiel profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SERAPHIEL_HOME.

    Returns ``"default"`` if SERAPHIEL_HOME is not set or points to ``~/.seraphiel``.
    Returns the profile name if SERAPHIEL_HOME points into ``~/.seraphiel/profiles/<name>``.
    Returns ``"custom"`` if SERAPHIEL_HOME is set to an unrecognized path.
    """
    from seraphiel_constants import get_seraphiel_home
    seraphiel_home = get_seraphiel_home()
    resolved = seraphiel_home.resolve()

    default_resolved = _get_default_seraphiel_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SERAPHIEL_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Seraphiel file the
      user may have lying alongside SERAPHIEL_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.seraphiel itself — its parent is ~/ and its
        # directory name is ".seraphiel", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: seraphiel profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.seraphiel itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.seraphiel). "
            "Specify a different name: seraphiel profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="seraphiel_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"seraphiel_{old_name}"
    legacy_old_host = f"seraphiel.{old_name}"
    new_host = f"seraphiel_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_seraphiel_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("seraphiel_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SERAPHIEL_HOME path string.

    Called early in the CLI entry point, before any seraphiel modules
    are imported, to set the SERAPHIEL_HOME environment variable.
    """
