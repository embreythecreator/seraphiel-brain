"""``seraphiel debug`` debug tools for Seraphiel Brain.

Currently supports:
    seraphiel debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.seraphiel/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[seraphiel debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.seraphiel/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``seraphiel debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``seraphiel debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `seraphiel debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "seraphiel-brain/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``seraphiel debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.seraphiel/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``seraphiel debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "seraphiel-brain/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----SeraphielDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from seraphiel_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_seraphiel_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``seraphiel debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``seraphiel dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``seraphiel dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "seraphiel-debug-share/1"
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    ...
    The JSON shape is what the discord-support viewer (Repo 3) parses::
        ...
        {"format": "seraphiel-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``seraphiel debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "seraphiel debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `seraphiel debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  seraphiel debug delete <url>")

    print(f"\nShare these links with the Seraphiel team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Seraphiel version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``seraphiel debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from seraphiel_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "seraphiel debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `seraphiel debug share --local` to print the report instead, "
            "or `seraphiel debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: seraphiel debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'seraphiel debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``seraphiel debug`` call.
    ...
    # best-effort — any failure is swallowed so ``seraphiel debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: seraphiel debug <command>")
        print()
        print("Commands:")
