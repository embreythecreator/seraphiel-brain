...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model Seraphiel agent
...
def resolve_moa_preset(config: Any, name: str | None = None) -> dict[str, Any]:
    ...
    if preset is None:
        ...
        raise MoAPresetNotFoundError(
            ...
            f"{available}. Run `seraphiel moa list`."
        )
...
def exact_moa_preset_name(config: Any, text: str) -> str | None:
    ...
    ``seraphiel_cli/model_switch.py``) to recognize a bare ``/model <preset>``
