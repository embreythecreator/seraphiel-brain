...
- Keep data: Remove code but keep ~/.seraphiel/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Seraphiel PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing seraphiel-brain or seraphiel PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Seraphiel Brain" comment and following line
                if '# Seraphiel Brain' in line or '# seraphiel-brain' in line:
                    skip_next = True
                    continue
                if skip_next and ('seraphiel' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing seraphiel
                if 'seraphiel' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the seraphiel wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "seraphiel",
        Path("/usr/local/bin/seraphiel"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains seraphiel_cli reference)
                content = wrapper.read_text()
                if 'seraphiel_cli' in content or 'seraphiel-brain' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(seraphiel_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``seraphiel`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Seraphiel home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (seraphiel_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``seraphiel gateway run`` processes
    ...
    try:
        from seraphiel_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from seraphiel_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SERAPHIEL_HOME`` and ``SERAPHIEL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\seraphiel\git\cmd``, ``%LOCALAPPDATA%\seraphiel\git\bin``,
#      ``%LOCALAPPDATA%\seraphiel\git\usr\bin``, ``%LOCALAPPDATA%\seraphiel\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\seraphiel\git\`` and Node to
#      ``%LOCALAPPDATA%\seraphiel\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``seraphiel dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\seraphiel\gateway-service\`` and sometimes
...
def _seraphiel_path_markers(seraphiel_home: Path) -> list[str]:
    """Path-entry substrings that identify Seraphiel-owned User-PATH entries."""
    root = str(seraphiel_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare seraphiel-brain install dir.
    markers = [root + "\\seraphiel-brain", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SERAPHIEL_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "seraphiel".  We don't want to catch
    # unrelated entries like "cseraphiel-foo" or "ephermeral", so we look for
    # backslash-seraphiel as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(seraphiel_home: Path) -> list[str]:
    """Strip Seraphiel-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _seraphiel_path_markers(seraphiel_home)
            kept: list[str] = []
...
def remove_seraphiel_env_vars_windows() -> list[str]:
    """Delete SERAPHIEL_HOME and SERAPHIEL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SERAPHIEL_HOME", "SERAPHIEL_GIT_BASH_PATH"):
                try:
...
def _is_default_seraphiel_home(seraphiel_home: Path) -> bool:
    """Return True when ``seraphiel_home`` points at the default (non-profile) root."""
    try:
        from seraphiel_constants import get_default_seraphiel_root
        return seraphiel_home.resolve() == get_default_seraphiel_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SERAPHIEL_HOME directory.

    We shell out to ``seraphiel -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SERAPHIEL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m seraphiel_cli.main` so we don't depend on a `seraphiel`
    #    wrapper that may be half-removed mid-uninstall.
    seraphiel_invocation = [_sys.executable, "-m", "seraphiel_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                seraphiel_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SERAPHIEL_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``seraphiel uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SERAPHIEL_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from seraphiel_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(seraphiel_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ✶ Seraphiel Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Seraphiel Chat GUI installation was found.")
        print(f"  Checked: {seraphiel_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Seraphiel agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(seraphiel_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Seraphiel agent at {seraphiel_home / 'seraphiel-brain'}")
        print(f"  • Your config, sessions, and secrets under {seraphiel_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Seraphiel agent is still installed. Run 'seraphiel' to use the CLI,")
    print("or 'seraphiel uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.seraphiel/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.seraphiel/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie SERAPHIEL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_seraphiel_home(seraphiel_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.seraphiel data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {seraphiel_home / 'config.yaml'}")
    print(f"  Secrets: {seraphiel_home / '.env'}")
    print(f"  Data:    {seraphiel_home / 'cron/'}, {seraphiel_home / 'sessions/'}, {seraphiel_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SERAPHIEL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Seraphiel data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Seraphiel code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, seraphiel_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Seraphiel PATH entries from shell configs / Windows User PATH")
    print("  • Seraphiel wrapper scripts and Seraphiel-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Seraphiel config/data: {seraphiel_home}")
        if _is_default_seraphiel_home(seraphiel_home):
        ...
        print(f"  • Keep Seraphiel config/data: {seraphiel_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``seraphiel`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SERAPHIEL_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in seraphiel_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\seraphiel\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(seraphiel_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Seraphiel-owned PATH entries in User environment")

        log_info("Removing SERAPHIEL_HOME / SERAPHIEL_GIT_BASH_PATH User env vars...")
        removed_env = remove_seraphiel_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Seraphiel-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing seraphiel command...")
    ...
    #     (only when they still point into this Seraphiel home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Seraphiel-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(seraphiel_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Seraphiel-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(seraphiel_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SERAPHIEL_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from seraphiel_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(seraphiel_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.seraphiel/, just remove the seraphiel-brain subdir
            if seraphiel_home in project_root.parents or project_root.parent == seraphiel_home:
    ...
    #     under SERAPHIEL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(seraphiel_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(seraphiel_home)
    ...
    # 5. Optionally remove ~/.seraphiel/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SERAPHIEL_HOME dirs live under
        ...
        try:
            if seraphiel_home.exists():
                shutil.rmtree(seraphiel_home)
                log_success(f"Removed {seraphiel_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {seraphiel_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {seraphiel_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {seraphiel_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://seraphiel-brain.embreythecreator.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://seraphiel-brain.embreythecreator.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m seraphiel_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import seraphiel_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``seraphiel_constants`` + ``seraphiel_cli.colors``
    (and lazily ``seraphiel_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m seraphiel_cli.uninstall")
    parser.add_argument(
        "--mode",
