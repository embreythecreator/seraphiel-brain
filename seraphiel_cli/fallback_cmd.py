"""
seraphiel fallback — manage the fallback provider chain.
...
Subcommands:
  seraphiel fallback [list]   Show the current fallback chain (default when no subcommand)
  seraphiel fallback add      Pick provider + model via the same picker as `seraphiel model`,
                           then append the selection to the chain
  seraphiel fallback remove   Pick an entry to delete from the chain
  seraphiel fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.seraphiel/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from seraphiel_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `seraphiel model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  seraphiel fallback add")
        print()
        return
    ...
    print("  Docs: https://seraphiel-brain.embreythecreator.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `seraphiel model`, then append the selection to the chain."""
    from seraphiel_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `seraphiel model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `seraphiel fallback list` to view, or `seraphiel fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``seraphiel fallback [subcommand]``."""
