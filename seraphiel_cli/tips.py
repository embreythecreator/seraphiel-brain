...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or seraphiel -c.",
    ...
    "Ctrl+Z suspends Seraphiel to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "seraphiel -c resumes your most recent CLI session. seraphiel -c \"project name\" resumes by title.",
    "seraphiel -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "seraphiel -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "seraphiel chat -t web,terminal enables only specific toolsets for a focused session.",
    "seraphiel chat -s github-pr-workflow preloads a skill at launch.",
    "seraphiel chat -q \"query\" runs a single non-interactive query and exits.",
    "seraphiel chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "seraphiel chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "seraphiel --yolo bypasses all dangerous command approval prompts for the entire session.",
    "seraphiel chat --source telegram tags the session for filtering in seraphiel sessions list.",
    "seraphiel -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "seraphiel doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "seraphiel dump outputs a compact setup summary — great for bug reports.",
    "seraphiel config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "seraphiel config edit opens config.yaml in your default editor.",
    "seraphiel config check scans for missing or stale configuration options.",
    "seraphiel sessions browse opens an interactive session picker with search.",
    "seraphiel sessions stats shows session counts by platform and database size.",
    "seraphiel sessions prune --older-than 30 cleans up old sessions.",
    "seraphiel skills search react --source skills-sh searches the skills.sh public directory.",
    "seraphiel skills check scans installed hub skills for upstream updates.",
    "seraphiel skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "seraphiel skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "seraphiel mcp add github --command npx adds MCP servers from the command line.",
    "seraphiel mcp serve runs Seraphiel itself as an MCP server for other agents.",
    "seraphiel auth add lets you add multiple API keys for credential pool rotation.",
    "seraphiel completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "seraphiel logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "seraphiel backup creates a zip backup of your entire Seraphiel home directory.",
    "seraphiel profile create coder creates an isolated profile that becomes its own command.",
    "seraphiel profile create work --clone copies your current config and keys to a new profile.",
    "seraphiel update syncs new bundled skills to ALL profiles automatically.",
    "seraphiel gateway install sets up Seraphiel as a system service (systemd/launchd).",
    "seraphiel memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "seraphiel webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: seraphiel tools disables unused tools, seraphiel skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "seraphiel models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as seraphiel -w).",
    ...
    "SERAPHIEL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Seraphiel tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'seraphiel profile create coder' creates the 'coder' command.",
    "seraphiel profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Seraphiel prints a resume command with session ID and stats.",
    "seraphiel sessions export backup.jsonl exports all sessions for backup or analysis.",
    "seraphiel -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "seraphiel skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .seraphiel/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: seraphiel cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Seraphiel runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "seraphiel gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Seraphiel via a local macOS server.",
    ...
    "Context files (.seraphiel.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Seraphiel loads project context from .seraphiel.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "seraphiel mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "seraphiel mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "seraphiel mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — seraphiel tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.seraphiel/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "seraphiel chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "seraphiel plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "seraphiel acp runs Seraphiel as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SERAPHIEL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "seraphiel auth add nous or seraphiel auth add openai-codex sets up OAuth-based providers.",
    ...
    "seraphiel status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.seraphiel/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Seraphiel your own.",
    ...
    "Before a gateway session reset, Seraphiel auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.seraphiel/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Seraphiel state via SERAPHIEL_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "SERAPHIEL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SERAPHIEL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.seraphiel/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'seraphiel profile create ls' would be rejected.",
    "seraphiel profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "seraphiel claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SERAPHIEL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via seraphiel tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.seraphiel/.env.",
    "Skills from trusted repos (embreythecreator) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.seraphiel/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Seraphiel auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Seraphiel config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Seraphiel is working.',
    ...
    '/reload reloads ~/.seraphiel/.env into the running session — pick up new API keys without restarting.',
    ...
    "SERAPHIEL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.seraphiel/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.seraphiel/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'seraphiel curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "seraphiel curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'seraphiel curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'seraphiel auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SERAPHIEL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SERAPHIEL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'seraphiel dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'seraphiel dashboard embeds the full Seraphiel TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.seraphiel/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.seraphiel/dashboard-plugins/ — no npm build required.',
    ...
    'SERAPHIEL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SERAPHIEL_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'SERAPHIEL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SERAPHIEL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.seraphiel/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.seraphiel/skills/ so drop-in skills appear without restarting the session.',
    ...
    'seraphiel -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'seraphiel chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'seraphiel chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'seraphiel chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "seraphiel chat --source tool tags programmatic chats so they don't clutter seraphiel sessions list.",
    'seraphiel dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'seraphiel sessions rename <ID> "new title" renames any past session; seraphiel sessions delete <ID> removes one.',
    'seraphiel import restores a full Seraphiel backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'seraphiel fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'seraphiel pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'seraphiel setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'seraphiel status --deep runs the full health sweep across every component; plain seraphiel status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SERAPHIEL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SERAPHIEL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.seraphiel/plugins/ — trust-gated by design.',
    "SERAPHIEL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SERAPHIEL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SERAPHIEL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SERAPHIEL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SERAPHIEL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SERAPHIEL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SERAPHIEL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SERAPHIEL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SERAPHIEL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SERAPHIEL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SERAPHIEL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Seraphiel block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from seraphiel sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.seraphiel/dashboard-plugins/.',
]
