...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the seraphiel args.

    The container PID 1 argv looks like
    ``/init /opt/seraphiel/docker/main-wrapper.sh <subcommand> [args...]`` and
    the wrapper re-execs ``seraphiel <subcommand>``. Peel ``init`` →
    ``main-wrapper.sh`` → ``seraphiel`` so callers can match on the bare
