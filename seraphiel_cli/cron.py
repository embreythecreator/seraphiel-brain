"""
Cron subcommand for seraphiel CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(seraphiel\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*seraphiel)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*seraphiel)"
    r"|(p?kill\s+.*seraphiel.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'seraphiel cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: seraphiel gateway install", Colors.DIM))
        print(color("                    sudo seraphiel gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    seraphiel gateway install    # Install as a user service")
        print("    sudo seraphiel gateway install --system  # Linux servers: boot-time system service")
        print("    seraphiel gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `seraphiel gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: seraphiel cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
