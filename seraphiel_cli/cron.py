...
# commands at execution time when ``_SERAPHIEL_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Check status:  seraphiel cron status", Colors.DIM))
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: seraphiel gateway restart")
...
def cron_command(args):
    ...
    print("Usage: seraphiel cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
