...
Profile-aware location: ``$SERAPHIEL_HOME/logs/dashboard-auth.log``.
...
from ``seraphiel_constants`` or other seraphiel_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SERAPHIEL_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``seraphiel_constants.get_seraphiel_home`` semantics: env var wins,
    else ``~/.seraphiel``. A local copy avoids an import cycle with the
    middleware which lives below ``seraphiel_cli``.
    """
    home = os.environ.get("SERAPHIEL_HOME") or str(Path.home() / ".seraphiel")
