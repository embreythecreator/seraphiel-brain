...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `seraphiel update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://seraphiel-brain.embreythecreator.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under SERAPHIEL_HOME, and print
    ...
    """
    try:
        from seraphiel_cli.slack_cli import _build_full_manifest
        from seraphiel_constants import get_seraphiel_home

        manifest = _build_full_manifest(
            bot_name="Seraphiel",
            bot_description="Your Seraphiel agent on Slack",
        )
        target = Path(get_seraphiel_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `seraphiel slack manifest --write` anytime to refresh after "
            "Seraphiel adds new commands."
        )
        ...
        print_info(
            ...
            "seraphiel slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Seraphiel delivers cron job results and notifications.")
    ...
    if any_messaging:
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo seraphiel gateway install --system")
