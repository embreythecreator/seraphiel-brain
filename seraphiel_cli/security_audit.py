"""On-demand supply-chain audit for Seraphiel Brain installs.

Scans three surfaces a Seraphiel user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Seraphiel venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.seraphiel/plugins``
...
def _discover_plugins(seraphiel_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.seraphiel/plugins``.
...
def run_audit(
    *,
    ...
    home = seraphiel_home or Path(get_seraphiel_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, seraphiel_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(seraphiel_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `seraphiel security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, seraphiel_home=home
    )
    if total == 0:
