"""seraphiel hooks — inspect and manage shell-script hooks.

Usage::

    seraphiel hooks list
    seraphiel hooks test <event> [--for-tool X] [--payload-file F]
    seraphiel hooks revoke <command>
    seraphiel hooks doctor

Consent records live under ``~/.seraphiel/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.seraphiel/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``seraphiel hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: seraphiel hooks {list|test|revoke|doctor}")
        print("Run 'seraphiel hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.seraphiel/config.yaml.")
        print("See `seraphiel hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `seraphiel hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `seraphiel hooks test` and `seraphiel hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Seraphiel wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `seraphiel hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `seraphiel hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `seraphiel hooks doctor`.")
