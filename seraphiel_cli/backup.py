"""
Backup and import commands for seraphiel CLI.

`seraphiel backup` creates a zip archive of the entire ~/.seraphiel/ directory
(excluding the seraphiel-brain repo and transient files).

`seraphiel import` restores from a backup zip, overlaying onto the current
SERAPHIEL_HOME root.
"""
...
from seraphiel_constants import get_default_seraphiel_root, get_seraphiel_home, display_seraphiel_home
...
_EXCLUDED_DIRS = {
    "seraphiel-brain",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to seraphiel root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SERAPHIEL_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Seraphiel home directory."""
    seraphiel_root = get_default_seraphiel_root()

    if not seraphiel_root.is_dir():
        print(f"Error: Seraphiel home directory not found at {seraphiel_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"seraphiel-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"seraphiel-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_seraphiel_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(seraphiel_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(seraphiel_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(seraphiel_root)
    ...
    print(f"\nRestore with: seraphiel import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Seraphiel backup.
    ...
    # Look for telltale files that a seraphiel home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Seraphiel backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.seraphiel/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a seraphiel dir name
        if prefix in {".seraphiel", "seraphiel"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_seraphiel_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (seraphiel_root / "config.yaml").exists()
        has_env = (seraphiel_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Seraphiel configuration.")
        ...
        for member in members:
            ...
            target = seraphiel_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(seraphiel_root.resolve())
            except ValueError:
        ...
        profiles_dir = seraphiel_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # seraphiel_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: seraphiel profile list  (after installing seraphiel)")

        # Guidance
        print()
        if not (seraphiel_root / "seraphiel-brain").is_dir():
            print("Note: The seraphiel-brain codebase was not included in the backup.")
            print("  If this is a fresh install, run: seraphiel update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  seraphiel -p {pname} gateway install")

        print("Done. Your Seraphiel configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and seraphiel backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SERAPHIEL_HOME).
...
# — `seraphiel update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(seraphiel_home: Optional[Path] = None) -> Path:
    home = seraphiel_home or get_seraphiel_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    seraphiel_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(seraphiel_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SERAPHIEL_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``seraphiel update``.
    ...
    Args:
        ...
        seraphiel_home: Override for the Seraphiel home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(seraphiel_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for seraphiel backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_seraphiel_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, seraphiel_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``seraphiel_root`` to ``out_path``.
...
def _pre_update_backup_dir(seraphiel_home: Optional[Path] = None) -> Path:
    home = seraphiel_home or get_seraphiel_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SERAPHIEL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SERAPHIEL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``seraphiel update``) should continue even if the backup fails.
    """
    seraphiel_root = seraphiel_home or get_default_seraphiel_root()
    if not seraphiel_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(seraphiel_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, seraphiel_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `seraphiel claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SERAPHIEL_HOME under ``backups/`` before a
    ``seraphiel claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``seraphiel import <archive>``.  Writes to
    ``<SERAPHIEL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `seraphiel import` and the
