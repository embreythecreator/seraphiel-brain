...
# ``seraphiel-brain`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/seraphiel-brain/``
...
# SERAPHIEL_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under SERAPHIEL_HOME) —
...
# File names that ``seraphiel import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE SERAPHIEL_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SERAPHIEL_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``seraphiel-brain`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/seraphiel-brain/`` — must be preserved.
        if part == "seraphiel-brain" and part != parts[0]:
            continue
        return True
...
def run_backup(args) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(seraphiel_root, followlinks=False):
        ...
        # ``seraphiel-brain`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "seraphiel-brain" and not is_root)
        ]
    ...
    # outside SERAPHIEL_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_seraphiel_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SERAPHIEL_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_seraphiel_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: seraphiel profile list  (after installing seraphiel)")
...
_QUICK_STATE_FILES = (
    ...
    # are at $SERAPHIEL_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SERAPHIEL_HOME and the entry is silently
