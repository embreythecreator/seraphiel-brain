...
- ``seraphiel_cli.providers``        -- canonical provider identity + overlays
- ``seraphiel_cli.model_normalize``  -- per-provider name formatting
...
_SERAPHIEL_MODEL_WARNING = (
    "Seraphiel Seraphiel 3 & 4 models are NOT agentic and are not designed "
    "for use with Seraphiel Brain. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Seraphiel Seraphiel 3 / Seraphiel 4 chat families.
# The previous substring check (`"seraphiel" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``seraphiel-brain:qwen3-14b-ctx16k`` that just
# happen to carry "seraphiel" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   embreythecreator/seraphiel-3-Llama-3.1-70B, seraphiel-4-405b, openrouter/seraphiel3:70b
# Negative examples it must NOT match:
#   seraphiel-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_SERAPHIEL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])seraphiel[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_seraphiel_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Seraphiel 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_SERAPHIEL_NON_AGENTIC_RE.search(model_name))


def _check_seraphiel_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Seraphiel 3/4 chat model."""
    if is_nous_seraphiel_non_agentic(model_name):
        return _SERAPHIEL_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``seraphiel config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from seraphiel_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'seraphiel model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'seraphiel doctor' — config issues detected:"
        ...
        from seraphiel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from seraphiel_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from seraphiel_cli.config import load_config
            from seraphiel_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from seraphiel_cli.providers import resolve_user_provider as _ruser
    ...
    # seraphiel_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from seraphiel_cli.models import normalize_opencode_base_url
    ...
    seraphiel_warn = _check_seraphiel_model_warning(new_model)
    if seraphiel_warn:
        warnings.append(seraphiel_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from seraphiel_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from seraphiel_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from seraphiel_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against seraphiel_id / mdev_id (section 1), pid / seraphiel_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from seraphiel_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by seraphiel provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://seraphiel-brain.embreythecreator.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Seraphiel release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from seraphiel_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from seraphiel_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Seraphiel-mapped providers ---
    from seraphiel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from seraphiel_cli.models import _PROVIDER_ALIASES as _CANON_ALIASES
    from seraphiel_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for seraphiel_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # same seraphiel_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if seraphiel_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(seraphiel_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (SERAPHIEL_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Seraphiel may not route yet.
        ...
        from seraphiel_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(seraphiel_id):
            continue
        ...
        if not has_creds:
            try:
                from seraphiel_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(seraphiel_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        seraphiel_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `seraphiel model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(seraphiel_id)
        if not model_ids:
            model_ids = curated.get(seraphiel_id, [])
            if seraphiel_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(seraphiel_id, model_ids)
        ...
        results.append({
            ...
            "is_current": (
                ...
                or seraphiel_id == current_provider
    ...
    # --- 2. Check Seraphiel-only providers (nous, openai-codex, copilot, opencode-go) ---
    from seraphiel_cli.providers import SERAPHIEL_OVERLAYS
    from seraphiel_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Seraphiel provider ID.
    # SERAPHIEL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Seraphiel IDs ("copilot").
    _mdev_to_seraphiel = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in SERAPHIEL_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Seraphiel slug — e.g. "github-copilot" → "copilot"
        seraphiel_slug = _mdev_to_seraphiel.get(pid, pid)
        if seraphiel_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or seraphiel_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(seraphiel_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, seraphiel_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or seraphiel_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(seraphiel_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(seraphiel_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", seraphiel_slug, exc)
        ...
        # configured.
        if not has_creds and seraphiel_slug == "anthropic":
            try:
                ...
                if (seraphiel_creds and seraphiel_creds.get("accessToken")) or \
        ...
        if seraphiel_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(seraphiel_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(seraphiel_slug)
                model_ids = _ids if _ids else (curated.get(seraphiel_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(seraphiel_slug, []) or curated.get(pid, [])
        elif seraphiel_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `seraphiel model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from seraphiel_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(seraphiel_slug, []) or curated.get(pid, [])
                if seraphiel_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(seraphiel_slug, model_ids)
        total = len(model_ids)
        if seraphiel_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": seraphiel_slug,
            "name": get_label(seraphiel_slug),
            "is_current": seraphiel_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "seraphiel",
        ...
        seen_slugs.add(seraphiel_slug.lower())
        _record_builtin_endpoint(seraphiel_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SERAPHIEL_OVERLAYS (keeps /model in sync
    # with `seraphiel model`).
    try:
        from seraphiel_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Seraphiel writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Seraphiel's own writer uses (mirrors section-4 grouping).
        ...
        for grp in ep_groups.values():
            ...
            if should_probe:
                try:
                    from seraphiel_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Seraphiel's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, seraphiel_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from seraphiel_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`seraphiel_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from seraphiel_cli.models import fetch_openrouter_models
