"""``seraphiel logs`` — view and filter Seraphiel log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.seraphiel/logs/``.

Usage examples::

    seraphiel logs                    # last 50 lines of agent.log
    seraphiel logs -f                 # follow agent.log in real time
    seraphiel logs errors             # last 50 lines of errors.log
    seraphiel logs gateway -n 100    # last 100 lines of gateway.log
    seraphiel logs gui -f            # follow gui.log (dashboard/pty/ws)
    seraphiel logs desktop -f        # follow desktop.log (Electron app boot/backend)
    seraphiel logs --level WARNING    # only WARNING+ lines
    seraphiel logs --session abc123   # filter by session ID substring
    seraphiel logs --component tools  # only tool-related lines
    seraphiel logs --since 1h         # lines from the last hour
    seraphiel logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_seraphiel_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Seraphiel runs — try 'seraphiel chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_seraphiel_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_seraphiel_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_seraphiel_home()}/logs/")
        return

    print(f"Log files in {display_seraphiel_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'seraphiel chat' to generate logs)")
