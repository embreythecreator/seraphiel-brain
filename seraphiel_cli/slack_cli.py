"""``seraphiel slack ...`` CLI subcommands.

Today only ``seraphiel slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ seraphiel slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Seraphiel. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Seraphiel deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Seraphiel on Slack's older Assistant messaging
    ...
    """
    from seraphiel_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Seraphiel in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Seraphiel in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Seraphiel agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``seraphiel_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SERAPHIEL_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Seraphiel")
    ...
    """
    name = getattr(args, "name", None) or "Seraphiel"
    description = getattr(args, "description", None) or "Your Seraphiel agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("SERAPHIEL_HOME") or str(Path.home() / ".seraphiel")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Seraphiel app\n"
