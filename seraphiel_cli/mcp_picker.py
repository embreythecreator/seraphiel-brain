"""MCP picker — interactive `seraphiel mcp picker` (also the default `seraphiel mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``seraphiel mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `seraphiel plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from seraphiel_cli.cli_output import prompt_yes_no
from seraphiel_cli.curses_ui import curses_single_select
from seraphiel_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Seraphiel session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from seraphiel_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `seraphiel mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: seraphiel mcp install <name>    Picker: seraphiel mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Seraphiel is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Seraphiel — run `seraphiel update` "
...
def show_catalog() -> None:
    """`seraphiel mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`seraphiel mcp picker` (and default `seraphiel mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`seraphiel mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `seraphiel mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
