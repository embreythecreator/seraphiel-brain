...
def create_task(
    ...
    *,
    ...
    each name to ``seraphiel --skills ...`` alongside the built-in
...
def _kanban_worker_skill_available(seraphiel_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``seraphiel -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset SERAPHIEL_HOME means the worker falls back to the default root
    # home (``~/.seraphiel``), which ships the bundled skill.
    base = _Path(seraphiel_home) if seraphiel_home else (_Path.home() / ".seraphiel")
...
def _default_spawn(
    ...
    *,
    ...
    if _kanban_worker_skill_available(env.get("SERAPHIEL_HOME")):
