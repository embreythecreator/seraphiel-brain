...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    SERAPHIEL_HOME — whichever process grabs the lock first wins the tick.
    """
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    if not _local_dashboard_request(request) or _default_seraphiel_root_is_opt_data():
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_seraphiel_action(["gateway", "restart"], "gateway-restart"), False
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Seraphiel from Slack via Socket Mode.",
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Seraphiel resolves Anthropic creds in this order at runtime:
    1. ``~/.seraphiel/.anthropic_oauth.json`` — Seraphiel-managed PKCE flow
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Seraphiel-managed PKCE file
...
async def list_mcp_catalog():
    ...
    `seraphiel mcp catalog` / `seraphiel mcp install` read.
    """
    try:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_seraphiel_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
async def start_gateway():
    try:
        proc = _spawn_seraphiel_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_seraphiel_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_seraphiel_action(["import", archive], "import")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_seraphiel_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_seraphiel_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_seraphiel_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
def start_server(
    ...
    print(f"  Seraphiel Web UI → http://{host}:{port}")
