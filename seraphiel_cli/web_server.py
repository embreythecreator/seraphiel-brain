...
from seraphiel_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _spawn_seraphiel_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SERAPHIEL_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `seraphiel model`
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _SERAPHIEL_OAUTH_FILE,
        )
    except ImportError:
        read_seraphiel_oauth_credentials = None  # type: ignore
        _SERAPHIEL_OAUTH_FILE = None  # type: ignore
    ...
    if seraphiel_creds and seraphiel_creds.get("accessToken"):
        return {
            ...
            "source_label": f"Seraphiel PKCE ({_SERAPHIEL_OAUTH_FILE})",
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _SERAPHIEL_OAUTH_FILE
                if _SERAPHIEL_OAUTH_FILE.exists():
                    _SERAPHIEL_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _SERAPHIEL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _SERAPHIEL_OAUTH_FILE.with_name(
        f"{_SERAPHIEL_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _SERAPHIEL_OAUTH_FILE)
        try:
            _SERAPHIEL_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
# the tokens exactly like `seraphiel auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("SERAPHIEL_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `seraphiel auth add xai-oauth`'s credential-pool insert.
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `seraphiel sessions prune`)."""
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process SERAPHIEL_HOME at import time. The dashboard is a single
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from seraphiel_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from seraphiel_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # SERAPHIEL_HOME override (see mcp_tool._wrap_with_home_override), so
...
# `seraphiel memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `seraphiel memory setup` to configure a new one.",
            )
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``seraphiel tools`` picker uses, so the GUI and CLI stay in
...
def _resolve_chat_argv(
    ...
    """
    from seraphiel_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_seraphiel_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    search_dirs = [
        (get_seraphiel_home() / "plugins", "user"),
...
def start_server(
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"SERAPHIEL_DASHBOARD_READY port={actual_port}", flush=True)
