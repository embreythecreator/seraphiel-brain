...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/seraphiel/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when seraphiel
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# seraphiel process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main seraphiel process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Seraphiel WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via SERAPHIEL_UID at runtime
RUN useradd -u 10000 -m -d /opt/data seraphiel
...
WORKDIR /opt/seraphiel

# ---------- Layer-cached dependency install ----------
# Copy only package manifests first so npm install + Playwright are cached
# unless the lockfiles themselves change.
#
# ui-tui/packages/seraphiel-ink/ is copied IN FULL (not just its manifests)
...
# lock on the @seraphiel/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/seraphiel/.venv at first
...
COPY --chown=seraphiel:seraphiel . .

# ---------- Permissions ----------
# Make install dir world-readable so any SERAPHIEL_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the seraphiel user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# seraphiel_cli/main.py succeeds (see #18800). /opt/seraphiel/web is build-time
# only (SERAPHIEL_WEB_DIST points at seraphiel_cli/web_dist) and is intentionally
# not chowned here.
# /opt/seraphiel/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the seraphiel UID is remapped at boot (#27221).
# The .venv MUST remain seraphiel-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/seraphiel && \
    chown -R seraphiel:seraphiel /opt/seraphiel/.venv /opt/seraphiel/ui-tui /opt/seraphiel/gateway /opt/seraphiel/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the seraphiel user via
# `s6-setuidgid seraphiel` in its run script. If SERAPHIEL_UID is unset, services
# run as the default seraphiel user (UID 10000).

# ---------- Link seraphiel-brain itself (editable) ----------
...
# container always returns nothing — meaning `seraphiel dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the SERAPHIEL_GIT_SHA build-arg to
# /opt/seraphiel/.seraphiel_build_sha at build time, and have
# seraphiel_cli/build_info.py read it at runtime.  Both `seraphiel dump` and
...
RUN if [ -n "${SERAPHIEL_GIT_SHA}" ]; then \
        ...
        chown seraphiel:seraphiel /opt/seraphiel/.seraphiel_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-seraphiel + dashboard.
...
# `exec seraphiel`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $SERAPHIEL_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/seraphiel/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-seraphiel-setup && \
    chmod +x /etc/cont-init.d/01-seraphiel-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV SERAPHIEL_WEB_DIST=/opt/seraphiel/seraphiel_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/seraphiel/ui-tui/dist/entry.js`) and skip the
...
ENV SERAPHIEL_TUI_DIR=/opt/seraphiel/ui-tui
ENV SERAPHIEL_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> seraphiel ...` they default to root, and any file the
# command writes under $SERAPHIEL_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/seraphiel/bin/seraphiel, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid seraphiel`
# when invoked as root. Non-root callers (supervised processes,
# `--user seraphiel`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/seraphiel/.venv/bin/seraphiel). See the shim source for
# the opt-out env var (SERAPHIEL_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/seraphiel-exec-shim.sh /opt/seraphiel/bin/seraphiel

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> seraphiel ...` and any
# subprocess that doesn't activate the venv first still find seraphiel.
#
# /opt/seraphiel/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/seraphiel/bin:/opt/seraphiel/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. seraphiel
# subcommand vs. no-args), drops to the seraphiel user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/seraphiel/docker/main-wrapper.sh" ]
CMD [ ]
