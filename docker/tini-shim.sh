...
# Seraphiel WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SERAPHIEL_TINI_SHIM_TARGET overrides the `/init` path (and
# SERAPHIEL_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SERAPHIEL_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SERAPHIEL_TINI_SHIM_WRAPPER:-/opt/seraphiel/docker/main-wrapper.sh}"
