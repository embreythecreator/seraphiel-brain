...
#
# Wired into the image as /etc/cont-init.d/01-seraphiel-setup by the
...
SERAPHIEL_HOME="${SERAPHIEL_HOME:-/opt/data}"
INSTALL_DIR="/opt/seraphiel"

# Drop to seraphiel via s6-setuidgid, but skip it when already non-root.
as_seraphiel() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid seraphiel "$@"; }
...
# `id -u seraphiel` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# seraphiel UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u seraphiel)" ]; then
    cat >&2 <<EOF
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SERAPHIEL_UID=\$(id -u) -e SERAPHIEL_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the seraphiel user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SERAPHIEL_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid seraphiel mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SERAPHIEL_HOME paths whose parents only
# root can create (e.g. `SERAPHIEL_HOME=/home/seraphiel/.seraphiel` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SERAPHIEL_UID/SERAPHIEL_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SERAPHIEL_UID/
# SERAPHIEL_GID still win when both are set.  See #15290, salvages #25872.
SERAPHIEL_UID="${SERAPHIEL_UID:-${PUID:-}}"
SERAPHIEL_GID="${SERAPHIEL_GID:-${PGID:-}}"

if [ -n "${SERAPHIEL_UID:-}" ] && validate_uid_gid "$SERAPHIEL_UID" && [ "$SERAPHIEL_UID" != "$(id -u seraphiel)" ]; then
    echo "[stage2] Changing seraphiel UID to $SERAPHIEL_UID"
    usermod -u "$SERAPHIEL_UID" seraphiel
fi
if [ -n "${SERAPHIEL_GID:-}" ] && validate_uid_gid "$SERAPHIEL_GID" && [ "$SERAPHIEL_GID" != "$(id -g seraphiel)" ]; then
    echo "[stage2] Changing seraphiel GID to $SERAPHIEL_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SERAPHIEL_GID" seraphiel 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised seraphiel user (UID 10000)
...
# seraphiel process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes seraphiel. Idempotent across container
...
#   - seraphiel is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G seraphiel 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] seraphiel already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" seraphiel 2>/dev/null; then
        echo "[stage2] Added seraphiel to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group seraphiel failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SERAPHIEL_UID is remapped or the top-level $SERAPHIEL_HOME isn't owned by
# the runtime seraphiel UID, restore ownership to seraphiel — but ONLY for the
# directories seraphiel actually writes to. The full $SERAPHIEL_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of seraphiel-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_seraphiel_uid=$(id -u seraphiel)

path_has_symlink_component() {
    path="$1"
    root="${2:-$SERAPHIEL_HOME}"
...
chown_seraphiel_tree() {
    target="$1"
    if refuse_symlinked_path "recursive chown" "$target"; then
        return 0
    fi
    chown -R seraphiel:seraphiel "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$SERAPHIEL_HOME" 2>/dev/null)" != "$actual_seraphiel_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SERAPHIEL_HOME (targeted) to seraphiel ($actual_seraphiel_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SERAPHIEL_HOME: chown the directory itself (not its contents)
    # so seraphiel can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SERAPHIEL_HOME"; then
        :
    else
        chown seraphiel:seraphiel "$SERAPHIEL_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SERAPHIEL_HOME failed (rootless container?) — continuing"
    fi
    # Seraphiel-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by seraphiel (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SERAPHIEL_HOME/$sub" ]; then
            chown_seraphiel_tree "$SERAPHIEL_HOME/$sub"
        fi
    done
fi
...
# $SERAPHIEL_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SERAPHIEL_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to seraphiel
...
# are invoked via `docker exec <container> seraphiel …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as seraphiel and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$SERAPHIEL_HOME/profiles" ]; then
    chown_seraphiel_tree "$SERAPHIEL_HOME/profiles"
fi

# Always reset ownership of $SERAPHIEL_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged seraphiel runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$SERAPHIEL_HOME/cron" ]; then
    chown_seraphiel_tree "$SERAPHIEL_HOME/cron"
fi
...
# seraphiel pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged seraphiel gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SERAPHIEL_HOME is
...
# is negligible.
if [ -d "$SERAPHIEL_HOME/platforms/pairing" ]; then
    chown_seraphiel_tree "$SERAPHIEL_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SERAPHIEL_HOME/pairing" ]; then
    chown_seraphiel_tree "$SERAPHIEL_HOME/pairing"
fi

# Reset ownership of seraphiel-owned top-level state files on every boot.
# The targeted data-volume chown above only covers seraphiel-owned
# *subdirectories*; loose state files living directly under $SERAPHIEL_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> seraphiel …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged seraphiel runtime then hits
...
# sweep so host-owned files in a bind-mounted $SERAPHIEL_HOME are never
...
# entries of seraphiel_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SERAPHIEL_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SERAPHIEL_HOME/$f"; then
            :
        else
            chown seraphiel:seraphiel "$SERAPHIEL_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the seraphiel runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SERAPHIEL_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SERAPHIEL_HOME/config.yaml"; then
        :
    else
        chown seraphiel:seraphiel "$SERAPHIEL_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SERAPHIEL_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as seraphiel user ---
# Run as seraphiel via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SERAPHIEL_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_seraphiel mkdir -p \
    "$SERAPHIEL_HOME/backups" \
    "$SERAPHIEL_HOME/cron" \
    ...
    "$SERAPHIEL_HOME/skins" \
    "$SERAPHIEL_HOME/plans" \
    "$SERAPHIEL_HOME/workspace" \
    "$SERAPHIEL_HOME/home" \
    "$SERAPHIEL_HOME/pairing" \
    "$SERAPHIEL_HOME/platforms/pairing" \
    "$SERAPHIEL_HOME/lazy-packages"
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SERAPHIEL_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SERAPHIEL_HOME/$dest"; then
            :
        else
            as_seraphiel cp "$INSTALL_DIR/$src" "$SERAPHIEL_HOME/$dest"
        fi
    fi
}
...
if [ -f "$SERAPHIEL_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SERAPHIEL_HOME/.env"; then
        :
    else
        chown seraphiel:seraphiel "$SERAPHIEL_HOME/.env" 2>/dev/null || true
        chmod 600 "$SERAPHIEL_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $SERAPHIEL_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `seraphiel update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SERAPHIEL_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SERAPHIEL_HOME/config.yaml" ]; then
    s6-setuidgid seraphiel "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SERAPHIEL_HOME/auth.json" ] && [ -n "${SERAPHIEL_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SERAPHIEL_HOME/auth.json"; then
        :
    else
        printf '%s' "$SERAPHIEL_AUTH_JSON_BOOTSTRAP" > "$SERAPHIEL_HOME/auth.json"
        chown seraphiel:seraphiel "$SERAPHIEL_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SERAPHIEL_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SERAPHIEL_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SERAPHIEL_HOME/auth.json" ] && [ -n "${SERAPHIEL_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SERAPHIEL_HOME/auth.json"; then
        :
    else
        s6-setuidgid seraphiel "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$SERAPHIEL_HOME/auth.json" \
...
# can set SERAPHIEL_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SERAPHIEL_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SERAPHIEL_HOME/gateway_state.json" ] && \
        [ "${SERAPHIEL_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SERAPHIEL_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SERAPHIEL_HOME/gateway_state.json"
        chown seraphiel:seraphiel "$SERAPHIEL_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SERAPHIEL_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_seraphiel "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/seraphiel/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Seraphiel spawns for the
...
# main-wrapper.sh propagates it into the supervised ``seraphiel`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-seraphiel, dashboard, etc.).
