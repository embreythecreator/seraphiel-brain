...
//!
//! Mirrors `seraphiel_constants.get_seraphiel_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\seraphiel
//!   macOS:   ~/.seraphiel
//!   Linux:   ~/.seraphiel  (override via $SERAPHIEL_HOME)
//!
//! NOTE (macOS): Python's get_seraphiel_home(), scripts/install.sh, and the
//! Electron desktop's resolveSeraphielHome() ALL use ~/.seraphiel on macOS — there
...
/// Returns the canonical Seraphiel home directory, respecting $SERAPHIEL_HOME if set.
pub fn seraphiel_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SERAPHIEL_HOME") {
    ...
    {
        // %LOCALAPPDATA%\seraphiel — matches scripts/install.ps1's $SeraphielHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("seraphiel");
        }
    }

    // macOS + Linux + fallback: ~/.seraphiel (matches Python get_seraphiel_home(),
    // install.sh, and the Electron desktop's resolveSeraphielHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".seraphiel");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".seraphiel")
}

pub fn log_dir() -> PathBuf {
    seraphiel_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    seraphiel_home().join("bootstrap-cache")
}
...
/// SERAPHIEL_HOME so it survives repo checkout deletion (unlike anything under
/// seraphiel-brain/).
///
/// On Windows this is `%LOCALAPPDATA%\seraphiel\seraphiel-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_seraphiel`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SERAPHIEL_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SERAPHIEL_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    seraphiel_home().join(".seraphiel-update-in-progress")
}
...
pub fn copy_self_to_seraphiel_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SERAPHIEL_HOME");
    Ok(())
}
...
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".seraphiel-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SERAPHIEL_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[seraphiel-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SERAPHIEL_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_seraphiel_home() -> String {
    seraphiel_home().to_string_lossy().into_owned()
}
