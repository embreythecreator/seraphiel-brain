...
//!
//!   1. wait for the old Seraphiel desktop process to fully exit (so the venv
//!      shim is free; otherwise `seraphiel update` aborts with exit code 2),
...
/// before giving up and letting `seraphiel update`'s own guard decide.
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    ...
    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Seraphiel to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend seraphiel.exe (or a grandchild it spawned)
            ...
            // every seraphiel.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] Seraphiel still holding the venv shim; force-killing stragglers…",
            );
