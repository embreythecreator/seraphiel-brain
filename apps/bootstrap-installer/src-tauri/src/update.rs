...
//!
//! Driven when the installer is launched as `Seraphiel-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Seraphiel desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `seraphiel update`
//!      or repair bootstrap can race locked files),
//!   2. run `seraphiel update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in seraphiel_cli/main.py),
//!   3. run `seraphiel desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `seraphiel update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_seraphiel) and
...
/// `seraphiel update` exit code meaning "another seraphiel process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// seraphiel_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `seraphiel update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let seraphiel_home = crate::paths::seraphiel_home();
    let install_root = seraphiel_home.join("seraphiel-brain");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_seraphiel`
    ...
    let seraphiel = resolve_seraphiel(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the seraphiel CLI under {}. Is Seraphiel installed? \
    ...
    // async on Windows. If it still holds the venv shim, `seraphiel update`
    ...
    // ---- stage 2: seraphiel update -----------------------------------------
    // Pass --branch so `seraphiel update` targets the branch this installer was
    ...
    // without --branch, `seraphiel update` switches the checkout to `main` (a
    ...
    // --force skips `seraphiel update`'s Windows running-exe guard (which would
    ...
    // time `seraphiel update` runs there is no legitimate seraphiel.exe to protect,
    // and the guard would only produce a false "Seraphiel is still running" stop.
    //
    ...
    // probe only checks the seraphiel.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Seraphiel windows" message.
    ...
    // Retry-once for the update-boundary crash. `seraphiel update` lazily imports
    ...
    // second `seraphiel update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Seraphiel" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Seraphiel is still running. Close all Seraphiel windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "seraphiel update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::seraphiel_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: seraphiel desktop --build-only ----------------------------
    // `seraphiel update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `seraphiel update` already does above, and `seraphiel update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `seraphiel desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Seraphiel manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_seraphiel_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Seraphiel to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend seraphiel.exe (or the desktop Seraphiel.exe
            ...
            // force-kill every Seraphiel.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Seraphiel still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_seraphiel();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_seraphiel(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Seraphiel.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Seraphiel.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `seraphiel.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `seraphiel.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\seraphiel.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startSeraphiel()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// seraphiel.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named seraphiel.exe.)
fn force_kill_other_seraphiel() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `seraphiel <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv seraphiel shim under an install root, regardless of existence.
fn venv_seraphiel(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("seraphiel.exe")
    } else {
        install_root.join("venv").join("bin").join("seraphiel")
    }
}

/// Resolve the seraphiel CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `seraphiel` on PATH.
fn resolve_seraphiel(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_seraphiel(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "seraphiel.exe" } else { "seraphiel" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let seraphiel_home = crate::paths::seraphiel_home();
    let mut envs = vec![(
        "SERAPHIEL_HOME".to_string(),
        seraphiel_home.as_os_str().to_os_string(),
    )];
    // `seraphiel update` is a Python CLI writing to a pipe here, so CPython
    ...
    if let Some(path) = path_with_prepended_entries(&[
        seraphiel_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_seraphiel_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Seraphiel.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.seraphiel-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.seraphiel-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::seraphiel_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_seraphiel_is_under_install_root() {
        let root = Path::new("/x/seraphiel-brain");
        let shim = venv_seraphiel(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/seraphiel-brain"));
        assert!(
            envs.iter()
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_seraphiel(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/seraphiel-brain");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".seraphiel-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Seraphiel.app"]),
            Some(PathBuf::from("/Applications/Seraphiel.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Seraphiel.app");
        let tmp = base.join("Seraphiel.app.seraphiel-update-new");
        let old = base.join("Seraphiel.app.seraphiel-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Seraphiel.app");
        let tmp = base.join("Seraphiel.app.seraphiel-update-new"); // intentionally absent
        let old = base.join("Seraphiel.app.seraphiel-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Seraphiel.app");
        let tmp = base.join("Seraphiel.app.seraphiel-update-new"); // absent
        let old = base.join("Seraphiel.app.seraphiel-update-old");
        write_marker(&target, "OLD");
