...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Seraphiel"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // ---- stage 2: seraphiel desktop --build-only ----------------------------
...
/// which completes BEFORE we spawn `venv\Scripts\seraphiel.exe update`. At this
/// point no update-driven seraphiel.exe exists yet, so the only seraphiel.exe images
