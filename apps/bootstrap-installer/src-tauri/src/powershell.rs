...
///
/// `seraphiel_home_override` propagates to the child as $SERAPHIEL_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, seraphiel_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = seraphiel_home_override {
        cmd.env("SERAPHIEL_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, seraphiel_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = seraphiel_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_seraphiel_home() {
