...
export const $seraphielHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.seraphiel/logs/bootstrap-installer.log')
    $seraphielHome.set('~/.seraphiel')
  ...
  try {
    const [logPath, seraphielHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_seraphiel_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $seraphielHome.set(seraphielHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Seraphiel" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Seraphiel-Setup.exe --update);
...
export async function launchSeraphielDesktop(): Promise<void> {
  ...
  await invoke('launch_seraphiel_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Seraphiel repository' },
