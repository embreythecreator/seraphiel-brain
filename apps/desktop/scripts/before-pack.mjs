 ...
 * product name (`Seraphiel`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Seraphiel'
 *
 * This is a hard failure with no obvious cause for the user — `seraphiel desktop`
 ...
 * (the mac-unpacked Seraphiel.app bundle) and Windows (win-unpacked), so we
