#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Seraphiel icon + version metadata onto the
// built Seraphiel.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Seraphiel.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `seraphiel desktop`, the installer's
...
// path (which rebuilds via `seraphiel desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Seraphiel.exe>
//
...
// Stamp the Seraphiel icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Seraphiel',
      FileDescription: 'Seraphiel',
      CompanyName: 'Seraphiel',
      LegalCopyright: 'Copyright (c) 2026 Seraphiel'
    }
  })

  console.log('[set-exe-identity] done — Seraphiel icon + identity stamped')
}
