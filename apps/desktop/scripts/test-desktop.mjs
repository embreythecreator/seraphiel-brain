...
// no longer bundles the Seraphiel Brain Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Seraphiel.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Seraphiel'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Seraphiel.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'seraphiel'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SERAPHIEL_HOME for non-sandboxed runs -- matches main.cjs's
// resolveSeraphielHome(). On Windows it's %LOCALAPPDATA%\seraphiel; elsewhere
// it's ~/.seraphiel. The fresh-install sandbox launchFresh() sets its own
// SERAPHIEL_HOME and never touches this.
const DEFAULT_SERAPHIEL_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'seraphiel')
  }
  return path.join(os.homedir(), '.seraphiel')
})()
const VENV_ROOT = path.join(DEFAULT_SERAPHIEL_HOME, 'seraphiel-brain', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'seraphiel-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SERAPHIEL_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Seraphiel-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Seraphiel-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Seraphiel-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Seraphiel-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SERAPHIEL_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SERAPHIEL_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const seraphielHome = path.join(sandbox, 'seraphiel-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(seraphielHome, { recursive: true })
  ...
  env.SERAPHIEL_DESKTOP_CWD = cwd
  env.SERAPHIEL_DESKTOP_IGNORE_EXISTING = '1'
  env.SERAPHIEL_DESKTOP_TEST_MODE = 'fresh-install'
  env.SERAPHIEL_DESKTOP_USER_DATA_DIR = userDataDir
  env.SERAPHIEL_HOME = seraphielHome
  delete env.SERAPHIEL_DESKTOP_SERAPHIEL
  delete env.SERAPHIEL_DESKTOP_SERAPHIEL_ROOT
  ...
  console.log(`  SERAPHIEL_HOME: ${seraphielHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(seraphielHome, 'seraphiel-brain', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Seraphiel Brain Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of seraphiel_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'seraphiel-brain', 'seraphiel_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Seraphiel
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SERAPHIEL_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SERAPHIEL_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
