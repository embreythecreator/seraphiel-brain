// Unit tests for the pure Windows `seraphiel` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvSeraphielCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `seraphiel` shim shadows
//      the real seraphiel.cmd/seraphiel.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the seraphiel.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvSeraphielCommand() — must probe the venv python via
//      canImportSeraphielCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvSeraphielCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportSeraphielCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    seraphielHome: '/fake/seraphiel-home',
...
test('resolveVenvSeraphielCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvSeraphielCommand('/root/venv/Scripts/seraphiel.exe', [], deps), null)
})

test('resolveVenvSeraphielCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvSeraphielCommand('/root/venv/Scripts/seraphiel.cmd', [], deps), null)
})

test('resolveVenvSeraphielCommand: returns null when the basename is not seraphiel/seraphiel.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSeraphielCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvSeraphielCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSeraphielCommand('/root/venv/bin/seraphiel.exe', [], deps), null)
})

test('resolveVenvSeraphielCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvSeraphielCommand('/root/venv/Scripts/seraphiel.exe', [], deps), null)
})

test('resolveVenvSeraphielCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportSeraphielCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvSeraphielCommand('/root/venv/Scripts/seraphiel.exe', ['serve'], deps)
...
test('resolveVenvSeraphielCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvSeraphielCommand('/root/venv/Scripts/seraphiel.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'seraphiel_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvSeraphielCommand: is case-insensitive on seraphiel.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvSeraphielCommand('/root/venv/Scripts/SERAPHIEL.EXE', [], deps))
  assert.ok(resolveVenvSeraphielCommand('/root/venv/SCRIPTS/seraphiel.exe', [], deps))
})
