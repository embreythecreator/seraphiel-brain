...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'seraphiel-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Seraphiel Test'], { cwd: dir })
