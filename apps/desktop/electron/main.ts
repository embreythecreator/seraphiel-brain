...
import { buildDesktopBackendEnv, normalizeSeraphielHomeRoot } from './backend-env'
import { canImportSeraphielCli, verifySeraphielCli } from './backend-probes'
...
import {
  ...
  resolveVenvSeraphielCommand
} from './windows-seraphiel-path'
import {
  alreadyHasNoSandbox,
...
const USER_DATA_OVERRIDE = process.env.SERAPHIEL_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SERAPHIEL_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SERAPHIEL_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SERAPHIEL_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[seraphiel] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[seraphiel] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `seraphiel desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Seraphiel sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[seraphiel] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[seraphiel] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[seraphiel] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[seraphiel] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[seraphiel] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('seraphiel:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[seraphiel] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[seraphiel] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[seraphiel] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[seraphiel] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SERAPHIEL_HOME — the user-facing root for everything Seraphiel-related. Mirrors
// scripts/install.ps1's $SeraphielHome and scripts/install.sh's $SERAPHIEL_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\seraphiel (matches install.ps1)
//   macOS / Linux: ~/.seraphiel (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.seraphiel directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\seraphiel yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SERAPHIEL_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SERAPHIEL_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.seraphiel / %LOCALAPPDATA%\seraphiel.
function resolveSeraphielHome() {
  if (process.env.SERAPHIEL_HOME) {
    return normalizeSeraphielHomeRoot(process.env.SERAPHIEL_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'seraphiel-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SERAPHIEL_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\seraphiel and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SERAPHIEL_HOME')

    if (fromRegistry) {
      return normalizeSeraphielHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'seraphiel')
    const legacy = path.join(app.getPath('home'), '.seraphiel')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.seraphiel setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.seraphiel')
}

const SERAPHIEL_HOME = resolveSeraphielHome()

function seraphielManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_seraphiel_node_dirs() in
  // seraphiel_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(SERAPHIEL_HOME, 'node')
...
function pathWithSeraphielManagedNode(...entries) {
  return [...seraphielManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SERAPHIEL_ROOT — the canonical mutable Seraphiel install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SERAPHIEL_ROOT = path.join(SERAPHIEL_HOME, 'seraphiel-brain')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SERAPHIEL_ROOT, 'venv')
...
// boots, going straight to `resolveSeraphielBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SERAPHIEL_ROOT (not alongside)
...
// active-profile.json records which Seraphiel profile the desktop launches its
// local backend as. When set, startSeraphiel() passes `seraphiel --profile <name>
// dashboard …`, which deterministically pins SERAPHIEL_HOME (see
// _apply_profile_override in seraphiel_cli/main.py) and bypasses the sticky
// ~/.seraphiel/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors seraphiel_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// seraphielDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SERAPHIEL_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by seraphiel_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SERAPHIEL_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (seraphiel_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SERAPHIEL_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SERAPHIEL_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SERAPHIEL_DESKTOP_APP_NAME || 'Seraphiel'
...
// 'seraphiel:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.embreythecreator.seraphiel) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.embreythecreator.seraphiel')
}

// Seed the native About panel with the live Seraphiel version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `seraphiel update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSeraphielVersion(),
  copyright: 'Copyright © 2026 Seraphiel'
})
...
const MEDIA_PROTOCOL = 'seraphiel-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(seraphiel-media://…)`.
...
// startSeraphiel(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SERAPHIEL_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SERAPHIEL_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startSeraphiel() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning seraphiel serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Seraphiel backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[seraphiel] ${line}`)
  seraphielLog.push(...lines)

  if (seraphielLog.length > 300) {
    seraphielLog.splice(0, seraphielLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-seraphiel-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('seraphiel:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the seraphiel:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('seraphiel:bootstrap:event', ev)
}
...
// The Tauri updater writes SERAPHIEL_HOME/.seraphiel-update-in-progress for the whole
...
// (`force_kill_other_seraphiel`, taskkill /IM seraphiel.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(SERAPHIEL_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Seraphiel will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(SERAPHIEL_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `seraphiel` — must not shadow `seraphiel.cmd`/`seraphiel.exe`.
...
function unwrapWindowsVenvSeraphielCommand(command, backendArgs) {
  return resolveVenvSeraphielCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
    canImportSeraphielCli,
    getVenvPython,
    getVenvSitePackagesEntries,
    buildDesktopBackendEnv,
    seraphielHome: SERAPHIEL_HOME,
...
// spawns `seraphiel serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `seraphiel` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'seraphiel_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SERAPHIEL_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isSeraphielSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'seraphiel_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SERAPHIEL_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Seraphiel deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Seraphiel' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\seraphiel\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'seraphiel', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'seraphiel', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal SERAPHIEL_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\seraphiel\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'seraphiel', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'seraphiel', 'git', 'bin', 'git.exe'))
  }
...
function recentSeraphielLog() {
  return seraphielLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's seraphiel root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SERAPHIEL_ROOT.
// SERAPHIEL_DESKTOP_SERAPHIEL_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SERAPHIEL_DESKTOP_SERAPHIEL_ROOT && path.resolve(process.env.SERAPHIEL_DESKTOP_SERAPHIEL_ROOT),
    !IS_PACKAGED && isSeraphielSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSeraphielSourceRoot(ACTIVE_SERAPHIEL_ROOT) ? ACTIVE_SERAPHIEL_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SERAPHIEL_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('seraphiel:updates:progress', merged)
  }
}
...
// SERAPHIEL_HOME/seraphiel-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_seraphiel_home). That binary owns
// ALL repo mutation — running `seraphiel update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'seraphiel-setup.exe' : 'seraphiel-setup'
  const candidate = path.join(SERAPHIEL_HOME, name)
...
// Path to the venv shim whose lock decides whether `seraphiel update` can write
// fresh entry points. On Windows this is the file the running backend
// `seraphiel.exe` holds open; on POSIX it's never mandatory-locked.
function venvSeraphielShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'seraphiel.exe')
    : path.join(updateRoot, 'venv', 'bin', 'seraphiel')
}
...
// only signals the direct child, so on Windows a backend `seraphiel.exe` that
// spawned its own grandchildren (a `seraphiel` REPL, a pty terminal session, the
...
// `seraphielProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `seraphiel.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a seraphiel
// REPL, a pty terminal, the gateway) keep `seraphiel.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const seraphielProcess = backendConnectionState.getProcess()

  if (seraphielProcess && Number.isInteger(seraphielProcess.pid)) {
    pids.push(seraphielProcess.pid)
  }
  ...
  if (seraphielProcess && !seraphielProcess.killed) {
    try {
      seraphielProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvSeraphielShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentSeraphielProcess = backendConnectionState.getProcess()

    if (currentSeraphielProcess && Number.isInteger(currentSeraphielProcess.pid)) {
      stragglers.push(currentSeraphielProcess.pid)
    }
...
// `seraphiel update`). Instead we spawn the staged Seraphiel-Setup binary with
// --update and quit, so it can run `seraphiel update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri seraphiel-setup. Unlike Windows
      ...
      // whole update itself: `seraphiel update` (backend) + `seraphiel desktop
    ...
    if (!updater) {
      ...
      // `seraphiel desktop`, never the Tauri installer that self-copies
      // seraphiel-setup.exe into SERAPHIEL_HOME). They DO have a working `seraphiel`
      ...
      // checkout they're on — bare `seraphiel update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `seraphiel update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `seraphiel update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, seraphielRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Seraphiel — this window will close and the updater will open. Don’t reopen Seraphiel yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // seraphiel.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running seraphiel, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Seraphiel install open ' +
        '(a second Seraphiel window or a terminal running seraphiel?). Close it and retry.'

      emitUpdateProgress({ stage: 'error', message, percent: null })
      startSeraphiel().catch(() => {})

      return { ok: false, error: message }
    }

    // Detached so the updater outlives this process — it needs us GONE before
    // `seraphiel update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: SERAPHIEL_HOME,
      env: {
        ...process.env,
        SERAPHIEL_HOME,
        PATH: pathWithSeraphielManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(SERAPHIEL_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSeraphiel = path.join(venvBin, IS_WINDOWS ? 'seraphiel.exe' : 'seraphiel')
  ...
  // not just the `seraphiel.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvSeraphiel) || fileExists(path.join(updateRoot, '.seraphiel-bootstrap-complete'))
...
// Resolve the seraphiel CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `seraphiel` on PATH.
function resolveSeraphielCliBinary(updateRoot) {
  const venvSeraphiel = path.join(updateRoot, 'venv', 'bin', 'seraphiel')

  if (fileExists(venvSeraphiel)) {
    return venvSeraphiel
  }

  return findOnPath('seraphiel') || null
}
...
// macOS/Linux in-app update: backend (`seraphiel update`) + OS-aware GUI rebuild
// (`seraphiel desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const seraphiel = resolveSeraphielCliBinary(updateRoot)

  if (!seraphiel) {
    emitUpdateProgress({ stage: 'manual', message: 'seraphiel update', percent: null })

    return { ok: true, manual: true, command: 'seraphiel update', seraphielRoot: updateRoot }
  }

  // Put the Seraphiel-managed Node and the venv on PATH so `seraphiel desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\seraphiel\node, not node\bin.
  // PYTHONUNBUFFERED: `seraphiel update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    SERAPHIEL_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithSeraphielManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `seraphiel update` reaps stale `seraphiel serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads SERAPHIEL_DESKTOP_CHILD_PID and excludes
  ...
  if (seraphielProcess && Number.isInteger(seraphielProcess.pid)) {
    desktopChildPids.push(seraphielProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.SERAPHIEL_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Seraphiel (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(seraphiel, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'seraphiel update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'seraphiel update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(seraphiel, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Seraphiel to retry.',
  ...
  // Linux in-app update terminal state (#45205). `seraphiel desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Seraphiel…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `seraphiel-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Seraphiel to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Seraphiel desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Seraphiel to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Seraphiel.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Seraphiel.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Seraphiel to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.seraphiel-update-new"; then
    rm -rf "$DST.seraphiel-update-old" 2>/dev/null || true
    mv "$DST" "$DST.seraphiel-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.seraphiel-update-new" "$DST"
    rm -rf "$DST.seraphiel-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Seraphiel to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_SERAPHIEL_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSeraphielSourceRoot(ACTIVE_SERAPHIEL_ROOT) &&
    fileExists(venvPython) &&
    canImportSeraphielCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SERAPHIEL_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `seraphiel
...
function resolveWebDist() {
  const override = process.env.SERAPHIEL_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SERAPHIEL_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: seraphiel desktop --force-build`
  )
...
function resolveSeraphielCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Seraphiel.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SERAPHIEL_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSeraphielCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'seraphiel_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_SERAPHIEL_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Seraphiel at ${ACTIVE_SERAPHIEL_ROOT}`,
    command,
    args: ['-m', 'seraphiel_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      seraphielHome: SERAPHIEL_HOME,
      pythonPathEntries: [ACTIVE_SERAPHIEL_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SERAPHIEL_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveSeraphielBackend(backendArgs) {
  // 1. Explicit override -- SERAPHIEL_DESKTOP_SERAPHIEL_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SERAPHIEL_DESKTOP_SERAPHIEL_ROOT && path.resolve(process.env.SERAPHIEL_DESKTOP_SERAPHIEL_ROOT)

  if (overrideRoot && isSeraphielSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Seraphiel source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `seraphiel` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSeraphielSourceRoot.)
  if (!IS_PACKAGED && isSeraphielSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Seraphiel source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_SERAPHIEL_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\seraphiel\seraphiel-brain (Windows) or ~/.seraphiel/seraphiel-brain.
  ...
  //    to spawning seraphiel. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `seraphiel update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `seraphiel` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SERAPHIEL_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SERAPHIEL_DESKTOP_IGNORE_EXISTING !== '1') {
    let seraphielCommand = null
    const seraphielOverride = process.env.SERAPHIEL_DESKTOP_SERAPHIEL

    if (seraphielOverride) {
      const resolvedOverride = findOnPath(seraphielOverride)

      if (resolvedOverride) {
        seraphielCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(seraphielOverride, { isWsl: IS_WSL })) {
        seraphielCommand = seraphielOverride
      } else {
        rememberLog(`Ignoring Windows Seraphiel override under WSL: ${seraphielOverride}`)
      }
    } else {
      seraphielCommand = findOnPath('seraphiel')
    }

    if (seraphielCommand) {
      if (looksLikeDesktopAppBinary(seraphielCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Seraphiel CLI: ${seraphielCommand}`)
        seraphielCommand = null
      }
    }

    if (seraphielCommand) {
      const unwrapped = unwrapWindowsVenvSeraphielCommand(seraphielCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `seraphiel` shim
      ...
      const shellForProbe = isCommandScript(seraphielCommand)

      if (verifySeraphielCli(seraphielCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvSeraphielCommand(seraphielCommand, backendArgs) || {
            label: `existing Seraphiel CLI at ${seraphielCommand}`,
            command: seraphielCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Seraphiel CLI at ${seraphielCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed seraphiel_cli module via system Python.
  ...
  if (python) {
    ...
    // having seraphiel_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\seraphiel\seraphiel-brain\venv.
    if (canImportSeraphielCli(python)) {
      return {
        kind: 'python',
        label: `installed seraphiel_cli module via ${python}`,
        command: python,
        args: ['-m', 'seraphiel_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: seraphiel_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSeraphielBackend was the old "no payload" path and forced the
  ...
  return {
    ...
    activeRoot: ACTIVE_SERAPHIEL_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSeraphielBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Seraphiel install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Seraphiel recovery was handed off to Seraphiel Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      seraphielHome: SERAPHIEL_HOME,
      logRoot: path.join(SERAPHIEL_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Seraphiel install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Seraphiel bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SERAPHIEL_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSeraphiel() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // seraphiel:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSeraphielBackend(backend.args))
  }
  ...
  if (!isSeraphielSourceRoot(ACTIVE_SERAPHIEL_ROOT)) {
    throw new Error(
      `Seraphiel install at ${ACTIVE_SERAPHIEL_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Seraphiel' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\seraphiel\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `seraphiel` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Seraphiel on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Seraphiel.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isSeraphielSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Seraphiel venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Seraphiel at ${ACTIVE_SERAPHIEL_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Seraphiel runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----seraphiel-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Seraphiel backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Seraphiel-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Seraphiel backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Seraphiel backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Seraphiel-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('seraphiel:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSeraphielCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('seraphiel:preview-file-changed', payload)
}
...
async function waitForSeraphiel(baseUrl, token) {
  ...
  throw new Error(`Seraphiel backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('seraphiel:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('seraphiel:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Seraphiel...".
function sendPowerResume() {
  ...
  webContents.send('seraphiel:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('seraphiel:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('seraphiel:window-state-changed', state)
}
...
//
// Hosted Seraphiel gateways gate the dashboard behind an OAuth provider (e.g.
// Seraphiel) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``seraphiel_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / seraphiel #37247).
//     Both are set as HttpOnly cookies (``seraphiel_session_at`` ~15 min,
//     ``seraphiel_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:seraphiel-remote-oauth'
...
// "not signed in", which on the initial boot path (startSeraphiel → the renderer's
// single-shot boot() with no retry) surfaced as the "Seraphiel couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Seraphiel
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Seraphiel Cloud agent…' : 'Sign in to Seraphiel gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Seraphiel backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Seraphiel Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:seraphiel-remote-oauth`:
...
// convention (seraphiel_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Seraphiel surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.embreythecreator.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SERAPHIEL_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Seraphiel gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for seraphiel_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Seraphiel Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Seraphiel Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Seraphiel Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Seraphiel Cloud. Open Settings → Gateway, choose Seraphiel Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Seraphiel Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Seraphiel Cloud session has expired. Sign in to Seraphiel Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Seraphiel Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root SERAPHIEL_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SERAPHIEL_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.SERAPHIEL_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a Seraphiel Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Seraphiel Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by SERAPHIEL_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Seraphiel Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Seraphiel Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (seraphiel #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Seraphiel gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Seraphiel gateway is selected, but no session token is saved. ' +
...
//   2. env override (SERAPHIEL_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.SERAPHIEL_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.SERAPHIEL_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'SERAPHIEL_DESKTOP_REMOTE_URL is set but SERAPHIEL_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Seraphiel backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.SERAPHIEL_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startSeraphiel() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Seraphiel
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Seraphiel" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSeraphiel())
  ...
  // connect to Seraphiel gateway". Mirror the renderer's connect here so the test
...
function resetSeraphielConnection({ soft = false } = {}) {
  backendStartFailure = null
  const seraphielProcess = backendConnectionState.invalidate()
  stopBackendChild(seraphielProcess)
...
// startSeraphiel() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSeraphielConnection() invalidates it.
  const seraphielProcess = backendConnectionState.getProcess()
  const dying = seraphielProcess && !seraphielProcess.killed ? seraphielProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('seraphiel:connection:applied')
}
...
// profile to startSeraphiel() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startSeraphiel() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForSeraphiel(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: seraphielLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited SERAPHIEL_HOME env (see _apply_profile_override
  // step 3 in seraphiel_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSeraphielBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const seraphielCwd = resolveSeraphielCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Seraphiel backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: seraphielCwd,
        SERAPHIEL_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        SERAPHIEL_DESKTOP: '1',
        SERAPHIEL_WEB_DIST: webDist,
        ...(readyFile ? { SERAPHIEL_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Seraphiel backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Seraphiel backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Seraphiel backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForSeraphiel(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Seraphiel backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_seraphiel_home() recreates the deleted profile directory.
//
...
async function startSeraphiel() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSeraphiel() call re-throws the same error
  ...
  const connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Seraphiel backend', 8)
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Seraphiel backend at ${remote.baseUrl}`, 24)
      await waitForSeraphiel(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Seraphiel backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.seraphiel/active_profile file) and
    // resolves SERAPHIEL_HOME the same way `seraphiel -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Seraphiel runtime', 28)
    const backend = await ensureRuntime(resolveSeraphielBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const seraphielCwd = resolveSeraphielCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Seraphiel backend via ${backend.label}`, 84)
    rememberLog(`Starting Seraphiel backend via ${backend.label}`)

    const seraphielProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: seraphielCwd,
        env: {
          ...process.env,
          // Explicitly pin SERAPHIEL_HOME for the child so Python's get_seraphiel_home()
          // resolves to the SAME location our resolveSeraphielHome() picked. Without
          // this pin, Python falls back to ~/.seraphiel on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\seraphiel, which differs from C:\Users\<u>\.seraphiel.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SERAPHIEL_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, seraphielProcess)

    if (!processOwner) {
      stopBackendChild(seraphielProcess)
      throw new Error('Seraphiel backend start was superseded by a newer connection attempt.')
    }

    seraphielProcess.stdout.on('data', rememberLog)
    seraphielProcess.stderr.on('data', rememberLog)
    ...
    seraphielProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Seraphiel backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Seraphiel backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Seraphiel backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Seraphiel backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    seraphielProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Seraphiel backend exit (${signal || code})`)
      ...
      rememberLog(`Seraphiel backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Seraphiel backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Seraphiel backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSeraphielLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Seraphiel backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(seraphielProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Seraphiel backend to become ready', 90)
    await Promise.race([waitForSeraphiel(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => seraphielProcess.exitCode === null && !seraphielProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Seraphiel backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Seraphiel is
...
// pushes pet state over IPC (seraphiel:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via seraphiel:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Seraphiel app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Seraphiel icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see seraphiel:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Seraphiel in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('seraphiel:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startSeraphiel().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('seraphiel:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Seraphiel…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('seraphiel:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetSeraphielConnection only
    // clears the connection promise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Seraphiel backend failed liveness probe; dropping stale connection.')
    resetSeraphielConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('seraphiel:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('seraphiel:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('seraphiel:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('seraphiel:window:openNewSession', async () => {
...
ipcMain.handle('seraphiel:zoom:get', event => {
...
ipcMain.on('seraphiel:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('seraphiel:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('seraphiel:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('seraphiel:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('seraphiel:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('seraphiel:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('seraphiel:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('seraphiel:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('seraphiel:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('seraphiel:pet-overlay:control', payload)
})
ipcMain.handle('seraphiel:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSeraphiel() call restarts the
...
ipcMain.handle('seraphiel:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startSeraphiel() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('seraphiel:bootstrap:cancel', async () => {
...
ipcMain.handle('seraphiel:boot-progress:get', async () => bootProgressState)
ipcMain.handle('seraphiel:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('seraphiel:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('seraphiel:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('seraphiel:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('seraphiel:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('seraphiel:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Seraphiel Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('seraphiel:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('seraphiel:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('seraphiel:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on seraphiel:connection:applied.
...
ipcMain.handle('seraphiel:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('seraphiel:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SERAPHIEL_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('seraphiel:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('seraphiel:requestMicrophoneAccess', async () => {
...
ipcMain.handle('seraphiel:api', async (_event, request) => {
  ...
  // backend calls ensure_seraphiel_home() which recreates the profile directory,
...
ipcMain.handle('seraphiel:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('seraphiel:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('seraphiel:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('seraphiel:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('seraphiel:readFileText', async (_event, filePath) => {
...
ipcMain.handle('seraphiel:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('seraphiel:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('seraphiel:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('seraphiel:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('seraphiel:saveClipboardImage', async () => {
...
ipcMain.handle('seraphiel:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('seraphiel:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('seraphiel:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('seraphiel:titlebar-theme', (_event, payload) => {
...
ipcMain.on('seraphiel:native-theme', (_event, mode) => {
...
ipcMain.on('seraphiel:translucency', (_event, payload) => {
...
ipcMain.handle('seraphiel:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('seraphiel:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSeraphielCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('seraphiel:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSeraphielCwd()
}))

ipcMain.handle('seraphiel:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('seraphiel:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('seraphiel:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('seraphiel:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('seraphiel:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('seraphiel:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: seraphielLog.slice(-200) }))
...
function terminalShellCommand() {
  // SERAPHIEL_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.SERAPHIEL_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Seraphiel' TUI into
  ...
  env.TERM_PROGRAM = 'Seraphiel'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a seraphiel/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from SERAPHIEL_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.SERAPHIEL_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `seraphiel:terminal:${id}:${suffix}`
}
...
ipcMain.handle('seraphiel:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('seraphiel:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('seraphiel:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('seraphiel:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('seraphiel:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('seraphiel:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('seraphiel:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('seraphiel:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('seraphiel:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('seraphiel:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('seraphiel:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('seraphiel:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('seraphiel:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('seraphiel:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('seraphiel:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('seraphiel:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('seraphiel:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('seraphiel:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('seraphiel:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('seraphiel:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('seraphiel:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('seraphiel:terminal:write', (_event, id, data) => {
...
ipcMain.handle('seraphiel:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('seraphiel:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('seraphiel:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('seraphiel:updates:check', async () =>
...
ipcMain.handle('seraphiel:updates:apply', async (_event, payload) =>
...
ipcMain.handle('seraphiel:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('seraphiel:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Seraphiel version (the one `release.py` bumps in
// seraphiel_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Seraphiel version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSeraphielVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'seraphiel_cli', '__init__.py')
...
// Re-resolve the live Seraphiel version and push it into the native About panel
// just before showing it, so an in-place `seraphiel update` is reflected without
...
ipcMain.handle('seraphiel:version', async () => ({
  appVersion: resolveSeraphielVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  seraphielRoot: resolveUpdateRoot()
}))
...
// the actual removal via `seraphiel uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (seraphiel_cli/uninstall.py + seraphiel_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SERAPHIEL_ROOT
  ...
  const fallback = () => ({
    seraphiel_home: SERAPHIEL_HOME,
    agent_installed: isSeraphielSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'seraphiel_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SERAPHIEL_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Seraphiel agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import seraphiel_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_SERAPHIEL_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_SERAPHIEL_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_SERAPHIEL_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `seraphiel-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `seraphiel-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('seraphiel:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('seraphiel:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('seraphiel:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('seraphiel:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// seraphiel:// deep links (e.g. seraphiel://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const SERAPHIEL_PROTOCOL = 'seraphiel'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${SERAPHIEL_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // seraphiel://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('seraphiel:deep-link', payload)
...
ipcMain.handle('seraphiel:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SERAPHIEL_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SERAPHIEL_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(SERAPHIEL_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `seraphiel://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching seraphiel:// URL is in our own argv.
