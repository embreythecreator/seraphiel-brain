...
// Regression guards for Windows `seraphiel` resolution in main.cjs.
//
...
//      Git-Bash `seraphiel` shim shadowed the real seraphiel.cmd/seraphiel.exe; the
...
//      --repair by checking ONLY venv\Scripts\seraphiel.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow seraphiel.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\seraphiel.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvSeraphiel\) \?/,
    'recovery regressed to gating only on the seraphiel.exe shim, which forces destructive --repair'
  )
})
