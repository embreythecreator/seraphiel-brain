// Backend subcommand routing for the desktop-managed Seraphiel process.
//
// The desktop app launches its own headless backend via `seraphiel serve` — it
...
// app hasn't updated yet, or an older `seraphiel` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Seraphiel profile to pin via `--profile`.
 */
 ...
 * `-m seraphiel_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `seraphiel_cli/subcommands/dashboard.py` source registers
