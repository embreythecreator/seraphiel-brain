// Profile-delete routing logic for the `seraphiel:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_seraphiel_home() and
...
/**
 * Parse a `seraphiel:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `seraphiel:api` request away from the primary/window backend
 ...
 * ensure_seraphiel_home() recreates the directory the delete just removed.
 */
