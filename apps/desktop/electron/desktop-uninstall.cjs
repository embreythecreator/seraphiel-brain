 ...
 * the three user-facing uninstall modes to the `seraphiel uninstall` CLI flags,
 ...
 *              `seraphiel uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `seraphiel uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `seraphiel uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `seraphiel` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m seraphiel_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `seraphiel_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'seraphiel_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Seraphiel.app/Contents/MacOS/Seraphiel  → …/Seraphiel.app
 *   Windows: …\Seraphiel\Seraphiel.exe                 → …\Seraphiel  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Seraphiel.app/Contents/MacOS/Seraphiel → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Seraphiel.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Seraphiel.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Seraphiel$/i.test(dir) || /[\\/]seraphiel-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/seraphiel
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import seraphiel_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, seraphielHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SERAPHIEL_HOME=${q(seraphielHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import seraphiel_cli` resolves from source
 ...
 */
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, seraphielHome }) {
  const pid = Number(desktopPid) || 0
  // cmd.exe has no string escaping inside quotes; strip embedded quotes (paths
  // under %LOCALAPPDATA% never contain them). `&`/`^` in a path would still be
  // a problem, but Seraphiel install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SERAPHIEL_HOME=${String(seraphielHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
