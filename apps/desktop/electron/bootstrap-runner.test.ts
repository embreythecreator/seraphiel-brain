...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/seraphiel-runner-test',
    sourceRepoRoot: null,
    seraphielHome: '/tmp/seraphiel-runner-test',
    logRoot: '/tmp/seraphiel-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'seraphiel-brain', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'seraphiel-brain')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/seraphiel-brain',
      seraphielHome: '/tmp/seraphiel'
    }),
    ['--dir', '/tmp/seraphiel-brain', '--seraphiel-home', '/tmp/seraphiel', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/seraphiel-brain', '--seraphiel-home', '/tmp/seraphiel', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/seraphiel',
      seraphielHome: '/tmp/home'
    }),
    ['--dir', '/tmp/seraphiel', '--seraphiel-home', '/tmp/home', '--branch', 'main']
  )
})
