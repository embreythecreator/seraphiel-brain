 ...
 *      internals; SERAPHIEL_HOME / SERAPHIEL_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.seraphiel/seraphiel-brain'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'seraphiel')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Seraphiel12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/seraphiel/seraphiel', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Seraphiel/seraphiel', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.seraphiel/seraphiel-brain/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'seraphiel'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'seraphiel')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'seraphiel://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'seraphiel://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves SERAPHIEL_HOME + SERAPHIEL_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    SERAPHIEL_HOME: '/home/u/.seraphiel',
    SERAPHIEL_DESKTOP_REMOTE_URL: 'http://box:9119',
    SERAPHIEL_DESKTOP_REMOTE_TOKEN: 'secret',
    SERAPHIEL_DESKTOP_SERAPHIEL_ROOT: '/home/u/dev/seraphiel',
    SERAPHIEL_DESKTOP_APP_NAME: 'SeraphielSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.seraphiel/seraphiel-brain/apps/desktop/release/linux-unpacked/Seraphiel',
    args: ['seraphiel://open/agent/42', "--note=it's fine"],
    env: { SERAPHIEL_HOME: '/home/u/.seraphiel', SERAPHIEL_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export SERAPHIEL_HOME='\/home\/u\/\.seraphiel'/)
  assert.match(script, /export SERAPHIEL_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Seraphiel' 'seraphiel:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `seraphiel-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Seraphiel/Seraphiel',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `seraphiel-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Seraphiel\/Seraphiel'\n/)
})
