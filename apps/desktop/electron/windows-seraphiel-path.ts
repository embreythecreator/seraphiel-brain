/**
 * windows-seraphiel-path.ts
 *
 * Pure, dependency-injected pieces of Windows `seraphiel` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvSeraphielCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `seraphiel` shim shadowed the real
 *      seraphiel.cmd/seraphiel.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\seraphiel.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvSeraphielCommand() — unwrapWindowsVenvSeraphielCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `seraphiel`) must
 * not shadow `seraphiel.cmd`/`seraphiel.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv seraphiel shim, bootstrap-complete
 * marker) — gating on just the seraphiel.exe console-script shim alone is the
...
/**
 * If `command` is a Windows venv `seraphiel`/`seraphiel.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/seraphiel(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m seraphiel_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportSeraphielCli(). A
 * venv whose update died mid-`pip install` still has python.exe + seraphiel.exe
 ...
 *
 * Returns null when `command` is not a venv seraphiel shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvSeraphielCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvSeraphielCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^seraphiel(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Seraphiel at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
