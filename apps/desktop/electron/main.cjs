...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Seraphiel updater…', percent: 100 })
    ...
    const child = spawn(updater, updaterArgs, {
      ...
      env: {
        ...process.env,
        SERAPHIEL_HOME,
        PATH: [path.join(SERAPHIEL_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const updaterArgs = fileExists(venvSeraphiel) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
async function applyUpdatesPosixInApp() {
  ...
  const extraPath = [path.join(SERAPHIEL_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    SERAPHIEL_HOME,
  ...
  // `seraphiel update` reaps stale `seraphiel dashboard` backends (a code update
...
function isBootstrapComplete() {
  ...
  return isSeraphielSourceRoot(ACTIVE_SERAPHIEL_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'seraphiel_cli.main', ...dashboardArgs],
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    ...
    env: buildDesktopBackendEnv({
      seraphielHome: SERAPHIEL_HOME,
      pythonPathEntries: [ACTIVE_SERAPHIEL_ROOT],
      venvRoot: VENV_ROOT
    }),
...
function resolveSeraphielBackend(dashboardArgs) {
  ...
  if (overrideRoot && isSeraphielSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Seraphiel source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isSeraphielSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Seraphiel source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Seraphiel recovery was handed off to Seraphiel Setup. The desktop will restart when recovery completes.')
...
async function spawnPoolBackend(profile, entry) {
  ...
  const backend = await ensureRuntime(resolveSeraphielBackend(dashboardArgs))
...
async function startSeraphiel() {
  ...
  connectionPromise = (async () => {
    ...
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(seraphielProcess), backendStartFailed])
...
ipcMain.handle('seraphiel:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))
