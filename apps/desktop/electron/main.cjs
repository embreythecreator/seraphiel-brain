...
const { buildDesktopBackendEnv, normalizeSeraphielHomeRoot } = require('./backend-env.cjs')
...
function resolveSeraphielHome() {
  if (process.env.SERAPHIEL_HOME) return normalizeSeraphielHomeRoot(process.env.SERAPHIEL_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'seraphiel-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeSeraphielHomeRoot(fromRegistry)
  }
...
let mainWindow = null
let seraphielProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by seraphielProcess +
// connectionPromise + startSeraphiel(); this pool only holds EXTRA profile
...
function unwrapWindowsVenvSeraphielCommand(command, backendArgs) {
  ...
  if (!/^seraphiel(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Seraphiel Python at ${python}`,
    command: python,
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(
        path.join(backend.root, 'seraphiel_cli', 'subcommands', 'dashboard.py'),
        'utf8'
      )
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (seraphielProcess && Number.isInteger(seraphielProcess.pid)) pids.push(seraphielProcess.pid)
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `seraphiel update --branch ${branch}`
        }
      } catch {
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const haveRealInstall =
    ...
    fileExists(venvSeraphiel) ||
    fileExists(path.join(updateRoot, '.seraphiel-bootstrap-complete'))
...
function resolveSeraphielCliBinary(updateRoot) {
  ...
  if (fileExists(venvSeraphiel)) return venvSeraphiel
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Seraphiel install was cancelled.')
...
const ZOOM_STORAGE_KEY = 'seraphiel:desktop:zoomLevel'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Seraphiel gateway',
...
function resetSeraphielConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(seraphielProcess)

  seraphielProcess = null
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetSeraphielConnection() nulls seraphielProcess.
...
async function startSeraphiel() {
  ...
  connectionPromise = (async () => {
    ...
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null seraphielProcess when the child dies.
      childAlive: () => seraphielProcess !== null && seraphielProcess.exitCode === null && !seraphielProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
...
ipcMain.handle('seraphiel:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('seraphiel:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Seraphiel.')
  }
