...
function resolveSeraphielHome() {
  if (process.env.SERAPHIEL_HOME) return path.resolve(process.env.SERAPHIEL_HOME)
...
async function applyUpdatesPosixInApp() {
  ...
  const rebuilt = await runStreamedUpdate(seraphiel, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
...
async function startSeraphiel() {
  ...
  connectionPromise = (async () => {
    ...
    seraphielProcess = spawn(backend.command, backend.args, hiddenWindowsChildOptions({
      cwd: seraphielCwd,
      env: {
        ...process.env,
...
ipcMain.handle('seraphiel:window:openSession', async (_event, sessionId) => {
...
ipcMain.handle('seraphiel:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('seraphiel:fs:gitRoot', async (_event, startPath) => {
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'seraphiel_cli.main', 'uninstall', '--gui-summary'], hiddenWindowsChildOptions({
        cwd: agentRoot,
