...
test('parseWorktrees: main checkout + linked worktree', () => {
  const out = [
    ...
    'HEAD def456',
    'branch refs/heads/seraphiel/feat',
    ''
  ].join('\n')
  ...
  assert.equal(trees[1].branch, 'seraphiel/feat')
})
...
test('ensureGitRepo: inits a plain dir with a root commit so worktrees branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-wt-'))
...
test('switchBranch: switches a normal checkout branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-switch-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: lists locals and flags the checked-out branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-branches-'))
...
test('listBranches: flags a free default branch as default, not checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-branches-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: a branch claimed by a worktree is flagged checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-branches-wt-'))
...
test('listBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-nonrepo-'))
...
test('addWorktree: existingBranch checks the branch out without a new branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-convert-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: existing default branch switches the main checkout, not .worktrees/main', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-convert-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: lists local branches and flags the default', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-base-branches-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-base-nonrepo-'))
...
test('addWorktree: base param branches off a specified local branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-base-add-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: base origin/main does not set up upstream tracking', async () => {
  ...
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-remote-'))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'seraphiel-clone-'))
  ...
  try {
    ...
    execFileSync('git', [
      ...
      remoteDir,
      '-c',
      'user.email=seraphiel@localhost',
      '-c',
      'user.name=Seraphiel',
      'commit',
      '--allow-empty',
