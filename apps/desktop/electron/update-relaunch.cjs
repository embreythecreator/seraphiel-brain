 ...
 * Background
 * ----------
 * After `seraphiel update` + `seraphiel desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything SERAPHIEL_DESKTOP_* is preserved
// plus SERAPHIEL_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['SERAPHIEL_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['SERAPHIEL_DESKTOP_']
