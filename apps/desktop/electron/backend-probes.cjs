 ...
 * resolveSeraphielBackend (main.cjs). The resolver walks a ladder of
...
/**
 * Return true iff `python -c "import seraphiel_cli"` exits 0.
 *
 ...
 * @returns {boolean}
 */
function canImportSeraphielCli(pythonPath) {
  if (!pythonPath) return false
  try {
    execFileSync(pythonPath, ['-c', 'import seraphiel_cli'], {
      stdio: 'ignore',
 ...
 * @returns {boolean}
 */
function verifySeraphielCli(seraphielCommand, opts = {}) {
  if (!seraphielCommand) return false
  try {
