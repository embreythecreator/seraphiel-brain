 ...
 *
 * Drives apps/desktop's first-launch install of Seraphiel Brain by spawning
...
function bootstrapCacheDir(seraphielHome) {
  return path.join(seraphielHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.seraphiel/seraphiel-brain. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(seraphielHome) {
  if (!seraphielHome) return null
  const candidate = path.join(seraphielHome, 'seraphiel-brain', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(seraphielHome, commit) {
  return path.join(bootstrapCacheDir(seraphielHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/embreythecreator/seraphiel-brain/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, seraphielHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(seraphielHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(seraphielHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, seraphielHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass SERAPHIEL_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        SERAPHIEL_HOME: seraphielHome || process.env.SERAPHIEL_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, seraphielHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, seraphielHome }) {
  const args = ['--dir', activeRoot, '--seraphiel-home', seraphielHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, seraphielHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, seraphielHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, seraphielHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, seraphielHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(seraphielHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, seraphielHome, emit })
