...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.seraphiel-bootstrap-complete'), 'utf8')
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/embreythecreator/seraphiel-brain/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(seraphielHome, installRef.cacheKey)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, seraphielHome }: any = {}) {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, seraphielHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, seraphielHome, pinCommit = true }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, seraphielHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, seraphielHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, seraphielHome, pinCommit })
      ]
