 ...
 * null to keep it enabled. `SERAPHIEL_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.SERAPHIEL_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (SERAPHIEL_DESKTOP_DISABLE_GPU)'
