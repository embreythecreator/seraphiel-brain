 ...
 *     `X-Seraphiel-Session-Token` header; WS uses `?token=`.
...
// seraphiel_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - seraphiel_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - seraphiel_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / seraphiel #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-seraphiel_session_at', '__Secure-seraphiel_session_at', 'seraphiel_session_at']
const RT_COOKIE_VARIANTS = ['__Host-seraphiel_session_rt', '__Secure-seraphiel_session_rt', 'seraphiel_session_rt']

// The Nous portal (NAS) does NOT use Seraphiel gateway session cookies — it is a
...
// Seraphiel Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://seraphiel.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a seraphiel session ACCESS-token cookie
 ...
 * the Seraphiel gateway session cookies, so cloud sign-in / discovery liveness
