...
export function markerPath(seraphielHome) {
  return path.join(seraphielHome, '.seraphiel-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(seraphielHome)
  let raw
 ...
 *
 * The Tauri-based seraphiel-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(seraphielHome, pid, { now = Date.now } = {}) {
  const file = markerPath(seraphielHome)
