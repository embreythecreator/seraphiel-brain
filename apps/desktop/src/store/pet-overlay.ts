 ...
 * app's bounds and stays visible while Seraphiel is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'seraphiel.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'seraphiel.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.seraphielDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.seraphielDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.seraphielDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.seraphielDesktop?.petOverlay?.close()
}
