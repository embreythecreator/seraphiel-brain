import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@seraphiel/shared'
...
const isOpen = (gateway: SeraphielGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<SeraphielGateway | null>(null)
...
let primaryGateway: SeraphielGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: SeraphielGateway | null, profile = 'default'): void {
...
export function activeGateway(): SeraphielGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<SeraphielGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
