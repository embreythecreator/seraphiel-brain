...
import type { SessionInfo, UsageStats } from '@/types/seraphiel'
...
const COMPOSER_MODEL_KEY = 'seraphiel.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'seraphiel.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'seraphiel.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'seraphiel.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'seraphiel.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'seraphiel.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'seraphiel.desktop.lastRoute'
...
function workspaceCwdKey(connection: SeraphielConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.seraphielDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.seraphielDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SeraphielConnection | null>(null)
...
export const setConnection = (next: Updater<SeraphielConnection | null>) => updateAtom($connection, next)
