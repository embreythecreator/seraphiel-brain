...
const STORAGE_KEY = 'seraphiel:native-notifications'
...
// "Backgrounded" = the user isn't on Seraphiel. `document.hidden` only flips when
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.seraphielDesktop
