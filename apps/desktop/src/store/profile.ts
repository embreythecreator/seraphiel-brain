...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/seraphiel'
...
import type { ProfileInfo } from '@/types/seraphiel'
...
// /api/profiles/active `current`). "default" is the root ~/.seraphiel. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'seraphiel.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'seraphiel.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.seraphielDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SERAPHIEL_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.seraphielDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.seraphielDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'seraphiel.desktop.showAllProfiles'
...
// Switch to the default (root ~/.seraphiel) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.seraphielDesktop?.touchBackend?.(target).catch(() => undefined)
}
