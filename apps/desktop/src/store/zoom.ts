...
export function setZoomPercent(percent: number): void {
  window.seraphielDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.seraphielDesktop?.zoom) {
  void window.seraphielDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.seraphielDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
