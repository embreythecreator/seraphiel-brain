...
import type { SeraphielReviewFile, SeraphielReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.seraphielDesktop.git,
...
function file(path: string, over: Partial<SeraphielReviewFile> = {}): SeraphielReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as SeraphielReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.seraphielDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { seraphielDesktop?: unknown }).seraphielDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
afterEach(() => {
  delete (window as unknown as { seraphielDesktop?: unknown }).seraphielDesktop
})
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as SeraphielReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.seraphielDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: SeraphielReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as SeraphielReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { seraphielDesktop?: unknown }).seraphielDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as SeraphielReviewShipInfo)
