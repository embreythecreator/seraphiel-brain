...
const { getProfiles } = await import('@/seraphiel')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/seraphiel/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<SeraphielConnection> = {}): SeraphielConnection =>
  ({ baseUrl: 'https://seraphiel-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SeraphielConnection

const localConn = (over: Partial<SeraphielConnection> = {}): SeraphielConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SeraphielConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SeraphielConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { seraphielDesktop: { getConnection } })
