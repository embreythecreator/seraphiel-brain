...
import type { SeraphielGitBaseBranch, SeraphielGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/seraphiel'
...
const PROJECT_SCOPE_KEY = 'seraphiel.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Seraphiel gateway is not connected')
  }
...
// repo, returning where Seraphiel should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SeraphielGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<SeraphielGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.seraphielDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.seraphielDesktop?.writeClipboard?.(path)
  }
}
