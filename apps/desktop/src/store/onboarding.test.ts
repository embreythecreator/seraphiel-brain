...
import type { OAuthProvider } from '@/types/seraphiel'
...
function provider(id: string, name = id): OAuthProvider {
  return {
    cli_command: `seraphiel login ${id}`,
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('seraphiel-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('seraphiel-desktop-onboarded-v1')).toBe('1')
  })
