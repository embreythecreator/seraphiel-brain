...
import type { SeraphielRepoStatus } from '@/global'

import { $repoStatus, $repoStatusLoading, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: SeraphielRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<SeraphielRepoStatus | null>) {
  ;(window as unknown as { seraphielDesktop?: unknown }).seraphielDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh', async () => {
    const resolvers: Array<(status: SeraphielRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
