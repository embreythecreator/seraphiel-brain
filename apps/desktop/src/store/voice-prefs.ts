...
import { getSeraphielConfigRecord, saveSeraphielConfig } from '@/seraphiel'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSeraphielConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSeraphielConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
