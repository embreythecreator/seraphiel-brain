...
//
// Scope is always "uncommitted" — Seraphiel' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'seraphiel.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'seraphiel.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'seraphiel.desktop.reviewTreeMode'
const SELECTED_KEY = 'seraphiel.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SeraphielReviewFile[]>([])
...
export const $reviewShipInfo = atom<SeraphielReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['seraphielDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SeraphielReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.seraphielDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.seraphielDesktop?.openExternal?.(url)
    }
