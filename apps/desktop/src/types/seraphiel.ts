...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Seraphiel doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in seraphiel_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `seraphiel model` uses — instead of
...
export interface SeraphielConfig {
  agent?: {
...
export type SeraphielConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `seraphiel model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors seraphiel_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Seraphiel, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `seraphiel tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Seraphiel); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/seraphiel/update/check` — the backend's own update state.
