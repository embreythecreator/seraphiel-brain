...
export function mediaStreamUrl(path: string): string {
  return `seraphiel-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (seraphiel_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.seraphielDesktop!.api<{ data_url: string }>({
