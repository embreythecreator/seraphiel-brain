...
/** Why a known Seraphiel command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Seraphiel profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Seraphiel Brain version', surface: exec() },
...
function isKnownSeraphielSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Seraphiel' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSeraphielSlashCommand(normalized)
}
