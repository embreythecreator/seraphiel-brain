...
// (window.seraphielDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['seraphielDesktop']>['git']>
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: SeraphielGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<SeraphielReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<SeraphielReviewShipInfo>('review/ship-info', { path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  return isDesktopFsRemoteMode() ? remoteGit : window.seraphielDesktop?.git
}
