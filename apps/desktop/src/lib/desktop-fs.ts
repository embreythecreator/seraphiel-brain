import type {
  SeraphielConnection,
  SeraphielReadDirResult,
  SeraphielReadFileTextResult,
  SeraphielSelectPathsOptions,
  SeraphielWorktreeInfo
} from '@/global'
...
function connectionCacheKey(connection: SeraphielConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Seraphiel Desktop bridge is unavailable')
  }
  return desktop
}

export async function readDesktopDir(path: string): Promise<SeraphielReadDirResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readDir(path)
  }
  return desktop.api<SeraphielReadDirResult>({ path: fsPath('list', path) })
}

export async function readDesktopFileText(path: string): Promise<SeraphielReadFileTextResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readFileText(path)
  }
  return desktop.api<SeraphielReadFileTextResult>({ path: fsPath('read-text', path) })
}
...
export async function desktopWorktrees(cwds: string[]): Promise<Record<string, SeraphielWorktreeInfo | null>> {
  if (isDesktopFsRemoteMode()) {
    return {}
  }
...
export async function selectDesktopPaths(options?: SeraphielSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
