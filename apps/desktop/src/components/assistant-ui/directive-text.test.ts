...
import { formatRefValue, seraphielDirectiveFormatter } from './directive-text'
...
describe('seraphielDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = seraphielDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = seraphielDirectiveFormatter.parse('@file:src/main.tsx the entry point')
