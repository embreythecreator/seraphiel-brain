...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.seraphielDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.seraphielDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.seraphielDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.seraphielDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.seraphielDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.seraphielDesktop && text && normalizeExternalUrl(text) === target) {
