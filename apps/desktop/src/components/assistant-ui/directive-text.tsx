...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...seraphielDirectiveFormatter.parse(text)]
  } catch {
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.seraphielDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.seraphielDesktop?.readFileDataUrl(id)
