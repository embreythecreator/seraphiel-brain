...
const SERAPHIEL_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type SeraphielRefType = (typeof SERAPHIEL_REF_TYPES)[number]
...
const ICON_PATHS: Record<SeraphielRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as SeraphielRefType] ?? ICON_FALLBACK
...
const SERAPHIEL_DIRECTIVE_RE = new RegExp(
...
export const seraphielDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SERAPHIEL_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as SeraphielRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: SeraphielRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Seraphiel directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => seraphielDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.seraphielDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.seraphielDesktop?.readFileDataUrl(id)
