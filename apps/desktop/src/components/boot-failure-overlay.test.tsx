...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Seraphiel gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.seraphielDesktop
  Object.defineProperty(window, 'seraphielDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'seraphielDesktop', { configurable: true, value: original })
}
