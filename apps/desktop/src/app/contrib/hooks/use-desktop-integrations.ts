...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.seraphielDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  // seraphiel:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
  ...
  useEffect(() => {
    const unsubscribe = window.seraphielDesktop?.onClosePreviewRequested?.(() => void closeActiveTab())
