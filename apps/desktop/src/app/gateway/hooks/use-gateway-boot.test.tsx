...
// bug that drives the REAL useGatewayBoot hook + REAL SeraphielGateway through a
...
function Harness({ refreshSessions }: { refreshSessions?: () => Promise<void> } = {}) {
  useGatewayBoot({
    ...
    refreshSeraphielConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { seraphielDesktop?: unknown }).seraphielDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { seraphielDesktop?: unknown }).seraphielDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSeraphiel) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSeraphiel()'s remote branch awaits waitForSeraphiel() for 45s before it
    ...
    ;(window as { seraphielDesktop?: unknown }).seraphielDesktop = desktop
    ...
    // After ~45s waitForSeraphiel gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Seraphiel backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Seraphiel couldn't start" overlay — the socket is open
