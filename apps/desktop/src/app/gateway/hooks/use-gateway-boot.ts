import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@seraphiel/shared'
import { useEffect, useRef } from 'react'

import type { SeraphielConnection } from '@/global'
import { SeraphielGateway } from '@/seraphiel'
...
import type { RpcEvent } from '@/types/seraphiel'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.seraphielDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SeraphielGateway | null) => void
  refreshSeraphielConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.seraphielDesktop

    const publish = (next: SeraphielConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Seraphiel...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Seraphiel…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Seraphiel gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSeraphielConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshSeraphielConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startSeraphiel re-emits progress — ignore so the
    ...
    const gateway = new SeraphielGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshSeraphielConfig(),
          ...
          // retry it), not brick boot behind the "Seraphiel couldn't start"
