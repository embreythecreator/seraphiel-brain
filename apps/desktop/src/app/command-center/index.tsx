...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateSeraphiel } from '@/seraphiel'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/seraphiel'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `seraphiel logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateSeraphiel()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.seraphielActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
