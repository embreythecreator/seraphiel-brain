...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshSeraphielConfig — two REST calls (config + defaults) per event, per
    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (apply) {
          ...
          // everything refreshSeraphielConfig applies is either active-session
        ...
        const errorMessage = payload?.message || 'Seraphiel reported an error'
