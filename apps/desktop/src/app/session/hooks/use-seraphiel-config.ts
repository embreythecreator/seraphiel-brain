...
import { getSeraphielConfig, getSeraphielConfigDefaults } from '@/seraphiel'
...
interface SeraphielConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useSeraphielConfig({ activeSessionIdRef, refreshProjectBranch }: SeraphielConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshSeraphielConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getSeraphielConfig(), getSeraphielConfigDefaults().catch(() => ({}))])
  ...
  return { refreshSeraphielConfig, sttEnabled, voiceMaxRecordingSeconds }
}
