        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.seraphielDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Seraphiel to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.seraphielDesktop?.watchPreviewFile ||
      !window.seraphielDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.seraphielDesktop.onPreviewFileChanged(payload => {
    ...
    void window.seraphielDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.seraphielDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.seraphielDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:seraphiel-preview')
