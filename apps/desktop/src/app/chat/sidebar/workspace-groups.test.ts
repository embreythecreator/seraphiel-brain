...
import type { SeraphielWorktreeInfo } from '@/global'
import type { SessionInfo } from '@/types/seraphiel'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/seraphiel-brain/apps/desktop'), makeSession('/a/seraphiel-brain-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/seraphiel-brain/apps/desktop'), makeSession('/a/seraphiel-brain-wt-rtl/apps/desktop')])
    ).toEqual(['seraphiel-brain/apps/desktop', 'seraphiel-brain-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/seraphiel-brain/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<SeraphielWorktreeInfo> & Pick<SeraphielWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): SeraphielWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/seraphiel-brain'), makeSession('/www/seraphiel-brain-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('seraphiel-brain')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['seraphiel-brain', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/seraphiel-brain') {
        return info({ repoRoot: '/www/seraphiel-brain', worktreeRoot: '/www/seraphiel-brain', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/seraphiel-brain', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/seraphiel-brain'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['seraphiel-brain', 'rtl'])
  })
