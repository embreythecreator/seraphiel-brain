...
const branchActionLabel = (branch: SeraphielGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `seraphiel/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<SeraphielGitBranch[]>([])
  ...
  const convert = async (branch: SeraphielGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
