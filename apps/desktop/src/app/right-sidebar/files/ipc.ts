...
import type { SeraphielReadDirEntry, SeraphielReadDirResult } from '@/global'

export type ProjectTreeEntry = SeraphielReadDirEntry
...
async function gitRootFor(start: string) {
  if (!window.seraphielDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.seraphielDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.seraphielDesktop?.readDir || !window.seraphielDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.seraphielDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.seraphielDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: SeraphielReadDirEntry) {
...
async function filterIgnored(entries: SeraphielReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SeraphielReadDirResult> {
  if (!window.seraphielDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.seraphielDesktop.readDir(dirPath)
