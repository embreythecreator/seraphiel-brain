...
import { invalidateSeraphielConfig, setSeraphielConfigCache, useSeraphielConfigRecord } from '../hooks/use-config-record'
...
// Seraphiel' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Seraphiel default (medium), shown as Medium.
...
// backend now surfaces the full `seraphiel model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in seraphiel_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useSeraphielConfigRecord()
  const setConfig = setSeraphielConfigCache
  ...
  const refresh = useCallback(async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
    ...
    try {
      ...
      void invalidateSeraphielConfig()
    } catch (err) {
      if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveSeraphielConfig(next)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `seraphiel model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Seraphiel runs the flow for you.`}
          </p>
        )}
