...
export const EMPTY_SELECT_VALUE = '__seraphiel_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Seraphiel & Nous-trained models',
    docsUrl: 'https://portal.embreythecreator.com',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'SERAPHIEL_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'SERAPHIEL_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // seraphiel_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Seraphiel needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Seraphiel stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Seraphiel can read from one file request.',
  approvals: {
    mode: 'How Seraphiel handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Seraphiel updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
