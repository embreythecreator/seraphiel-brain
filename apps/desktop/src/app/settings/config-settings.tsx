...
import { getElevenLabsVoices, getSeraphielConfigSchema, saveSeraphielConfig } from '@/seraphiel'
import { useI18n } from '@/i18n'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, SeraphielConfigRecord } from '@/types/seraphiel'

import { setSeraphielConfigCache, useSeraphielConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: SeraphielConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<SeraphielConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSeraphielConfigRecord()
  ...
  const {
    ...
    queryKey: ['seraphiel-config-schema'],
    queryFn: getSeraphielConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: SeraphielConfigRecord) => {
