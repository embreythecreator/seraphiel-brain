...
import { disconnectOAuthProvider, listOAuthProviders } from '@/seraphiel'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/seraphiel'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.seraphielDesktop?.terminal)
...
//      in seraphiel_cli/provider_catalog.py) — the SAME provider identity
//      `seraphiel model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Seraphiel can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Seraphiel can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Seraphiel never deletes creds another tool owns behind a silent API call.
