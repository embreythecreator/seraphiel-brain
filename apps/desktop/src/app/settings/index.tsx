...
import { getSeraphielConfigDefaults, getSeraphielConfigRecord, saveSeraphielConfig } from '@/seraphiel'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getSeraphielConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'seraphiel-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveSeraphielConfig(await getSeraphielConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
