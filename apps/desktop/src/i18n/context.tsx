...
import { getSeraphielConfigRecord, type SeraphielConfigRecord, saveSeraphielConfig } from '@/seraphiel'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SeraphielConfigRecord>
  saveConfig: (config: SeraphielConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.seraphielDesktop?.api) {
      return Promise.resolve({})
    }

    return getSeraphielConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.seraphielDesktop?.api) {
    ...
    return saveSeraphielConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: SeraphielConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SeraphielConfigRecord, locale: Locale): SeraphielConfigRecord {
