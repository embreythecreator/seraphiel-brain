...
declare global {
  interface Window {
    seraphielDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SeraphielConnection>
      ...
      // Seraphiel Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SERAPHIEL_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SeraphielApiRequest) => Promise<T>
      notify: (payload: SeraphielNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<SeraphielReadFileTextResult>
      selectPaths: (options?: SeraphielSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SeraphielPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SeraphielPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: SeraphielTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SeraphielReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SeraphielGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SeraphielGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SeraphielGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SeraphielRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SeraphielReviewScope, baseRef?: null | string) => Promise<SeraphielReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SeraphielReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SeraphielReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SeraphielTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SeraphielTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SeraphielWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: SeraphielPreviewFileChanged) => void) => () => void
...
export interface SeraphielTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SeraphielTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `seraphiel update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  seraphielRoot?: string
...
export interface SeraphielConnection {
  baseUrl: string
...
export interface SeraphielTitleBarTheme {
  background: string
  foreground: string
}

export interface SeraphielWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Seraphiel Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Seraphiel Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Seraphiel Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the seraphiel gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Seraphiel Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface SeraphielApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SeraphielNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SeraphielPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SeraphielReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SeraphielPreviewWatch {
  id: string
  path: string
}
...
export interface SeraphielGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SeraphielGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface SeraphielGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SeraphielRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SeraphielRepoStatus {
  branch: null | string
...
// turn began.
export type SeraphielReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SeraphielReviewFile {
  path: string
  added: number
  removed: number
...
export interface SeraphielReviewList {
  files: SeraphielReviewFile[]
...
export interface SeraphielReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SeraphielReviewShipInfo {
  ghReady: boolean
  pr: SeraphielReviewPr | null
}

export interface SeraphielReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SeraphielReadDirResult {
  entries: SeraphielReadDirEntry[]
  error?: string
}

export interface SeraphielPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SeraphielSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
