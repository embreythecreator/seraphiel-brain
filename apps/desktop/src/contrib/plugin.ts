 ...
 * `SeraphielPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/seraphiel'
...
export type { PluginRestOptions } from '@/seraphiel'
 ...
 *  under `seraphiel.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface SeraphielPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `seraphiel.plugin.${pluginId}.${key}`
