 ...
 *   (`@seraphiel/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default SeraphielPlugin -> register(ctx)
 *
 ...
 * and `<seraphiel home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { getStatus } from '@/seraphiel'
import { installPluginSdk, sdkImportMap } from '@/sdk/runtime'
import { notifyError } from '@/store/notifications'

import { createPluginContext, type SeraphielPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@seraphiel/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @seraphiel/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: SeraphielPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default SeraphielPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<seraphiel home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    const { seraphiel_home } = await getStatus()
    const { entries } = await desktop.readDir(`${seraphiel_home}/desktop-plugins`)
