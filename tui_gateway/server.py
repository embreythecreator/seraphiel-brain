...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over SERAPHIEL_MODEL / SERAPHIEL_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set SERAPHIEL_INFERENCE_MODEL in the
...
def _resolve_runtime_with_fallback(
    ...
    ``seraphiel_cli/cli_agent_setup_mixin.py``: when the primary provider
...
# (agent/billing_view.py + seraphiel_cli/nous_billing.py) — same as /credits.
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from seraphiel_cli.model_switch import parse_model_flags
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from seraphiel_cli.config import is_managed, save_env_value
        from seraphiel_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from seraphiel_cli.config import remove_env_value
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from seraphiel_cli.browser_connect import try_launch_chrome_debug
