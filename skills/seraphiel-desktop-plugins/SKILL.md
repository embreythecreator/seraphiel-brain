---
name: seraphiel-desktop-plugins
...
# Seraphiel Desktop Plugins Skill

Write plugins for the Seraphiel desktop app: statusbar items, layout panes,
...
system (`~/.seraphiel/plugins/`) is otherwise documented separately.
...
- The Seraphiel desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SERAPHIEL_HOME/desktop-plugins/` (usually
  `~/.seraphiel/desktop-plugins/`).

## How to Run

1. Create `$SERAPHIEL_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.seraphiel/...` by default, or `~/.seraphiel/profiles/<profile>/...` under a
...
The ONLY import surface is `@seraphiel/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.seraphiel/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@seraphiel/plugin-sdk`, `react`, and
