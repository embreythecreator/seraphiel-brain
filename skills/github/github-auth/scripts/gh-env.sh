#!/usr/bin/env bash
# GitHub environment detection helper for Seraphiel Brain skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _seraphiel_env="${SERAPHIEL_HOME:-$HOME/.seraphiel}/.env"; [ -f "$_seraphiel_env" ] && grep -q "^GITHUB_TOKEN=" "$_seraphiel_env" 2>/dev/null; then
