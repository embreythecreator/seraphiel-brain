...
## 5. Seraphiel Configuration

### Config location

`$SERAPHIEL_HOME/config.yaml` (defaults to `~/.seraphiel/config.yaml` when `SERAPHIEL_HOME` is unset)
...
Restart the Seraphiel session for changes to take effect. The MCP connection is
