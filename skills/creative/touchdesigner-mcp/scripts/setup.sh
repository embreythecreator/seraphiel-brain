...
SERAPHIEL_HOME_DIR="${SERAPHIEL_HOME:-$HOME/.seraphiel}"
SERAPHIEL_CFG="${SERAPHIEL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Seraphiel config has twozero_td MCP entry ──
if [[ ! -f "$SERAPHIEL_CFG" ]]; then
    echo -e " ${FAIL} Seraphiel config not found at ${SERAPHIEL_CFG}"
    manual_steps+=("Create ${SERAPHIEL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SERAPHIEL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Seraphiel config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Seraphiel config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$SERAPHIEL_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SERAPHIEL_CFG} manually"); }
    manual_steps+=("Restart Seraphiel session to pick up config change")
fi
