#!/usr/bin/env python3
"""Google Workspace API CLI for Seraphiel Brain.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Seraphiel-facing JSON contract and falls back to the Python client
...
from _seraphiel_home import get_seraphiel_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SERAPHIEL_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Seraphiel Brain")
